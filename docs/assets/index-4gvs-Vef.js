(function () {
  const l = document.createElement("link").relList;
  if (l && l.supports && l.supports("modulepreload")) return;
  for (const c of document.querySelectorAll('link[rel="modulepreload"]')) o(c);
  new MutationObserver((c) => {
    for (const h of c)
      if (h.type === "childList")
        for (const d of h.addedNodes) d.tagName === "LINK" && d.rel === "modulepreload" && o(d);
  }).observe(document, { childList: !0, subtree: !0 });
  function u(c) {
    const h = {};
    return (
      c.integrity && (h.integrity = c.integrity),
      c.referrerPolicy && (h.referrerPolicy = c.referrerPolicy),
      c.crossOrigin === "use-credentials"
        ? (h.credentials = "include")
        : c.crossOrigin === "anonymous"
          ? (h.credentials = "omit")
          : (h.credentials = "same-origin"),
      h
    );
  }
  function o(c) {
    if (c.ep) return;
    c.ep = !0;
    const h = u(c);
    fetch(c.href, h);
  }
})();
function ex(i) {
  return i && i.__esModule && Object.prototype.hasOwnProperty.call(i, "default") ? i.default : i;
}
var _r = { exports: {} },
  fl = {};
/**
 * @license React
 * react-jsx-runtime.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var rp;
function nx() {
  if (rp) return fl;
  rp = 1;
  var i = Symbol.for("react.transitional.element"),
    l = Symbol.for("react.fragment");
  function u(o, c, h) {
    var d = null;
    if ((h !== void 0 && (d = "" + h), c.key !== void 0 && (d = "" + c.key), "key" in c)) {
      h = {};
      for (var m in c) m !== "key" && (h[m] = c[m]);
    } else h = c;
    return ((c = h.ref), { $$typeof: i, type: o, key: d, ref: c !== void 0 ? c : null, props: h });
  }
  return ((fl.Fragment = l), (fl.jsx = u), (fl.jsxs = u), fl);
}
var cp;
function ix() {
  return (cp || ((cp = 1), (_r.exports = nx())), _r.exports);
}
var b = ix(),
  Br = { exports: {} },
  at = {};
/**
 * @license React
 * react.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var fp;
function ax() {
  if (fp) return at;
  fp = 1;
  var i = Symbol.for("react.transitional.element"),
    l = Symbol.for("react.portal"),
    u = Symbol.for("react.fragment"),
    o = Symbol.for("react.strict_mode"),
    c = Symbol.for("react.profiler"),
    h = Symbol.for("react.consumer"),
    d = Symbol.for("react.context"),
    m = Symbol.for("react.forward_ref"),
    g = Symbol.for("react.suspense"),
    p = Symbol.for("react.memo"),
    v = Symbol.for("react.lazy"),
    x = Symbol.for("react.activity"),
    S = Symbol.iterator;
  function N(E) {
    return E === null || typeof E != "object"
      ? null
      : ((E = (S && E[S]) || E["@@iterator"]), typeof E == "function" ? E : null);
  }
  var w = {
      isMounted: function () {
        return !1;
      },
      enqueueForceUpdate: function () {},
      enqueueReplaceState: function () {},
      enqueueSetState: function () {},
    },
    U = Object.assign,
    H = {};
  function L(E, B, Q) {
    ((this.props = E), (this.context = B), (this.refs = H), (this.updater = Q || w));
  }
  ((L.prototype.isReactComponent = {}),
    (L.prototype.setState = function (E, B) {
      if (typeof E != "object" && typeof E != "function" && E != null)
        throw Error(
          "takes an object of state variables to update or a function which returns an object of state variables.",
        );
      this.updater.enqueueSetState(this, E, B, "setState");
    }),
    (L.prototype.forceUpdate = function (E) {
      this.updater.enqueueForceUpdate(this, E, "forceUpdate");
    }));
  function Y() {}
  Y.prototype = L.prototype;
  function q(E, B, Q) {
    ((this.props = E), (this.context = B), (this.refs = H), (this.updater = Q || w));
  }
  var G = (q.prototype = new Y());
  ((G.constructor = q), U(G, L.prototype), (G.isPureReactComponent = !0));
  var F = Array.isArray;
  function lt() {}
  var I = { H: null, A: null, T: null, S: null },
    $ = Object.prototype.hasOwnProperty;
  function it(E, B, Q) {
    var P = Q.ref;
    return { $$typeof: i, type: E, key: B, ref: P !== void 0 ? P : null, props: Q };
  }
  function tt(E, B) {
    return it(E.type, B, E.props);
  }
  function pt(E) {
    return typeof E == "object" && E !== null && E.$$typeof === i;
  }
  function vt(E) {
    var B = { "=": "=0", ":": "=2" };
    return (
      "$" +
      E.replace(/[=:]/g, function (Q) {
        return B[Q];
      })
    );
  }
  var $t = /\/+/g;
  function Gt(E, B) {
    return typeof E == "object" && E !== null && E.key != null ? vt("" + E.key) : B.toString(36);
  }
  function Lt(E) {
    switch (E.status) {
      case "fulfilled":
        return E.value;
      case "rejected":
        throw E.reason;
      default:
        switch (
          (typeof E.status == "string"
            ? E.then(lt, lt)
            : ((E.status = "pending"),
              E.then(
                function (B) {
                  E.status === "pending" && ((E.status = "fulfilled"), (E.value = B));
                },
                function (B) {
                  E.status === "pending" && ((E.status = "rejected"), (E.reason = B));
                },
              )),
          E.status)
        ) {
          case "fulfilled":
            return E.value;
          case "rejected":
            throw E.reason;
        }
    }
    throw E;
  }
  function R(E, B, Q, P, st) {
    var ct = typeof E;
    (ct === "undefined" || ct === "boolean") && (E = null);
    var Tt = !1;
    if (E === null) Tt = !0;
    else
      switch (ct) {
        case "bigint":
        case "string":
        case "number":
          Tt = !0;
          break;
        case "object":
          switch (E.$$typeof) {
            case i:
            case l:
              Tt = !0;
              break;
            case v:
              return ((Tt = E._init), R(Tt(E._payload), B, Q, P, st));
          }
      }
    if (Tt)
      return (
        (st = st(E)),
        (Tt = P === "" ? "." + Gt(E, 0) : P),
        F(st)
          ? ((Q = ""),
            Tt != null && (Q = Tt.replace($t, "$&/") + "/"),
            R(st, B, Q, "", function (va) {
              return va;
            }))
          : st != null &&
            (pt(st) &&
              (st = tt(
                st,
                Q +
                  (st.key == null || (E && E.key === st.key)
                    ? ""
                    : ("" + st.key).replace($t, "$&/") + "/") +
                  Tt,
              )),
            B.push(st)),
        1
      );
    Tt = 0;
    var oe = P === "" ? "." : P + ":";
    if (F(E))
      for (var Ht = 0; Ht < E.length; Ht++)
        ((P = E[Ht]), (ct = oe + Gt(P, Ht)), (Tt += R(P, B, Q, ct, st)));
    else if (((Ht = N(E)), typeof Ht == "function"))
      for (E = Ht.call(E), Ht = 0; !(P = E.next()).done; )
        ((P = P.value), (ct = oe + Gt(P, Ht++)), (Tt += R(P, B, Q, ct, st)));
    else if (ct === "object") {
      if (typeof E.then == "function") return R(Lt(E), B, Q, P, st);
      throw (
        (B = String(E)),
        Error(
          "Objects are not valid as a React child (found: " +
            (B === "[object Object]" ? "object with keys {" + Object.keys(E).join(", ") + "}" : B) +
            "). If you meant to render a collection of children, use an array instead.",
        )
      );
    }
    return Tt;
  }
  function X(E, B, Q) {
    if (E == null) return E;
    var P = [],
      st = 0;
    return (
      R(E, P, "", "", function (ct) {
        return B.call(Q, ct, st++);
      }),
      P
    );
  }
  function Z(E) {
    if (E._status === -1) {
      var B = E._result;
      ((B = B()),
        B.then(
          function (Q) {
            (E._status === 0 || E._status === -1) && ((E._status = 1), (E._result = Q));
          },
          function (Q) {
            (E._status === 0 || E._status === -1) && ((E._status = 2), (E._result = Q));
          },
        ),
        E._status === -1 && ((E._status = 0), (E._result = B)));
    }
    if (E._status === 1) return E._result.default;
    throw E._result;
  }
  var ot =
      typeof reportError == "function"
        ? reportError
        : function (E) {
            if (typeof window == "object" && typeof window.ErrorEvent == "function") {
              var B = new window.ErrorEvent("error", {
                bubbles: !0,
                cancelable: !0,
                message:
                  typeof E == "object" && E !== null && typeof E.message == "string"
                    ? String(E.message)
                    : String(E),
                error: E,
              });
              if (!window.dispatchEvent(B)) return;
            } else if (typeof process == "object" && typeof process.emit == "function") {
              process.emit("uncaughtException", E);
              return;
            }
            console.error(E);
          },
    gt = {
      map: X,
      forEach: function (E, B, Q) {
        X(
          E,
          function () {
            B.apply(this, arguments);
          },
          Q,
        );
      },
      count: function (E) {
        var B = 0;
        return (
          X(E, function () {
            B++;
          }),
          B
        );
      },
      toArray: function (E) {
        return (
          X(E, function (B) {
            return B;
          }) || []
        );
      },
      only: function (E) {
        if (!pt(E))
          throw Error("React.Children.only expected to receive a single React element child.");
        return E;
      },
    };
  return (
    (at.Activity = x),
    (at.Children = gt),
    (at.Component = L),
    (at.Fragment = u),
    (at.Profiler = c),
    (at.PureComponent = q),
    (at.StrictMode = o),
    (at.Suspense = g),
    (at.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = I),
    (at.__COMPILER_RUNTIME = {
      __proto__: null,
      c: function (E) {
        return I.H.useMemoCache(E);
      },
    }),
    (at.cache = function (E) {
      return function () {
        return E.apply(null, arguments);
      };
    }),
    (at.cacheSignal = function () {
      return null;
    }),
    (at.cloneElement = function (E, B, Q) {
      if (E == null) throw Error("The argument must be a React element, but you passed " + E + ".");
      var P = U({}, E.props),
        st = E.key;
      if (B != null)
        for (ct in (B.key !== void 0 && (st = "" + B.key), B))
          !$.call(B, ct) ||
            ct === "key" ||
            ct === "__self" ||
            ct === "__source" ||
            (ct === "ref" && B.ref === void 0) ||
            (P[ct] = B[ct]);
      var ct = arguments.length - 2;
      if (ct === 1) P.children = Q;
      else if (1 < ct) {
        for (var Tt = Array(ct), oe = 0; oe < ct; oe++) Tt[oe] = arguments[oe + 2];
        P.children = Tt;
      }
      return it(E.type, st, P);
    }),
    (at.createContext = function (E) {
      return (
        (E = {
          $$typeof: d,
          _currentValue: E,
          _currentValue2: E,
          _threadCount: 0,
          Provider: null,
          Consumer: null,
        }),
        (E.Provider = E),
        (E.Consumer = { $$typeof: h, _context: E }),
        E
      );
    }),
    (at.createElement = function (E, B, Q) {
      var P,
        st = {},
        ct = null;
      if (B != null)
        for (P in (B.key !== void 0 && (ct = "" + B.key), B))
          $.call(B, P) && P !== "key" && P !== "__self" && P !== "__source" && (st[P] = B[P]);
      var Tt = arguments.length - 2;
      if (Tt === 1) st.children = Q;
      else if (1 < Tt) {
        for (var oe = Array(Tt), Ht = 0; Ht < Tt; Ht++) oe[Ht] = arguments[Ht + 2];
        st.children = oe;
      }
      if (E && E.defaultProps)
        for (P in ((Tt = E.defaultProps), Tt)) st[P] === void 0 && (st[P] = Tt[P]);
      return it(E, ct, st);
    }),
    (at.createRef = function () {
      return { current: null };
    }),
    (at.forwardRef = function (E) {
      return { $$typeof: m, render: E };
    }),
    (at.isValidElement = pt),
    (at.lazy = function (E) {
      return { $$typeof: v, _payload: { _status: -1, _result: E }, _init: Z };
    }),
    (at.memo = function (E, B) {
      return { $$typeof: p, type: E, compare: B === void 0 ? null : B };
    }),
    (at.startTransition = function (E) {
      var B = I.T,
        Q = {};
      I.T = Q;
      try {
        var P = E(),
          st = I.S;
        (st !== null && st(Q, P),
          typeof P == "object" && P !== null && typeof P.then == "function" && P.then(lt, ot));
      } catch (ct) {
        ot(ct);
      } finally {
        (B !== null && Q.types !== null && (B.types = Q.types), (I.T = B));
      }
    }),
    (at.unstable_useCacheRefresh = function () {
      return I.H.useCacheRefresh();
    }),
    (at.use = function (E) {
      return I.H.use(E);
    }),
    (at.useActionState = function (E, B, Q) {
      return I.H.useActionState(E, B, Q);
    }),
    (at.useCallback = function (E, B) {
      return I.H.useCallback(E, B);
    }),
    (at.useContext = function (E) {
      return I.H.useContext(E);
    }),
    (at.useDebugValue = function () {}),
    (at.useDeferredValue = function (E, B) {
      return I.H.useDeferredValue(E, B);
    }),
    (at.useEffect = function (E, B) {
      return I.H.useEffect(E, B);
    }),
    (at.useEffectEvent = function (E) {
      return I.H.useEffectEvent(E);
    }),
    (at.useId = function () {
      return I.H.useId();
    }),
    (at.useImperativeHandle = function (E, B, Q) {
      return I.H.useImperativeHandle(E, B, Q);
    }),
    (at.useInsertionEffect = function (E, B) {
      return I.H.useInsertionEffect(E, B);
    }),
    (at.useLayoutEffect = function (E, B) {
      return I.H.useLayoutEffect(E, B);
    }),
    (at.useMemo = function (E, B) {
      return I.H.useMemo(E, B);
    }),
    (at.useOptimistic = function (E, B) {
      return I.H.useOptimistic(E, B);
    }),
    (at.useReducer = function (E, B, Q) {
      return I.H.useReducer(E, B, Q);
    }),
    (at.useRef = function (E) {
      return I.H.useRef(E);
    }),
    (at.useState = function (E) {
      return I.H.useState(E);
    }),
    (at.useSyncExternalStore = function (E, B, Q) {
      return I.H.useSyncExternalStore(E, B, Q);
    }),
    (at.useTransition = function () {
      return I.H.useTransition();
    }),
    (at.version = "19.2.4"),
    at
  );
}
var dp;
function zc() {
  return (dp || ((dp = 1), (Br.exports = ax())), Br.exports);
}
var k = zc();
const lx = ex(k);
var Ur = { exports: {} },
  dl = {},
  Lr = { exports: {} },
  Hr = {};
/**
 * @license React
 * scheduler.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var hp;
function sx() {
  return (
    hp ||
      ((hp = 1),
      (function (i) {
        function l(R, X) {
          var Z = R.length;
          R.push(X);
          t: for (; 0 < Z; ) {
            var ot = (Z - 1) >>> 1,
              gt = R[ot];
            if (0 < c(gt, X)) ((R[ot] = X), (R[Z] = gt), (Z = ot));
            else break t;
          }
        }
        function u(R) {
          return R.length === 0 ? null : R[0];
        }
        function o(R) {
          if (R.length === 0) return null;
          var X = R[0],
            Z = R.pop();
          if (Z !== X) {
            R[0] = Z;
            t: for (var ot = 0, gt = R.length, E = gt >>> 1; ot < E; ) {
              var B = 2 * (ot + 1) - 1,
                Q = R[B],
                P = B + 1,
                st = R[P];
              if (0 > c(Q, Z))
                P < gt && 0 > c(st, Q)
                  ? ((R[ot] = st), (R[P] = Z), (ot = P))
                  : ((R[ot] = Q), (R[B] = Z), (ot = B));
              else if (P < gt && 0 > c(st, Z)) ((R[ot] = st), (R[P] = Z), (ot = P));
              else break t;
            }
          }
          return X;
        }
        function c(R, X) {
          var Z = R.sortIndex - X.sortIndex;
          return Z !== 0 ? Z : R.id - X.id;
        }
        if (
          ((i.unstable_now = void 0),
          typeof performance == "object" && typeof performance.now == "function")
        ) {
          var h = performance;
          i.unstable_now = function () {
            return h.now();
          };
        } else {
          var d = Date,
            m = d.now();
          i.unstable_now = function () {
            return d.now() - m;
          };
        }
        var g = [],
          p = [],
          v = 1,
          x = null,
          S = 3,
          N = !1,
          w = !1,
          U = !1,
          H = !1,
          L = typeof setTimeout == "function" ? setTimeout : null,
          Y = typeof clearTimeout == "function" ? clearTimeout : null,
          q = typeof setImmediate < "u" ? setImmediate : null;
        function G(R) {
          for (var X = u(p); X !== null; ) {
            if (X.callback === null) o(p);
            else if (X.startTime <= R) (o(p), (X.sortIndex = X.expirationTime), l(g, X));
            else break;
            X = u(p);
          }
        }
        function F(R) {
          if (((U = !1), G(R), !w))
            if (u(g) !== null) ((w = !0), lt || ((lt = !0), vt()));
            else {
              var X = u(p);
              X !== null && Lt(F, X.startTime - R);
            }
        }
        var lt = !1,
          I = -1,
          $ = 5,
          it = -1;
        function tt() {
          return H ? !0 : !(i.unstable_now() - it < $);
        }
        function pt() {
          if (((H = !1), lt)) {
            var R = i.unstable_now();
            it = R;
            var X = !0;
            try {
              t: {
                ((w = !1), U && ((U = !1), Y(I), (I = -1)), (N = !0));
                var Z = S;
                try {
                  e: {
                    for (G(R), x = u(g); x !== null && !(x.expirationTime > R && tt()); ) {
                      var ot = x.callback;
                      if (typeof ot == "function") {
                        ((x.callback = null), (S = x.priorityLevel));
                        var gt = ot(x.expirationTime <= R);
                        if (((R = i.unstable_now()), typeof gt == "function")) {
                          ((x.callback = gt), G(R), (X = !0));
                          break e;
                        }
                        (x === u(g) && o(g), G(R));
                      } else o(g);
                      x = u(g);
                    }
                    if (x !== null) X = !0;
                    else {
                      var E = u(p);
                      (E !== null && Lt(F, E.startTime - R), (X = !1));
                    }
                  }
                  break t;
                } finally {
                  ((x = null), (S = Z), (N = !1));
                }
                X = void 0;
              }
            } finally {
              X ? vt() : (lt = !1);
            }
          }
        }
        var vt;
        if (typeof q == "function")
          vt = function () {
            q(pt);
          };
        else if (typeof MessageChannel < "u") {
          var $t = new MessageChannel(),
            Gt = $t.port2;
          (($t.port1.onmessage = pt),
            (vt = function () {
              Gt.postMessage(null);
            }));
        } else
          vt = function () {
            L(pt, 0);
          };
        function Lt(R, X) {
          I = L(function () {
            R(i.unstable_now());
          }, X);
        }
        ((i.unstable_IdlePriority = 5),
          (i.unstable_ImmediatePriority = 1),
          (i.unstable_LowPriority = 4),
          (i.unstable_NormalPriority = 3),
          (i.unstable_Profiling = null),
          (i.unstable_UserBlockingPriority = 2),
          (i.unstable_cancelCallback = function (R) {
            R.callback = null;
          }),
          (i.unstable_forceFrameRate = function (R) {
            0 > R || 125 < R
              ? console.error(
                  "forceFrameRate takes a positive int between 0 and 125, forcing frame rates higher than 125 fps is not supported",
                )
              : ($ = 0 < R ? Math.floor(1e3 / R) : 5);
          }),
          (i.unstable_getCurrentPriorityLevel = function () {
            return S;
          }),
          (i.unstable_next = function (R) {
            switch (S) {
              case 1:
              case 2:
              case 3:
                var X = 3;
                break;
              default:
                X = S;
            }
            var Z = S;
            S = X;
            try {
              return R();
            } finally {
              S = Z;
            }
          }),
          (i.unstable_requestPaint = function () {
            H = !0;
          }),
          (i.unstable_runWithPriority = function (R, X) {
            switch (R) {
              case 1:
              case 2:
              case 3:
              case 4:
              case 5:
                break;
              default:
                R = 3;
            }
            var Z = S;
            S = R;
            try {
              return X();
            } finally {
              S = Z;
            }
          }),
          (i.unstable_scheduleCallback = function (R, X, Z) {
            var ot = i.unstable_now();
            switch (
              (typeof Z == "object" && Z !== null
                ? ((Z = Z.delay), (Z = typeof Z == "number" && 0 < Z ? ot + Z : ot))
                : (Z = ot),
              R)
            ) {
              case 1:
                var gt = -1;
                break;
              case 2:
                gt = 250;
                break;
              case 5:
                gt = 1073741823;
                break;
              case 4:
                gt = 1e4;
                break;
              default:
                gt = 5e3;
            }
            return (
              (gt = Z + gt),
              (R = {
                id: v++,
                callback: X,
                priorityLevel: R,
                startTime: Z,
                expirationTime: gt,
                sortIndex: -1,
              }),
              Z > ot
                ? ((R.sortIndex = Z),
                  l(p, R),
                  u(g) === null && R === u(p) && (U ? (Y(I), (I = -1)) : (U = !0), Lt(F, Z - ot)))
                : ((R.sortIndex = gt), l(g, R), w || N || ((w = !0), lt || ((lt = !0), vt()))),
              R
            );
          }),
          (i.unstable_shouldYield = tt),
          (i.unstable_wrapCallback = function (R) {
            var X = S;
            return function () {
              var Z = S;
              S = X;
              try {
                return R.apply(this, arguments);
              } finally {
                S = Z;
              }
            };
          }));
      })(Hr)),
    Hr
  );
}
var mp;
function ux() {
  return (mp || ((mp = 1), (Lr.exports = sx())), Lr.exports);
}
var qr = { exports: {} },
  ue = {};
/**
 * @license React
 * react-dom.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var pp;
function ox() {
  if (pp) return ue;
  pp = 1;
  var i = zc();
  function l(g) {
    var p = "https://react.dev/errors/" + g;
    if (1 < arguments.length) {
      p += "?args[]=" + encodeURIComponent(arguments[1]);
      for (var v = 2; v < arguments.length; v++) p += "&args[]=" + encodeURIComponent(arguments[v]);
    }
    return (
      "Minified React error #" +
      g +
      "; visit " +
      p +
      " for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
    );
  }
  function u() {}
  var o = {
      d: {
        f: u,
        r: function () {
          throw Error(l(522));
        },
        D: u,
        C: u,
        L: u,
        m: u,
        X: u,
        S: u,
        M: u,
      },
      p: 0,
      findDOMNode: null,
    },
    c = Symbol.for("react.portal");
  function h(g, p, v) {
    var x = 3 < arguments.length && arguments[3] !== void 0 ? arguments[3] : null;
    return {
      $$typeof: c,
      key: x == null ? null : "" + x,
      children: g,
      containerInfo: p,
      implementation: v,
    };
  }
  var d = i.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
  function m(g, p) {
    if (g === "font") return "";
    if (typeof p == "string") return p === "use-credentials" ? p : "";
  }
  return (
    (ue.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = o),
    (ue.createPortal = function (g, p) {
      var v = 2 < arguments.length && arguments[2] !== void 0 ? arguments[2] : null;
      if (!p || (p.nodeType !== 1 && p.nodeType !== 9 && p.nodeType !== 11)) throw Error(l(299));
      return h(g, p, null, v);
    }),
    (ue.flushSync = function (g) {
      var p = d.T,
        v = o.p;
      try {
        if (((d.T = null), (o.p = 2), g)) return g();
      } finally {
        ((d.T = p), (o.p = v), o.d.f());
      }
    }),
    (ue.preconnect = function (g, p) {
      typeof g == "string" &&
        (p
          ? ((p = p.crossOrigin),
            (p = typeof p == "string" ? (p === "use-credentials" ? p : "") : void 0))
          : (p = null),
        o.d.C(g, p));
    }),
    (ue.prefetchDNS = function (g) {
      typeof g == "string" && o.d.D(g);
    }),
    (ue.preinit = function (g, p) {
      if (typeof g == "string" && p && typeof p.as == "string") {
        var v = p.as,
          x = m(v, p.crossOrigin),
          S = typeof p.integrity == "string" ? p.integrity : void 0,
          N = typeof p.fetchPriority == "string" ? p.fetchPriority : void 0;
        v === "style"
          ? o.d.S(g, typeof p.precedence == "string" ? p.precedence : void 0, {
              crossOrigin: x,
              integrity: S,
              fetchPriority: N,
            })
          : v === "script" &&
            o.d.X(g, {
              crossOrigin: x,
              integrity: S,
              fetchPriority: N,
              nonce: typeof p.nonce == "string" ? p.nonce : void 0,
            });
      }
    }),
    (ue.preinitModule = function (g, p) {
      if (typeof g == "string")
        if (typeof p == "object" && p !== null) {
          if (p.as == null || p.as === "script") {
            var v = m(p.as, p.crossOrigin);
            o.d.M(g, {
              crossOrigin: v,
              integrity: typeof p.integrity == "string" ? p.integrity : void 0,
              nonce: typeof p.nonce == "string" ? p.nonce : void 0,
            });
          }
        } else p == null && o.d.M(g);
    }),
    (ue.preload = function (g, p) {
      if (typeof g == "string" && typeof p == "object" && p !== null && typeof p.as == "string") {
        var v = p.as,
          x = m(v, p.crossOrigin);
        o.d.L(g, v, {
          crossOrigin: x,
          integrity: typeof p.integrity == "string" ? p.integrity : void 0,
          nonce: typeof p.nonce == "string" ? p.nonce : void 0,
          type: typeof p.type == "string" ? p.type : void 0,
          fetchPriority: typeof p.fetchPriority == "string" ? p.fetchPriority : void 0,
          referrerPolicy: typeof p.referrerPolicy == "string" ? p.referrerPolicy : void 0,
          imageSrcSet: typeof p.imageSrcSet == "string" ? p.imageSrcSet : void 0,
          imageSizes: typeof p.imageSizes == "string" ? p.imageSizes : void 0,
          media: typeof p.media == "string" ? p.media : void 0,
        });
      }
    }),
    (ue.preloadModule = function (g, p) {
      if (typeof g == "string")
        if (p) {
          var v = m(p.as, p.crossOrigin);
          o.d.m(g, {
            as: typeof p.as == "string" && p.as !== "script" ? p.as : void 0,
            crossOrigin: v,
            integrity: typeof p.integrity == "string" ? p.integrity : void 0,
          });
        } else o.d.m(g);
    }),
    (ue.requestFormReset = function (g) {
      o.d.r(g);
    }),
    (ue.unstable_batchedUpdates = function (g, p) {
      return g(p);
    }),
    (ue.useFormState = function (g, p, v) {
      return d.H.useFormState(g, p, v);
    }),
    (ue.useFormStatus = function () {
      return d.H.useHostTransitionStatus();
    }),
    (ue.version = "19.2.4"),
    ue
  );
}
var gp;
function rx() {
  if (gp) return qr.exports;
  gp = 1;
  function i() {
    if (
      !(
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
      )
    )
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(i);
      } catch (l) {
        console.error(l);
      }
  }
  return (i(), (qr.exports = ox()), qr.exports);
}
/**
 * @license React
 * react-dom-client.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var yp;
function cx() {
  if (yp) return dl;
  yp = 1;
  var i = ux(),
    l = zc(),
    u = rx();
  function o(t) {
    var e = "https://react.dev/errors/" + t;
    if (1 < arguments.length) {
      e += "?args[]=" + encodeURIComponent(arguments[1]);
      for (var n = 2; n < arguments.length; n++) e += "&args[]=" + encodeURIComponent(arguments[n]);
    }
    return (
      "Minified React error #" +
      t +
      "; visit " +
      e +
      " for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
    );
  }
  function c(t) {
    return !(!t || (t.nodeType !== 1 && t.nodeType !== 9 && t.nodeType !== 11));
  }
  function h(t) {
    var e = t,
      n = t;
    if (t.alternate) for (; e.return; ) e = e.return;
    else {
      t = e;
      do ((e = t), (e.flags & 4098) !== 0 && (n = e.return), (t = e.return));
      while (t);
    }
    return e.tag === 3 ? n : null;
  }
  function d(t) {
    if (t.tag === 13) {
      var e = t.memoizedState;
      if ((e === null && ((t = t.alternate), t !== null && (e = t.memoizedState)), e !== null))
        return e.dehydrated;
    }
    return null;
  }
  function m(t) {
    if (t.tag === 31) {
      var e = t.memoizedState;
      if ((e === null && ((t = t.alternate), t !== null && (e = t.memoizedState)), e !== null))
        return e.dehydrated;
    }
    return null;
  }
  function g(t) {
    if (h(t) !== t) throw Error(o(188));
  }
  function p(t) {
    var e = t.alternate;
    if (!e) {
      if (((e = h(t)), e === null)) throw Error(o(188));
      return e !== t ? null : t;
    }
    for (var n = t, a = e; ; ) {
      var s = n.return;
      if (s === null) break;
      var r = s.alternate;
      if (r === null) {
        if (((a = s.return), a !== null)) {
          n = a;
          continue;
        }
        break;
      }
      if (s.child === r.child) {
        for (r = s.child; r; ) {
          if (r === n) return (g(s), t);
          if (r === a) return (g(s), e);
          r = r.sibling;
        }
        throw Error(o(188));
      }
      if (n.return !== a.return) ((n = s), (a = r));
      else {
        for (var f = !1, y = s.child; y; ) {
          if (y === n) {
            ((f = !0), (n = s), (a = r));
            break;
          }
          if (y === a) {
            ((f = !0), (a = s), (n = r));
            break;
          }
          y = y.sibling;
        }
        if (!f) {
          for (y = r.child; y; ) {
            if (y === n) {
              ((f = !0), (n = r), (a = s));
              break;
            }
            if (y === a) {
              ((f = !0), (a = r), (n = s));
              break;
            }
            y = y.sibling;
          }
          if (!f) throw Error(o(189));
        }
      }
      if (n.alternate !== a) throw Error(o(190));
    }
    if (n.tag !== 3) throw Error(o(188));
    return n.stateNode.current === n ? t : e;
  }
  function v(t) {
    var e = t.tag;
    if (e === 5 || e === 26 || e === 27 || e === 6) return t;
    for (t = t.child; t !== null; ) {
      if (((e = v(t)), e !== null)) return e;
      t = t.sibling;
    }
    return null;
  }
  var x = Object.assign,
    S = Symbol.for("react.element"),
    N = Symbol.for("react.transitional.element"),
    w = Symbol.for("react.portal"),
    U = Symbol.for("react.fragment"),
    H = Symbol.for("react.strict_mode"),
    L = Symbol.for("react.profiler"),
    Y = Symbol.for("react.consumer"),
    q = Symbol.for("react.context"),
    G = Symbol.for("react.forward_ref"),
    F = Symbol.for("react.suspense"),
    lt = Symbol.for("react.suspense_list"),
    I = Symbol.for("react.memo"),
    $ = Symbol.for("react.lazy"),
    it = Symbol.for("react.activity"),
    tt = Symbol.for("react.memo_cache_sentinel"),
    pt = Symbol.iterator;
  function vt(t) {
    return t === null || typeof t != "object"
      ? null
      : ((t = (pt && t[pt]) || t["@@iterator"]), typeof t == "function" ? t : null);
  }
  var $t = Symbol.for("react.client.reference");
  function Gt(t) {
    if (t == null) return null;
    if (typeof t == "function") return t.$$typeof === $t ? null : t.displayName || t.name || null;
    if (typeof t == "string") return t;
    switch (t) {
      case U:
        return "Fragment";
      case L:
        return "Profiler";
      case H:
        return "StrictMode";
      case F:
        return "Suspense";
      case lt:
        return "SuspenseList";
      case it:
        return "Activity";
    }
    if (typeof t == "object")
      switch (t.$$typeof) {
        case w:
          return "Portal";
        case q:
          return t.displayName || "Context";
        case Y:
          return (t._context.displayName || "Context") + ".Consumer";
        case G:
          var e = t.render;
          return (
            (t = t.displayName),
            t ||
              ((t = e.displayName || e.name || ""),
              (t = t !== "" ? "ForwardRef(" + t + ")" : "ForwardRef")),
            t
          );
        case I:
          return ((e = t.displayName || null), e !== null ? e : Gt(t.type) || "Memo");
        case $:
          ((e = t._payload), (t = t._init));
          try {
            return Gt(t(e));
          } catch {}
      }
    return null;
  }
  var Lt = Array.isArray,
    R = l.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE,
    X = u.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE,
    Z = { pending: !1, data: null, method: null, action: null },
    ot = [],
    gt = -1;
  function E(t) {
    return { current: t };
  }
  function B(t) {
    0 > gt || ((t.current = ot[gt]), (ot[gt] = null), gt--);
  }
  function Q(t, e) {
    (gt++, (ot[gt] = t.current), (t.current = e));
  }
  var P = E(null),
    st = E(null),
    ct = E(null),
    Tt = E(null);
  function oe(t, e) {
    switch ((Q(ct, e), Q(st, t), Q(P, null), e.nodeType)) {
      case 9:
      case 11:
        t = (t = e.documentElement) && (t = t.namespaceURI) ? Rm(t) : 0;
        break;
      default:
        if (((t = e.tagName), (e = e.namespaceURI))) ((e = Rm(e)), (t = Om(e, t)));
        else
          switch (t) {
            case "svg":
              t = 1;
              break;
            case "math":
              t = 2;
              break;
            default:
              t = 0;
          }
    }
    (B(P), Q(P, t));
  }
  function Ht() {
    (B(P), B(st), B(ct));
  }
  function va(t) {
    t.memoizedState !== null && Q(Tt, t);
    var e = P.current,
      n = Om(e, t.type);
    e !== n && (Q(st, t), Q(P, n));
  }
  function Cl(t) {
    (st.current === t && (B(P), B(st)), Tt.current === t && (B(Tt), (ul._currentValue = Z)));
  }
  var gu, of;
  function $n(t) {
    if (gu === void 0)
      try {
        throw Error();
      } catch (n) {
        var e = n.stack.trim().match(/\n( *(at )?)/);
        ((gu = (e && e[1]) || ""),
          (of =
            -1 <
            n.stack.indexOf(`
    at`)
              ? " (<anonymous>)"
              : -1 < n.stack.indexOf("@")
                ? "@unknown:0:0"
                : ""));
      }
    return (
      `
` +
      gu +
      t +
      of
    );
  }
  var yu = !1;
  function vu(t, e) {
    if (!t || yu) return "";
    yu = !0;
    var n = Error.prepareStackTrace;
    Error.prepareStackTrace = void 0;
    try {
      var a = {
        DetermineComponentFrameRoot: function () {
          try {
            if (e) {
              var _ = function () {
                throw Error();
              };
              if (
                (Object.defineProperty(_.prototype, "props", {
                  set: function () {
                    throw Error();
                  },
                }),
                typeof Reflect == "object" && Reflect.construct)
              ) {
                try {
                  Reflect.construct(_, []);
                } catch (j) {
                  var z = j;
                }
                Reflect.construct(t, [], _);
              } else {
                try {
                  _.call();
                } catch (j) {
                  z = j;
                }
                t.call(_.prototype);
              }
            } else {
              try {
                throw Error();
              } catch (j) {
                z = j;
              }
              (_ = t()) && typeof _.catch == "function" && _.catch(function () {});
            }
          } catch (j) {
            if (j && z && typeof j.stack == "string") return [j.stack, z.stack];
          }
          return [null, null];
        },
      };
      a.DetermineComponentFrameRoot.displayName = "DetermineComponentFrameRoot";
      var s = Object.getOwnPropertyDescriptor(a.DetermineComponentFrameRoot, "name");
      s &&
        s.configurable &&
        Object.defineProperty(a.DetermineComponentFrameRoot, "name", {
          value: "DetermineComponentFrameRoot",
        });
      var r = a.DetermineComponentFrameRoot(),
        f = r[0],
        y = r[1];
      if (f && y) {
        var T = f.split(`
`),
          C = y.split(`
`);
        for (s = a = 0; a < T.length && !T[a].includes("DetermineComponentFrameRoot"); ) a++;
        for (; s < C.length && !C[s].includes("DetermineComponentFrameRoot"); ) s++;
        if (a === T.length || s === C.length)
          for (a = T.length - 1, s = C.length - 1; 1 <= a && 0 <= s && T[a] !== C[s]; ) s--;
        for (; 1 <= a && 0 <= s; a--, s--)
          if (T[a] !== C[s]) {
            if (a !== 1 || s !== 1)
              do
                if ((a--, s--, 0 > s || T[a] !== C[s])) {
                  var O =
                    `
` + T[a].replace(" at new ", " at ");
                  return (
                    t.displayName &&
                      O.includes("<anonymous>") &&
                      (O = O.replace("<anonymous>", t.displayName)),
                    O
                  );
                }
              while (1 <= a && 0 <= s);
            break;
          }
      }
    } finally {
      ((yu = !1), (Error.prepareStackTrace = n));
    }
    return (n = t ? t.displayName || t.name : "") ? $n(n) : "";
  }
  function Ry(t, e) {
    switch (t.tag) {
      case 26:
      case 27:
      case 5:
        return $n(t.type);
      case 16:
        return $n("Lazy");
      case 13:
        return t.child !== e && e !== null ? $n("Suspense Fallback") : $n("Suspense");
      case 19:
        return $n("SuspenseList");
      case 0:
      case 15:
        return vu(t.type, !1);
      case 11:
        return vu(t.type.render, !1);
      case 1:
        return vu(t.type, !0);
      case 31:
        return $n("Activity");
      default:
        return "";
    }
  }
  function rf(t) {
    try {
      var e = "",
        n = null;
      do ((e += Ry(t, n)), (n = t), (t = t.return));
      while (t);
      return e;
    } catch (a) {
      return (
        `
Error generating stack: ` +
        a.message +
        `
` +
        a.stack
      );
    }
  }
  var xu = Object.prototype.hasOwnProperty,
    bu = i.unstable_scheduleCallback,
    Su = i.unstable_cancelCallback,
    Oy = i.unstable_shouldYield,
    Vy = i.unstable_requestPaint,
    xe = i.unstable_now,
    _y = i.unstable_getCurrentPriorityLevel,
    cf = i.unstable_ImmediatePriority,
    ff = i.unstable_UserBlockingPriority,
    Nl = i.unstable_NormalPriority,
    By = i.unstable_LowPriority,
    df = i.unstable_IdlePriority,
    Uy = i.log,
    Ly = i.unstable_setDisableYieldValue,
    xa = null,
    be = null;
  function Sn(t) {
    if ((typeof Uy == "function" && Ly(t), be && typeof be.setStrictMode == "function"))
      try {
        be.setStrictMode(xa, t);
      } catch {}
  }
  var Se = Math.clz32 ? Math.clz32 : Yy,
    Hy = Math.log,
    qy = Math.LN2;
  function Yy(t) {
    return ((t >>>= 0), t === 0 ? 32 : (31 - ((Hy(t) / qy) | 0)) | 0);
  }
  var zl = 256,
    jl = 262144,
    wl = 4194304;
  function In(t) {
    var e = t & 42;
    if (e !== 0) return e;
    switch (t & -t) {
      case 1:
        return 1;
      case 2:
        return 2;
      case 4:
        return 4;
      case 8:
        return 8;
      case 16:
        return 16;
      case 32:
        return 32;
      case 64:
        return 64;
      case 128:
        return 128;
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
        return t & 261888;
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
        return t & 3932160;
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        return t & 62914560;
      case 67108864:
        return 67108864;
      case 134217728:
        return 134217728;
      case 268435456:
        return 268435456;
      case 536870912:
        return 536870912;
      case 1073741824:
        return 0;
      default:
        return t;
    }
  }
  function Rl(t, e, n) {
    var a = t.pendingLanes;
    if (a === 0) return 0;
    var s = 0,
      r = t.suspendedLanes,
      f = t.pingedLanes;
    t = t.warmLanes;
    var y = a & 134217727;
    return (
      y !== 0
        ? ((a = y & ~r),
          a !== 0
            ? (s = In(a))
            : ((f &= y), f !== 0 ? (s = In(f)) : n || ((n = y & ~t), n !== 0 && (s = In(n)))))
        : ((y = a & ~r),
          y !== 0
            ? (s = In(y))
            : f !== 0
              ? (s = In(f))
              : n || ((n = a & ~t), n !== 0 && (s = In(n)))),
      s === 0
        ? 0
        : e !== 0 &&
            e !== s &&
            (e & r) === 0 &&
            ((r = s & -s), (n = e & -e), r >= n || (r === 32 && (n & 4194048) !== 0))
          ? e
          : s
    );
  }
  function ba(t, e) {
    return (t.pendingLanes & ~(t.suspendedLanes & ~t.pingedLanes) & e) === 0;
  }
  function Gy(t, e) {
    switch (t) {
      case 1:
      case 2:
      case 4:
      case 8:
      case 64:
        return e + 250;
      case 16:
      case 32:
      case 128:
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
        return e + 5e3;
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        return -1;
      case 67108864:
      case 134217728:
      case 268435456:
      case 536870912:
      case 1073741824:
        return -1;
      default:
        return -1;
    }
  }
  function hf() {
    var t = wl;
    return ((wl <<= 1), (wl & 62914560) === 0 && (wl = 4194304), t);
  }
  function Tu(t) {
    for (var e = [], n = 0; 31 > n; n++) e.push(t);
    return e;
  }
  function Sa(t, e) {
    ((t.pendingLanes |= e),
      e !== 268435456 && ((t.suspendedLanes = 0), (t.pingedLanes = 0), (t.warmLanes = 0)));
  }
  function Xy(t, e, n, a, s, r) {
    var f = t.pendingLanes;
    ((t.pendingLanes = n),
      (t.suspendedLanes = 0),
      (t.pingedLanes = 0),
      (t.warmLanes = 0),
      (t.expiredLanes &= n),
      (t.entangledLanes &= n),
      (t.errorRecoveryDisabledLanes &= n),
      (t.shellSuspendCounter = 0));
    var y = t.entanglements,
      T = t.expirationTimes,
      C = t.hiddenUpdates;
    for (n = f & ~n; 0 < n; ) {
      var O = 31 - Se(n),
        _ = 1 << O;
      ((y[O] = 0), (T[O] = -1));
      var z = C[O];
      if (z !== null)
        for (C[O] = null, O = 0; O < z.length; O++) {
          var j = z[O];
          j !== null && (j.lane &= -536870913);
        }
      n &= ~_;
    }
    (a !== 0 && mf(t, a, 0),
      r !== 0 && s === 0 && t.tag !== 0 && (t.suspendedLanes |= r & ~(f & ~e)));
  }
  function mf(t, e, n) {
    ((t.pendingLanes |= e), (t.suspendedLanes &= ~e));
    var a = 31 - Se(e);
    ((t.entangledLanes |= e),
      (t.entanglements[a] = t.entanglements[a] | 1073741824 | (n & 261930)));
  }
  function pf(t, e) {
    var n = (t.entangledLanes |= e);
    for (t = t.entanglements; n; ) {
      var a = 31 - Se(n),
        s = 1 << a;
      ((s & e) | (t[a] & e) && (t[a] |= e), (n &= ~s));
    }
  }
  function gf(t, e) {
    var n = e & -e;
    return ((n = (n & 42) !== 0 ? 1 : Au(n)), (n & (t.suspendedLanes | e)) !== 0 ? 0 : n);
  }
  function Au(t) {
    switch (t) {
      case 2:
        t = 1;
        break;
      case 8:
        t = 4;
        break;
      case 32:
        t = 16;
        break;
      case 256:
      case 512:
      case 1024:
      case 2048:
      case 4096:
      case 8192:
      case 16384:
      case 32768:
      case 65536:
      case 131072:
      case 262144:
      case 524288:
      case 1048576:
      case 2097152:
      case 4194304:
      case 8388608:
      case 16777216:
      case 33554432:
        t = 128;
        break;
      case 268435456:
        t = 134217728;
        break;
      default:
        t = 0;
    }
    return t;
  }
  function Eu(t) {
    return ((t &= -t), 2 < t ? (8 < t ? ((t & 134217727) !== 0 ? 32 : 268435456) : 8) : 2);
  }
  function yf() {
    var t = X.p;
    return t !== 0 ? t : ((t = window.event), t === void 0 ? 32 : np(t.type));
  }
  function vf(t, e) {
    var n = X.p;
    try {
      return ((X.p = t), e());
    } finally {
      X.p = n;
    }
  }
  var Tn = Math.random().toString(36).slice(2),
    It = "__reactFiber$" + Tn,
    de = "__reactProps$" + Tn,
    Ai = "__reactContainer$" + Tn,
    Mu = "__reactEvents$" + Tn,
    ky = "__reactListeners$" + Tn,
    Qy = "__reactHandles$" + Tn,
    xf = "__reactResources$" + Tn,
    Ta = "__reactMarker$" + Tn;
  function Du(t) {
    (delete t[It], delete t[de], delete t[Mu], delete t[ky], delete t[Qy]);
  }
  function Ei(t) {
    var e = t[It];
    if (e) return e;
    for (var n = t.parentNode; n; ) {
      if ((e = n[Ai] || n[It])) {
        if (((n = e.alternate), e.child !== null || (n !== null && n.child !== null)))
          for (t = qm(t); t !== null; ) {
            if ((n = t[It])) return n;
            t = qm(t);
          }
        return e;
      }
      ((t = n), (n = t.parentNode));
    }
    return null;
  }
  function Mi(t) {
    if ((t = t[It] || t[Ai])) {
      var e = t.tag;
      if (e === 5 || e === 6 || e === 13 || e === 31 || e === 26 || e === 27 || e === 3) return t;
    }
    return null;
  }
  function Aa(t) {
    var e = t.tag;
    if (e === 5 || e === 26 || e === 27 || e === 6) return t.stateNode;
    throw Error(o(33));
  }
  function Di(t) {
    var e = t[xf];
    return (e || (e = t[xf] = { hoistableStyles: new Map(), hoistableScripts: new Map() }), e);
  }
  function Pt(t) {
    t[Ta] = !0;
  }
  var bf = new Set(),
    Sf = {};
  function ti(t, e) {
    (Ci(t, e), Ci(t + "Capture", e));
  }
  function Ci(t, e) {
    for (Sf[t] = e, t = 0; t < e.length; t++) bf.add(e[t]);
  }
  var Zy = RegExp(
      "^[:A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD][:A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD\\-.0-9\\u00B7\\u0300-\\u036F\\u203F-\\u2040]*$",
    ),
    Tf = {},
    Af = {};
  function Ky(t) {
    return xu.call(Af, t)
      ? !0
      : xu.call(Tf, t)
        ? !1
        : Zy.test(t)
          ? (Af[t] = !0)
          : ((Tf[t] = !0), !1);
  }
  function Ol(t, e, n) {
    if (Ky(e))
      if (n === null) t.removeAttribute(e);
      else {
        switch (typeof n) {
          case "undefined":
          case "function":
          case "symbol":
            t.removeAttribute(e);
            return;
          case "boolean":
            var a = e.toLowerCase().slice(0, 5);
            if (a !== "data-" && a !== "aria-") {
              t.removeAttribute(e);
              return;
            }
        }
        t.setAttribute(e, "" + n);
      }
  }
  function Vl(t, e, n) {
    if (n === null) t.removeAttribute(e);
    else {
      switch (typeof n) {
        case "undefined":
        case "function":
        case "symbol":
        case "boolean":
          t.removeAttribute(e);
          return;
      }
      t.setAttribute(e, "" + n);
    }
  }
  function nn(t, e, n, a) {
    if (a === null) t.removeAttribute(n);
    else {
      switch (typeof a) {
        case "undefined":
        case "function":
        case "symbol":
        case "boolean":
          t.removeAttribute(n);
          return;
      }
      t.setAttributeNS(e, n, "" + a);
    }
  }
  function we(t) {
    switch (typeof t) {
      case "bigint":
      case "boolean":
      case "number":
      case "string":
      case "undefined":
        return t;
      case "object":
        return t;
      default:
        return "";
    }
  }
  function Ef(t) {
    var e = t.type;
    return (t = t.nodeName) && t.toLowerCase() === "input" && (e === "checkbox" || e === "radio");
  }
  function Jy(t, e, n) {
    var a = Object.getOwnPropertyDescriptor(t.constructor.prototype, e);
    if (
      !t.hasOwnProperty(e) &&
      typeof a < "u" &&
      typeof a.get == "function" &&
      typeof a.set == "function"
    ) {
      var s = a.get,
        r = a.set;
      return (
        Object.defineProperty(t, e, {
          configurable: !0,
          get: function () {
            return s.call(this);
          },
          set: function (f) {
            ((n = "" + f), r.call(this, f));
          },
        }),
        Object.defineProperty(t, e, { enumerable: a.enumerable }),
        {
          getValue: function () {
            return n;
          },
          setValue: function (f) {
            n = "" + f;
          },
          stopTracking: function () {
            ((t._valueTracker = null), delete t[e]);
          },
        }
      );
    }
  }
  function Cu(t) {
    if (!t._valueTracker) {
      var e = Ef(t) ? "checked" : "value";
      t._valueTracker = Jy(t, e, "" + t[e]);
    }
  }
  function Mf(t) {
    if (!t) return !1;
    var e = t._valueTracker;
    if (!e) return !0;
    var n = e.getValue(),
      a = "";
    return (
      t && (a = Ef(t) ? (t.checked ? "true" : "false") : t.value),
      (t = a),
      t !== n ? (e.setValue(t), !0) : !1
    );
  }
  function _l(t) {
    if (((t = t || (typeof document < "u" ? document : void 0)), typeof t > "u")) return null;
    try {
      return t.activeElement || t.body;
    } catch {
      return t.body;
    }
  }
  var Fy = /[\n"\\]/g;
  function Re(t) {
    return t.replace(Fy, function (e) {
      return "\\" + e.charCodeAt(0).toString(16) + " ";
    });
  }
  function Nu(t, e, n, a, s, r, f, y) {
    ((t.name = ""),
      f != null && typeof f != "function" && typeof f != "symbol" && typeof f != "boolean"
        ? (t.type = f)
        : t.removeAttribute("type"),
      e != null
        ? f === "number"
          ? ((e === 0 && t.value === "") || t.value != e) && (t.value = "" + we(e))
          : t.value !== "" + we(e) && (t.value = "" + we(e))
        : (f !== "submit" && f !== "reset") || t.removeAttribute("value"),
      e != null
        ? zu(t, f, we(e))
        : n != null
          ? zu(t, f, we(n))
          : a != null && t.removeAttribute("value"),
      s == null && r != null && (t.defaultChecked = !!r),
      s != null && (t.checked = s && typeof s != "function" && typeof s != "symbol"),
      y != null && typeof y != "function" && typeof y != "symbol" && typeof y != "boolean"
        ? (t.name = "" + we(y))
        : t.removeAttribute("name"));
  }
  function Df(t, e, n, a, s, r, f, y) {
    if (
      (r != null &&
        typeof r != "function" &&
        typeof r != "symbol" &&
        typeof r != "boolean" &&
        (t.type = r),
      e != null || n != null)
    ) {
      if (!((r !== "submit" && r !== "reset") || e != null)) {
        Cu(t);
        return;
      }
      ((n = n != null ? "" + we(n) : ""),
        (e = e != null ? "" + we(e) : n),
        y || e === t.value || (t.value = e),
        (t.defaultValue = e));
    }
    ((a = a ?? s),
      (a = typeof a != "function" && typeof a != "symbol" && !!a),
      (t.checked = y ? t.checked : !!a),
      (t.defaultChecked = !!a),
      f != null &&
        typeof f != "function" &&
        typeof f != "symbol" &&
        typeof f != "boolean" &&
        (t.name = f),
      Cu(t));
  }
  function zu(t, e, n) {
    (e === "number" && _l(t.ownerDocument) === t) ||
      t.defaultValue === "" + n ||
      (t.defaultValue = "" + n);
  }
  function Ni(t, e, n, a) {
    if (((t = t.options), e)) {
      e = {};
      for (var s = 0; s < n.length; s++) e["$" + n[s]] = !0;
      for (n = 0; n < t.length; n++)
        ((s = e.hasOwnProperty("$" + t[n].value)),
          t[n].selected !== s && (t[n].selected = s),
          s && a && (t[n].defaultSelected = !0));
    } else {
      for (n = "" + we(n), e = null, s = 0; s < t.length; s++) {
        if (t[s].value === n) {
          ((t[s].selected = !0), a && (t[s].defaultSelected = !0));
          return;
        }
        e !== null || t[s].disabled || (e = t[s]);
      }
      e !== null && (e.selected = !0);
    }
  }
  function Cf(t, e, n) {
    if (e != null && ((e = "" + we(e)), e !== t.value && (t.value = e), n == null)) {
      t.defaultValue !== e && (t.defaultValue = e);
      return;
    }
    t.defaultValue = n != null ? "" + we(n) : "";
  }
  function Nf(t, e, n, a) {
    if (e == null) {
      if (a != null) {
        if (n != null) throw Error(o(92));
        if (Lt(a)) {
          if (1 < a.length) throw Error(o(93));
          a = a[0];
        }
        n = a;
      }
      (n == null && (n = ""), (e = n));
    }
    ((n = we(e)),
      (t.defaultValue = n),
      (a = t.textContent),
      a === n && a !== "" && a !== null && (t.value = a),
      Cu(t));
  }
  function zi(t, e) {
    if (e) {
      var n = t.firstChild;
      if (n && n === t.lastChild && n.nodeType === 3) {
        n.nodeValue = e;
        return;
      }
    }
    t.textContent = e;
  }
  var Py = new Set(
    "animationIterationCount aspectRatio borderImageOutset borderImageSlice borderImageWidth boxFlex boxFlexGroup boxOrdinalGroup columnCount columns flex flexGrow flexPositive flexShrink flexNegative flexOrder gridArea gridRow gridRowEnd gridRowSpan gridRowStart gridColumn gridColumnEnd gridColumnSpan gridColumnStart fontWeight lineClamp lineHeight opacity order orphans scale tabSize widows zIndex zoom fillOpacity floodOpacity stopOpacity strokeDasharray strokeDashoffset strokeMiterlimit strokeOpacity strokeWidth MozAnimationIterationCount MozBoxFlex MozBoxFlexGroup MozLineClamp msAnimationIterationCount msFlex msZoom msFlexGrow msFlexNegative msFlexOrder msFlexPositive msFlexShrink msGridColumn msGridColumnSpan msGridRow msGridRowSpan WebkitAnimationIterationCount WebkitBoxFlex WebKitBoxFlexGroup WebkitBoxOrdinalGroup WebkitColumnCount WebkitColumns WebkitFlex WebkitFlexGrow WebkitFlexPositive WebkitFlexShrink WebkitLineClamp".split(
      " ",
    ),
  );
  function zf(t, e, n) {
    var a = e.indexOf("--") === 0;
    n == null || typeof n == "boolean" || n === ""
      ? a
        ? t.setProperty(e, "")
        : e === "float"
          ? (t.cssFloat = "")
          : (t[e] = "")
      : a
        ? t.setProperty(e, n)
        : typeof n != "number" || n === 0 || Py.has(e)
          ? e === "float"
            ? (t.cssFloat = n)
            : (t[e] = ("" + n).trim())
          : (t[e] = n + "px");
  }
  function jf(t, e, n) {
    if (e != null && typeof e != "object") throw Error(o(62));
    if (((t = t.style), n != null)) {
      for (var a in n)
        !n.hasOwnProperty(a) ||
          (e != null && e.hasOwnProperty(a)) ||
          (a.indexOf("--") === 0
            ? t.setProperty(a, "")
            : a === "float"
              ? (t.cssFloat = "")
              : (t[a] = ""));
      for (var s in e) ((a = e[s]), e.hasOwnProperty(s) && n[s] !== a && zf(t, s, a));
    } else for (var r in e) e.hasOwnProperty(r) && zf(t, r, e[r]);
  }
  function ju(t) {
    if (t.indexOf("-") === -1) return !1;
    switch (t) {
      case "annotation-xml":
      case "color-profile":
      case "font-face":
      case "font-face-src":
      case "font-face-uri":
      case "font-face-format":
      case "font-face-name":
      case "missing-glyph":
        return !1;
      default:
        return !0;
    }
  }
  var Wy = new Map([
      ["acceptCharset", "accept-charset"],
      ["htmlFor", "for"],
      ["httpEquiv", "http-equiv"],
      ["crossOrigin", "crossorigin"],
      ["accentHeight", "accent-height"],
      ["alignmentBaseline", "alignment-baseline"],
      ["arabicForm", "arabic-form"],
      ["baselineShift", "baseline-shift"],
      ["capHeight", "cap-height"],
      ["clipPath", "clip-path"],
      ["clipRule", "clip-rule"],
      ["colorInterpolation", "color-interpolation"],
      ["colorInterpolationFilters", "color-interpolation-filters"],
      ["colorProfile", "color-profile"],
      ["colorRendering", "color-rendering"],
      ["dominantBaseline", "dominant-baseline"],
      ["enableBackground", "enable-background"],
      ["fillOpacity", "fill-opacity"],
      ["fillRule", "fill-rule"],
      ["floodColor", "flood-color"],
      ["floodOpacity", "flood-opacity"],
      ["fontFamily", "font-family"],
      ["fontSize", "font-size"],
      ["fontSizeAdjust", "font-size-adjust"],
      ["fontStretch", "font-stretch"],
      ["fontStyle", "font-style"],
      ["fontVariant", "font-variant"],
      ["fontWeight", "font-weight"],
      ["glyphName", "glyph-name"],
      ["glyphOrientationHorizontal", "glyph-orientation-horizontal"],
      ["glyphOrientationVertical", "glyph-orientation-vertical"],
      ["horizAdvX", "horiz-adv-x"],
      ["horizOriginX", "horiz-origin-x"],
      ["imageRendering", "image-rendering"],
      ["letterSpacing", "letter-spacing"],
      ["lightingColor", "lighting-color"],
      ["markerEnd", "marker-end"],
      ["markerMid", "marker-mid"],
      ["markerStart", "marker-start"],
      ["overlinePosition", "overline-position"],
      ["overlineThickness", "overline-thickness"],
      ["paintOrder", "paint-order"],
      ["panose-1", "panose-1"],
      ["pointerEvents", "pointer-events"],
      ["renderingIntent", "rendering-intent"],
      ["shapeRendering", "shape-rendering"],
      ["stopColor", "stop-color"],
      ["stopOpacity", "stop-opacity"],
      ["strikethroughPosition", "strikethrough-position"],
      ["strikethroughThickness", "strikethrough-thickness"],
      ["strokeDasharray", "stroke-dasharray"],
      ["strokeDashoffset", "stroke-dashoffset"],
      ["strokeLinecap", "stroke-linecap"],
      ["strokeLinejoin", "stroke-linejoin"],
      ["strokeMiterlimit", "stroke-miterlimit"],
      ["strokeOpacity", "stroke-opacity"],
      ["strokeWidth", "stroke-width"],
      ["textAnchor", "text-anchor"],
      ["textDecoration", "text-decoration"],
      ["textRendering", "text-rendering"],
      ["transformOrigin", "transform-origin"],
      ["underlinePosition", "underline-position"],
      ["underlineThickness", "underline-thickness"],
      ["unicodeBidi", "unicode-bidi"],
      ["unicodeRange", "unicode-range"],
      ["unitsPerEm", "units-per-em"],
      ["vAlphabetic", "v-alphabetic"],
      ["vHanging", "v-hanging"],
      ["vIdeographic", "v-ideographic"],
      ["vMathematical", "v-mathematical"],
      ["vectorEffect", "vector-effect"],
      ["vertAdvY", "vert-adv-y"],
      ["vertOriginX", "vert-origin-x"],
      ["vertOriginY", "vert-origin-y"],
      ["wordSpacing", "word-spacing"],
      ["writingMode", "writing-mode"],
      ["xmlnsXlink", "xmlns:xlink"],
      ["xHeight", "x-height"],
    ]),
    $y =
      /^[\u0000-\u001F ]*j[\r\n\t]*a[\r\n\t]*v[\r\n\t]*a[\r\n\t]*s[\r\n\t]*c[\r\n\t]*r[\r\n\t]*i[\r\n\t]*p[\r\n\t]*t[\r\n\t]*:/i;
  function Bl(t) {
    return $y.test("" + t)
      ? "javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')"
      : t;
  }
  function an() {}
  var wu = null;
  function Ru(t) {
    return (
      (t = t.target || t.srcElement || window),
      t.correspondingUseElement && (t = t.correspondingUseElement),
      t.nodeType === 3 ? t.parentNode : t
    );
  }
  var ji = null,
    wi = null;
  function wf(t) {
    var e = Mi(t);
    if (e && (t = e.stateNode)) {
      var n = t[de] || null;
      t: switch (((t = e.stateNode), e.type)) {
        case "input":
          if (
            (Nu(
              t,
              n.value,
              n.defaultValue,
              n.defaultValue,
              n.checked,
              n.defaultChecked,
              n.type,
              n.name,
            ),
            (e = n.name),
            n.type === "radio" && e != null)
          ) {
            for (n = t; n.parentNode; ) n = n.parentNode;
            for (
              n = n.querySelectorAll('input[name="' + Re("" + e) + '"][type="radio"]'), e = 0;
              e < n.length;
              e++
            ) {
              var a = n[e];
              if (a !== t && a.form === t.form) {
                var s = a[de] || null;
                if (!s) throw Error(o(90));
                Nu(
                  a,
                  s.value,
                  s.defaultValue,
                  s.defaultValue,
                  s.checked,
                  s.defaultChecked,
                  s.type,
                  s.name,
                );
              }
            }
            for (e = 0; e < n.length; e++) ((a = n[e]), a.form === t.form && Mf(a));
          }
          break t;
        case "textarea":
          Cf(t, n.value, n.defaultValue);
          break t;
        case "select":
          ((e = n.value), e != null && Ni(t, !!n.multiple, e, !1));
      }
    }
  }
  var Ou = !1;
  function Rf(t, e, n) {
    if (Ou) return t(e, n);
    Ou = !0;
    try {
      var a = t(e);
      return a;
    } finally {
      if (
        ((Ou = !1),
        (ji !== null || wi !== null) &&
          (As(), ji && ((e = ji), (t = wi), (wi = ji = null), wf(e), t)))
      )
        for (e = 0; e < t.length; e++) wf(t[e]);
    }
  }
  function Ea(t, e) {
    var n = t.stateNode;
    if (n === null) return null;
    var a = n[de] || null;
    if (a === null) return null;
    n = a[e];
    t: switch (e) {
      case "onClick":
      case "onClickCapture":
      case "onDoubleClick":
      case "onDoubleClickCapture":
      case "onMouseDown":
      case "onMouseDownCapture":
      case "onMouseMove":
      case "onMouseMoveCapture":
      case "onMouseUp":
      case "onMouseUpCapture":
      case "onMouseEnter":
        ((a = !a.disabled) ||
          ((t = t.type),
          (a = !(t === "button" || t === "input" || t === "select" || t === "textarea"))),
          (t = !a));
        break t;
      default:
        t = !1;
    }
    if (t) return null;
    if (n && typeof n != "function") throw Error(o(231, e, typeof n));
    return n;
  }
  var ln = !(
      typeof window > "u" ||
      typeof window.document > "u" ||
      typeof window.document.createElement > "u"
    ),
    Vu = !1;
  if (ln)
    try {
      var Ma = {};
      (Object.defineProperty(Ma, "passive", {
        get: function () {
          Vu = !0;
        },
      }),
        window.addEventListener("test", Ma, Ma),
        window.removeEventListener("test", Ma, Ma));
    } catch {
      Vu = !1;
    }
  var An = null,
    _u = null,
    Ul = null;
  function Of() {
    if (Ul) return Ul;
    var t,
      e = _u,
      n = e.length,
      a,
      s = "value" in An ? An.value : An.textContent,
      r = s.length;
    for (t = 0; t < n && e[t] === s[t]; t++);
    var f = n - t;
    for (a = 1; a <= f && e[n - a] === s[r - a]; a++);
    return (Ul = s.slice(t, 1 < a ? 1 - a : void 0));
  }
  function Ll(t) {
    var e = t.keyCode;
    return (
      "charCode" in t ? ((t = t.charCode), t === 0 && e === 13 && (t = 13)) : (t = e),
      t === 10 && (t = 13),
      32 <= t || t === 13 ? t : 0
    );
  }
  function Hl() {
    return !0;
  }
  function Vf() {
    return !1;
  }
  function he(t) {
    function e(n, a, s, r, f) {
      ((this._reactName = n),
        (this._targetInst = s),
        (this.type = a),
        (this.nativeEvent = r),
        (this.target = f),
        (this.currentTarget = null));
      for (var y in t) t.hasOwnProperty(y) && ((n = t[y]), (this[y] = n ? n(r) : r[y]));
      return (
        (this.isDefaultPrevented = (
          r.defaultPrevented != null ? r.defaultPrevented : r.returnValue === !1
        )
          ? Hl
          : Vf),
        (this.isPropagationStopped = Vf),
        this
      );
    }
    return (
      x(e.prototype, {
        preventDefault: function () {
          this.defaultPrevented = !0;
          var n = this.nativeEvent;
          n &&
            (n.preventDefault
              ? n.preventDefault()
              : typeof n.returnValue != "unknown" && (n.returnValue = !1),
            (this.isDefaultPrevented = Hl));
        },
        stopPropagation: function () {
          var n = this.nativeEvent;
          n &&
            (n.stopPropagation
              ? n.stopPropagation()
              : typeof n.cancelBubble != "unknown" && (n.cancelBubble = !0),
            (this.isPropagationStopped = Hl));
        },
        persist: function () {},
        isPersistent: Hl,
      }),
      e
    );
  }
  var ei = {
      eventPhase: 0,
      bubbles: 0,
      cancelable: 0,
      timeStamp: function (t) {
        return t.timeStamp || Date.now();
      },
      defaultPrevented: 0,
      isTrusted: 0,
    },
    ql = he(ei),
    Da = x({}, ei, { view: 0, detail: 0 }),
    Iy = he(Da),
    Bu,
    Uu,
    Ca,
    Yl = x({}, Da, {
      screenX: 0,
      screenY: 0,
      clientX: 0,
      clientY: 0,
      pageX: 0,
      pageY: 0,
      ctrlKey: 0,
      shiftKey: 0,
      altKey: 0,
      metaKey: 0,
      getModifierState: Hu,
      button: 0,
      buttons: 0,
      relatedTarget: function (t) {
        return t.relatedTarget === void 0
          ? t.fromElement === t.srcElement
            ? t.toElement
            : t.fromElement
          : t.relatedTarget;
      },
      movementX: function (t) {
        return "movementX" in t
          ? t.movementX
          : (t !== Ca &&
              (Ca && t.type === "mousemove"
                ? ((Bu = t.screenX - Ca.screenX), (Uu = t.screenY - Ca.screenY))
                : (Uu = Bu = 0),
              (Ca = t)),
            Bu);
      },
      movementY: function (t) {
        return "movementY" in t ? t.movementY : Uu;
      },
    }),
    _f = he(Yl),
    tv = x({}, Yl, { dataTransfer: 0 }),
    ev = he(tv),
    nv = x({}, Da, { relatedTarget: 0 }),
    Lu = he(nv),
    iv = x({}, ei, { animationName: 0, elapsedTime: 0, pseudoElement: 0 }),
    av = he(iv),
    lv = x({}, ei, {
      clipboardData: function (t) {
        return "clipboardData" in t ? t.clipboardData : window.clipboardData;
      },
    }),
    sv = he(lv),
    uv = x({}, ei, { data: 0 }),
    Bf = he(uv),
    ov = {
      Esc: "Escape",
      Spacebar: " ",
      Left: "ArrowLeft",
      Up: "ArrowUp",
      Right: "ArrowRight",
      Down: "ArrowDown",
      Del: "Delete",
      Win: "OS",
      Menu: "ContextMenu",
      Apps: "ContextMenu",
      Scroll: "ScrollLock",
      MozPrintableKey: "Unidentified",
    },
    rv = {
      8: "Backspace",
      9: "Tab",
      12: "Clear",
      13: "Enter",
      16: "Shift",
      17: "Control",
      18: "Alt",
      19: "Pause",
      20: "CapsLock",
      27: "Escape",
      32: " ",
      33: "PageUp",
      34: "PageDown",
      35: "End",
      36: "Home",
      37: "ArrowLeft",
      38: "ArrowUp",
      39: "ArrowRight",
      40: "ArrowDown",
      45: "Insert",
      46: "Delete",
      112: "F1",
      113: "F2",
      114: "F3",
      115: "F4",
      116: "F5",
      117: "F6",
      118: "F7",
      119: "F8",
      120: "F9",
      121: "F10",
      122: "F11",
      123: "F12",
      144: "NumLock",
      145: "ScrollLock",
      224: "Meta",
    },
    cv = { Alt: "altKey", Control: "ctrlKey", Meta: "metaKey", Shift: "shiftKey" };
  function fv(t) {
    var e = this.nativeEvent;
    return e.getModifierState ? e.getModifierState(t) : (t = cv[t]) ? !!e[t] : !1;
  }
  function Hu() {
    return fv;
  }
  var dv = x({}, Da, {
      key: function (t) {
        if (t.key) {
          var e = ov[t.key] || t.key;
          if (e !== "Unidentified") return e;
        }
        return t.type === "keypress"
          ? ((t = Ll(t)), t === 13 ? "Enter" : String.fromCharCode(t))
          : t.type === "keydown" || t.type === "keyup"
            ? rv[t.keyCode] || "Unidentified"
            : "";
      },
      code: 0,
      location: 0,
      ctrlKey: 0,
      shiftKey: 0,
      altKey: 0,
      metaKey: 0,
      repeat: 0,
      locale: 0,
      getModifierState: Hu,
      charCode: function (t) {
        return t.type === "keypress" ? Ll(t) : 0;
      },
      keyCode: function (t) {
        return t.type === "keydown" || t.type === "keyup" ? t.keyCode : 0;
      },
      which: function (t) {
        return t.type === "keypress"
          ? Ll(t)
          : t.type === "keydown" || t.type === "keyup"
            ? t.keyCode
            : 0;
      },
    }),
    hv = he(dv),
    mv = x({}, Yl, {
      pointerId: 0,
      width: 0,
      height: 0,
      pressure: 0,
      tangentialPressure: 0,
      tiltX: 0,
      tiltY: 0,
      twist: 0,
      pointerType: 0,
      isPrimary: 0,
    }),
    Uf = he(mv),
    pv = x({}, Da, {
      touches: 0,
      targetTouches: 0,
      changedTouches: 0,
      altKey: 0,
      metaKey: 0,
      ctrlKey: 0,
      shiftKey: 0,
      getModifierState: Hu,
    }),
    gv = he(pv),
    yv = x({}, ei, { propertyName: 0, elapsedTime: 0, pseudoElement: 0 }),
    vv = he(yv),
    xv = x({}, Yl, {
      deltaX: function (t) {
        return "deltaX" in t ? t.deltaX : "wheelDeltaX" in t ? -t.wheelDeltaX : 0;
      },
      deltaY: function (t) {
        return "deltaY" in t
          ? t.deltaY
          : "wheelDeltaY" in t
            ? -t.wheelDeltaY
            : "wheelDelta" in t
              ? -t.wheelDelta
              : 0;
      },
      deltaZ: 0,
      deltaMode: 0,
    }),
    bv = he(xv),
    Sv = x({}, ei, { newState: 0, oldState: 0 }),
    Tv = he(Sv),
    Av = [9, 13, 27, 32],
    qu = ln && "CompositionEvent" in window,
    Na = null;
  ln && "documentMode" in document && (Na = document.documentMode);
  var Ev = ln && "TextEvent" in window && !Na,
    Lf = ln && (!qu || (Na && 8 < Na && 11 >= Na)),
    Hf = " ",
    qf = !1;
  function Yf(t, e) {
    switch (t) {
      case "keyup":
        return Av.indexOf(e.keyCode) !== -1;
      case "keydown":
        return e.keyCode !== 229;
      case "keypress":
      case "mousedown":
      case "focusout":
        return !0;
      default:
        return !1;
    }
  }
  function Gf(t) {
    return ((t = t.detail), typeof t == "object" && "data" in t ? t.data : null);
  }
  var Ri = !1;
  function Mv(t, e) {
    switch (t) {
      case "compositionend":
        return Gf(e);
      case "keypress":
        return e.which !== 32 ? null : ((qf = !0), Hf);
      case "textInput":
        return ((t = e.data), t === Hf && qf ? null : t);
      default:
        return null;
    }
  }
  function Dv(t, e) {
    if (Ri)
      return t === "compositionend" || (!qu && Yf(t, e))
        ? ((t = Of()), (Ul = _u = An = null), (Ri = !1), t)
        : null;
    switch (t) {
      case "paste":
        return null;
      case "keypress":
        if (!(e.ctrlKey || e.altKey || e.metaKey) || (e.ctrlKey && e.altKey)) {
          if (e.char && 1 < e.char.length) return e.char;
          if (e.which) return String.fromCharCode(e.which);
        }
        return null;
      case "compositionend":
        return Lf && e.locale !== "ko" ? null : e.data;
      default:
        return null;
    }
  }
  var Cv = {
    color: !0,
    date: !0,
    datetime: !0,
    "datetime-local": !0,
    email: !0,
    month: !0,
    number: !0,
    password: !0,
    range: !0,
    search: !0,
    tel: !0,
    text: !0,
    time: !0,
    url: !0,
    week: !0,
  };
  function Xf(t) {
    var e = t && t.nodeName && t.nodeName.toLowerCase();
    return e === "input" ? !!Cv[t.type] : e === "textarea";
  }
  function kf(t, e, n, a) {
    (ji ? (wi ? wi.push(a) : (wi = [a])) : (ji = a),
      (e = js(e, "onChange")),
      0 < e.length &&
        ((n = new ql("onChange", "change", null, n, a)), t.push({ event: n, listeners: e })));
  }
  var za = null,
    ja = null;
  function Nv(t) {
    Dm(t, 0);
  }
  function Gl(t) {
    var e = Aa(t);
    if (Mf(e)) return t;
  }
  function Qf(t, e) {
    if (t === "change") return e;
  }
  var Zf = !1;
  if (ln) {
    var Yu;
    if (ln) {
      var Gu = "oninput" in document;
      if (!Gu) {
        var Kf = document.createElement("div");
        (Kf.setAttribute("oninput", "return;"), (Gu = typeof Kf.oninput == "function"));
      }
      Yu = Gu;
    } else Yu = !1;
    Zf = Yu && (!document.documentMode || 9 < document.documentMode);
  }
  function Jf() {
    za && (za.detachEvent("onpropertychange", Ff), (ja = za = null));
  }
  function Ff(t) {
    if (t.propertyName === "value" && Gl(ja)) {
      var e = [];
      (kf(e, ja, t, Ru(t)), Rf(Nv, e));
    }
  }
  function zv(t, e, n) {
    t === "focusin"
      ? (Jf(), (za = e), (ja = n), za.attachEvent("onpropertychange", Ff))
      : t === "focusout" && Jf();
  }
  function jv(t) {
    if (t === "selectionchange" || t === "keyup" || t === "keydown") return Gl(ja);
  }
  function wv(t, e) {
    if (t === "click") return Gl(e);
  }
  function Rv(t, e) {
    if (t === "input" || t === "change") return Gl(e);
  }
  function Ov(t, e) {
    return (t === e && (t !== 0 || 1 / t === 1 / e)) || (t !== t && e !== e);
  }
  var Te = typeof Object.is == "function" ? Object.is : Ov;
  function wa(t, e) {
    if (Te(t, e)) return !0;
    if (typeof t != "object" || t === null || typeof e != "object" || e === null) return !1;
    var n = Object.keys(t),
      a = Object.keys(e);
    if (n.length !== a.length) return !1;
    for (a = 0; a < n.length; a++) {
      var s = n[a];
      if (!xu.call(e, s) || !Te(t[s], e[s])) return !1;
    }
    return !0;
  }
  function Pf(t) {
    for (; t && t.firstChild; ) t = t.firstChild;
    return t;
  }
  function Wf(t, e) {
    var n = Pf(t);
    t = 0;
    for (var a; n; ) {
      if (n.nodeType === 3) {
        if (((a = t + n.textContent.length), t <= e && a >= e)) return { node: n, offset: e - t };
        t = a;
      }
      t: {
        for (; n; ) {
          if (n.nextSibling) {
            n = n.nextSibling;
            break t;
          }
          n = n.parentNode;
        }
        n = void 0;
      }
      n = Pf(n);
    }
  }
  function $f(t, e) {
    return t && e
      ? t === e
        ? !0
        : t && t.nodeType === 3
          ? !1
          : e && e.nodeType === 3
            ? $f(t, e.parentNode)
            : "contains" in t
              ? t.contains(e)
              : t.compareDocumentPosition
                ? !!(t.compareDocumentPosition(e) & 16)
                : !1
      : !1;
  }
  function If(t) {
    t =
      t != null && t.ownerDocument != null && t.ownerDocument.defaultView != null
        ? t.ownerDocument.defaultView
        : window;
    for (var e = _l(t.document); e instanceof t.HTMLIFrameElement; ) {
      try {
        var n = typeof e.contentWindow.location.href == "string";
      } catch {
        n = !1;
      }
      if (n) t = e.contentWindow;
      else break;
      e = _l(t.document);
    }
    return e;
  }
  function Xu(t) {
    var e = t && t.nodeName && t.nodeName.toLowerCase();
    return (
      e &&
      ((e === "input" &&
        (t.type === "text" ||
          t.type === "search" ||
          t.type === "tel" ||
          t.type === "url" ||
          t.type === "password")) ||
        e === "textarea" ||
        t.contentEditable === "true")
    );
  }
  var Vv = ln && "documentMode" in document && 11 >= document.documentMode,
    Oi = null,
    ku = null,
    Ra = null,
    Qu = !1;
  function td(t, e, n) {
    var a = n.window === n ? n.document : n.nodeType === 9 ? n : n.ownerDocument;
    Qu ||
      Oi == null ||
      Oi !== _l(a) ||
      ((a = Oi),
      "selectionStart" in a && Xu(a)
        ? (a = { start: a.selectionStart, end: a.selectionEnd })
        : ((a = ((a.ownerDocument && a.ownerDocument.defaultView) || window).getSelection()),
          (a = {
            anchorNode: a.anchorNode,
            anchorOffset: a.anchorOffset,
            focusNode: a.focusNode,
            focusOffset: a.focusOffset,
          })),
      (Ra && wa(Ra, a)) ||
        ((Ra = a),
        (a = js(ku, "onSelect")),
        0 < a.length &&
          ((e = new ql("onSelect", "select", null, e, n)),
          t.push({ event: e, listeners: a }),
          (e.target = Oi))));
  }
  function ni(t, e) {
    var n = {};
    return (
      (n[t.toLowerCase()] = e.toLowerCase()),
      (n["Webkit" + t] = "webkit" + e),
      (n["Moz" + t] = "moz" + e),
      n
    );
  }
  var Vi = {
      animationend: ni("Animation", "AnimationEnd"),
      animationiteration: ni("Animation", "AnimationIteration"),
      animationstart: ni("Animation", "AnimationStart"),
      transitionrun: ni("Transition", "TransitionRun"),
      transitionstart: ni("Transition", "TransitionStart"),
      transitioncancel: ni("Transition", "TransitionCancel"),
      transitionend: ni("Transition", "TransitionEnd"),
    },
    Zu = {},
    ed = {};
  ln &&
    ((ed = document.createElement("div").style),
    "AnimationEvent" in window ||
      (delete Vi.animationend.animation,
      delete Vi.animationiteration.animation,
      delete Vi.animationstart.animation),
    "TransitionEvent" in window || delete Vi.transitionend.transition);
  function ii(t) {
    if (Zu[t]) return Zu[t];
    if (!Vi[t]) return t;
    var e = Vi[t],
      n;
    for (n in e) if (e.hasOwnProperty(n) && n in ed) return (Zu[t] = e[n]);
    return t;
  }
  var nd = ii("animationend"),
    id = ii("animationiteration"),
    ad = ii("animationstart"),
    _v = ii("transitionrun"),
    Bv = ii("transitionstart"),
    Uv = ii("transitioncancel"),
    ld = ii("transitionend"),
    sd = new Map(),
    Ku =
      "abort auxClick beforeToggle cancel canPlay canPlayThrough click close contextMenu copy cut drag dragEnd dragEnter dragExit dragLeave dragOver dragStart drop durationChange emptied encrypted ended error gotPointerCapture input invalid keyDown keyPress keyUp load loadedData loadedMetadata loadStart lostPointerCapture mouseDown mouseMove mouseOut mouseOver mouseUp paste pause play playing pointerCancel pointerDown pointerMove pointerOut pointerOver pointerUp progress rateChange reset resize seeked seeking stalled submit suspend timeUpdate touchCancel touchEnd touchStart volumeChange scroll toggle touchMove waiting wheel".split(
        " ",
      );
  Ku.push("scrollEnd");
  function Xe(t, e) {
    (sd.set(t, e), ti(e, [t]));
  }
  var Xl =
      typeof reportError == "function"
        ? reportError
        : function (t) {
            if (typeof window == "object" && typeof window.ErrorEvent == "function") {
              var e = new window.ErrorEvent("error", {
                bubbles: !0,
                cancelable: !0,
                message:
                  typeof t == "object" && t !== null && typeof t.message == "string"
                    ? String(t.message)
                    : String(t),
                error: t,
              });
              if (!window.dispatchEvent(e)) return;
            } else if (typeof process == "object" && typeof process.emit == "function") {
              process.emit("uncaughtException", t);
              return;
            }
            console.error(t);
          },
    Oe = [],
    _i = 0,
    Ju = 0;
  function kl() {
    for (var t = _i, e = (Ju = _i = 0); e < t; ) {
      var n = Oe[e];
      Oe[e++] = null;
      var a = Oe[e];
      Oe[e++] = null;
      var s = Oe[e];
      Oe[e++] = null;
      var r = Oe[e];
      if (((Oe[e++] = null), a !== null && s !== null)) {
        var f = a.pending;
        (f === null ? (s.next = s) : ((s.next = f.next), (f.next = s)), (a.pending = s));
      }
      r !== 0 && ud(n, s, r);
    }
  }
  function Ql(t, e, n, a) {
    ((Oe[_i++] = t),
      (Oe[_i++] = e),
      (Oe[_i++] = n),
      (Oe[_i++] = a),
      (Ju |= a),
      (t.lanes |= a),
      (t = t.alternate),
      t !== null && (t.lanes |= a));
  }
  function Fu(t, e, n, a) {
    return (Ql(t, e, n, a), Zl(t));
  }
  function ai(t, e) {
    return (Ql(t, null, null, e), Zl(t));
  }
  function ud(t, e, n) {
    t.lanes |= n;
    var a = t.alternate;
    a !== null && (a.lanes |= n);
    for (var s = !1, r = t.return; r !== null; )
      ((r.childLanes |= n),
        (a = r.alternate),
        a !== null && (a.childLanes |= n),
        r.tag === 22 && ((t = r.stateNode), t === null || t._visibility & 1 || (s = !0)),
        (t = r),
        (r = r.return));
    return t.tag === 3
      ? ((r = t.stateNode),
        s &&
          e !== null &&
          ((s = 31 - Se(n)),
          (t = r.hiddenUpdates),
          (a = t[s]),
          a === null ? (t[s] = [e]) : a.push(e),
          (e.lane = n | 536870912)),
        r)
      : null;
  }
  function Zl(t) {
    if (50 < tl) throw ((tl = 0), (lr = null), Error(o(185)));
    for (var e = t.return; e !== null; ) ((t = e), (e = t.return));
    return t.tag === 3 ? t.stateNode : null;
  }
  var Bi = {};
  function Lv(t, e, n, a) {
    ((this.tag = t),
      (this.key = n),
      (this.sibling =
        this.child =
        this.return =
        this.stateNode =
        this.type =
        this.elementType =
          null),
      (this.index = 0),
      (this.refCleanup = this.ref = null),
      (this.pendingProps = e),
      (this.dependencies = this.memoizedState = this.updateQueue = this.memoizedProps = null),
      (this.mode = a),
      (this.subtreeFlags = this.flags = 0),
      (this.deletions = null),
      (this.childLanes = this.lanes = 0),
      (this.alternate = null));
  }
  function Ae(t, e, n, a) {
    return new Lv(t, e, n, a);
  }
  function Pu(t) {
    return ((t = t.prototype), !(!t || !t.isReactComponent));
  }
  function sn(t, e) {
    var n = t.alternate;
    return (
      n === null
        ? ((n = Ae(t.tag, e, t.key, t.mode)),
          (n.elementType = t.elementType),
          (n.type = t.type),
          (n.stateNode = t.stateNode),
          (n.alternate = t),
          (t.alternate = n))
        : ((n.pendingProps = e),
          (n.type = t.type),
          (n.flags = 0),
          (n.subtreeFlags = 0),
          (n.deletions = null)),
      (n.flags = t.flags & 65011712),
      (n.childLanes = t.childLanes),
      (n.lanes = t.lanes),
      (n.child = t.child),
      (n.memoizedProps = t.memoizedProps),
      (n.memoizedState = t.memoizedState),
      (n.updateQueue = t.updateQueue),
      (e = t.dependencies),
      (n.dependencies = e === null ? null : { lanes: e.lanes, firstContext: e.firstContext }),
      (n.sibling = t.sibling),
      (n.index = t.index),
      (n.ref = t.ref),
      (n.refCleanup = t.refCleanup),
      n
    );
  }
  function od(t, e) {
    t.flags &= 65011714;
    var n = t.alternate;
    return (
      n === null
        ? ((t.childLanes = 0),
          (t.lanes = e),
          (t.child = null),
          (t.subtreeFlags = 0),
          (t.memoizedProps = null),
          (t.memoizedState = null),
          (t.updateQueue = null),
          (t.dependencies = null),
          (t.stateNode = null))
        : ((t.childLanes = n.childLanes),
          (t.lanes = n.lanes),
          (t.child = n.child),
          (t.subtreeFlags = 0),
          (t.deletions = null),
          (t.memoizedProps = n.memoizedProps),
          (t.memoizedState = n.memoizedState),
          (t.updateQueue = n.updateQueue),
          (t.type = n.type),
          (e = n.dependencies),
          (t.dependencies = e === null ? null : { lanes: e.lanes, firstContext: e.firstContext })),
      t
    );
  }
  function Kl(t, e, n, a, s, r) {
    var f = 0;
    if (((a = t), typeof t == "function")) Pu(t) && (f = 1);
    else if (typeof t == "string")
      f = X1(t, n, P.current) ? 26 : t === "html" || t === "head" || t === "body" ? 27 : 5;
    else
      t: switch (t) {
        case it:
          return ((t = Ae(31, n, e, s)), (t.elementType = it), (t.lanes = r), t);
        case U:
          return li(n.children, s, r, e);
        case H:
          ((f = 8), (s |= 24));
          break;
        case L:
          return ((t = Ae(12, n, e, s | 2)), (t.elementType = L), (t.lanes = r), t);
        case F:
          return ((t = Ae(13, n, e, s)), (t.elementType = F), (t.lanes = r), t);
        case lt:
          return ((t = Ae(19, n, e, s)), (t.elementType = lt), (t.lanes = r), t);
        default:
          if (typeof t == "object" && t !== null)
            switch (t.$$typeof) {
              case q:
                f = 10;
                break t;
              case Y:
                f = 9;
                break t;
              case G:
                f = 11;
                break t;
              case I:
                f = 14;
                break t;
              case $:
                ((f = 16), (a = null));
                break t;
            }
          ((f = 29), (n = Error(o(130, t === null ? "null" : typeof t, ""))), (a = null));
      }
    return ((e = Ae(f, n, e, s)), (e.elementType = t), (e.type = a), (e.lanes = r), e);
  }
  function li(t, e, n, a) {
    return ((t = Ae(7, t, a, e)), (t.lanes = n), t);
  }
  function Wu(t, e, n) {
    return ((t = Ae(6, t, null, e)), (t.lanes = n), t);
  }
  function rd(t) {
    var e = Ae(18, null, null, 0);
    return ((e.stateNode = t), e);
  }
  function $u(t, e, n) {
    return (
      (e = Ae(4, t.children !== null ? t.children : [], t.key, e)),
      (e.lanes = n),
      (e.stateNode = {
        containerInfo: t.containerInfo,
        pendingChildren: null,
        implementation: t.implementation,
      }),
      e
    );
  }
  var cd = new WeakMap();
  function Ve(t, e) {
    if (typeof t == "object" && t !== null) {
      var n = cd.get(t);
      return n !== void 0 ? n : ((e = { value: t, source: e, stack: rf(e) }), cd.set(t, e), e);
    }
    return { value: t, source: e, stack: rf(e) };
  }
  var Ui = [],
    Li = 0,
    Jl = null,
    Oa = 0,
    _e = [],
    Be = 0,
    En = null,
    Fe = 1,
    Pe = "";
  function un(t, e) {
    ((Ui[Li++] = Oa), (Ui[Li++] = Jl), (Jl = t), (Oa = e));
  }
  function fd(t, e, n) {
    ((_e[Be++] = Fe), (_e[Be++] = Pe), (_e[Be++] = En), (En = t));
    var a = Fe;
    t = Pe;
    var s = 32 - Se(a) - 1;
    ((a &= ~(1 << s)), (n += 1));
    var r = 32 - Se(e) + s;
    if (30 < r) {
      var f = s - (s % 5);
      ((r = (a & ((1 << f) - 1)).toString(32)),
        (a >>= f),
        (s -= f),
        (Fe = (1 << (32 - Se(e) + s)) | (n << s) | a),
        (Pe = r + t));
    } else ((Fe = (1 << r) | (n << s) | a), (Pe = t));
  }
  function Iu(t) {
    t.return !== null && (un(t, 1), fd(t, 1, 0));
  }
  function to(t) {
    for (; t === Jl; ) ((Jl = Ui[--Li]), (Ui[Li] = null), (Oa = Ui[--Li]), (Ui[Li] = null));
    for (; t === En; )
      ((En = _e[--Be]),
        (_e[Be] = null),
        (Pe = _e[--Be]),
        (_e[Be] = null),
        (Fe = _e[--Be]),
        (_e[Be] = null));
  }
  function dd(t, e) {
    ((_e[Be++] = Fe), (_e[Be++] = Pe), (_e[Be++] = En), (Fe = e.id), (Pe = e.overflow), (En = t));
  }
  var te = null,
    jt = null,
    yt = !1,
    Mn = null,
    Ue = !1,
    eo = Error(o(519));
  function Dn(t) {
    var e = Error(
      o(418, 1 < arguments.length && arguments[1] !== void 0 && arguments[1] ? "text" : "HTML", ""),
    );
    throw (Va(Ve(e, t)), eo);
  }
  function hd(t) {
    var e = t.stateNode,
      n = t.type,
      a = t.memoizedProps;
    switch (((e[It] = t), (e[de] = a), n)) {
      case "dialog":
        (dt("cancel", e), dt("close", e));
        break;
      case "iframe":
      case "object":
      case "embed":
        dt("load", e);
        break;
      case "video":
      case "audio":
        for (n = 0; n < nl.length; n++) dt(nl[n], e);
        break;
      case "source":
        dt("error", e);
        break;
      case "img":
      case "image":
      case "link":
        (dt("error", e), dt("load", e));
        break;
      case "details":
        dt("toggle", e);
        break;
      case "input":
        (dt("invalid", e),
          Df(e, a.value, a.defaultValue, a.checked, a.defaultChecked, a.type, a.name, !0));
        break;
      case "select":
        dt("invalid", e);
        break;
      case "textarea":
        (dt("invalid", e), Nf(e, a.value, a.defaultValue, a.children));
    }
    ((n = a.children),
      (typeof n != "string" && typeof n != "number" && typeof n != "bigint") ||
      e.textContent === "" + n ||
      a.suppressHydrationWarning === !0 ||
      jm(e.textContent, n)
        ? (a.popover != null && (dt("beforetoggle", e), dt("toggle", e)),
          a.onScroll != null && dt("scroll", e),
          a.onScrollEnd != null && dt("scrollend", e),
          a.onClick != null && (e.onclick = an),
          (e = !0))
        : (e = !1),
      e || Dn(t, !0));
  }
  function md(t) {
    for (te = t.return; te; )
      switch (te.tag) {
        case 5:
        case 31:
        case 13:
          Ue = !1;
          return;
        case 27:
        case 3:
          Ue = !0;
          return;
        default:
          te = te.return;
      }
  }
  function Hi(t) {
    if (t !== te) return !1;
    if (!yt) return (md(t), (yt = !0), !1);
    var e = t.tag,
      n;
    if (
      ((n = e !== 3 && e !== 27) &&
        ((n = e === 5) &&
          ((n = t.type), (n = !(n !== "form" && n !== "button") || br(t.type, t.memoizedProps))),
        (n = !n)),
      n && jt && Dn(t),
      md(t),
      e === 13)
    ) {
      if (((t = t.memoizedState), (t = t !== null ? t.dehydrated : null), !t)) throw Error(o(317));
      jt = Hm(t);
    } else if (e === 31) {
      if (((t = t.memoizedState), (t = t !== null ? t.dehydrated : null), !t)) throw Error(o(317));
      jt = Hm(t);
    } else
      e === 27
        ? ((e = jt), qn(t.type) ? ((t = Mr), (Mr = null), (jt = t)) : (jt = e))
        : (jt = te ? He(t.stateNode.nextSibling) : null);
    return !0;
  }
  function si() {
    ((jt = te = null), (yt = !1));
  }
  function no() {
    var t = Mn;
    return (t !== null && (ye === null ? (ye = t) : ye.push.apply(ye, t), (Mn = null)), t);
  }
  function Va(t) {
    Mn === null ? (Mn = [t]) : Mn.push(t);
  }
  var io = E(null),
    ui = null,
    on = null;
  function Cn(t, e, n) {
    (Q(io, e._currentValue), (e._currentValue = n));
  }
  function rn(t) {
    ((t._currentValue = io.current), B(io));
  }
  function ao(t, e, n) {
    for (; t !== null; ) {
      var a = t.alternate;
      if (
        ((t.childLanes & e) !== e
          ? ((t.childLanes |= e), a !== null && (a.childLanes |= e))
          : a !== null && (a.childLanes & e) !== e && (a.childLanes |= e),
        t === n)
      )
        break;
      t = t.return;
    }
  }
  function lo(t, e, n, a) {
    var s = t.child;
    for (s !== null && (s.return = t); s !== null; ) {
      var r = s.dependencies;
      if (r !== null) {
        var f = s.child;
        r = r.firstContext;
        t: for (; r !== null; ) {
          var y = r;
          r = s;
          for (var T = 0; T < e.length; T++)
            if (y.context === e[T]) {
              ((r.lanes |= n),
                (y = r.alternate),
                y !== null && (y.lanes |= n),
                ao(r.return, n, t),
                a || (f = null));
              break t;
            }
          r = y.next;
        }
      } else if (s.tag === 18) {
        if (((f = s.return), f === null)) throw Error(o(341));
        ((f.lanes |= n), (r = f.alternate), r !== null && (r.lanes |= n), ao(f, n, t), (f = null));
      } else f = s.child;
      if (f !== null) f.return = s;
      else
        for (f = s; f !== null; ) {
          if (f === t) {
            f = null;
            break;
          }
          if (((s = f.sibling), s !== null)) {
            ((s.return = f.return), (f = s));
            break;
          }
          f = f.return;
        }
      s = f;
    }
  }
  function qi(t, e, n, a) {
    t = null;
    for (var s = e, r = !1; s !== null; ) {
      if (!r) {
        if ((s.flags & 524288) !== 0) r = !0;
        else if ((s.flags & 262144) !== 0) break;
      }
      if (s.tag === 10) {
        var f = s.alternate;
        if (f === null) throw Error(o(387));
        if (((f = f.memoizedProps), f !== null)) {
          var y = s.type;
          Te(s.pendingProps.value, f.value) || (t !== null ? t.push(y) : (t = [y]));
        }
      } else if (s === Tt.current) {
        if (((f = s.alternate), f === null)) throw Error(o(387));
        f.memoizedState.memoizedState !== s.memoizedState.memoizedState &&
          (t !== null ? t.push(ul) : (t = [ul]));
      }
      s = s.return;
    }
    (t !== null && lo(e, t, n, a), (e.flags |= 262144));
  }
  function Fl(t) {
    for (t = t.firstContext; t !== null; ) {
      if (!Te(t.context._currentValue, t.memoizedValue)) return !0;
      t = t.next;
    }
    return !1;
  }
  function oi(t) {
    ((ui = t), (on = null), (t = t.dependencies), t !== null && (t.firstContext = null));
  }
  function ee(t) {
    return pd(ui, t);
  }
  function Pl(t, e) {
    return (ui === null && oi(t), pd(t, e));
  }
  function pd(t, e) {
    var n = e._currentValue;
    if (((e = { context: e, memoizedValue: n, next: null }), on === null)) {
      if (t === null) throw Error(o(308));
      ((on = e), (t.dependencies = { lanes: 0, firstContext: e }), (t.flags |= 524288));
    } else on = on.next = e;
    return n;
  }
  var Hv =
      typeof AbortController < "u"
        ? AbortController
        : function () {
            var t = [],
              e = (this.signal = {
                aborted: !1,
                addEventListener: function (n, a) {
                  t.push(a);
                },
              });
            this.abort = function () {
              ((e.aborted = !0),
                t.forEach(function (n) {
                  return n();
                }));
            };
          },
    qv = i.unstable_scheduleCallback,
    Yv = i.unstable_NormalPriority,
    Xt = {
      $$typeof: q,
      Consumer: null,
      Provider: null,
      _currentValue: null,
      _currentValue2: null,
      _threadCount: 0,
    };
  function so() {
    return { controller: new Hv(), data: new Map(), refCount: 0 };
  }
  function _a(t) {
    (t.refCount--,
      t.refCount === 0 &&
        qv(Yv, function () {
          t.controller.abort();
        }));
  }
  var Ba = null,
    uo = 0,
    Yi = 0,
    Gi = null;
  function Gv(t, e) {
    if (Ba === null) {
      var n = (Ba = []);
      ((uo = 0),
        (Yi = fr()),
        (Gi = {
          status: "pending",
          value: void 0,
          then: function (a) {
            n.push(a);
          },
        }));
    }
    return (uo++, e.then(gd, gd), e);
  }
  function gd() {
    if (--uo === 0 && Ba !== null) {
      Gi !== null && (Gi.status = "fulfilled");
      var t = Ba;
      ((Ba = null), (Yi = 0), (Gi = null));
      for (var e = 0; e < t.length; e++) (0, t[e])();
    }
  }
  function Xv(t, e) {
    var n = [],
      a = {
        status: "pending",
        value: null,
        reason: null,
        then: function (s) {
          n.push(s);
        },
      };
    return (
      t.then(
        function () {
          ((a.status = "fulfilled"), (a.value = e));
          for (var s = 0; s < n.length; s++) (0, n[s])(e);
        },
        function (s) {
          for (a.status = "rejected", a.reason = s, s = 0; s < n.length; s++) (0, n[s])(void 0);
        },
      ),
      a
    );
  }
  var yd = R.S;
  R.S = function (t, e) {
    ((tm = xe()),
      typeof e == "object" && e !== null && typeof e.then == "function" && Gv(t, e),
      yd !== null && yd(t, e));
  };
  var ri = E(null);
  function oo() {
    var t = ri.current;
    return t !== null ? t : Nt.pooledCache;
  }
  function Wl(t, e) {
    e === null ? Q(ri, ri.current) : Q(ri, e.pool);
  }
  function vd() {
    var t = oo();
    return t === null ? null : { parent: Xt._currentValue, pool: t };
  }
  var Xi = Error(o(460)),
    ro = Error(o(474)),
    $l = Error(o(542)),
    Il = { then: function () {} };
  function xd(t) {
    return ((t = t.status), t === "fulfilled" || t === "rejected");
  }
  function bd(t, e, n) {
    switch (
      ((n = t[n]), n === void 0 ? t.push(e) : n !== e && (e.then(an, an), (e = n)), e.status)
    ) {
      case "fulfilled":
        return e.value;
      case "rejected":
        throw ((t = e.reason), Td(t), t);
      default:
        if (typeof e.status == "string") e.then(an, an);
        else {
          if (((t = Nt), t !== null && 100 < t.shellSuspendCounter)) throw Error(o(482));
          ((t = e),
            (t.status = "pending"),
            t.then(
              function (a) {
                if (e.status === "pending") {
                  var s = e;
                  ((s.status = "fulfilled"), (s.value = a));
                }
              },
              function (a) {
                if (e.status === "pending") {
                  var s = e;
                  ((s.status = "rejected"), (s.reason = a));
                }
              },
            ));
        }
        switch (e.status) {
          case "fulfilled":
            return e.value;
          case "rejected":
            throw ((t = e.reason), Td(t), t);
        }
        throw ((fi = e), Xi);
    }
  }
  function ci(t) {
    try {
      var e = t._init;
      return e(t._payload);
    } catch (n) {
      throw n !== null && typeof n == "object" && typeof n.then == "function" ? ((fi = n), Xi) : n;
    }
  }
  var fi = null;
  function Sd() {
    if (fi === null) throw Error(o(459));
    var t = fi;
    return ((fi = null), t);
  }
  function Td(t) {
    if (t === Xi || t === $l) throw Error(o(483));
  }
  var ki = null,
    Ua = 0;
  function ts(t) {
    var e = Ua;
    return ((Ua += 1), ki === null && (ki = []), bd(ki, t, e));
  }
  function La(t, e) {
    ((e = e.props.ref), (t.ref = e !== void 0 ? e : null));
  }
  function es(t, e) {
    throw e.$$typeof === S
      ? Error(o(525))
      : ((t = Object.prototype.toString.call(e)),
        Error(
          o(
            31,
            t === "[object Object]" ? "object with keys {" + Object.keys(e).join(", ") + "}" : t,
          ),
        ));
  }
  function Ad(t) {
    function e(M, A) {
      if (t) {
        var D = M.deletions;
        D === null ? ((M.deletions = [A]), (M.flags |= 16)) : D.push(A);
      }
    }
    function n(M, A) {
      if (!t) return null;
      for (; A !== null; ) (e(M, A), (A = A.sibling));
      return null;
    }
    function a(M) {
      for (var A = new Map(); M !== null; )
        (M.key !== null ? A.set(M.key, M) : A.set(M.index, M), (M = M.sibling));
      return A;
    }
    function s(M, A) {
      return ((M = sn(M, A)), (M.index = 0), (M.sibling = null), M);
    }
    function r(M, A, D) {
      return (
        (M.index = D),
        t
          ? ((D = M.alternate),
            D !== null
              ? ((D = D.index), D < A ? ((M.flags |= 67108866), A) : D)
              : ((M.flags |= 67108866), A))
          : ((M.flags |= 1048576), A)
      );
    }
    function f(M) {
      return (t && M.alternate === null && (M.flags |= 67108866), M);
    }
    function y(M, A, D, V) {
      return A === null || A.tag !== 6
        ? ((A = Wu(D, M.mode, V)), (A.return = M), A)
        : ((A = s(A, D)), (A.return = M), A);
    }
    function T(M, A, D, V) {
      var et = D.type;
      return et === U
        ? O(M, A, D.props.children, V, D.key)
        : A !== null &&
            (A.elementType === et ||
              (typeof et == "object" && et !== null && et.$$typeof === $ && ci(et) === A.type))
          ? ((A = s(A, D.props)), La(A, D), (A.return = M), A)
          : ((A = Kl(D.type, D.key, D.props, null, M.mode, V)), La(A, D), (A.return = M), A);
    }
    function C(M, A, D, V) {
      return A === null ||
        A.tag !== 4 ||
        A.stateNode.containerInfo !== D.containerInfo ||
        A.stateNode.implementation !== D.implementation
        ? ((A = $u(D, M.mode, V)), (A.return = M), A)
        : ((A = s(A, D.children || [])), (A.return = M), A);
    }
    function O(M, A, D, V, et) {
      return A === null || A.tag !== 7
        ? ((A = li(D, M.mode, V, et)), (A.return = M), A)
        : ((A = s(A, D)), (A.return = M), A);
    }
    function _(M, A, D) {
      if ((typeof A == "string" && A !== "") || typeof A == "number" || typeof A == "bigint")
        return ((A = Wu("" + A, M.mode, D)), (A.return = M), A);
      if (typeof A == "object" && A !== null) {
        switch (A.$$typeof) {
          case N:
            return ((D = Kl(A.type, A.key, A.props, null, M.mode, D)), La(D, A), (D.return = M), D);
          case w:
            return ((A = $u(A, M.mode, D)), (A.return = M), A);
          case $:
            return ((A = ci(A)), _(M, A, D));
        }
        if (Lt(A) || vt(A)) return ((A = li(A, M.mode, D, null)), (A.return = M), A);
        if (typeof A.then == "function") return _(M, ts(A), D);
        if (A.$$typeof === q) return _(M, Pl(M, A), D);
        es(M, A);
      }
      return null;
    }
    function z(M, A, D, V) {
      var et = A !== null ? A.key : null;
      if ((typeof D == "string" && D !== "") || typeof D == "number" || typeof D == "bigint")
        return et !== null ? null : y(M, A, "" + D, V);
      if (typeof D == "object" && D !== null) {
        switch (D.$$typeof) {
          case N:
            return D.key === et ? T(M, A, D, V) : null;
          case w:
            return D.key === et ? C(M, A, D, V) : null;
          case $:
            return ((D = ci(D)), z(M, A, D, V));
        }
        if (Lt(D) || vt(D)) return et !== null ? null : O(M, A, D, V, null);
        if (typeof D.then == "function") return z(M, A, ts(D), V);
        if (D.$$typeof === q) return z(M, A, Pl(M, D), V);
        es(M, D);
      }
      return null;
    }
    function j(M, A, D, V, et) {
      if ((typeof V == "string" && V !== "") || typeof V == "number" || typeof V == "bigint")
        return ((M = M.get(D) || null), y(A, M, "" + V, et));
      if (typeof V == "object" && V !== null) {
        switch (V.$$typeof) {
          case N:
            return ((M = M.get(V.key === null ? D : V.key) || null), T(A, M, V, et));
          case w:
            return ((M = M.get(V.key === null ? D : V.key) || null), C(A, M, V, et));
          case $:
            return ((V = ci(V)), j(M, A, D, V, et));
        }
        if (Lt(V) || vt(V)) return ((M = M.get(D) || null), O(A, M, V, et, null));
        if (typeof V.then == "function") return j(M, A, D, ts(V), et);
        if (V.$$typeof === q) return j(M, A, D, Pl(A, V), et);
        es(A, V);
      }
      return null;
    }
    function K(M, A, D, V) {
      for (
        var et = null, xt = null, W = A, rt = (A = 0), mt = null;
        W !== null && rt < D.length;
        rt++
      ) {
        W.index > rt ? ((mt = W), (W = null)) : (mt = W.sibling);
        var bt = z(M, W, D[rt], V);
        if (bt === null) {
          W === null && (W = mt);
          break;
        }
        (t && W && bt.alternate === null && e(M, W),
          (A = r(bt, A, rt)),
          xt === null ? (et = bt) : (xt.sibling = bt),
          (xt = bt),
          (W = mt));
      }
      if (rt === D.length) return (n(M, W), yt && un(M, rt), et);
      if (W === null) {
        for (; rt < D.length; rt++)
          ((W = _(M, D[rt], V)),
            W !== null && ((A = r(W, A, rt)), xt === null ? (et = W) : (xt.sibling = W), (xt = W)));
        return (yt && un(M, rt), et);
      }
      for (W = a(W); rt < D.length; rt++)
        ((mt = j(W, M, rt, D[rt], V)),
          mt !== null &&
            (t && mt.alternate !== null && W.delete(mt.key === null ? rt : mt.key),
            (A = r(mt, A, rt)),
            xt === null ? (et = mt) : (xt.sibling = mt),
            (xt = mt)));
      return (
        t &&
          W.forEach(function (Qn) {
            return e(M, Qn);
          }),
        yt && un(M, rt),
        et
      );
    }
    function nt(M, A, D, V) {
      if (D == null) throw Error(o(151));
      for (
        var et = null, xt = null, W = A, rt = (A = 0), mt = null, bt = D.next();
        W !== null && !bt.done;
        rt++, bt = D.next()
      ) {
        W.index > rt ? ((mt = W), (W = null)) : (mt = W.sibling);
        var Qn = z(M, W, bt.value, V);
        if (Qn === null) {
          W === null && (W = mt);
          break;
        }
        (t && W && Qn.alternate === null && e(M, W),
          (A = r(Qn, A, rt)),
          xt === null ? (et = Qn) : (xt.sibling = Qn),
          (xt = Qn),
          (W = mt));
      }
      if (bt.done) return (n(M, W), yt && un(M, rt), et);
      if (W === null) {
        for (; !bt.done; rt++, bt = D.next())
          ((bt = _(M, bt.value, V)),
            bt !== null &&
              ((A = r(bt, A, rt)), xt === null ? (et = bt) : (xt.sibling = bt), (xt = bt)));
        return (yt && un(M, rt), et);
      }
      for (W = a(W); !bt.done; rt++, bt = D.next())
        ((bt = j(W, M, rt, bt.value, V)),
          bt !== null &&
            (t && bt.alternate !== null && W.delete(bt.key === null ? rt : bt.key),
            (A = r(bt, A, rt)),
            xt === null ? (et = bt) : (xt.sibling = bt),
            (xt = bt)));
      return (
        t &&
          W.forEach(function (tx) {
            return e(M, tx);
          }),
        yt && un(M, rt),
        et
      );
    }
    function Ct(M, A, D, V) {
      if (
        (typeof D == "object" &&
          D !== null &&
          D.type === U &&
          D.key === null &&
          (D = D.props.children),
        typeof D == "object" && D !== null)
      ) {
        switch (D.$$typeof) {
          case N:
            t: {
              for (var et = D.key; A !== null; ) {
                if (A.key === et) {
                  if (((et = D.type), et === U)) {
                    if (A.tag === 7) {
                      (n(M, A.sibling), (V = s(A, D.props.children)), (V.return = M), (M = V));
                      break t;
                    }
                  } else if (
                    A.elementType === et ||
                    (typeof et == "object" && et !== null && et.$$typeof === $ && ci(et) === A.type)
                  ) {
                    (n(M, A.sibling), (V = s(A, D.props)), La(V, D), (V.return = M), (M = V));
                    break t;
                  }
                  n(M, A);
                  break;
                } else e(M, A);
                A = A.sibling;
              }
              D.type === U
                ? ((V = li(D.props.children, M.mode, V, D.key)), (V.return = M), (M = V))
                : ((V = Kl(D.type, D.key, D.props, null, M.mode, V)),
                  La(V, D),
                  (V.return = M),
                  (M = V));
            }
            return f(M);
          case w:
            t: {
              for (et = D.key; A !== null; ) {
                if (A.key === et)
                  if (
                    A.tag === 4 &&
                    A.stateNode.containerInfo === D.containerInfo &&
                    A.stateNode.implementation === D.implementation
                  ) {
                    (n(M, A.sibling), (V = s(A, D.children || [])), (V.return = M), (M = V));
                    break t;
                  } else {
                    n(M, A);
                    break;
                  }
                else e(M, A);
                A = A.sibling;
              }
              ((V = $u(D, M.mode, V)), (V.return = M), (M = V));
            }
            return f(M);
          case $:
            return ((D = ci(D)), Ct(M, A, D, V));
        }
        if (Lt(D)) return K(M, A, D, V);
        if (vt(D)) {
          if (((et = vt(D)), typeof et != "function")) throw Error(o(150));
          return ((D = et.call(D)), nt(M, A, D, V));
        }
        if (typeof D.then == "function") return Ct(M, A, ts(D), V);
        if (D.$$typeof === q) return Ct(M, A, Pl(M, D), V);
        es(M, D);
      }
      return (typeof D == "string" && D !== "") || typeof D == "number" || typeof D == "bigint"
        ? ((D = "" + D),
          A !== null && A.tag === 6
            ? (n(M, A.sibling), (V = s(A, D)), (V.return = M), (M = V))
            : (n(M, A), (V = Wu(D, M.mode, V)), (V.return = M), (M = V)),
          f(M))
        : n(M, A);
    }
    return function (M, A, D, V) {
      try {
        Ua = 0;
        var et = Ct(M, A, D, V);
        return ((ki = null), et);
      } catch (W) {
        if (W === Xi || W === $l) throw W;
        var xt = Ae(29, W, null, M.mode);
        return ((xt.lanes = V), (xt.return = M), xt);
      } finally {
      }
    };
  }
  var di = Ad(!0),
    Ed = Ad(!1),
    Nn = !1;
  function co(t) {
    t.updateQueue = {
      baseState: t.memoizedState,
      firstBaseUpdate: null,
      lastBaseUpdate: null,
      shared: { pending: null, lanes: 0, hiddenCallbacks: null },
      callbacks: null,
    };
  }
  function fo(t, e) {
    ((t = t.updateQueue),
      e.updateQueue === t &&
        (e.updateQueue = {
          baseState: t.baseState,
          firstBaseUpdate: t.firstBaseUpdate,
          lastBaseUpdate: t.lastBaseUpdate,
          shared: t.shared,
          callbacks: null,
        }));
  }
  function zn(t) {
    return { lane: t, tag: 0, payload: null, callback: null, next: null };
  }
  function jn(t, e, n) {
    var a = t.updateQueue;
    if (a === null) return null;
    if (((a = a.shared), (St & 2) !== 0)) {
      var s = a.pending;
      return (
        s === null ? (e.next = e) : ((e.next = s.next), (s.next = e)),
        (a.pending = e),
        (e = Zl(t)),
        ud(t, null, n),
        e
      );
    }
    return (Ql(t, a, e, n), Zl(t));
  }
  function Ha(t, e, n) {
    if (((e = e.updateQueue), e !== null && ((e = e.shared), (n & 4194048) !== 0))) {
      var a = e.lanes;
      ((a &= t.pendingLanes), (n |= a), (e.lanes = n), pf(t, n));
    }
  }
  function ho(t, e) {
    var n = t.updateQueue,
      a = t.alternate;
    if (a !== null && ((a = a.updateQueue), n === a)) {
      var s = null,
        r = null;
      if (((n = n.firstBaseUpdate), n !== null)) {
        do {
          var f = { lane: n.lane, tag: n.tag, payload: n.payload, callback: null, next: null };
          (r === null ? (s = r = f) : (r = r.next = f), (n = n.next));
        } while (n !== null);
        r === null ? (s = r = e) : (r = r.next = e);
      } else s = r = e;
      ((n = {
        baseState: a.baseState,
        firstBaseUpdate: s,
        lastBaseUpdate: r,
        shared: a.shared,
        callbacks: a.callbacks,
      }),
        (t.updateQueue = n));
      return;
    }
    ((t = n.lastBaseUpdate),
      t === null ? (n.firstBaseUpdate = e) : (t.next = e),
      (n.lastBaseUpdate = e));
  }
  var mo = !1;
  function qa() {
    if (mo) {
      var t = Gi;
      if (t !== null) throw t;
    }
  }
  function Ya(t, e, n, a) {
    mo = !1;
    var s = t.updateQueue;
    Nn = !1;
    var r = s.firstBaseUpdate,
      f = s.lastBaseUpdate,
      y = s.shared.pending;
    if (y !== null) {
      s.shared.pending = null;
      var T = y,
        C = T.next;
      ((T.next = null), f === null ? (r = C) : (f.next = C), (f = T));
      var O = t.alternate;
      O !== null &&
        ((O = O.updateQueue),
        (y = O.lastBaseUpdate),
        y !== f && (y === null ? (O.firstBaseUpdate = C) : (y.next = C), (O.lastBaseUpdate = T)));
    }
    if (r !== null) {
      var _ = s.baseState;
      ((f = 0), (O = C = T = null), (y = r));
      do {
        var z = y.lane & -536870913,
          j = z !== y.lane;
        if (j ? (ht & z) === z : (a & z) === z) {
          (z !== 0 && z === Yi && (mo = !0),
            O !== null &&
              (O = O.next =
                { lane: 0, tag: y.tag, payload: y.payload, callback: null, next: null }));
          t: {
            var K = t,
              nt = y;
            z = e;
            var Ct = n;
            switch (nt.tag) {
              case 1:
                if (((K = nt.payload), typeof K == "function")) {
                  _ = K.call(Ct, _, z);
                  break t;
                }
                _ = K;
                break t;
              case 3:
                K.flags = (K.flags & -65537) | 128;
              case 0:
                if (
                  ((K = nt.payload), (z = typeof K == "function" ? K.call(Ct, _, z) : K), z == null)
                )
                  break t;
                _ = x({}, _, z);
                break t;
              case 2:
                Nn = !0;
            }
          }
          ((z = y.callback),
            z !== null &&
              ((t.flags |= 64),
              j && (t.flags |= 8192),
              (j = s.callbacks),
              j === null ? (s.callbacks = [z]) : j.push(z)));
        } else
          ((j = { lane: z, tag: y.tag, payload: y.payload, callback: y.callback, next: null }),
            O === null ? ((C = O = j), (T = _)) : (O = O.next = j),
            (f |= z));
        if (((y = y.next), y === null)) {
          if (((y = s.shared.pending), y === null)) break;
          ((j = y),
            (y = j.next),
            (j.next = null),
            (s.lastBaseUpdate = j),
            (s.shared.pending = null));
        }
      } while (!0);
      (O === null && (T = _),
        (s.baseState = T),
        (s.firstBaseUpdate = C),
        (s.lastBaseUpdate = O),
        r === null && (s.shared.lanes = 0),
        (_n |= f),
        (t.lanes = f),
        (t.memoizedState = _));
    }
  }
  function Md(t, e) {
    if (typeof t != "function") throw Error(o(191, t));
    t.call(e);
  }
  function Dd(t, e) {
    var n = t.callbacks;
    if (n !== null) for (t.callbacks = null, t = 0; t < n.length; t++) Md(n[t], e);
  }
  var Qi = E(null),
    ns = E(0);
  function Cd(t, e) {
    ((t = vn), Q(ns, t), Q(Qi, e), (vn = t | e.baseLanes));
  }
  function po() {
    (Q(ns, vn), Q(Qi, Qi.current));
  }
  function go() {
    ((vn = ns.current), B(Qi), B(ns));
  }
  var Ee = E(null),
    Le = null;
  function wn(t) {
    var e = t.alternate;
    (Q(qt, qt.current & 1),
      Q(Ee, t),
      Le === null && (e === null || Qi.current !== null || e.memoizedState !== null) && (Le = t));
  }
  function yo(t) {
    (Q(qt, qt.current), Q(Ee, t), Le === null && (Le = t));
  }
  function Nd(t) {
    t.tag === 22 ? (Q(qt, qt.current), Q(Ee, t), Le === null && (Le = t)) : Rn();
  }
  function Rn() {
    (Q(qt, qt.current), Q(Ee, Ee.current));
  }
  function Me(t) {
    (B(Ee), Le === t && (Le = null), B(qt));
  }
  var qt = E(0);
  function is(t) {
    for (var e = t; e !== null; ) {
      if (e.tag === 13) {
        var n = e.memoizedState;
        if (n !== null && ((n = n.dehydrated), n === null || Ar(n) || Er(n))) return e;
      } else if (
        e.tag === 19 &&
        (e.memoizedProps.revealOrder === "forwards" ||
          e.memoizedProps.revealOrder === "backwards" ||
          e.memoizedProps.revealOrder === "unstable_legacy-backwards" ||
          e.memoizedProps.revealOrder === "together")
      ) {
        if ((e.flags & 128) !== 0) return e;
      } else if (e.child !== null) {
        ((e.child.return = e), (e = e.child));
        continue;
      }
      if (e === t) break;
      for (; e.sibling === null; ) {
        if (e.return === null || e.return === t) return null;
        e = e.return;
      }
      ((e.sibling.return = e.return), (e = e.sibling));
    }
    return null;
  }
  var cn = 0,
    ut = null,
    Mt = null,
    kt = null,
    as = !1,
    Zi = !1,
    hi = !1,
    ls = 0,
    Ga = 0,
    Ki = null,
    kv = 0;
  function _t() {
    throw Error(o(321));
  }
  function vo(t, e) {
    if (e === null) return !1;
    for (var n = 0; n < e.length && n < t.length; n++) if (!Te(t[n], e[n])) return !1;
    return !0;
  }
  function xo(t, e, n, a, s, r) {
    return (
      (cn = r),
      (ut = e),
      (e.memoizedState = null),
      (e.updateQueue = null),
      (e.lanes = 0),
      (R.H = t === null || t.memoizedState === null ? fh : Vo),
      (hi = !1),
      (r = n(a, s)),
      (hi = !1),
      Zi && (r = jd(e, n, a, s)),
      zd(t),
      r
    );
  }
  function zd(t) {
    R.H = Qa;
    var e = Mt !== null && Mt.next !== null;
    if (((cn = 0), (kt = Mt = ut = null), (as = !1), (Ga = 0), (Ki = null), e)) throw Error(o(300));
    t === null || Qt || ((t = t.dependencies), t !== null && Fl(t) && (Qt = !0));
  }
  function jd(t, e, n, a) {
    ut = t;
    var s = 0;
    do {
      if ((Zi && (Ki = null), (Ga = 0), (Zi = !1), 25 <= s)) throw Error(o(301));
      if (((s += 1), (kt = Mt = null), t.updateQueue != null)) {
        var r = t.updateQueue;
        ((r.lastEffect = null),
          (r.events = null),
          (r.stores = null),
          r.memoCache != null && (r.memoCache.index = 0));
      }
      ((R.H = dh), (r = e(n, a)));
    } while (Zi);
    return r;
  }
  function Qv() {
    var t = R.H,
      e = t.useState()[0];
    return (
      (e = typeof e.then == "function" ? Xa(e) : e),
      (t = t.useState()[0]),
      (Mt !== null ? Mt.memoizedState : null) !== t && (ut.flags |= 1024),
      e
    );
  }
  function bo() {
    var t = ls !== 0;
    return ((ls = 0), t);
  }
  function So(t, e, n) {
    ((e.updateQueue = t.updateQueue), (e.flags &= -2053), (t.lanes &= ~n));
  }
  function To(t) {
    if (as) {
      for (t = t.memoizedState; t !== null; ) {
        var e = t.queue;
        (e !== null && (e.pending = null), (t = t.next));
      }
      as = !1;
    }
    ((cn = 0), (kt = Mt = ut = null), (Zi = !1), (Ga = ls = 0), (Ki = null));
  }
  function re() {
    var t = { memoizedState: null, baseState: null, baseQueue: null, queue: null, next: null };
    return (kt === null ? (ut.memoizedState = kt = t) : (kt = kt.next = t), kt);
  }
  function Yt() {
    if (Mt === null) {
      var t = ut.alternate;
      t = t !== null ? t.memoizedState : null;
    } else t = Mt.next;
    var e = kt === null ? ut.memoizedState : kt.next;
    if (e !== null) ((kt = e), (Mt = t));
    else {
      if (t === null) throw ut.alternate === null ? Error(o(467)) : Error(o(310));
      ((Mt = t),
        (t = {
          memoizedState: Mt.memoizedState,
          baseState: Mt.baseState,
          baseQueue: Mt.baseQueue,
          queue: Mt.queue,
          next: null,
        }),
        kt === null ? (ut.memoizedState = kt = t) : (kt = kt.next = t));
    }
    return kt;
  }
  function ss() {
    return { lastEffect: null, events: null, stores: null, memoCache: null };
  }
  function Xa(t) {
    var e = Ga;
    return (
      (Ga += 1),
      Ki === null && (Ki = []),
      (t = bd(Ki, t, e)),
      (e = ut),
      (kt === null ? e.memoizedState : kt.next) === null &&
        ((e = e.alternate), (R.H = e === null || e.memoizedState === null ? fh : Vo)),
      t
    );
  }
  function us(t) {
    if (t !== null && typeof t == "object") {
      if (typeof t.then == "function") return Xa(t);
      if (t.$$typeof === q) return ee(t);
    }
    throw Error(o(438, String(t)));
  }
  function Ao(t) {
    var e = null,
      n = ut.updateQueue;
    if ((n !== null && (e = n.memoCache), e == null)) {
      var a = ut.alternate;
      a !== null &&
        ((a = a.updateQueue),
        a !== null &&
          ((a = a.memoCache),
          a != null &&
            (e = {
              data: a.data.map(function (s) {
                return s.slice();
              }),
              index: 0,
            })));
    }
    if (
      (e == null && (e = { data: [], index: 0 }),
      n === null && ((n = ss()), (ut.updateQueue = n)),
      (n.memoCache = e),
      (n = e.data[e.index]),
      n === void 0)
    )
      for (n = e.data[e.index] = Array(t), a = 0; a < t; a++) n[a] = tt;
    return (e.index++, n);
  }
  function fn(t, e) {
    return typeof e == "function" ? e(t) : e;
  }
  function os(t) {
    var e = Yt();
    return Eo(e, Mt, t);
  }
  function Eo(t, e, n) {
    var a = t.queue;
    if (a === null) throw Error(o(311));
    a.lastRenderedReducer = n;
    var s = t.baseQueue,
      r = a.pending;
    if (r !== null) {
      if (s !== null) {
        var f = s.next;
        ((s.next = r.next), (r.next = f));
      }
      ((e.baseQueue = s = r), (a.pending = null));
    }
    if (((r = t.baseState), s === null)) t.memoizedState = r;
    else {
      e = s.next;
      var y = (f = null),
        T = null,
        C = e,
        O = !1;
      do {
        var _ = C.lane & -536870913;
        if (_ !== C.lane ? (ht & _) === _ : (cn & _) === _) {
          var z = C.revertLane;
          if (z === 0)
            (T !== null &&
              (T = T.next =
                {
                  lane: 0,
                  revertLane: 0,
                  gesture: null,
                  action: C.action,
                  hasEagerState: C.hasEagerState,
                  eagerState: C.eagerState,
                  next: null,
                }),
              _ === Yi && (O = !0));
          else if ((cn & z) === z) {
            ((C = C.next), z === Yi && (O = !0));
            continue;
          } else
            ((_ = {
              lane: 0,
              revertLane: C.revertLane,
              gesture: null,
              action: C.action,
              hasEagerState: C.hasEagerState,
              eagerState: C.eagerState,
              next: null,
            }),
              T === null ? ((y = T = _), (f = r)) : (T = T.next = _),
              (ut.lanes |= z),
              (_n |= z));
          ((_ = C.action), hi && n(r, _), (r = C.hasEagerState ? C.eagerState : n(r, _)));
        } else
          ((z = {
            lane: _,
            revertLane: C.revertLane,
            gesture: C.gesture,
            action: C.action,
            hasEagerState: C.hasEagerState,
            eagerState: C.eagerState,
            next: null,
          }),
            T === null ? ((y = T = z), (f = r)) : (T = T.next = z),
            (ut.lanes |= _),
            (_n |= _));
        C = C.next;
      } while (C !== null && C !== e);
      if (
        (T === null ? (f = r) : (T.next = y),
        !Te(r, t.memoizedState) && ((Qt = !0), O && ((n = Gi), n !== null)))
      )
        throw n;
      ((t.memoizedState = r), (t.baseState = f), (t.baseQueue = T), (a.lastRenderedState = r));
    }
    return (s === null && (a.lanes = 0), [t.memoizedState, a.dispatch]);
  }
  function Mo(t) {
    var e = Yt(),
      n = e.queue;
    if (n === null) throw Error(o(311));
    n.lastRenderedReducer = t;
    var a = n.dispatch,
      s = n.pending,
      r = e.memoizedState;
    if (s !== null) {
      n.pending = null;
      var f = (s = s.next);
      do ((r = t(r, f.action)), (f = f.next));
      while (f !== s);
      (Te(r, e.memoizedState) || (Qt = !0),
        (e.memoizedState = r),
        e.baseQueue === null && (e.baseState = r),
        (n.lastRenderedState = r));
    }
    return [r, a];
  }
  function wd(t, e, n) {
    var a = ut,
      s = Yt(),
      r = yt;
    if (r) {
      if (n === void 0) throw Error(o(407));
      n = n();
    } else n = e();
    var f = !Te((Mt || s).memoizedState, n);
    if (
      (f && ((s.memoizedState = n), (Qt = !0)),
      (s = s.queue),
      No(Vd.bind(null, a, s, t), [t]),
      s.getSnapshot !== e || f || (kt !== null && kt.memoizedState.tag & 1))
    ) {
      if (
        ((a.flags |= 2048),
        Ji(9, { destroy: void 0 }, Od.bind(null, a, s, n, e), null),
        Nt === null)
      )
        throw Error(o(349));
      r || (cn & 127) !== 0 || Rd(a, e, n);
    }
    return n;
  }
  function Rd(t, e, n) {
    ((t.flags |= 16384),
      (t = { getSnapshot: e, value: n }),
      (e = ut.updateQueue),
      e === null
        ? ((e = ss()), (ut.updateQueue = e), (e.stores = [t]))
        : ((n = e.stores), n === null ? (e.stores = [t]) : n.push(t)));
  }
  function Od(t, e, n, a) {
    ((e.value = n), (e.getSnapshot = a), _d(e) && Bd(t));
  }
  function Vd(t, e, n) {
    return n(function () {
      _d(e) && Bd(t);
    });
  }
  function _d(t) {
    var e = t.getSnapshot;
    t = t.value;
    try {
      var n = e();
      return !Te(t, n);
    } catch {
      return !0;
    }
  }
  function Bd(t) {
    var e = ai(t, 2);
    e !== null && ve(e, t, 2);
  }
  function Do(t) {
    var e = re();
    if (typeof t == "function") {
      var n = t;
      if (((t = n()), hi)) {
        Sn(!0);
        try {
          n();
        } finally {
          Sn(!1);
        }
      }
    }
    return (
      (e.memoizedState = e.baseState = t),
      (e.queue = {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: fn,
        lastRenderedState: t,
      }),
      e
    );
  }
  function Ud(t, e, n, a) {
    return ((t.baseState = n), Eo(t, Mt, typeof a == "function" ? a : fn));
  }
  function Zv(t, e, n, a, s) {
    if (fs(t)) throw Error(o(485));
    if (((t = e.action), t !== null)) {
      var r = {
        payload: s,
        action: t,
        next: null,
        isTransition: !0,
        status: "pending",
        value: null,
        reason: null,
        listeners: [],
        then: function (f) {
          r.listeners.push(f);
        },
      };
      (R.T !== null ? n(!0) : (r.isTransition = !1),
        a(r),
        (n = e.pending),
        n === null
          ? ((r.next = e.pending = r), Ld(e, r))
          : ((r.next = n.next), (e.pending = n.next = r)));
    }
  }
  function Ld(t, e) {
    var n = e.action,
      a = e.payload,
      s = t.state;
    if (e.isTransition) {
      var r = R.T,
        f = {};
      R.T = f;
      try {
        var y = n(s, a),
          T = R.S;
        (T !== null && T(f, y), Hd(t, e, y));
      } catch (C) {
        Co(t, e, C);
      } finally {
        (r !== null && f.types !== null && (r.types = f.types), (R.T = r));
      }
    } else
      try {
        ((r = n(s, a)), Hd(t, e, r));
      } catch (C) {
        Co(t, e, C);
      }
  }
  function Hd(t, e, n) {
    n !== null && typeof n == "object" && typeof n.then == "function"
      ? n.then(
          function (a) {
            qd(t, e, a);
          },
          function (a) {
            return Co(t, e, a);
          },
        )
      : qd(t, e, n);
  }
  function qd(t, e, n) {
    ((e.status = "fulfilled"),
      (e.value = n),
      Yd(e),
      (t.state = n),
      (e = t.pending),
      e !== null &&
        ((n = e.next), n === e ? (t.pending = null) : ((n = n.next), (e.next = n), Ld(t, n))));
  }
  function Co(t, e, n) {
    var a = t.pending;
    if (((t.pending = null), a !== null)) {
      a = a.next;
      do ((e.status = "rejected"), (e.reason = n), Yd(e), (e = e.next));
      while (e !== a);
    }
    t.action = null;
  }
  function Yd(t) {
    t = t.listeners;
    for (var e = 0; e < t.length; e++) (0, t[e])();
  }
  function Gd(t, e) {
    return e;
  }
  function Xd(t, e) {
    if (yt) {
      var n = Nt.formState;
      if (n !== null) {
        t: {
          var a = ut;
          if (yt) {
            if (jt) {
              e: {
                for (var s = jt, r = Ue; s.nodeType !== 8; ) {
                  if (!r) {
                    s = null;
                    break e;
                  }
                  if (((s = He(s.nextSibling)), s === null)) {
                    s = null;
                    break e;
                  }
                }
                ((r = s.data), (s = r === "F!" || r === "F" ? s : null));
              }
              if (s) {
                ((jt = He(s.nextSibling)), (a = s.data === "F!"));
                break t;
              }
            }
            Dn(a);
          }
          a = !1;
        }
        a && (e = n[0]);
      }
    }
    return (
      (n = re()),
      (n.memoizedState = n.baseState = e),
      (a = {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: Gd,
        lastRenderedState: e,
      }),
      (n.queue = a),
      (n = oh.bind(null, ut, a)),
      (a.dispatch = n),
      (a = Do(!1)),
      (r = Oo.bind(null, ut, !1, a.queue)),
      (a = re()),
      (s = { state: e, dispatch: null, action: t, pending: null }),
      (a.queue = s),
      (n = Zv.bind(null, ut, s, r, n)),
      (s.dispatch = n),
      (a.memoizedState = t),
      [e, n, !1]
    );
  }
  function kd(t) {
    var e = Yt();
    return Qd(e, Mt, t);
  }
  function Qd(t, e, n) {
    if (
      ((e = Eo(t, e, Gd)[0]),
      (t = os(fn)[0]),
      typeof e == "object" && e !== null && typeof e.then == "function")
    )
      try {
        var a = Xa(e);
      } catch (f) {
        throw f === Xi ? $l : f;
      }
    else a = e;
    e = Yt();
    var s = e.queue,
      r = s.dispatch;
    return (
      n !== e.memoizedState &&
        ((ut.flags |= 2048), Ji(9, { destroy: void 0 }, Kv.bind(null, s, n), null)),
      [a, r, t]
    );
  }
  function Kv(t, e) {
    t.action = e;
  }
  function Zd(t) {
    var e = Yt(),
      n = Mt;
    if (n !== null) return Qd(e, n, t);
    (Yt(), (e = e.memoizedState), (n = Yt()));
    var a = n.queue.dispatch;
    return ((n.memoizedState = t), [e, a, !1]);
  }
  function Ji(t, e, n, a) {
    return (
      (t = { tag: t, create: n, deps: a, inst: e, next: null }),
      (e = ut.updateQueue),
      e === null && ((e = ss()), (ut.updateQueue = e)),
      (n = e.lastEffect),
      n === null
        ? (e.lastEffect = t.next = t)
        : ((a = n.next), (n.next = t), (t.next = a), (e.lastEffect = t)),
      t
    );
  }
  function Kd() {
    return Yt().memoizedState;
  }
  function rs(t, e, n, a) {
    var s = re();
    ((ut.flags |= t),
      (s.memoizedState = Ji(1 | e, { destroy: void 0 }, n, a === void 0 ? null : a)));
  }
  function cs(t, e, n, a) {
    var s = Yt();
    a = a === void 0 ? null : a;
    var r = s.memoizedState.inst;
    Mt !== null && a !== null && vo(a, Mt.memoizedState.deps)
      ? (s.memoizedState = Ji(e, r, n, a))
      : ((ut.flags |= t), (s.memoizedState = Ji(1 | e, r, n, a)));
  }
  function Jd(t, e) {
    rs(8390656, 8, t, e);
  }
  function No(t, e) {
    cs(2048, 8, t, e);
  }
  function Jv(t) {
    ut.flags |= 4;
    var e = ut.updateQueue;
    if (e === null) ((e = ss()), (ut.updateQueue = e), (e.events = [t]));
    else {
      var n = e.events;
      n === null ? (e.events = [t]) : n.push(t);
    }
  }
  function Fd(t) {
    var e = Yt().memoizedState;
    return (
      Jv({ ref: e, nextImpl: t }),
      function () {
        if ((St & 2) !== 0) throw Error(o(440));
        return e.impl.apply(void 0, arguments);
      }
    );
  }
  function Pd(t, e) {
    return cs(4, 2, t, e);
  }
  function Wd(t, e) {
    return cs(4, 4, t, e);
  }
  function $d(t, e) {
    if (typeof e == "function") {
      t = t();
      var n = e(t);
      return function () {
        typeof n == "function" ? n() : e(null);
      };
    }
    if (e != null)
      return (
        (t = t()),
        (e.current = t),
        function () {
          e.current = null;
        }
      );
  }
  function Id(t, e, n) {
    ((n = n != null ? n.concat([t]) : null), cs(4, 4, $d.bind(null, e, t), n));
  }
  function zo() {}
  function th(t, e) {
    var n = Yt();
    e = e === void 0 ? null : e;
    var a = n.memoizedState;
    return e !== null && vo(e, a[1]) ? a[0] : ((n.memoizedState = [t, e]), t);
  }
  function eh(t, e) {
    var n = Yt();
    e = e === void 0 ? null : e;
    var a = n.memoizedState;
    if (e !== null && vo(e, a[1])) return a[0];
    if (((a = t()), hi)) {
      Sn(!0);
      try {
        t();
      } finally {
        Sn(!1);
      }
    }
    return ((n.memoizedState = [a, e]), a);
  }
  function jo(t, e, n) {
    return n === void 0 || ((cn & 1073741824) !== 0 && (ht & 261930) === 0)
      ? (t.memoizedState = e)
      : ((t.memoizedState = n), (t = nm()), (ut.lanes |= t), (_n |= t), n);
  }
  function nh(t, e, n, a) {
    return Te(n, e)
      ? n
      : Qi.current !== null
        ? ((t = jo(t, n, a)), Te(t, e) || (Qt = !0), t)
        : (cn & 42) === 0 || ((cn & 1073741824) !== 0 && (ht & 261930) === 0)
          ? ((Qt = !0), (t.memoizedState = n))
          : ((t = nm()), (ut.lanes |= t), (_n |= t), e);
  }
  function ih(t, e, n, a, s) {
    var r = X.p;
    X.p = r !== 0 && 8 > r ? r : 8;
    var f = R.T,
      y = {};
    ((R.T = y), Oo(t, !1, e, n));
    try {
      var T = s(),
        C = R.S;
      if (
        (C !== null && C(y, T), T !== null && typeof T == "object" && typeof T.then == "function")
      ) {
        var O = Xv(T, a);
        ka(t, e, O, Ne(t));
      } else ka(t, e, a, Ne(t));
    } catch (_) {
      ka(t, e, { then: function () {}, status: "rejected", reason: _ }, Ne());
    } finally {
      ((X.p = r), f !== null && y.types !== null && (f.types = y.types), (R.T = f));
    }
  }
  function Fv() {}
  function wo(t, e, n, a) {
    if (t.tag !== 5) throw Error(o(476));
    var s = ah(t).queue;
    ih(
      t,
      s,
      e,
      Z,
      n === null
        ? Fv
        : function () {
            return (lh(t), n(a));
          },
    );
  }
  function ah(t) {
    var e = t.memoizedState;
    if (e !== null) return e;
    e = {
      memoizedState: Z,
      baseState: Z,
      baseQueue: null,
      queue: {
        pending: null,
        lanes: 0,
        dispatch: null,
        lastRenderedReducer: fn,
        lastRenderedState: Z,
      },
      next: null,
    };
    var n = {};
    return (
      (e.next = {
        memoizedState: n,
        baseState: n,
        baseQueue: null,
        queue: {
          pending: null,
          lanes: 0,
          dispatch: null,
          lastRenderedReducer: fn,
          lastRenderedState: n,
        },
        next: null,
      }),
      (t.memoizedState = e),
      (t = t.alternate),
      t !== null && (t.memoizedState = e),
      e
    );
  }
  function lh(t) {
    var e = ah(t);
    (e.next === null && (e = t.alternate.memoizedState), ka(t, e.next.queue, {}, Ne()));
  }
  function Ro() {
    return ee(ul);
  }
  function sh() {
    return Yt().memoizedState;
  }
  function uh() {
    return Yt().memoizedState;
  }
  function Pv(t) {
    for (var e = t.return; e !== null; ) {
      switch (e.tag) {
        case 24:
        case 3:
          var n = Ne();
          t = zn(n);
          var a = jn(e, t, n);
          (a !== null && (ve(a, e, n), Ha(a, e, n)), (e = { cache: so() }), (t.payload = e));
          return;
      }
      e = e.return;
    }
  }
  function Wv(t, e, n) {
    var a = Ne();
    ((n = {
      lane: a,
      revertLane: 0,
      gesture: null,
      action: n,
      hasEagerState: !1,
      eagerState: null,
      next: null,
    }),
      fs(t) ? rh(e, n) : ((n = Fu(t, e, n, a)), n !== null && (ve(n, t, a), ch(n, e, a))));
  }
  function oh(t, e, n) {
    var a = Ne();
    ka(t, e, n, a);
  }
  function ka(t, e, n, a) {
    var s = {
      lane: a,
      revertLane: 0,
      gesture: null,
      action: n,
      hasEagerState: !1,
      eagerState: null,
      next: null,
    };
    if (fs(t)) rh(e, s);
    else {
      var r = t.alternate;
      if (
        t.lanes === 0 &&
        (r === null || r.lanes === 0) &&
        ((r = e.lastRenderedReducer), r !== null)
      )
        try {
          var f = e.lastRenderedState,
            y = r(f, n);
          if (((s.hasEagerState = !0), (s.eagerState = y), Te(y, f)))
            return (Ql(t, e, s, 0), Nt === null && kl(), !1);
        } catch {
        } finally {
        }
      if (((n = Fu(t, e, s, a)), n !== null)) return (ve(n, t, a), ch(n, e, a), !0);
    }
    return !1;
  }
  function Oo(t, e, n, a) {
    if (
      ((a = {
        lane: 2,
        revertLane: fr(),
        gesture: null,
        action: a,
        hasEagerState: !1,
        eagerState: null,
        next: null,
      }),
      fs(t))
    ) {
      if (e) throw Error(o(479));
    } else ((e = Fu(t, n, a, 2)), e !== null && ve(e, t, 2));
  }
  function fs(t) {
    var e = t.alternate;
    return t === ut || (e !== null && e === ut);
  }
  function rh(t, e) {
    Zi = as = !0;
    var n = t.pending;
    (n === null ? (e.next = e) : ((e.next = n.next), (n.next = e)), (t.pending = e));
  }
  function ch(t, e, n) {
    if ((n & 4194048) !== 0) {
      var a = e.lanes;
      ((a &= t.pendingLanes), (n |= a), (e.lanes = n), pf(t, n));
    }
  }
  var Qa = {
    readContext: ee,
    use: us,
    useCallback: _t,
    useContext: _t,
    useEffect: _t,
    useImperativeHandle: _t,
    useLayoutEffect: _t,
    useInsertionEffect: _t,
    useMemo: _t,
    useReducer: _t,
    useRef: _t,
    useState: _t,
    useDebugValue: _t,
    useDeferredValue: _t,
    useTransition: _t,
    useSyncExternalStore: _t,
    useId: _t,
    useHostTransitionStatus: _t,
    useFormState: _t,
    useActionState: _t,
    useOptimistic: _t,
    useMemoCache: _t,
    useCacheRefresh: _t,
  };
  Qa.useEffectEvent = _t;
  var fh = {
      readContext: ee,
      use: us,
      useCallback: function (t, e) {
        return ((re().memoizedState = [t, e === void 0 ? null : e]), t);
      },
      useContext: ee,
      useEffect: Jd,
      useImperativeHandle: function (t, e, n) {
        ((n = n != null ? n.concat([t]) : null), rs(4194308, 4, $d.bind(null, e, t), n));
      },
      useLayoutEffect: function (t, e) {
        return rs(4194308, 4, t, e);
      },
      useInsertionEffect: function (t, e) {
        rs(4, 2, t, e);
      },
      useMemo: function (t, e) {
        var n = re();
        e = e === void 0 ? null : e;
        var a = t();
        if (hi) {
          Sn(!0);
          try {
            t();
          } finally {
            Sn(!1);
          }
        }
        return ((n.memoizedState = [a, e]), a);
      },
      useReducer: function (t, e, n) {
        var a = re();
        if (n !== void 0) {
          var s = n(e);
          if (hi) {
            Sn(!0);
            try {
              n(e);
            } finally {
              Sn(!1);
            }
          }
        } else s = e;
        return (
          (a.memoizedState = a.baseState = s),
          (t = {
            pending: null,
            lanes: 0,
            dispatch: null,
            lastRenderedReducer: t,
            lastRenderedState: s,
          }),
          (a.queue = t),
          (t = t.dispatch = Wv.bind(null, ut, t)),
          [a.memoizedState, t]
        );
      },
      useRef: function (t) {
        var e = re();
        return ((t = { current: t }), (e.memoizedState = t));
      },
      useState: function (t) {
        t = Do(t);
        var e = t.queue,
          n = oh.bind(null, ut, e);
        return ((e.dispatch = n), [t.memoizedState, n]);
      },
      useDebugValue: zo,
      useDeferredValue: function (t, e) {
        var n = re();
        return jo(n, t, e);
      },
      useTransition: function () {
        var t = Do(!1);
        return ((t = ih.bind(null, ut, t.queue, !0, !1)), (re().memoizedState = t), [!1, t]);
      },
      useSyncExternalStore: function (t, e, n) {
        var a = ut,
          s = re();
        if (yt) {
          if (n === void 0) throw Error(o(407));
          n = n();
        } else {
          if (((n = e()), Nt === null)) throw Error(o(349));
          (ht & 127) !== 0 || Rd(a, e, n);
        }
        s.memoizedState = n;
        var r = { value: n, getSnapshot: e };
        return (
          (s.queue = r),
          Jd(Vd.bind(null, a, r, t), [t]),
          (a.flags |= 2048),
          Ji(9, { destroy: void 0 }, Od.bind(null, a, r, n, e), null),
          n
        );
      },
      useId: function () {
        var t = re(),
          e = Nt.identifierPrefix;
        if (yt) {
          var n = Pe,
            a = Fe;
          ((n = (a & ~(1 << (32 - Se(a) - 1))).toString(32) + n),
            (e = "_" + e + "R_" + n),
            (n = ls++),
            0 < n && (e += "H" + n.toString(32)),
            (e += "_"));
        } else ((n = kv++), (e = "_" + e + "r_" + n.toString(32) + "_"));
        return (t.memoizedState = e);
      },
      useHostTransitionStatus: Ro,
      useFormState: Xd,
      useActionState: Xd,
      useOptimistic: function (t) {
        var e = re();
        e.memoizedState = e.baseState = t;
        var n = {
          pending: null,
          lanes: 0,
          dispatch: null,
          lastRenderedReducer: null,
          lastRenderedState: null,
        };
        return ((e.queue = n), (e = Oo.bind(null, ut, !0, n)), (n.dispatch = e), [t, e]);
      },
      useMemoCache: Ao,
      useCacheRefresh: function () {
        return (re().memoizedState = Pv.bind(null, ut));
      },
      useEffectEvent: function (t) {
        var e = re(),
          n = { impl: t };
        return (
          (e.memoizedState = n),
          function () {
            if ((St & 2) !== 0) throw Error(o(440));
            return n.impl.apply(void 0, arguments);
          }
        );
      },
    },
    Vo = {
      readContext: ee,
      use: us,
      useCallback: th,
      useContext: ee,
      useEffect: No,
      useImperativeHandle: Id,
      useInsertionEffect: Pd,
      useLayoutEffect: Wd,
      useMemo: eh,
      useReducer: os,
      useRef: Kd,
      useState: function () {
        return os(fn);
      },
      useDebugValue: zo,
      useDeferredValue: function (t, e) {
        var n = Yt();
        return nh(n, Mt.memoizedState, t, e);
      },
      useTransition: function () {
        var t = os(fn)[0],
          e = Yt().memoizedState;
        return [typeof t == "boolean" ? t : Xa(t), e];
      },
      useSyncExternalStore: wd,
      useId: sh,
      useHostTransitionStatus: Ro,
      useFormState: kd,
      useActionState: kd,
      useOptimistic: function (t, e) {
        var n = Yt();
        return Ud(n, Mt, t, e);
      },
      useMemoCache: Ao,
      useCacheRefresh: uh,
    };
  Vo.useEffectEvent = Fd;
  var dh = {
    readContext: ee,
    use: us,
    useCallback: th,
    useContext: ee,
    useEffect: No,
    useImperativeHandle: Id,
    useInsertionEffect: Pd,
    useLayoutEffect: Wd,
    useMemo: eh,
    useReducer: Mo,
    useRef: Kd,
    useState: function () {
      return Mo(fn);
    },
    useDebugValue: zo,
    useDeferredValue: function (t, e) {
      var n = Yt();
      return Mt === null ? jo(n, t, e) : nh(n, Mt.memoizedState, t, e);
    },
    useTransition: function () {
      var t = Mo(fn)[0],
        e = Yt().memoizedState;
      return [typeof t == "boolean" ? t : Xa(t), e];
    },
    useSyncExternalStore: wd,
    useId: sh,
    useHostTransitionStatus: Ro,
    useFormState: Zd,
    useActionState: Zd,
    useOptimistic: function (t, e) {
      var n = Yt();
      return Mt !== null ? Ud(n, Mt, t, e) : ((n.baseState = t), [t, n.queue.dispatch]);
    },
    useMemoCache: Ao,
    useCacheRefresh: uh,
  };
  dh.useEffectEvent = Fd;
  function _o(t, e, n, a) {
    ((e = t.memoizedState),
      (n = n(a, e)),
      (n = n == null ? e : x({}, e, n)),
      (t.memoizedState = n),
      t.lanes === 0 && (t.updateQueue.baseState = n));
  }
  var Bo = {
    enqueueSetState: function (t, e, n) {
      t = t._reactInternals;
      var a = Ne(),
        s = zn(a);
      ((s.payload = e),
        n != null && (s.callback = n),
        (e = jn(t, s, a)),
        e !== null && (ve(e, t, a), Ha(e, t, a)));
    },
    enqueueReplaceState: function (t, e, n) {
      t = t._reactInternals;
      var a = Ne(),
        s = zn(a);
      ((s.tag = 1),
        (s.payload = e),
        n != null && (s.callback = n),
        (e = jn(t, s, a)),
        e !== null && (ve(e, t, a), Ha(e, t, a)));
    },
    enqueueForceUpdate: function (t, e) {
      t = t._reactInternals;
      var n = Ne(),
        a = zn(n);
      ((a.tag = 2),
        e != null && (a.callback = e),
        (e = jn(t, a, n)),
        e !== null && (ve(e, t, n), Ha(e, t, n)));
    },
  };
  function hh(t, e, n, a, s, r, f) {
    return (
      (t = t.stateNode),
      typeof t.shouldComponentUpdate == "function"
        ? t.shouldComponentUpdate(a, r, f)
        : e.prototype && e.prototype.isPureReactComponent
          ? !wa(n, a) || !wa(s, r)
          : !0
    );
  }
  function mh(t, e, n, a) {
    ((t = e.state),
      typeof e.componentWillReceiveProps == "function" && e.componentWillReceiveProps(n, a),
      typeof e.UNSAFE_componentWillReceiveProps == "function" &&
        e.UNSAFE_componentWillReceiveProps(n, a),
      e.state !== t && Bo.enqueueReplaceState(e, e.state, null));
  }
  function mi(t, e) {
    var n = e;
    if ("ref" in e) {
      n = {};
      for (var a in e) a !== "ref" && (n[a] = e[a]);
    }
    if ((t = t.defaultProps)) {
      n === e && (n = x({}, n));
      for (var s in t) n[s] === void 0 && (n[s] = t[s]);
    }
    return n;
  }
  function ph(t) {
    Xl(t);
  }
  function gh(t) {
    console.error(t);
  }
  function yh(t) {
    Xl(t);
  }
  function ds(t, e) {
    try {
      var n = t.onUncaughtError;
      n(e.value, { componentStack: e.stack });
    } catch (a) {
      setTimeout(function () {
        throw a;
      });
    }
  }
  function vh(t, e, n) {
    try {
      var a = t.onCaughtError;
      a(n.value, { componentStack: n.stack, errorBoundary: e.tag === 1 ? e.stateNode : null });
    } catch (s) {
      setTimeout(function () {
        throw s;
      });
    }
  }
  function Uo(t, e, n) {
    return (
      (n = zn(n)),
      (n.tag = 3),
      (n.payload = { element: null }),
      (n.callback = function () {
        ds(t, e);
      }),
      n
    );
  }
  function xh(t) {
    return ((t = zn(t)), (t.tag = 3), t);
  }
  function bh(t, e, n, a) {
    var s = n.type.getDerivedStateFromError;
    if (typeof s == "function") {
      var r = a.value;
      ((t.payload = function () {
        return s(r);
      }),
        (t.callback = function () {
          vh(e, n, a);
        }));
    }
    var f = n.stateNode;
    f !== null &&
      typeof f.componentDidCatch == "function" &&
      (t.callback = function () {
        (vh(e, n, a),
          typeof s != "function" && (Bn === null ? (Bn = new Set([this])) : Bn.add(this)));
        var y = a.stack;
        this.componentDidCatch(a.value, { componentStack: y !== null ? y : "" });
      });
  }
  function $v(t, e, n, a, s) {
    if (((n.flags |= 32768), a !== null && typeof a == "object" && typeof a.then == "function")) {
      if (((e = n.alternate), e !== null && qi(e, n, s, !0), (n = Ee.current), n !== null)) {
        switch (n.tag) {
          case 31:
          case 13:
            return (
              Le === null ? Es() : n.alternate === null && Bt === 0 && (Bt = 3),
              (n.flags &= -257),
              (n.flags |= 65536),
              (n.lanes = s),
              a === Il
                ? (n.flags |= 16384)
                : ((e = n.updateQueue),
                  e === null ? (n.updateQueue = new Set([a])) : e.add(a),
                  or(t, a, s)),
              !1
            );
          case 22:
            return (
              (n.flags |= 65536),
              a === Il
                ? (n.flags |= 16384)
                : ((e = n.updateQueue),
                  e === null
                    ? ((e = { transitions: null, markerInstances: null, retryQueue: new Set([a]) }),
                      (n.updateQueue = e))
                    : ((n = e.retryQueue), n === null ? (e.retryQueue = new Set([a])) : n.add(a)),
                  or(t, a, s)),
              !1
            );
        }
        throw Error(o(435, n.tag));
      }
      return (or(t, a, s), Es(), !1);
    }
    if (yt)
      return (
        (e = Ee.current),
        e !== null
          ? ((e.flags & 65536) === 0 && (e.flags |= 256),
            (e.flags |= 65536),
            (e.lanes = s),
            a !== eo && ((t = Error(o(422), { cause: a })), Va(Ve(t, n))))
          : (a !== eo && ((e = Error(o(423), { cause: a })), Va(Ve(e, n))),
            (t = t.current.alternate),
            (t.flags |= 65536),
            (s &= -s),
            (t.lanes |= s),
            (a = Ve(a, n)),
            (s = Uo(t.stateNode, a, s)),
            ho(t, s),
            Bt !== 4 && (Bt = 2)),
        !1
      );
    var r = Error(o(520), { cause: a });
    if (((r = Ve(r, n)), Ia === null ? (Ia = [r]) : Ia.push(r), Bt !== 4 && (Bt = 2), e === null))
      return !0;
    ((a = Ve(a, n)), (n = e));
    do {
      switch (n.tag) {
        case 3:
          return (
            (n.flags |= 65536),
            (t = s & -s),
            (n.lanes |= t),
            (t = Uo(n.stateNode, a, t)),
            ho(n, t),
            !1
          );
        case 1:
          if (
            ((e = n.type),
            (r = n.stateNode),
            (n.flags & 128) === 0 &&
              (typeof e.getDerivedStateFromError == "function" ||
                (r !== null &&
                  typeof r.componentDidCatch == "function" &&
                  (Bn === null || !Bn.has(r)))))
          )
            return (
              (n.flags |= 65536),
              (s &= -s),
              (n.lanes |= s),
              (s = xh(s)),
              bh(s, t, n, a),
              ho(n, s),
              !1
            );
      }
      n = n.return;
    } while (n !== null);
    return !1;
  }
  var Lo = Error(o(461)),
    Qt = !1;
  function ne(t, e, n, a) {
    e.child = t === null ? Ed(e, null, n, a) : di(e, t.child, n, a);
  }
  function Sh(t, e, n, a, s) {
    n = n.render;
    var r = e.ref;
    if ("ref" in a) {
      var f = {};
      for (var y in a) y !== "ref" && (f[y] = a[y]);
    } else f = a;
    return (
      oi(e),
      (a = xo(t, e, n, f, r, s)),
      (y = bo()),
      t !== null && !Qt
        ? (So(t, e, s), dn(t, e, s))
        : (yt && y && Iu(e), (e.flags |= 1), ne(t, e, a, s), e.child)
    );
  }
  function Th(t, e, n, a, s) {
    if (t === null) {
      var r = n.type;
      return typeof r == "function" && !Pu(r) && r.defaultProps === void 0 && n.compare === null
        ? ((e.tag = 15), (e.type = r), Ah(t, e, r, a, s))
        : ((t = Kl(n.type, null, a, e, e.mode, s)), (t.ref = e.ref), (t.return = e), (e.child = t));
    }
    if (((r = t.child), !Zo(t, s))) {
      var f = r.memoizedProps;
      if (((n = n.compare), (n = n !== null ? n : wa), n(f, a) && t.ref === e.ref))
        return dn(t, e, s);
    }
    return ((e.flags |= 1), (t = sn(r, a)), (t.ref = e.ref), (t.return = e), (e.child = t));
  }
  function Ah(t, e, n, a, s) {
    if (t !== null) {
      var r = t.memoizedProps;
      if (wa(r, a) && t.ref === e.ref)
        if (((Qt = !1), (e.pendingProps = a = r), Zo(t, s))) (t.flags & 131072) !== 0 && (Qt = !0);
        else return ((e.lanes = t.lanes), dn(t, e, s));
    }
    return Ho(t, e, n, a, s);
  }
  function Eh(t, e, n, a) {
    var s = a.children,
      r = t !== null ? t.memoizedState : null;
    if (
      (t === null &&
        e.stateNode === null &&
        (e.stateNode = {
          _visibility: 1,
          _pendingMarkers: null,
          _retryCache: null,
          _transitions: null,
        }),
      a.mode === "hidden")
    ) {
      if ((e.flags & 128) !== 0) {
        if (((r = r !== null ? r.baseLanes | n : n), t !== null)) {
          for (a = e.child = t.child, s = 0; a !== null; )
            ((s = s | a.lanes | a.childLanes), (a = a.sibling));
          a = s & ~r;
        } else ((a = 0), (e.child = null));
        return Mh(t, e, r, n, a);
      }
      if ((n & 536870912) !== 0)
        ((e.memoizedState = { baseLanes: 0, cachePool: null }),
          t !== null && Wl(e, r !== null ? r.cachePool : null),
          r !== null ? Cd(e, r) : po(),
          Nd(e));
      else return ((a = e.lanes = 536870912), Mh(t, e, r !== null ? r.baseLanes | n : n, n, a));
    } else
      r !== null
        ? (Wl(e, r.cachePool), Cd(e, r), Rn(), (e.memoizedState = null))
        : (t !== null && Wl(e, null), po(), Rn());
    return (ne(t, e, s, n), e.child);
  }
  function Za(t, e) {
    return (
      (t !== null && t.tag === 22) ||
        e.stateNode !== null ||
        (e.stateNode = {
          _visibility: 1,
          _pendingMarkers: null,
          _retryCache: null,
          _transitions: null,
        }),
      e.sibling
    );
  }
  function Mh(t, e, n, a, s) {
    var r = oo();
    return (
      (r = r === null ? null : { parent: Xt._currentValue, pool: r }),
      (e.memoizedState = { baseLanes: n, cachePool: r }),
      t !== null && Wl(e, null),
      po(),
      Nd(e),
      t !== null && qi(t, e, a, !0),
      (e.childLanes = s),
      null
    );
  }
  function hs(t, e) {
    return (
      (e = ps({ mode: e.mode, children: e.children }, t.mode)),
      (e.ref = t.ref),
      (t.child = e),
      (e.return = t),
      e
    );
  }
  function Dh(t, e, n) {
    return (
      di(e, t.child, null, n),
      (t = hs(e, e.pendingProps)),
      (t.flags |= 2),
      Me(e),
      (e.memoizedState = null),
      t
    );
  }
  function Iv(t, e, n) {
    var a = e.pendingProps,
      s = (e.flags & 128) !== 0;
    if (((e.flags &= -129), t === null)) {
      if (yt) {
        if (a.mode === "hidden") return ((t = hs(e, a)), (e.lanes = 536870912), Za(null, t));
        if (
          (yo(e),
          (t = jt)
            ? ((t = Lm(t, Ue)),
              (t = t !== null && t.data === "&" ? t : null),
              t !== null &&
                ((e.memoizedState = {
                  dehydrated: t,
                  treeContext: En !== null ? { id: Fe, overflow: Pe } : null,
                  retryLane: 536870912,
                  hydrationErrors: null,
                }),
                (n = rd(t)),
                (n.return = e),
                (e.child = n),
                (te = e),
                (jt = null)))
            : (t = null),
          t === null)
        )
          throw Dn(e);
        return ((e.lanes = 536870912), null);
      }
      return hs(e, a);
    }
    var r = t.memoizedState;
    if (r !== null) {
      var f = r.dehydrated;
      if ((yo(e), s))
        if (e.flags & 256) ((e.flags &= -257), (e = Dh(t, e, n)));
        else if (e.memoizedState !== null) ((e.child = t.child), (e.flags |= 128), (e = null));
        else throw Error(o(558));
      else if ((Qt || qi(t, e, n, !1), (s = (n & t.childLanes) !== 0), Qt || s)) {
        if (((a = Nt), a !== null && ((f = gf(a, n)), f !== 0 && f !== r.retryLane)))
          throw ((r.retryLane = f), ai(t, f), ve(a, t, f), Lo);
        (Es(), (e = Dh(t, e, n)));
      } else
        ((t = r.treeContext),
          (jt = He(f.nextSibling)),
          (te = e),
          (yt = !0),
          (Mn = null),
          (Ue = !1),
          t !== null && dd(e, t),
          (e = hs(e, a)),
          (e.flags |= 4096));
      return e;
    }
    return (
      (t = sn(t.child, { mode: a.mode, children: a.children })),
      (t.ref = e.ref),
      (e.child = t),
      (t.return = e),
      t
    );
  }
  function ms(t, e) {
    var n = e.ref;
    if (n === null) t !== null && t.ref !== null && (e.flags |= 4194816);
    else {
      if (typeof n != "function" && typeof n != "object") throw Error(o(284));
      (t === null || t.ref !== n) && (e.flags |= 4194816);
    }
  }
  function Ho(t, e, n, a, s) {
    return (
      oi(e),
      (n = xo(t, e, n, a, void 0, s)),
      (a = bo()),
      t !== null && !Qt
        ? (So(t, e, s), dn(t, e, s))
        : (yt && a && Iu(e), (e.flags |= 1), ne(t, e, n, s), e.child)
    );
  }
  function Ch(t, e, n, a, s, r) {
    return (
      oi(e),
      (e.updateQueue = null),
      (n = jd(e, a, n, s)),
      zd(t),
      (a = bo()),
      t !== null && !Qt
        ? (So(t, e, r), dn(t, e, r))
        : (yt && a && Iu(e), (e.flags |= 1), ne(t, e, n, r), e.child)
    );
  }
  function Nh(t, e, n, a, s) {
    if ((oi(e), e.stateNode === null)) {
      var r = Bi,
        f = n.contextType;
      (typeof f == "object" && f !== null && (r = ee(f)),
        (r = new n(a, r)),
        (e.memoizedState = r.state !== null && r.state !== void 0 ? r.state : null),
        (r.updater = Bo),
        (e.stateNode = r),
        (r._reactInternals = e),
        (r = e.stateNode),
        (r.props = a),
        (r.state = e.memoizedState),
        (r.refs = {}),
        co(e),
        (f = n.contextType),
        (r.context = typeof f == "object" && f !== null ? ee(f) : Bi),
        (r.state = e.memoizedState),
        (f = n.getDerivedStateFromProps),
        typeof f == "function" && (_o(e, n, f, a), (r.state = e.memoizedState)),
        typeof n.getDerivedStateFromProps == "function" ||
          typeof r.getSnapshotBeforeUpdate == "function" ||
          (typeof r.UNSAFE_componentWillMount != "function" &&
            typeof r.componentWillMount != "function") ||
          ((f = r.state),
          typeof r.componentWillMount == "function" && r.componentWillMount(),
          typeof r.UNSAFE_componentWillMount == "function" && r.UNSAFE_componentWillMount(),
          f !== r.state && Bo.enqueueReplaceState(r, r.state, null),
          Ya(e, a, r, s),
          qa(),
          (r.state = e.memoizedState)),
        typeof r.componentDidMount == "function" && (e.flags |= 4194308),
        (a = !0));
    } else if (t === null) {
      r = e.stateNode;
      var y = e.memoizedProps,
        T = mi(n, y);
      r.props = T;
      var C = r.context,
        O = n.contextType;
      ((f = Bi), typeof O == "object" && O !== null && (f = ee(O)));
      var _ = n.getDerivedStateFromProps;
      ((O = typeof _ == "function" || typeof r.getSnapshotBeforeUpdate == "function"),
        (y = e.pendingProps !== y),
        O ||
          (typeof r.UNSAFE_componentWillReceiveProps != "function" &&
            typeof r.componentWillReceiveProps != "function") ||
          ((y || C !== f) && mh(e, r, a, f)),
        (Nn = !1));
      var z = e.memoizedState;
      ((r.state = z),
        Ya(e, a, r, s),
        qa(),
        (C = e.memoizedState),
        y || z !== C || Nn
          ? (typeof _ == "function" && (_o(e, n, _, a), (C = e.memoizedState)),
            (T = Nn || hh(e, n, T, a, z, C, f))
              ? (O ||
                  (typeof r.UNSAFE_componentWillMount != "function" &&
                    typeof r.componentWillMount != "function") ||
                  (typeof r.componentWillMount == "function" && r.componentWillMount(),
                  typeof r.UNSAFE_componentWillMount == "function" &&
                    r.UNSAFE_componentWillMount()),
                typeof r.componentDidMount == "function" && (e.flags |= 4194308))
              : (typeof r.componentDidMount == "function" && (e.flags |= 4194308),
                (e.memoizedProps = a),
                (e.memoizedState = C)),
            (r.props = a),
            (r.state = C),
            (r.context = f),
            (a = T))
          : (typeof r.componentDidMount == "function" && (e.flags |= 4194308), (a = !1)));
    } else {
      ((r = e.stateNode),
        fo(t, e),
        (f = e.memoizedProps),
        (O = mi(n, f)),
        (r.props = O),
        (_ = e.pendingProps),
        (z = r.context),
        (C = n.contextType),
        (T = Bi),
        typeof C == "object" && C !== null && (T = ee(C)),
        (y = n.getDerivedStateFromProps),
        (C = typeof y == "function" || typeof r.getSnapshotBeforeUpdate == "function") ||
          (typeof r.UNSAFE_componentWillReceiveProps != "function" &&
            typeof r.componentWillReceiveProps != "function") ||
          ((f !== _ || z !== T) && mh(e, r, a, T)),
        (Nn = !1),
        (z = e.memoizedState),
        (r.state = z),
        Ya(e, a, r, s),
        qa());
      var j = e.memoizedState;
      f !== _ || z !== j || Nn || (t !== null && t.dependencies !== null && Fl(t.dependencies))
        ? (typeof y == "function" && (_o(e, n, y, a), (j = e.memoizedState)),
          (O =
            Nn ||
            hh(e, n, O, a, z, j, T) ||
            (t !== null && t.dependencies !== null && Fl(t.dependencies)))
            ? (C ||
                (typeof r.UNSAFE_componentWillUpdate != "function" &&
                  typeof r.componentWillUpdate != "function") ||
                (typeof r.componentWillUpdate == "function" && r.componentWillUpdate(a, j, T),
                typeof r.UNSAFE_componentWillUpdate == "function" &&
                  r.UNSAFE_componentWillUpdate(a, j, T)),
              typeof r.componentDidUpdate == "function" && (e.flags |= 4),
              typeof r.getSnapshotBeforeUpdate == "function" && (e.flags |= 1024))
            : (typeof r.componentDidUpdate != "function" ||
                (f === t.memoizedProps && z === t.memoizedState) ||
                (e.flags |= 4),
              typeof r.getSnapshotBeforeUpdate != "function" ||
                (f === t.memoizedProps && z === t.memoizedState) ||
                (e.flags |= 1024),
              (e.memoizedProps = a),
              (e.memoizedState = j)),
          (r.props = a),
          (r.state = j),
          (r.context = T),
          (a = O))
        : (typeof r.componentDidUpdate != "function" ||
            (f === t.memoizedProps && z === t.memoizedState) ||
            (e.flags |= 4),
          typeof r.getSnapshotBeforeUpdate != "function" ||
            (f === t.memoizedProps && z === t.memoizedState) ||
            (e.flags |= 1024),
          (a = !1));
    }
    return (
      (r = a),
      ms(t, e),
      (a = (e.flags & 128) !== 0),
      r || a
        ? ((r = e.stateNode),
          (n = a && typeof n.getDerivedStateFromError != "function" ? null : r.render()),
          (e.flags |= 1),
          t !== null && a
            ? ((e.child = di(e, t.child, null, s)), (e.child = di(e, null, n, s)))
            : ne(t, e, n, s),
          (e.memoizedState = r.state),
          (t = e.child))
        : (t = dn(t, e, s)),
      t
    );
  }
  function zh(t, e, n, a) {
    return (si(), (e.flags |= 256), ne(t, e, n, a), e.child);
  }
  var qo = { dehydrated: null, treeContext: null, retryLane: 0, hydrationErrors: null };
  function Yo(t) {
    return { baseLanes: t, cachePool: vd() };
  }
  function Go(t, e, n) {
    return ((t = t !== null ? t.childLanes & ~n : 0), e && (t |= Ce), t);
  }
  function jh(t, e, n) {
    var a = e.pendingProps,
      s = !1,
      r = (e.flags & 128) !== 0,
      f;
    if (
      ((f = r) || (f = t !== null && t.memoizedState === null ? !1 : (qt.current & 2) !== 0),
      f && ((s = !0), (e.flags &= -129)),
      (f = (e.flags & 32) !== 0),
      (e.flags &= -33),
      t === null)
    ) {
      if (yt) {
        if (
          (s ? wn(e) : Rn(),
          (t = jt)
            ? ((t = Lm(t, Ue)),
              (t = t !== null && t.data !== "&" ? t : null),
              t !== null &&
                ((e.memoizedState = {
                  dehydrated: t,
                  treeContext: En !== null ? { id: Fe, overflow: Pe } : null,
                  retryLane: 536870912,
                  hydrationErrors: null,
                }),
                (n = rd(t)),
                (n.return = e),
                (e.child = n),
                (te = e),
                (jt = null)))
            : (t = null),
          t === null)
        )
          throw Dn(e);
        return (Er(t) ? (e.lanes = 32) : (e.lanes = 536870912), null);
      }
      var y = a.children;
      return (
        (a = a.fallback),
        s
          ? (Rn(),
            (s = e.mode),
            (y = ps({ mode: "hidden", children: y }, s)),
            (a = li(a, s, n, null)),
            (y.return = e),
            (a.return = e),
            (y.sibling = a),
            (e.child = y),
            (a = e.child),
            (a.memoizedState = Yo(n)),
            (a.childLanes = Go(t, f, n)),
            (e.memoizedState = qo),
            Za(null, a))
          : (wn(e), Xo(e, y))
      );
    }
    var T = t.memoizedState;
    if (T !== null && ((y = T.dehydrated), y !== null)) {
      if (r)
        e.flags & 256
          ? (wn(e), (e.flags &= -257), (e = ko(t, e, n)))
          : e.memoizedState !== null
            ? (Rn(), (e.child = t.child), (e.flags |= 128), (e = null))
            : (Rn(),
              (y = a.fallback),
              (s = e.mode),
              (a = ps({ mode: "visible", children: a.children }, s)),
              (y = li(y, s, n, null)),
              (y.flags |= 2),
              (a.return = e),
              (y.return = e),
              (a.sibling = y),
              (e.child = a),
              di(e, t.child, null, n),
              (a = e.child),
              (a.memoizedState = Yo(n)),
              (a.childLanes = Go(t, f, n)),
              (e.memoizedState = qo),
              (e = Za(null, a)));
      else if ((wn(e), Er(y))) {
        if (((f = y.nextSibling && y.nextSibling.dataset), f)) var C = f.dgst;
        ((f = C),
          (a = Error(o(419))),
          (a.stack = ""),
          (a.digest = f),
          Va({ value: a, source: null, stack: null }),
          (e = ko(t, e, n)));
      } else if ((Qt || qi(t, e, n, !1), (f = (n & t.childLanes) !== 0), Qt || f)) {
        if (((f = Nt), f !== null && ((a = gf(f, n)), a !== 0 && a !== T.retryLane)))
          throw ((T.retryLane = a), ai(t, a), ve(f, t, a), Lo);
        (Ar(y) || Es(), (e = ko(t, e, n)));
      } else
        Ar(y)
          ? ((e.flags |= 192), (e.child = t.child), (e = null))
          : ((t = T.treeContext),
            (jt = He(y.nextSibling)),
            (te = e),
            (yt = !0),
            (Mn = null),
            (Ue = !1),
            t !== null && dd(e, t),
            (e = Xo(e, a.children)),
            (e.flags |= 4096));
      return e;
    }
    return s
      ? (Rn(),
        (y = a.fallback),
        (s = e.mode),
        (T = t.child),
        (C = T.sibling),
        (a = sn(T, { mode: "hidden", children: a.children })),
        (a.subtreeFlags = T.subtreeFlags & 65011712),
        C !== null ? (y = sn(C, y)) : ((y = li(y, s, n, null)), (y.flags |= 2)),
        (y.return = e),
        (a.return = e),
        (a.sibling = y),
        (e.child = a),
        Za(null, a),
        (a = e.child),
        (y = t.child.memoizedState),
        y === null
          ? (y = Yo(n))
          : ((s = y.cachePool),
            s !== null
              ? ((T = Xt._currentValue), (s = s.parent !== T ? { parent: T, pool: T } : s))
              : (s = vd()),
            (y = { baseLanes: y.baseLanes | n, cachePool: s })),
        (a.memoizedState = y),
        (a.childLanes = Go(t, f, n)),
        (e.memoizedState = qo),
        Za(t.child, a))
      : (wn(e),
        (n = t.child),
        (t = n.sibling),
        (n = sn(n, { mode: "visible", children: a.children })),
        (n.return = e),
        (n.sibling = null),
        t !== null &&
          ((f = e.deletions), f === null ? ((e.deletions = [t]), (e.flags |= 16)) : f.push(t)),
        (e.child = n),
        (e.memoizedState = null),
        n);
  }
  function Xo(t, e) {
    return ((e = ps({ mode: "visible", children: e }, t.mode)), (e.return = t), (t.child = e));
  }
  function ps(t, e) {
    return ((t = Ae(22, t, null, e)), (t.lanes = 0), t);
  }
  function ko(t, e, n) {
    return (
      di(e, t.child, null, n),
      (t = Xo(e, e.pendingProps.children)),
      (t.flags |= 2),
      (e.memoizedState = null),
      t
    );
  }
  function wh(t, e, n) {
    t.lanes |= e;
    var a = t.alternate;
    (a !== null && (a.lanes |= e), ao(t.return, e, n));
  }
  function Qo(t, e, n, a, s, r) {
    var f = t.memoizedState;
    f === null
      ? (t.memoizedState = {
          isBackwards: e,
          rendering: null,
          renderingStartTime: 0,
          last: a,
          tail: n,
          tailMode: s,
          treeForkCount: r,
        })
      : ((f.isBackwards = e),
        (f.rendering = null),
        (f.renderingStartTime = 0),
        (f.last = a),
        (f.tail = n),
        (f.tailMode = s),
        (f.treeForkCount = r));
  }
  function Rh(t, e, n) {
    var a = e.pendingProps,
      s = a.revealOrder,
      r = a.tail;
    a = a.children;
    var f = qt.current,
      y = (f & 2) !== 0;
    if (
      (y ? ((f = (f & 1) | 2), (e.flags |= 128)) : (f &= 1),
      Q(qt, f),
      ne(t, e, a, n),
      (a = yt ? Oa : 0),
      !y && t !== null && (t.flags & 128) !== 0)
    )
      t: for (t = e.child; t !== null; ) {
        if (t.tag === 13) t.memoizedState !== null && wh(t, n, e);
        else if (t.tag === 19) wh(t, n, e);
        else if (t.child !== null) {
          ((t.child.return = t), (t = t.child));
          continue;
        }
        if (t === e) break t;
        for (; t.sibling === null; ) {
          if (t.return === null || t.return === e) break t;
          t = t.return;
        }
        ((t.sibling.return = t.return), (t = t.sibling));
      }
    switch (s) {
      case "forwards":
        for (n = e.child, s = null; n !== null; )
          ((t = n.alternate), t !== null && is(t) === null && (s = n), (n = n.sibling));
        ((n = s),
          n === null ? ((s = e.child), (e.child = null)) : ((s = n.sibling), (n.sibling = null)),
          Qo(e, !1, s, n, r, a));
        break;
      case "backwards":
      case "unstable_legacy-backwards":
        for (n = null, s = e.child, e.child = null; s !== null; ) {
          if (((t = s.alternate), t !== null && is(t) === null)) {
            e.child = s;
            break;
          }
          ((t = s.sibling), (s.sibling = n), (n = s), (s = t));
        }
        Qo(e, !0, n, null, r, a);
        break;
      case "together":
        Qo(e, !1, null, null, void 0, a);
        break;
      default:
        e.memoizedState = null;
    }
    return e.child;
  }
  function dn(t, e, n) {
    if (
      (t !== null && (e.dependencies = t.dependencies), (_n |= e.lanes), (n & e.childLanes) === 0)
    )
      if (t !== null) {
        if ((qi(t, e, n, !1), (n & e.childLanes) === 0)) return null;
      } else return null;
    if (t !== null && e.child !== t.child) throw Error(o(153));
    if (e.child !== null) {
      for (t = e.child, n = sn(t, t.pendingProps), e.child = n, n.return = e; t.sibling !== null; )
        ((t = t.sibling), (n = n.sibling = sn(t, t.pendingProps)), (n.return = e));
      n.sibling = null;
    }
    return e.child;
  }
  function Zo(t, e) {
    return (t.lanes & e) !== 0 ? !0 : ((t = t.dependencies), !!(t !== null && Fl(t)));
  }
  function t1(t, e, n) {
    switch (e.tag) {
      case 3:
        (oe(e, e.stateNode.containerInfo), Cn(e, Xt, t.memoizedState.cache), si());
        break;
      case 27:
      case 5:
        va(e);
        break;
      case 4:
        oe(e, e.stateNode.containerInfo);
        break;
      case 10:
        Cn(e, e.type, e.memoizedProps.value);
        break;
      case 31:
        if (e.memoizedState !== null) return ((e.flags |= 128), yo(e), null);
        break;
      case 13:
        var a = e.memoizedState;
        if (a !== null)
          return a.dehydrated !== null
            ? (wn(e), (e.flags |= 128), null)
            : (n & e.child.childLanes) !== 0
              ? jh(t, e, n)
              : (wn(e), (t = dn(t, e, n)), t !== null ? t.sibling : null);
        wn(e);
        break;
      case 19:
        var s = (t.flags & 128) !== 0;
        if (
          ((a = (n & e.childLanes) !== 0),
          a || (qi(t, e, n, !1), (a = (n & e.childLanes) !== 0)),
          s)
        ) {
          if (a) return Rh(t, e, n);
          e.flags |= 128;
        }
        if (
          ((s = e.memoizedState),
          s !== null && ((s.rendering = null), (s.tail = null), (s.lastEffect = null)),
          Q(qt, qt.current),
          a)
        )
          break;
        return null;
      case 22:
        return ((e.lanes = 0), Eh(t, e, n, e.pendingProps));
      case 24:
        Cn(e, Xt, t.memoizedState.cache);
    }
    return dn(t, e, n);
  }
  function Oh(t, e, n) {
    if (t !== null)
      if (t.memoizedProps !== e.pendingProps) Qt = !0;
      else {
        if (!Zo(t, n) && (e.flags & 128) === 0) return ((Qt = !1), t1(t, e, n));
        Qt = (t.flags & 131072) !== 0;
      }
    else ((Qt = !1), yt && (e.flags & 1048576) !== 0 && fd(e, Oa, e.index));
    switch (((e.lanes = 0), e.tag)) {
      case 16:
        t: {
          var a = e.pendingProps;
          if (((t = ci(e.elementType)), (e.type = t), typeof t == "function"))
            Pu(t)
              ? ((a = mi(t, a)), (e.tag = 1), (e = Nh(null, e, t, a, n)))
              : ((e.tag = 0), (e = Ho(null, e, t, a, n)));
          else {
            if (t != null) {
              var s = t.$$typeof;
              if (s === G) {
                ((e.tag = 11), (e = Sh(null, e, t, a, n)));
                break t;
              } else if (s === I) {
                ((e.tag = 14), (e = Th(null, e, t, a, n)));
                break t;
              }
            }
            throw ((e = Gt(t) || t), Error(o(306, e, "")));
          }
        }
        return e;
      case 0:
        return Ho(t, e, e.type, e.pendingProps, n);
      case 1:
        return ((a = e.type), (s = mi(a, e.pendingProps)), Nh(t, e, a, s, n));
      case 3:
        t: {
          if ((oe(e, e.stateNode.containerInfo), t === null)) throw Error(o(387));
          a = e.pendingProps;
          var r = e.memoizedState;
          ((s = r.element), fo(t, e), Ya(e, a, null, n));
          var f = e.memoizedState;
          if (
            ((a = f.cache),
            Cn(e, Xt, a),
            a !== r.cache && lo(e, [Xt], n, !0),
            qa(),
            (a = f.element),
            r.isDehydrated)
          )
            if (
              ((r = { element: a, isDehydrated: !1, cache: f.cache }),
              (e.updateQueue.baseState = r),
              (e.memoizedState = r),
              e.flags & 256)
            ) {
              e = zh(t, e, a, n);
              break t;
            } else if (a !== s) {
              ((s = Ve(Error(o(424)), e)), Va(s), (e = zh(t, e, a, n)));
              break t;
            } else {
              switch (((t = e.stateNode.containerInfo), t.nodeType)) {
                case 9:
                  t = t.body;
                  break;
                default:
                  t = t.nodeName === "HTML" ? t.ownerDocument.body : t;
              }
              for (
                jt = He(t.firstChild),
                  te = e,
                  yt = !0,
                  Mn = null,
                  Ue = !0,
                  n = Ed(e, null, a, n),
                  e.child = n;
                n;
              )
                ((n.flags = (n.flags & -3) | 4096), (n = n.sibling));
            }
          else {
            if ((si(), a === s)) {
              e = dn(t, e, n);
              break t;
            }
            ne(t, e, a, n);
          }
          e = e.child;
        }
        return e;
      case 26:
        return (
          ms(t, e),
          t === null
            ? (n = km(e.type, null, e.pendingProps, null))
              ? (e.memoizedState = n)
              : yt ||
                ((n = e.type),
                (t = e.pendingProps),
                (a = ws(ct.current).createElement(n)),
                (a[It] = e),
                (a[de] = t),
                ie(a, n, t),
                Pt(a),
                (e.stateNode = a))
            : (e.memoizedState = km(e.type, t.memoizedProps, e.pendingProps, t.memoizedState)),
          null
        );
      case 27:
        return (
          va(e),
          t === null &&
            yt &&
            ((a = e.stateNode = Ym(e.type, e.pendingProps, ct.current)),
            (te = e),
            (Ue = !0),
            (s = jt),
            qn(e.type) ? ((Mr = s), (jt = He(a.firstChild))) : (jt = s)),
          ne(t, e, e.pendingProps.children, n),
          ms(t, e),
          t === null && (e.flags |= 4194304),
          e.child
        );
      case 5:
        return (
          t === null &&
            yt &&
            ((s = a = jt) &&
              ((a = j1(a, e.type, e.pendingProps, Ue)),
              a !== null
                ? ((e.stateNode = a), (te = e), (jt = He(a.firstChild)), (Ue = !1), (s = !0))
                : (s = !1)),
            s || Dn(e)),
          va(e),
          (s = e.type),
          (r = e.pendingProps),
          (f = t !== null ? t.memoizedProps : null),
          (a = r.children),
          br(s, r) ? (a = null) : f !== null && br(s, f) && (e.flags |= 32),
          e.memoizedState !== null && ((s = xo(t, e, Qv, null, null, n)), (ul._currentValue = s)),
          ms(t, e),
          ne(t, e, a, n),
          e.child
        );
      case 6:
        return (
          t === null &&
            yt &&
            ((t = n = jt) &&
              ((n = w1(n, e.pendingProps, Ue)),
              n !== null ? ((e.stateNode = n), (te = e), (jt = null), (t = !0)) : (t = !1)),
            t || Dn(e)),
          null
        );
      case 13:
        return jh(t, e, n);
      case 4:
        return (
          oe(e, e.stateNode.containerInfo),
          (a = e.pendingProps),
          t === null ? (e.child = di(e, null, a, n)) : ne(t, e, a, n),
          e.child
        );
      case 11:
        return Sh(t, e, e.type, e.pendingProps, n);
      case 7:
        return (ne(t, e, e.pendingProps, n), e.child);
      case 8:
        return (ne(t, e, e.pendingProps.children, n), e.child);
      case 12:
        return (ne(t, e, e.pendingProps.children, n), e.child);
      case 10:
        return ((a = e.pendingProps), Cn(e, e.type, a.value), ne(t, e, a.children, n), e.child);
      case 9:
        return (
          (s = e.type._context),
          (a = e.pendingProps.children),
          oi(e),
          (s = ee(s)),
          (a = a(s)),
          (e.flags |= 1),
          ne(t, e, a, n),
          e.child
        );
      case 14:
        return Th(t, e, e.type, e.pendingProps, n);
      case 15:
        return Ah(t, e, e.type, e.pendingProps, n);
      case 19:
        return Rh(t, e, n);
      case 31:
        return Iv(t, e, n);
      case 22:
        return Eh(t, e, n, e.pendingProps);
      case 24:
        return (
          oi(e),
          (a = ee(Xt)),
          t === null
            ? ((s = oo()),
              s === null &&
                ((s = Nt),
                (r = so()),
                (s.pooledCache = r),
                r.refCount++,
                r !== null && (s.pooledCacheLanes |= n),
                (s = r)),
              (e.memoizedState = { parent: a, cache: s }),
              co(e),
              Cn(e, Xt, s))
            : ((t.lanes & n) !== 0 && (fo(t, e), Ya(e, null, null, n), qa()),
              (s = t.memoizedState),
              (r = e.memoizedState),
              s.parent !== a
                ? ((s = { parent: a, cache: a }),
                  (e.memoizedState = s),
                  e.lanes === 0 && (e.memoizedState = e.updateQueue.baseState = s),
                  Cn(e, Xt, a))
                : ((a = r.cache), Cn(e, Xt, a), a !== s.cache && lo(e, [Xt], n, !0))),
          ne(t, e, e.pendingProps.children, n),
          e.child
        );
      case 29:
        throw e.pendingProps;
    }
    throw Error(o(156, e.tag));
  }
  function hn(t) {
    t.flags |= 4;
  }
  function Ko(t, e, n, a, s) {
    if (((e = (t.mode & 32) !== 0) && (e = !1), e)) {
      if (((t.flags |= 16777216), (s & 335544128) === s))
        if (t.stateNode.complete) t.flags |= 8192;
        else if (sm()) t.flags |= 8192;
        else throw ((fi = Il), ro);
    } else t.flags &= -16777217;
  }
  function Vh(t, e) {
    if (e.type !== "stylesheet" || (e.state.loading & 4) !== 0) t.flags &= -16777217;
    else if (((t.flags |= 16777216), !Fm(e)))
      if (sm()) t.flags |= 8192;
      else throw ((fi = Il), ro);
  }
  function gs(t, e) {
    (e !== null && (t.flags |= 4),
      t.flags & 16384 && ((e = t.tag !== 22 ? hf() : 536870912), (t.lanes |= e), ($i |= e)));
  }
  function Ka(t, e) {
    if (!yt)
      switch (t.tailMode) {
        case "hidden":
          e = t.tail;
          for (var n = null; e !== null; ) (e.alternate !== null && (n = e), (e = e.sibling));
          n === null ? (t.tail = null) : (n.sibling = null);
          break;
        case "collapsed":
          n = t.tail;
          for (var a = null; n !== null; ) (n.alternate !== null && (a = n), (n = n.sibling));
          a === null
            ? e || t.tail === null
              ? (t.tail = null)
              : (t.tail.sibling = null)
            : (a.sibling = null);
      }
  }
  function wt(t) {
    var e = t.alternate !== null && t.alternate.child === t.child,
      n = 0,
      a = 0;
    if (e)
      for (var s = t.child; s !== null; )
        ((n |= s.lanes | s.childLanes),
          (a |= s.subtreeFlags & 65011712),
          (a |= s.flags & 65011712),
          (s.return = t),
          (s = s.sibling));
    else
      for (s = t.child; s !== null; )
        ((n |= s.lanes | s.childLanes),
          (a |= s.subtreeFlags),
          (a |= s.flags),
          (s.return = t),
          (s = s.sibling));
    return ((t.subtreeFlags |= a), (t.childLanes = n), e);
  }
  function e1(t, e, n) {
    var a = e.pendingProps;
    switch ((to(e), e.tag)) {
      case 16:
      case 15:
      case 0:
      case 11:
      case 7:
      case 8:
      case 12:
      case 9:
      case 14:
        return (wt(e), null);
      case 1:
        return (wt(e), null);
      case 3:
        return (
          (n = e.stateNode),
          (a = null),
          t !== null && (a = t.memoizedState.cache),
          e.memoizedState.cache !== a && (e.flags |= 2048),
          rn(Xt),
          Ht(),
          n.pendingContext && ((n.context = n.pendingContext), (n.pendingContext = null)),
          (t === null || t.child === null) &&
            (Hi(e)
              ? hn(e)
              : t === null ||
                (t.memoizedState.isDehydrated && (e.flags & 256) === 0) ||
                ((e.flags |= 1024), no())),
          wt(e),
          null
        );
      case 26:
        var s = e.type,
          r = e.memoizedState;
        return (
          t === null
            ? (hn(e), r !== null ? (wt(e), Vh(e, r)) : (wt(e), Ko(e, s, null, a, n)))
            : r
              ? r !== t.memoizedState
                ? (hn(e), wt(e), Vh(e, r))
                : (wt(e), (e.flags &= -16777217))
              : ((t = t.memoizedProps), t !== a && hn(e), wt(e), Ko(e, s, t, a, n)),
          null
        );
      case 27:
        if ((Cl(e), (n = ct.current), (s = e.type), t !== null && e.stateNode != null))
          t.memoizedProps !== a && hn(e);
        else {
          if (!a) {
            if (e.stateNode === null) throw Error(o(166));
            return (wt(e), null);
          }
          ((t = P.current), Hi(e) ? hd(e) : ((t = Ym(s, a, n)), (e.stateNode = t), hn(e)));
        }
        return (wt(e), null);
      case 5:
        if ((Cl(e), (s = e.type), t !== null && e.stateNode != null))
          t.memoizedProps !== a && hn(e);
        else {
          if (!a) {
            if (e.stateNode === null) throw Error(o(166));
            return (wt(e), null);
          }
          if (((r = P.current), Hi(e))) hd(e);
          else {
            var f = ws(ct.current);
            switch (r) {
              case 1:
                r = f.createElementNS("http://www.w3.org/2000/svg", s);
                break;
              case 2:
                r = f.createElementNS("http://www.w3.org/1998/Math/MathML", s);
                break;
              default:
                switch (s) {
                  case "svg":
                    r = f.createElementNS("http://www.w3.org/2000/svg", s);
                    break;
                  case "math":
                    r = f.createElementNS("http://www.w3.org/1998/Math/MathML", s);
                    break;
                  case "script":
                    ((r = f.createElement("div")),
                      (r.innerHTML = "<script><\/script>"),
                      (r = r.removeChild(r.firstChild)));
                    break;
                  case "select":
                    ((r =
                      typeof a.is == "string"
                        ? f.createElement("select", { is: a.is })
                        : f.createElement("select")),
                      a.multiple ? (r.multiple = !0) : a.size && (r.size = a.size));
                    break;
                  default:
                    r =
                      typeof a.is == "string"
                        ? f.createElement(s, { is: a.is })
                        : f.createElement(s);
                }
            }
            ((r[It] = e), (r[de] = a));
            t: for (f = e.child; f !== null; ) {
              if (f.tag === 5 || f.tag === 6) r.appendChild(f.stateNode);
              else if (f.tag !== 4 && f.tag !== 27 && f.child !== null) {
                ((f.child.return = f), (f = f.child));
                continue;
              }
              if (f === e) break t;
              for (; f.sibling === null; ) {
                if (f.return === null || f.return === e) break t;
                f = f.return;
              }
              ((f.sibling.return = f.return), (f = f.sibling));
            }
            e.stateNode = r;
            t: switch ((ie(r, s, a), s)) {
              case "button":
              case "input":
              case "select":
              case "textarea":
                a = !!a.autoFocus;
                break t;
              case "img":
                a = !0;
                break t;
              default:
                a = !1;
            }
            a && hn(e);
          }
        }
        return (wt(e), Ko(e, e.type, t === null ? null : t.memoizedProps, e.pendingProps, n), null);
      case 6:
        if (t && e.stateNode != null) t.memoizedProps !== a && hn(e);
        else {
          if (typeof a != "string" && e.stateNode === null) throw Error(o(166));
          if (((t = ct.current), Hi(e))) {
            if (((t = e.stateNode), (n = e.memoizedProps), (a = null), (s = te), s !== null))
              switch (s.tag) {
                case 27:
                case 5:
                  a = s.memoizedProps;
              }
            ((t[It] = e),
              (t = !!(
                t.nodeValue === n ||
                (a !== null && a.suppressHydrationWarning === !0) ||
                jm(t.nodeValue, n)
              )),
              t || Dn(e, !0));
          } else ((t = ws(t).createTextNode(a)), (t[It] = e), (e.stateNode = t));
        }
        return (wt(e), null);
      case 31:
        if (((n = e.memoizedState), t === null || t.memoizedState !== null)) {
          if (((a = Hi(e)), n !== null)) {
            if (t === null) {
              if (!a) throw Error(o(318));
              if (((t = e.memoizedState), (t = t !== null ? t.dehydrated : null), !t))
                throw Error(o(557));
              t[It] = e;
            } else (si(), (e.flags & 128) === 0 && (e.memoizedState = null), (e.flags |= 4));
            (wt(e), (t = !1));
          } else
            ((n = no()),
              t !== null && t.memoizedState !== null && (t.memoizedState.hydrationErrors = n),
              (t = !0));
          if (!t) return e.flags & 256 ? (Me(e), e) : (Me(e), null);
          if ((e.flags & 128) !== 0) throw Error(o(558));
        }
        return (wt(e), null);
      case 13:
        if (
          ((a = e.memoizedState),
          t === null || (t.memoizedState !== null && t.memoizedState.dehydrated !== null))
        ) {
          if (((s = Hi(e)), a !== null && a.dehydrated !== null)) {
            if (t === null) {
              if (!s) throw Error(o(318));
              if (((s = e.memoizedState), (s = s !== null ? s.dehydrated : null), !s))
                throw Error(o(317));
              s[It] = e;
            } else (si(), (e.flags & 128) === 0 && (e.memoizedState = null), (e.flags |= 4));
            (wt(e), (s = !1));
          } else
            ((s = no()),
              t !== null && t.memoizedState !== null && (t.memoizedState.hydrationErrors = s),
              (s = !0));
          if (!s) return e.flags & 256 ? (Me(e), e) : (Me(e), null);
        }
        return (
          Me(e),
          (e.flags & 128) !== 0
            ? ((e.lanes = n), e)
            : ((n = a !== null),
              (t = t !== null && t.memoizedState !== null),
              n &&
                ((a = e.child),
                (s = null),
                a.alternate !== null &&
                  a.alternate.memoizedState !== null &&
                  a.alternate.memoizedState.cachePool !== null &&
                  (s = a.alternate.memoizedState.cachePool.pool),
                (r = null),
                a.memoizedState !== null &&
                  a.memoizedState.cachePool !== null &&
                  (r = a.memoizedState.cachePool.pool),
                r !== s && (a.flags |= 2048)),
              n !== t && n && (e.child.flags |= 8192),
              gs(e, e.updateQueue),
              wt(e),
              null)
        );
      case 4:
        return (Ht(), t === null && pr(e.stateNode.containerInfo), wt(e), null);
      case 10:
        return (rn(e.type), wt(e), null);
      case 19:
        if ((B(qt), (a = e.memoizedState), a === null)) return (wt(e), null);
        if (((s = (e.flags & 128) !== 0), (r = a.rendering), r === null))
          if (s) Ka(a, !1);
          else {
            if (Bt !== 0 || (t !== null && (t.flags & 128) !== 0))
              for (t = e.child; t !== null; ) {
                if (((r = is(t)), r !== null)) {
                  for (
                    e.flags |= 128,
                      Ka(a, !1),
                      t = r.updateQueue,
                      e.updateQueue = t,
                      gs(e, t),
                      e.subtreeFlags = 0,
                      t = n,
                      n = e.child;
                    n !== null;
                  )
                    (od(n, t), (n = n.sibling));
                  return (Q(qt, (qt.current & 1) | 2), yt && un(e, a.treeForkCount), e.child);
                }
                t = t.sibling;
              }
            a.tail !== null &&
              xe() > Ss &&
              ((e.flags |= 128), (s = !0), Ka(a, !1), (e.lanes = 4194304));
          }
        else {
          if (!s)
            if (((t = is(r)), t !== null)) {
              if (
                ((e.flags |= 128),
                (s = !0),
                (t = t.updateQueue),
                (e.updateQueue = t),
                gs(e, t),
                Ka(a, !0),
                a.tail === null && a.tailMode === "hidden" && !r.alternate && !yt)
              )
                return (wt(e), null);
            } else
              2 * xe() - a.renderingStartTime > Ss &&
                n !== 536870912 &&
                ((e.flags |= 128), (s = !0), Ka(a, !1), (e.lanes = 4194304));
          a.isBackwards
            ? ((r.sibling = e.child), (e.child = r))
            : ((t = a.last), t !== null ? (t.sibling = r) : (e.child = r), (a.last = r));
        }
        return a.tail !== null
          ? ((t = a.tail),
            (a.rendering = t),
            (a.tail = t.sibling),
            (a.renderingStartTime = xe()),
            (t.sibling = null),
            (n = qt.current),
            Q(qt, s ? (n & 1) | 2 : n & 1),
            yt && un(e, a.treeForkCount),
            t)
          : (wt(e), null);
      case 22:
      case 23:
        return (
          Me(e),
          go(),
          (a = e.memoizedState !== null),
          t !== null
            ? (t.memoizedState !== null) !== a && (e.flags |= 8192)
            : a && (e.flags |= 8192),
          a
            ? (n & 536870912) !== 0 &&
              (e.flags & 128) === 0 &&
              (wt(e), e.subtreeFlags & 6 && (e.flags |= 8192))
            : wt(e),
          (n = e.updateQueue),
          n !== null && gs(e, n.retryQueue),
          (n = null),
          t !== null &&
            t.memoizedState !== null &&
            t.memoizedState.cachePool !== null &&
            (n = t.memoizedState.cachePool.pool),
          (a = null),
          e.memoizedState !== null &&
            e.memoizedState.cachePool !== null &&
            (a = e.memoizedState.cachePool.pool),
          a !== n && (e.flags |= 2048),
          t !== null && B(ri),
          null
        );
      case 24:
        return (
          (n = null),
          t !== null && (n = t.memoizedState.cache),
          e.memoizedState.cache !== n && (e.flags |= 2048),
          rn(Xt),
          wt(e),
          null
        );
      case 25:
        return null;
      case 30:
        return null;
    }
    throw Error(o(156, e.tag));
  }
  function n1(t, e) {
    switch ((to(e), e.tag)) {
      case 1:
        return ((t = e.flags), t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null);
      case 3:
        return (
          rn(Xt),
          Ht(),
          (t = e.flags),
          (t & 65536) !== 0 && (t & 128) === 0 ? ((e.flags = (t & -65537) | 128), e) : null
        );
      case 26:
      case 27:
      case 5:
        return (Cl(e), null);
      case 31:
        if (e.memoizedState !== null) {
          if ((Me(e), e.alternate === null)) throw Error(o(340));
          si();
        }
        return ((t = e.flags), t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null);
      case 13:
        if ((Me(e), (t = e.memoizedState), t !== null && t.dehydrated !== null)) {
          if (e.alternate === null) throw Error(o(340));
          si();
        }
        return ((t = e.flags), t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null);
      case 19:
        return (B(qt), null);
      case 4:
        return (Ht(), null);
      case 10:
        return (rn(e.type), null);
      case 22:
      case 23:
        return (
          Me(e),
          go(),
          t !== null && B(ri),
          (t = e.flags),
          t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
        );
      case 24:
        return (rn(Xt), null);
      case 25:
        return null;
      default:
        return null;
    }
  }
  function _h(t, e) {
    switch ((to(e), e.tag)) {
      case 3:
        (rn(Xt), Ht());
        break;
      case 26:
      case 27:
      case 5:
        Cl(e);
        break;
      case 4:
        Ht();
        break;
      case 31:
        e.memoizedState !== null && Me(e);
        break;
      case 13:
        Me(e);
        break;
      case 19:
        B(qt);
        break;
      case 10:
        rn(e.type);
        break;
      case 22:
      case 23:
        (Me(e), go(), t !== null && B(ri));
        break;
      case 24:
        rn(Xt);
    }
  }
  function Ja(t, e) {
    try {
      var n = e.updateQueue,
        a = n !== null ? n.lastEffect : null;
      if (a !== null) {
        var s = a.next;
        n = s;
        do {
          if ((n.tag & t) === t) {
            a = void 0;
            var r = n.create,
              f = n.inst;
            ((a = r()), (f.destroy = a));
          }
          n = n.next;
        } while (n !== s);
      }
    } catch (y) {
      Et(e, e.return, y);
    }
  }
  function On(t, e, n) {
    try {
      var a = e.updateQueue,
        s = a !== null ? a.lastEffect : null;
      if (s !== null) {
        var r = s.next;
        a = r;
        do {
          if ((a.tag & t) === t) {
            var f = a.inst,
              y = f.destroy;
            if (y !== void 0) {
              ((f.destroy = void 0), (s = e));
              var T = n,
                C = y;
              try {
                C();
              } catch (O) {
                Et(s, T, O);
              }
            }
          }
          a = a.next;
        } while (a !== r);
      }
    } catch (O) {
      Et(e, e.return, O);
    }
  }
  function Bh(t) {
    var e = t.updateQueue;
    if (e !== null) {
      var n = t.stateNode;
      try {
        Dd(e, n);
      } catch (a) {
        Et(t, t.return, a);
      }
    }
  }
  function Uh(t, e, n) {
    ((n.props = mi(t.type, t.memoizedProps)), (n.state = t.memoizedState));
    try {
      n.componentWillUnmount();
    } catch (a) {
      Et(t, e, a);
    }
  }
  function Fa(t, e) {
    try {
      var n = t.ref;
      if (n !== null) {
        switch (t.tag) {
          case 26:
          case 27:
          case 5:
            var a = t.stateNode;
            break;
          case 30:
            a = t.stateNode;
            break;
          default:
            a = t.stateNode;
        }
        typeof n == "function" ? (t.refCleanup = n(a)) : (n.current = a);
      }
    } catch (s) {
      Et(t, e, s);
    }
  }
  function We(t, e) {
    var n = t.ref,
      a = t.refCleanup;
    if (n !== null)
      if (typeof a == "function")
        try {
          a();
        } catch (s) {
          Et(t, e, s);
        } finally {
          ((t.refCleanup = null), (t = t.alternate), t != null && (t.refCleanup = null));
        }
      else if (typeof n == "function")
        try {
          n(null);
        } catch (s) {
          Et(t, e, s);
        }
      else n.current = null;
  }
  function Lh(t) {
    var e = t.type,
      n = t.memoizedProps,
      a = t.stateNode;
    try {
      t: switch (e) {
        case "button":
        case "input":
        case "select":
        case "textarea":
          n.autoFocus && a.focus();
          break t;
        case "img":
          n.src ? (a.src = n.src) : n.srcSet && (a.srcset = n.srcSet);
      }
    } catch (s) {
      Et(t, t.return, s);
    }
  }
  function Jo(t, e, n) {
    try {
      var a = t.stateNode;
      (E1(a, t.type, n, e), (a[de] = e));
    } catch (s) {
      Et(t, t.return, s);
    }
  }
  function Hh(t) {
    return (
      t.tag === 5 || t.tag === 3 || t.tag === 26 || (t.tag === 27 && qn(t.type)) || t.tag === 4
    );
  }
  function Fo(t) {
    t: for (;;) {
      for (; t.sibling === null; ) {
        if (t.return === null || Hh(t.return)) return null;
        t = t.return;
      }
      for (
        t.sibling.return = t.return, t = t.sibling;
        t.tag !== 5 && t.tag !== 6 && t.tag !== 18;
      ) {
        if ((t.tag === 27 && qn(t.type)) || t.flags & 2 || t.child === null || t.tag === 4)
          continue t;
        ((t.child.return = t), (t = t.child));
      }
      if (!(t.flags & 2)) return t.stateNode;
    }
  }
  function Po(t, e, n) {
    var a = t.tag;
    if (a === 5 || a === 6)
      ((t = t.stateNode),
        e
          ? (n.nodeType === 9
              ? n.body
              : n.nodeName === "HTML"
                ? n.ownerDocument.body
                : n
            ).insertBefore(t, e)
          : ((e = n.nodeType === 9 ? n.body : n.nodeName === "HTML" ? n.ownerDocument.body : n),
            e.appendChild(t),
            (n = n._reactRootContainer),
            n != null || e.onclick !== null || (e.onclick = an)));
    else if (
      a !== 4 &&
      (a === 27 && qn(t.type) && ((n = t.stateNode), (e = null)), (t = t.child), t !== null)
    )
      for (Po(t, e, n), t = t.sibling; t !== null; ) (Po(t, e, n), (t = t.sibling));
  }
  function ys(t, e, n) {
    var a = t.tag;
    if (a === 5 || a === 6) ((t = t.stateNode), e ? n.insertBefore(t, e) : n.appendChild(t));
    else if (a !== 4 && (a === 27 && qn(t.type) && (n = t.stateNode), (t = t.child), t !== null))
      for (ys(t, e, n), t = t.sibling; t !== null; ) (ys(t, e, n), (t = t.sibling));
  }
  function qh(t) {
    var e = t.stateNode,
      n = t.memoizedProps;
    try {
      for (var a = t.type, s = e.attributes; s.length; ) e.removeAttributeNode(s[0]);
      (ie(e, a, n), (e[It] = t), (e[de] = n));
    } catch (r) {
      Et(t, t.return, r);
    }
  }
  var mn = !1,
    Zt = !1,
    Wo = !1,
    Yh = typeof WeakSet == "function" ? WeakSet : Set,
    Wt = null;
  function i1(t, e) {
    if (((t = t.containerInfo), (vr = Ls), (t = If(t)), Xu(t))) {
      if ("selectionStart" in t) var n = { start: t.selectionStart, end: t.selectionEnd };
      else
        t: {
          n = ((n = t.ownerDocument) && n.defaultView) || window;
          var a = n.getSelection && n.getSelection();
          if (a && a.rangeCount !== 0) {
            n = a.anchorNode;
            var s = a.anchorOffset,
              r = a.focusNode;
            a = a.focusOffset;
            try {
              (n.nodeType, r.nodeType);
            } catch {
              n = null;
              break t;
            }
            var f = 0,
              y = -1,
              T = -1,
              C = 0,
              O = 0,
              _ = t,
              z = null;
            e: for (;;) {
              for (
                var j;
                _ !== n || (s !== 0 && _.nodeType !== 3) || (y = f + s),
                  _ !== r || (a !== 0 && _.nodeType !== 3) || (T = f + a),
                  _.nodeType === 3 && (f += _.nodeValue.length),
                  (j = _.firstChild) !== null;
              )
                ((z = _), (_ = j));
              for (;;) {
                if (_ === t) break e;
                if (
                  (z === n && ++C === s && (y = f),
                  z === r && ++O === a && (T = f),
                  (j = _.nextSibling) !== null)
                )
                  break;
                ((_ = z), (z = _.parentNode));
              }
              _ = j;
            }
            n = y === -1 || T === -1 ? null : { start: y, end: T };
          } else n = null;
        }
      n = n || { start: 0, end: 0 };
    } else n = null;
    for (xr = { focusedElem: t, selectionRange: n }, Ls = !1, Wt = e; Wt !== null; )
      if (((e = Wt), (t = e.child), (e.subtreeFlags & 1028) !== 0 && t !== null))
        ((t.return = e), (Wt = t));
      else
        for (; Wt !== null; ) {
          switch (((e = Wt), (r = e.alternate), (t = e.flags), e.tag)) {
            case 0:
              if (
                (t & 4) !== 0 &&
                ((t = e.updateQueue), (t = t !== null ? t.events : null), t !== null)
              )
                for (n = 0; n < t.length; n++) ((s = t[n]), (s.ref.impl = s.nextImpl));
              break;
            case 11:
            case 15:
              break;
            case 1:
              if ((t & 1024) !== 0 && r !== null) {
                ((t = void 0),
                  (n = e),
                  (s = r.memoizedProps),
                  (r = r.memoizedState),
                  (a = n.stateNode));
                try {
                  var K = mi(n.type, s);
                  ((t = a.getSnapshotBeforeUpdate(K, r)),
                    (a.__reactInternalSnapshotBeforeUpdate = t));
                } catch (nt) {
                  Et(n, n.return, nt);
                }
              }
              break;
            case 3:
              if ((t & 1024) !== 0) {
                if (((t = e.stateNode.containerInfo), (n = t.nodeType), n === 9)) Tr(t);
                else if (n === 1)
                  switch (t.nodeName) {
                    case "HEAD":
                    case "HTML":
                    case "BODY":
                      Tr(t);
                      break;
                    default:
                      t.textContent = "";
                  }
              }
              break;
            case 5:
            case 26:
            case 27:
            case 6:
            case 4:
            case 17:
              break;
            default:
              if ((t & 1024) !== 0) throw Error(o(163));
          }
          if (((t = e.sibling), t !== null)) {
            ((t.return = e.return), (Wt = t));
            break;
          }
          Wt = e.return;
        }
  }
  function Gh(t, e, n) {
    var a = n.flags;
    switch (n.tag) {
      case 0:
      case 11:
      case 15:
        (gn(t, n), a & 4 && Ja(5, n));
        break;
      case 1:
        if ((gn(t, n), a & 4))
          if (((t = n.stateNode), e === null))
            try {
              t.componentDidMount();
            } catch (f) {
              Et(n, n.return, f);
            }
          else {
            var s = mi(n.type, e.memoizedProps);
            e = e.memoizedState;
            try {
              t.componentDidUpdate(s, e, t.__reactInternalSnapshotBeforeUpdate);
            } catch (f) {
              Et(n, n.return, f);
            }
          }
        (a & 64 && Bh(n), a & 512 && Fa(n, n.return));
        break;
      case 3:
        if ((gn(t, n), a & 64 && ((t = n.updateQueue), t !== null))) {
          if (((e = null), n.child !== null))
            switch (n.child.tag) {
              case 27:
              case 5:
                e = n.child.stateNode;
                break;
              case 1:
                e = n.child.stateNode;
            }
          try {
            Dd(t, e);
          } catch (f) {
            Et(n, n.return, f);
          }
        }
        break;
      case 27:
        e === null && a & 4 && qh(n);
      case 26:
      case 5:
        (gn(t, n), e === null && a & 4 && Lh(n), a & 512 && Fa(n, n.return));
        break;
      case 12:
        gn(t, n);
        break;
      case 31:
        (gn(t, n), a & 4 && Qh(t, n));
        break;
      case 13:
        (gn(t, n),
          a & 4 && Zh(t, n),
          a & 64 &&
            ((t = n.memoizedState),
            t !== null && ((t = t.dehydrated), t !== null && ((n = d1.bind(null, n)), R1(t, n)))));
        break;
      case 22:
        if (((a = n.memoizedState !== null || mn), !a)) {
          ((e = (e !== null && e.memoizedState !== null) || Zt), (s = mn));
          var r = Zt;
          ((mn = a),
            (Zt = e) && !r ? yn(t, n, (n.subtreeFlags & 8772) !== 0) : gn(t, n),
            (mn = s),
            (Zt = r));
        }
        break;
      case 30:
        break;
      default:
        gn(t, n);
    }
  }
  function Xh(t) {
    var e = t.alternate;
    (e !== null && ((t.alternate = null), Xh(e)),
      (t.child = null),
      (t.deletions = null),
      (t.sibling = null),
      t.tag === 5 && ((e = t.stateNode), e !== null && Du(e)),
      (t.stateNode = null),
      (t.return = null),
      (t.dependencies = null),
      (t.memoizedProps = null),
      (t.memoizedState = null),
      (t.pendingProps = null),
      (t.stateNode = null),
      (t.updateQueue = null));
  }
  var Rt = null,
    me = !1;
  function pn(t, e, n) {
    for (n = n.child; n !== null; ) (kh(t, e, n), (n = n.sibling));
  }
  function kh(t, e, n) {
    if (be && typeof be.onCommitFiberUnmount == "function")
      try {
        be.onCommitFiberUnmount(xa, n);
      } catch {}
    switch (n.tag) {
      case 26:
        (Zt || We(n, e),
          pn(t, e, n),
          n.memoizedState
            ? n.memoizedState.count--
            : n.stateNode && ((n = n.stateNode), n.parentNode.removeChild(n)));
        break;
      case 27:
        Zt || We(n, e);
        var a = Rt,
          s = me;
        (qn(n.type) && ((Rt = n.stateNode), (me = !1)),
          pn(t, e, n),
          al(n.stateNode),
          (Rt = a),
          (me = s));
        break;
      case 5:
        Zt || We(n, e);
      case 6:
        if (((a = Rt), (s = me), (Rt = null), pn(t, e, n), (Rt = a), (me = s), Rt !== null))
          if (me)
            try {
              (Rt.nodeType === 9
                ? Rt.body
                : Rt.nodeName === "HTML"
                  ? Rt.ownerDocument.body
                  : Rt
              ).removeChild(n.stateNode);
            } catch (r) {
              Et(n, e, r);
            }
          else
            try {
              Rt.removeChild(n.stateNode);
            } catch (r) {
              Et(n, e, r);
            }
        break;
      case 18:
        Rt !== null &&
          (me
            ? ((t = Rt),
              Bm(
                t.nodeType === 9 ? t.body : t.nodeName === "HTML" ? t.ownerDocument.body : t,
                n.stateNode,
              ),
              sa(t))
            : Bm(Rt, n.stateNode));
        break;
      case 4:
        ((a = Rt),
          (s = me),
          (Rt = n.stateNode.containerInfo),
          (me = !0),
          pn(t, e, n),
          (Rt = a),
          (me = s));
        break;
      case 0:
      case 11:
      case 14:
      case 15:
        (On(2, n, e), Zt || On(4, n, e), pn(t, e, n));
        break;
      case 1:
        (Zt ||
          (We(n, e), (a = n.stateNode), typeof a.componentWillUnmount == "function" && Uh(n, e, a)),
          pn(t, e, n));
        break;
      case 21:
        pn(t, e, n);
        break;
      case 22:
        ((Zt = (a = Zt) || n.memoizedState !== null), pn(t, e, n), (Zt = a));
        break;
      default:
        pn(t, e, n);
    }
  }
  function Qh(t, e) {
    if (
      e.memoizedState === null &&
      ((t = e.alternate), t !== null && ((t = t.memoizedState), t !== null))
    ) {
      t = t.dehydrated;
      try {
        sa(t);
      } catch (n) {
        Et(e, e.return, n);
      }
    }
  }
  function Zh(t, e) {
    if (
      e.memoizedState === null &&
      ((t = e.alternate),
      t !== null && ((t = t.memoizedState), t !== null && ((t = t.dehydrated), t !== null)))
    )
      try {
        sa(t);
      } catch (n) {
        Et(e, e.return, n);
      }
  }
  function a1(t) {
    switch (t.tag) {
      case 31:
      case 13:
      case 19:
        var e = t.stateNode;
        return (e === null && (e = t.stateNode = new Yh()), e);
      case 22:
        return (
          (t = t.stateNode),
          (e = t._retryCache),
          e === null && (e = t._retryCache = new Yh()),
          e
        );
      default:
        throw Error(o(435, t.tag));
    }
  }
  function vs(t, e) {
    var n = a1(t);
    e.forEach(function (a) {
      if (!n.has(a)) {
        n.add(a);
        var s = h1.bind(null, t, a);
        a.then(s, s);
      }
    });
  }
  function pe(t, e) {
    var n = e.deletions;
    if (n !== null)
      for (var a = 0; a < n.length; a++) {
        var s = n[a],
          r = t,
          f = e,
          y = f;
        t: for (; y !== null; ) {
          switch (y.tag) {
            case 27:
              if (qn(y.type)) {
                ((Rt = y.stateNode), (me = !1));
                break t;
              }
              break;
            case 5:
              ((Rt = y.stateNode), (me = !1));
              break t;
            case 3:
            case 4:
              ((Rt = y.stateNode.containerInfo), (me = !0));
              break t;
          }
          y = y.return;
        }
        if (Rt === null) throw Error(o(160));
        (kh(r, f, s),
          (Rt = null),
          (me = !1),
          (r = s.alternate),
          r !== null && (r.return = null),
          (s.return = null));
      }
    if (e.subtreeFlags & 13886) for (e = e.child; e !== null; ) (Kh(e, t), (e = e.sibling));
  }
  var ke = null;
  function Kh(t, e) {
    var n = t.alternate,
      a = t.flags;
    switch (t.tag) {
      case 0:
      case 11:
      case 14:
      case 15:
        (pe(e, t), ge(t), a & 4 && (On(3, t, t.return), Ja(3, t), On(5, t, t.return)));
        break;
      case 1:
        (pe(e, t),
          ge(t),
          a & 512 && (Zt || n === null || We(n, n.return)),
          a & 64 &&
            mn &&
            ((t = t.updateQueue),
            t !== null &&
              ((a = t.callbacks),
              a !== null &&
                ((n = t.shared.hiddenCallbacks),
                (t.shared.hiddenCallbacks = n === null ? a : n.concat(a))))));
        break;
      case 26:
        var s = ke;
        if ((pe(e, t), ge(t), a & 512 && (Zt || n === null || We(n, n.return)), a & 4)) {
          var r = n !== null ? n.memoizedState : null;
          if (((a = t.memoizedState), n === null))
            if (a === null)
              if (t.stateNode === null) {
                t: {
                  ((a = t.type), (n = t.memoizedProps), (s = s.ownerDocument || s));
                  e: switch (a) {
                    case "title":
                      ((r = s.getElementsByTagName("title")[0]),
                        (!r ||
                          r[Ta] ||
                          r[It] ||
                          r.namespaceURI === "http://www.w3.org/2000/svg" ||
                          r.hasAttribute("itemprop")) &&
                          ((r = s.createElement(a)),
                          s.head.insertBefore(r, s.querySelector("head > title"))),
                        ie(r, a, n),
                        (r[It] = t),
                        Pt(r),
                        (a = r));
                      break t;
                    case "link":
                      var f = Km("link", "href", s).get(a + (n.href || ""));
                      if (f) {
                        for (var y = 0; y < f.length; y++)
                          if (
                            ((r = f[y]),
                            r.getAttribute("href") ===
                              (n.href == null || n.href === "" ? null : n.href) &&
                              r.getAttribute("rel") === (n.rel == null ? null : n.rel) &&
                              r.getAttribute("title") === (n.title == null ? null : n.title) &&
                              r.getAttribute("crossorigin") ===
                                (n.crossOrigin == null ? null : n.crossOrigin))
                          ) {
                            f.splice(y, 1);
                            break e;
                          }
                      }
                      ((r = s.createElement(a)), ie(r, a, n), s.head.appendChild(r));
                      break;
                    case "meta":
                      if ((f = Km("meta", "content", s).get(a + (n.content || "")))) {
                        for (y = 0; y < f.length; y++)
                          if (
                            ((r = f[y]),
                            r.getAttribute("content") ===
                              (n.content == null ? null : "" + n.content) &&
                              r.getAttribute("name") === (n.name == null ? null : n.name) &&
                              r.getAttribute("property") ===
                                (n.property == null ? null : n.property) &&
                              r.getAttribute("http-equiv") ===
                                (n.httpEquiv == null ? null : n.httpEquiv) &&
                              r.getAttribute("charset") === (n.charSet == null ? null : n.charSet))
                          ) {
                            f.splice(y, 1);
                            break e;
                          }
                      }
                      ((r = s.createElement(a)), ie(r, a, n), s.head.appendChild(r));
                      break;
                    default:
                      throw Error(o(468, a));
                  }
                  ((r[It] = t), Pt(r), (a = r));
                }
                t.stateNode = a;
              } else Jm(s, t.type, t.stateNode);
            else t.stateNode = Zm(s, a, t.memoizedProps);
          else
            r !== a
              ? (r === null
                  ? n.stateNode !== null && ((n = n.stateNode), n.parentNode.removeChild(n))
                  : r.count--,
                a === null ? Jm(s, t.type, t.stateNode) : Zm(s, a, t.memoizedProps))
              : a === null && t.stateNode !== null && Jo(t, t.memoizedProps, n.memoizedProps);
        }
        break;
      case 27:
        (pe(e, t),
          ge(t),
          a & 512 && (Zt || n === null || We(n, n.return)),
          n !== null && a & 4 && Jo(t, t.memoizedProps, n.memoizedProps));
        break;
      case 5:
        if ((pe(e, t), ge(t), a & 512 && (Zt || n === null || We(n, n.return)), t.flags & 32)) {
          s = t.stateNode;
          try {
            zi(s, "");
          } catch (K) {
            Et(t, t.return, K);
          }
        }
        (a & 4 &&
          t.stateNode != null &&
          ((s = t.memoizedProps), Jo(t, s, n !== null ? n.memoizedProps : s)),
          a & 1024 && (Wo = !0));
        break;
      case 6:
        if ((pe(e, t), ge(t), a & 4)) {
          if (t.stateNode === null) throw Error(o(162));
          ((a = t.memoizedProps), (n = t.stateNode));
          try {
            n.nodeValue = a;
          } catch (K) {
            Et(t, t.return, K);
          }
        }
        break;
      case 3:
        if (
          ((Vs = null),
          (s = ke),
          (ke = Rs(e.containerInfo)),
          pe(e, t),
          (ke = s),
          ge(t),
          a & 4 && n !== null && n.memoizedState.isDehydrated)
        )
          try {
            sa(e.containerInfo);
          } catch (K) {
            Et(t, t.return, K);
          }
        Wo && ((Wo = !1), Jh(t));
        break;
      case 4:
        ((a = ke), (ke = Rs(t.stateNode.containerInfo)), pe(e, t), ge(t), (ke = a));
        break;
      case 12:
        (pe(e, t), ge(t));
        break;
      case 31:
        (pe(e, t),
          ge(t),
          a & 4 && ((a = t.updateQueue), a !== null && ((t.updateQueue = null), vs(t, a))));
        break;
      case 13:
        (pe(e, t),
          ge(t),
          t.child.flags & 8192 &&
            (t.memoizedState !== null) != (n !== null && n.memoizedState !== null) &&
            (bs = xe()),
          a & 4 && ((a = t.updateQueue), a !== null && ((t.updateQueue = null), vs(t, a))));
        break;
      case 22:
        s = t.memoizedState !== null;
        var T = n !== null && n.memoizedState !== null,
          C = mn,
          O = Zt;
        if (((mn = C || s), (Zt = O || T), pe(e, t), (Zt = O), (mn = C), ge(t), a & 8192))
          t: for (
            e = t.stateNode,
              e._visibility = s ? e._visibility & -2 : e._visibility | 1,
              s && (n === null || T || mn || Zt || pi(t)),
              n = null,
              e = t;
            ;
          ) {
            if (e.tag === 5 || e.tag === 26) {
              if (n === null) {
                T = n = e;
                try {
                  if (((r = T.stateNode), s))
                    ((f = r.style),
                      typeof f.setProperty == "function"
                        ? f.setProperty("display", "none", "important")
                        : (f.display = "none"));
                  else {
                    y = T.stateNode;
                    var _ = T.memoizedProps.style,
                      z = _ != null && _.hasOwnProperty("display") ? _.display : null;
                    y.style.display = z == null || typeof z == "boolean" ? "" : ("" + z).trim();
                  }
                } catch (K) {
                  Et(T, T.return, K);
                }
              }
            } else if (e.tag === 6) {
              if (n === null) {
                T = e;
                try {
                  T.stateNode.nodeValue = s ? "" : T.memoizedProps;
                } catch (K) {
                  Et(T, T.return, K);
                }
              }
            } else if (e.tag === 18) {
              if (n === null) {
                T = e;
                try {
                  var j = T.stateNode;
                  s ? Um(j, !0) : Um(T.stateNode, !1);
                } catch (K) {
                  Et(T, T.return, K);
                }
              }
            } else if (
              ((e.tag !== 22 && e.tag !== 23) || e.memoizedState === null || e === t) &&
              e.child !== null
            ) {
              ((e.child.return = e), (e = e.child));
              continue;
            }
            if (e === t) break t;
            for (; e.sibling === null; ) {
              if (e.return === null || e.return === t) break t;
              (n === e && (n = null), (e = e.return));
            }
            (n === e && (n = null), (e.sibling.return = e.return), (e = e.sibling));
          }
        a & 4 &&
          ((a = t.updateQueue),
          a !== null && ((n = a.retryQueue), n !== null && ((a.retryQueue = null), vs(t, n))));
        break;
      case 19:
        (pe(e, t),
          ge(t),
          a & 4 && ((a = t.updateQueue), a !== null && ((t.updateQueue = null), vs(t, a))));
        break;
      case 30:
        break;
      case 21:
        break;
      default:
        (pe(e, t), ge(t));
    }
  }
  function ge(t) {
    var e = t.flags;
    if (e & 2) {
      try {
        for (var n, a = t.return; a !== null; ) {
          if (Hh(a)) {
            n = a;
            break;
          }
          a = a.return;
        }
        if (n == null) throw Error(o(160));
        switch (n.tag) {
          case 27:
            var s = n.stateNode,
              r = Fo(t);
            ys(t, r, s);
            break;
          case 5:
            var f = n.stateNode;
            n.flags & 32 && (zi(f, ""), (n.flags &= -33));
            var y = Fo(t);
            ys(t, y, f);
            break;
          case 3:
          case 4:
            var T = n.stateNode.containerInfo,
              C = Fo(t);
            Po(t, C, T);
            break;
          default:
            throw Error(o(161));
        }
      } catch (O) {
        Et(t, t.return, O);
      }
      t.flags &= -3;
    }
    e & 4096 && (t.flags &= -4097);
  }
  function Jh(t) {
    if (t.subtreeFlags & 1024)
      for (t = t.child; t !== null; ) {
        var e = t;
        (Jh(e), e.tag === 5 && e.flags & 1024 && e.stateNode.reset(), (t = t.sibling));
      }
  }
  function gn(t, e) {
    if (e.subtreeFlags & 8772)
      for (e = e.child; e !== null; ) (Gh(t, e.alternate, e), (e = e.sibling));
  }
  function pi(t) {
    for (t = t.child; t !== null; ) {
      var e = t;
      switch (e.tag) {
        case 0:
        case 11:
        case 14:
        case 15:
          (On(4, e, e.return), pi(e));
          break;
        case 1:
          We(e, e.return);
          var n = e.stateNode;
          (typeof n.componentWillUnmount == "function" && Uh(e, e.return, n), pi(e));
          break;
        case 27:
          al(e.stateNode);
        case 26:
        case 5:
          (We(e, e.return), pi(e));
          break;
        case 22:
          e.memoizedState === null && pi(e);
          break;
        case 30:
          pi(e);
          break;
        default:
          pi(e);
      }
      t = t.sibling;
    }
  }
  function yn(t, e, n) {
    for (n = n && (e.subtreeFlags & 8772) !== 0, e = e.child; e !== null; ) {
      var a = e.alternate,
        s = t,
        r = e,
        f = r.flags;
      switch (r.tag) {
        case 0:
        case 11:
        case 15:
          (yn(s, r, n), Ja(4, r));
          break;
        case 1:
          if ((yn(s, r, n), (a = r), (s = a.stateNode), typeof s.componentDidMount == "function"))
            try {
              s.componentDidMount();
            } catch (C) {
              Et(a, a.return, C);
            }
          if (((a = r), (s = a.updateQueue), s !== null)) {
            var y = a.stateNode;
            try {
              var T = s.shared.hiddenCallbacks;
              if (T !== null)
                for (s.shared.hiddenCallbacks = null, s = 0; s < T.length; s++) Md(T[s], y);
            } catch (C) {
              Et(a, a.return, C);
            }
          }
          (n && f & 64 && Bh(r), Fa(r, r.return));
          break;
        case 27:
          qh(r);
        case 26:
        case 5:
          (yn(s, r, n), n && a === null && f & 4 && Lh(r), Fa(r, r.return));
          break;
        case 12:
          yn(s, r, n);
          break;
        case 31:
          (yn(s, r, n), n && f & 4 && Qh(s, r));
          break;
        case 13:
          (yn(s, r, n), n && f & 4 && Zh(s, r));
          break;
        case 22:
          (r.memoizedState === null && yn(s, r, n), Fa(r, r.return));
          break;
        case 30:
          break;
        default:
          yn(s, r, n);
      }
      e = e.sibling;
    }
  }
  function $o(t, e) {
    var n = null;
    (t !== null &&
      t.memoizedState !== null &&
      t.memoizedState.cachePool !== null &&
      (n = t.memoizedState.cachePool.pool),
      (t = null),
      e.memoizedState !== null &&
        e.memoizedState.cachePool !== null &&
        (t = e.memoizedState.cachePool.pool),
      t !== n && (t != null && t.refCount++, n != null && _a(n)));
  }
  function Io(t, e) {
    ((t = null),
      e.alternate !== null && (t = e.alternate.memoizedState.cache),
      (e = e.memoizedState.cache),
      e !== t && (e.refCount++, t != null && _a(t)));
  }
  function Qe(t, e, n, a) {
    if (e.subtreeFlags & 10256) for (e = e.child; e !== null; ) (Fh(t, e, n, a), (e = e.sibling));
  }
  function Fh(t, e, n, a) {
    var s = e.flags;
    switch (e.tag) {
      case 0:
      case 11:
      case 15:
        (Qe(t, e, n, a), s & 2048 && Ja(9, e));
        break;
      case 1:
        Qe(t, e, n, a);
        break;
      case 3:
        (Qe(t, e, n, a),
          s & 2048 &&
            ((t = null),
            e.alternate !== null && (t = e.alternate.memoizedState.cache),
            (e = e.memoizedState.cache),
            e !== t && (e.refCount++, t != null && _a(t))));
        break;
      case 12:
        if (s & 2048) {
          (Qe(t, e, n, a), (t = e.stateNode));
          try {
            var r = e.memoizedProps,
              f = r.id,
              y = r.onPostCommit;
            typeof y == "function" &&
              y(f, e.alternate === null ? "mount" : "update", t.passiveEffectDuration, -0);
          } catch (T) {
            Et(e, e.return, T);
          }
        } else Qe(t, e, n, a);
        break;
      case 31:
        Qe(t, e, n, a);
        break;
      case 13:
        Qe(t, e, n, a);
        break;
      case 23:
        break;
      case 22:
        ((r = e.stateNode),
          (f = e.alternate),
          e.memoizedState !== null
            ? r._visibility & 2
              ? Qe(t, e, n, a)
              : Pa(t, e)
            : r._visibility & 2
              ? Qe(t, e, n, a)
              : ((r._visibility |= 2), Fi(t, e, n, a, (e.subtreeFlags & 10256) !== 0 || !1)),
          s & 2048 && $o(f, e));
        break;
      case 24:
        (Qe(t, e, n, a), s & 2048 && Io(e.alternate, e));
        break;
      default:
        Qe(t, e, n, a);
    }
  }
  function Fi(t, e, n, a, s) {
    for (s = s && ((e.subtreeFlags & 10256) !== 0 || !1), e = e.child; e !== null; ) {
      var r = t,
        f = e,
        y = n,
        T = a,
        C = f.flags;
      switch (f.tag) {
        case 0:
        case 11:
        case 15:
          (Fi(r, f, y, T, s), Ja(8, f));
          break;
        case 23:
          break;
        case 22:
          var O = f.stateNode;
          (f.memoizedState !== null
            ? O._visibility & 2
              ? Fi(r, f, y, T, s)
              : Pa(r, f)
            : ((O._visibility |= 2), Fi(r, f, y, T, s)),
            s && C & 2048 && $o(f.alternate, f));
          break;
        case 24:
          (Fi(r, f, y, T, s), s && C & 2048 && Io(f.alternate, f));
          break;
        default:
          Fi(r, f, y, T, s);
      }
      e = e.sibling;
    }
  }
  function Pa(t, e) {
    if (e.subtreeFlags & 10256)
      for (e = e.child; e !== null; ) {
        var n = t,
          a = e,
          s = a.flags;
        switch (a.tag) {
          case 22:
            (Pa(n, a), s & 2048 && $o(a.alternate, a));
            break;
          case 24:
            (Pa(n, a), s & 2048 && Io(a.alternate, a));
            break;
          default:
            Pa(n, a);
        }
        e = e.sibling;
      }
  }
  var Wa = 8192;
  function Pi(t, e, n) {
    if (t.subtreeFlags & Wa) for (t = t.child; t !== null; ) (Ph(t, e, n), (t = t.sibling));
  }
  function Ph(t, e, n) {
    switch (t.tag) {
      case 26:
        (Pi(t, e, n),
          t.flags & Wa && t.memoizedState !== null && k1(n, ke, t.memoizedState, t.memoizedProps));
        break;
      case 5:
        Pi(t, e, n);
        break;
      case 3:
      case 4:
        var a = ke;
        ((ke = Rs(t.stateNode.containerInfo)), Pi(t, e, n), (ke = a));
        break;
      case 22:
        t.memoizedState === null &&
          ((a = t.alternate),
          a !== null && a.memoizedState !== null
            ? ((a = Wa), (Wa = 16777216), Pi(t, e, n), (Wa = a))
            : Pi(t, e, n));
        break;
      default:
        Pi(t, e, n);
    }
  }
  function Wh(t) {
    var e = t.alternate;
    if (e !== null && ((t = e.child), t !== null)) {
      e.child = null;
      do ((e = t.sibling), (t.sibling = null), (t = e));
      while (t !== null);
    }
  }
  function $a(t) {
    var e = t.deletions;
    if ((t.flags & 16) !== 0) {
      if (e !== null)
        for (var n = 0; n < e.length; n++) {
          var a = e[n];
          ((Wt = a), Ih(a, t));
        }
      Wh(t);
    }
    if (t.subtreeFlags & 10256) for (t = t.child; t !== null; ) ($h(t), (t = t.sibling));
  }
  function $h(t) {
    switch (t.tag) {
      case 0:
      case 11:
      case 15:
        ($a(t), t.flags & 2048 && On(9, t, t.return));
        break;
      case 3:
        $a(t);
        break;
      case 12:
        $a(t);
        break;
      case 22:
        var e = t.stateNode;
        t.memoizedState !== null && e._visibility & 2 && (t.return === null || t.return.tag !== 13)
          ? ((e._visibility &= -3), xs(t))
          : $a(t);
        break;
      default:
        $a(t);
    }
  }
  function xs(t) {
    var e = t.deletions;
    if ((t.flags & 16) !== 0) {
      if (e !== null)
        for (var n = 0; n < e.length; n++) {
          var a = e[n];
          ((Wt = a), Ih(a, t));
        }
      Wh(t);
    }
    for (t = t.child; t !== null; ) {
      switch (((e = t), e.tag)) {
        case 0:
        case 11:
        case 15:
          (On(8, e, e.return), xs(e));
          break;
        case 22:
          ((n = e.stateNode), n._visibility & 2 && ((n._visibility &= -3), xs(e)));
          break;
        default:
          xs(e);
      }
      t = t.sibling;
    }
  }
  function Ih(t, e) {
    for (; Wt !== null; ) {
      var n = Wt;
      switch (n.tag) {
        case 0:
        case 11:
        case 15:
          On(8, n, e);
          break;
        case 23:
        case 22:
          if (n.memoizedState !== null && n.memoizedState.cachePool !== null) {
            var a = n.memoizedState.cachePool.pool;
            a != null && a.refCount++;
          }
          break;
        case 24:
          _a(n.memoizedState.cache);
      }
      if (((a = n.child), a !== null)) ((a.return = n), (Wt = a));
      else
        t: for (n = t; Wt !== null; ) {
          a = Wt;
          var s = a.sibling,
            r = a.return;
          if ((Xh(a), a === n)) {
            Wt = null;
            break t;
          }
          if (s !== null) {
            ((s.return = r), (Wt = s));
            break t;
          }
          Wt = r;
        }
    }
  }
  var l1 = {
      getCacheForType: function (t) {
        var e = ee(Xt),
          n = e.data.get(t);
        return (n === void 0 && ((n = t()), e.data.set(t, n)), n);
      },
      cacheSignal: function () {
        return ee(Xt).controller.signal;
      },
    },
    s1 = typeof WeakMap == "function" ? WeakMap : Map,
    St = 0,
    Nt = null,
    ft = null,
    ht = 0,
    At = 0,
    De = null,
    Vn = !1,
    Wi = !1,
    tr = !1,
    vn = 0,
    Bt = 0,
    _n = 0,
    gi = 0,
    er = 0,
    Ce = 0,
    $i = 0,
    Ia = null,
    ye = null,
    nr = !1,
    bs = 0,
    tm = 0,
    Ss = 1 / 0,
    Ts = null,
    Bn = null,
    Jt = 0,
    Un = null,
    Ii = null,
    xn = 0,
    ir = 0,
    ar = null,
    em = null,
    tl = 0,
    lr = null;
  function Ne() {
    return (St & 2) !== 0 && ht !== 0 ? ht & -ht : R.T !== null ? fr() : yf();
  }
  function nm() {
    if (Ce === 0)
      if ((ht & 536870912) === 0 || yt) {
        var t = jl;
        ((jl <<= 1), (jl & 3932160) === 0 && (jl = 262144), (Ce = t));
      } else Ce = 536870912;
    return ((t = Ee.current), t !== null && (t.flags |= 32), Ce);
  }
  function ve(t, e, n) {
    (((t === Nt && (At === 2 || At === 9)) || t.cancelPendingCommit !== null) &&
      (ta(t, 0), Ln(t, ht, Ce, !1)),
      Sa(t, n),
      ((St & 2) === 0 || t !== Nt) &&
        (t === Nt && ((St & 2) === 0 && (gi |= n), Bt === 4 && Ln(t, ht, Ce, !1)), $e(t)));
  }
  function im(t, e, n) {
    if ((St & 6) !== 0) throw Error(o(327));
    var a = (!n && (e & 127) === 0 && (e & t.expiredLanes) === 0) || ba(t, e),
      s = a ? r1(t, e) : ur(t, e, !0),
      r = a;
    do {
      if (s === 0) {
        Wi && !a && Ln(t, e, 0, !1);
        break;
      } else {
        if (((n = t.current.alternate), r && !u1(n))) {
          ((s = ur(t, e, !1)), (r = !1));
          continue;
        }
        if (s === 2) {
          if (((r = e), t.errorRecoveryDisabledLanes & r)) var f = 0;
          else
            ((f = t.pendingLanes & -536870913), (f = f !== 0 ? f : f & 536870912 ? 536870912 : 0));
          if (f !== 0) {
            e = f;
            t: {
              var y = t;
              s = Ia;
              var T = y.current.memoizedState.isDehydrated;
              if ((T && (ta(y, f).flags |= 256), (f = ur(y, f, !1)), f !== 2)) {
                if (tr && !T) {
                  ((y.errorRecoveryDisabledLanes |= r), (gi |= r), (s = 4));
                  break t;
                }
                ((r = ye), (ye = s), r !== null && (ye === null ? (ye = r) : ye.push.apply(ye, r)));
              }
              s = f;
            }
            if (((r = !1), s !== 2)) continue;
          }
        }
        if (s === 1) {
          (ta(t, 0), Ln(t, e, 0, !0));
          break;
        }
        t: {
          switch (((a = t), (r = s), r)) {
            case 0:
            case 1:
              throw Error(o(345));
            case 4:
              if ((e & 4194048) !== e) break;
            case 6:
              Ln(a, e, Ce, !Vn);
              break t;
            case 2:
              ye = null;
              break;
            case 3:
            case 5:
              break;
            default:
              throw Error(o(329));
          }
          if ((e & 62914560) === e && ((s = bs + 300 - xe()), 10 < s)) {
            if ((Ln(a, e, Ce, !Vn), Rl(a, 0, !0) !== 0)) break t;
            ((xn = e),
              (a.timeoutHandle = Vm(
                am.bind(null, a, n, ye, Ts, nr, e, Ce, gi, $i, Vn, r, "Throttled", -0, 0),
                s,
              )));
            break t;
          }
          am(a, n, ye, Ts, nr, e, Ce, gi, $i, Vn, r, null, -0, 0);
        }
      }
      break;
    } while (!0);
    $e(t);
  }
  function am(t, e, n, a, s, r, f, y, T, C, O, _, z, j) {
    if (((t.timeoutHandle = -1), (_ = e.subtreeFlags), _ & 8192 || (_ & 16785408) === 16785408)) {
      ((_ = {
        stylesheets: null,
        count: 0,
        imgCount: 0,
        imgBytes: 0,
        suspenseyImages: [],
        waitingForImages: !0,
        waitingForViewTransition: !1,
        unsuspend: an,
      }),
        Ph(e, r, _));
      var K = (r & 62914560) === r ? bs - xe() : (r & 4194048) === r ? tm - xe() : 0;
      if (((K = Q1(_, K)), K !== null)) {
        ((xn = r),
          (t.cancelPendingCommit = K(dm.bind(null, t, e, r, n, a, s, f, y, T, O, _, null, z, j))),
          Ln(t, r, f, !C));
        return;
      }
    }
    dm(t, e, r, n, a, s, f, y, T);
  }
  function u1(t) {
    for (var e = t; ; ) {
      var n = e.tag;
      if (
        (n === 0 || n === 11 || n === 15) &&
        e.flags & 16384 &&
        ((n = e.updateQueue), n !== null && ((n = n.stores), n !== null))
      )
        for (var a = 0; a < n.length; a++) {
          var s = n[a],
            r = s.getSnapshot;
          s = s.value;
          try {
            if (!Te(r(), s)) return !1;
          } catch {
            return !1;
          }
        }
      if (((n = e.child), e.subtreeFlags & 16384 && n !== null)) ((n.return = e), (e = n));
      else {
        if (e === t) break;
        for (; e.sibling === null; ) {
          if (e.return === null || e.return === t) return !0;
          e = e.return;
        }
        ((e.sibling.return = e.return), (e = e.sibling));
      }
    }
    return !0;
  }
  function Ln(t, e, n, a) {
    ((e &= ~er),
      (e &= ~gi),
      (t.suspendedLanes |= e),
      (t.pingedLanes &= ~e),
      a && (t.warmLanes |= e),
      (a = t.expirationTimes));
    for (var s = e; 0 < s; ) {
      var r = 31 - Se(s),
        f = 1 << r;
      ((a[r] = -1), (s &= ~f));
    }
    n !== 0 && mf(t, n, e);
  }
  function As() {
    return (St & 6) === 0 ? (el(0), !1) : !0;
  }
  function sr() {
    if (ft !== null) {
      if (At === 0) var t = ft.return;
      else ((t = ft), (on = ui = null), To(t), (ki = null), (Ua = 0), (t = ft));
      for (; t !== null; ) (_h(t.alternate, t), (t = t.return));
      ft = null;
    }
  }
  function ta(t, e) {
    var n = t.timeoutHandle;
    (n !== -1 && ((t.timeoutHandle = -1), C1(n)),
      (n = t.cancelPendingCommit),
      n !== null && ((t.cancelPendingCommit = null), n()),
      (xn = 0),
      sr(),
      (Nt = t),
      (ft = n = sn(t.current, null)),
      (ht = e),
      (At = 0),
      (De = null),
      (Vn = !1),
      (Wi = ba(t, e)),
      (tr = !1),
      ($i = Ce = er = gi = _n = Bt = 0),
      (ye = Ia = null),
      (nr = !1),
      (e & 8) !== 0 && (e |= e & 32));
    var a = t.entangledLanes;
    if (a !== 0)
      for (t = t.entanglements, a &= e; 0 < a; ) {
        var s = 31 - Se(a),
          r = 1 << s;
        ((e |= t[s]), (a &= ~r));
      }
    return ((vn = e), kl(), n);
  }
  function lm(t, e) {
    ((ut = null),
      (R.H = Qa),
      e === Xi || e === $l
        ? ((e = Sd()), (At = 3))
        : e === ro
          ? ((e = Sd()), (At = 4))
          : (At =
              e === Lo
                ? 8
                : e !== null && typeof e == "object" && typeof e.then == "function"
                  ? 6
                  : 1),
      (De = e),
      ft === null && ((Bt = 1), ds(t, Ve(e, t.current))));
  }
  function sm() {
    var t = Ee.current;
    return t === null
      ? !0
      : (ht & 4194048) === ht
        ? Le === null
        : (ht & 62914560) === ht || (ht & 536870912) !== 0
          ? t === Le
          : !1;
  }
  function um() {
    var t = R.H;
    return ((R.H = Qa), t === null ? Qa : t);
  }
  function om() {
    var t = R.A;
    return ((R.A = l1), t);
  }
  function Es() {
    ((Bt = 4),
      Vn || ((ht & 4194048) !== ht && Ee.current !== null) || (Wi = !0),
      ((_n & 134217727) === 0 && (gi & 134217727) === 0) || Nt === null || Ln(Nt, ht, Ce, !1));
  }
  function ur(t, e, n) {
    var a = St;
    St |= 2;
    var s = um(),
      r = om();
    ((Nt !== t || ht !== e) && ((Ts = null), ta(t, e)), (e = !1));
    var f = Bt;
    t: do
      try {
        if (At !== 0 && ft !== null) {
          var y = ft,
            T = De;
          switch (At) {
            case 8:
              (sr(), (f = 6));
              break t;
            case 3:
            case 2:
            case 9:
            case 6:
              Ee.current === null && (e = !0);
              var C = At;
              if (((At = 0), (De = null), ea(t, y, T, C), n && Wi)) {
                f = 0;
                break t;
              }
              break;
            default:
              ((C = At), (At = 0), (De = null), ea(t, y, T, C));
          }
        }
        (o1(), (f = Bt));
        break;
      } catch (O) {
        lm(t, O);
      }
    while (!0);
    return (
      e && t.shellSuspendCounter++,
      (on = ui = null),
      (St = a),
      (R.H = s),
      (R.A = r),
      ft === null && ((Nt = null), (ht = 0), kl()),
      f
    );
  }
  function o1() {
    for (; ft !== null; ) rm(ft);
  }
  function r1(t, e) {
    var n = St;
    St |= 2;
    var a = um(),
      s = om();
    Nt !== t || ht !== e ? ((Ts = null), (Ss = xe() + 500), ta(t, e)) : (Wi = ba(t, e));
    t: do
      try {
        if (At !== 0 && ft !== null) {
          e = ft;
          var r = De;
          e: switch (At) {
            case 1:
              ((At = 0), (De = null), ea(t, e, r, 1));
              break;
            case 2:
            case 9:
              if (xd(r)) {
                ((At = 0), (De = null), cm(e));
                break;
              }
              ((e = function () {
                ((At !== 2 && At !== 9) || Nt !== t || (At = 7), $e(t));
              }),
                r.then(e, e));
              break t;
            case 3:
              At = 7;
              break t;
            case 4:
              At = 5;
              break t;
            case 7:
              xd(r) ? ((At = 0), (De = null), cm(e)) : ((At = 0), (De = null), ea(t, e, r, 7));
              break;
            case 5:
              var f = null;
              switch (ft.tag) {
                case 26:
                  f = ft.memoizedState;
                case 5:
                case 27:
                  var y = ft;
                  if (f ? Fm(f) : y.stateNode.complete) {
                    ((At = 0), (De = null));
                    var T = y.sibling;
                    if (T !== null) ft = T;
                    else {
                      var C = y.return;
                      C !== null ? ((ft = C), Ms(C)) : (ft = null);
                    }
                    break e;
                  }
              }
              ((At = 0), (De = null), ea(t, e, r, 5));
              break;
            case 6:
              ((At = 0), (De = null), ea(t, e, r, 6));
              break;
            case 8:
              (sr(), (Bt = 6));
              break t;
            default:
              throw Error(o(462));
          }
        }
        c1();
        break;
      } catch (O) {
        lm(t, O);
      }
    while (!0);
    return (
      (on = ui = null),
      (R.H = a),
      (R.A = s),
      (St = n),
      ft !== null ? 0 : ((Nt = null), (ht = 0), kl(), Bt)
    );
  }
  function c1() {
    for (; ft !== null && !Oy(); ) rm(ft);
  }
  function rm(t) {
    var e = Oh(t.alternate, t, vn);
    ((t.memoizedProps = t.pendingProps), e === null ? Ms(t) : (ft = e));
  }
  function cm(t) {
    var e = t,
      n = e.alternate;
    switch (e.tag) {
      case 15:
      case 0:
        e = Ch(n, e, e.pendingProps, e.type, void 0, ht);
        break;
      case 11:
        e = Ch(n, e, e.pendingProps, e.type.render, e.ref, ht);
        break;
      case 5:
        To(e);
      default:
        (_h(n, e), (e = ft = od(e, vn)), (e = Oh(n, e, vn)));
    }
    ((t.memoizedProps = t.pendingProps), e === null ? Ms(t) : (ft = e));
  }
  function ea(t, e, n, a) {
    ((on = ui = null), To(e), (ki = null), (Ua = 0));
    var s = e.return;
    try {
      if ($v(t, s, e, n, ht)) {
        ((Bt = 1), ds(t, Ve(n, t.current)), (ft = null));
        return;
      }
    } catch (r) {
      if (s !== null) throw ((ft = s), r);
      ((Bt = 1), ds(t, Ve(n, t.current)), (ft = null));
      return;
    }
    e.flags & 32768
      ? (yt || a === 1
          ? (t = !0)
          : Wi || (ht & 536870912) !== 0
            ? (t = !1)
            : ((Vn = t = !0),
              (a === 2 || a === 9 || a === 3 || a === 6) &&
                ((a = Ee.current), a !== null && a.tag === 13 && (a.flags |= 16384))),
        fm(e, t))
      : Ms(e);
  }
  function Ms(t) {
    var e = t;
    do {
      if ((e.flags & 32768) !== 0) {
        fm(e, Vn);
        return;
      }
      t = e.return;
      var n = e1(e.alternate, e, vn);
      if (n !== null) {
        ft = n;
        return;
      }
      if (((e = e.sibling), e !== null)) {
        ft = e;
        return;
      }
      ft = e = t;
    } while (e !== null);
    Bt === 0 && (Bt = 5);
  }
  function fm(t, e) {
    do {
      var n = n1(t.alternate, t);
      if (n !== null) {
        ((n.flags &= 32767), (ft = n));
        return;
      }
      if (
        ((n = t.return),
        n !== null && ((n.flags |= 32768), (n.subtreeFlags = 0), (n.deletions = null)),
        !e && ((t = t.sibling), t !== null))
      ) {
        ft = t;
        return;
      }
      ft = t = n;
    } while (t !== null);
    ((Bt = 6), (ft = null));
  }
  function dm(t, e, n, a, s, r, f, y, T) {
    t.cancelPendingCommit = null;
    do Ds();
    while (Jt !== 0);
    if ((St & 6) !== 0) throw Error(o(327));
    if (e !== null) {
      if (e === t.current) throw Error(o(177));
      if (
        ((r = e.lanes | e.childLanes),
        (r |= Ju),
        Xy(t, n, r, f, y, T),
        t === Nt && ((ft = Nt = null), (ht = 0)),
        (Ii = e),
        (Un = t),
        (xn = n),
        (ir = r),
        (ar = s),
        (em = a),
        (e.subtreeFlags & 10256) !== 0 || (e.flags & 10256) !== 0
          ? ((t.callbackNode = null),
            (t.callbackPriority = 0),
            m1(Nl, function () {
              return (ym(), null);
            }))
          : ((t.callbackNode = null), (t.callbackPriority = 0)),
        (a = (e.flags & 13878) !== 0),
        (e.subtreeFlags & 13878) !== 0 || a)
      ) {
        ((a = R.T), (R.T = null), (s = X.p), (X.p = 2), (f = St), (St |= 4));
        try {
          i1(t, e, n);
        } finally {
          ((St = f), (X.p = s), (R.T = a));
        }
      }
      ((Jt = 1), hm(), mm(), pm());
    }
  }
  function hm() {
    if (Jt === 1) {
      Jt = 0;
      var t = Un,
        e = Ii,
        n = (e.flags & 13878) !== 0;
      if ((e.subtreeFlags & 13878) !== 0 || n) {
        ((n = R.T), (R.T = null));
        var a = X.p;
        X.p = 2;
        var s = St;
        St |= 4;
        try {
          Kh(e, t);
          var r = xr,
            f = If(t.containerInfo),
            y = r.focusedElem,
            T = r.selectionRange;
          if (f !== y && y && y.ownerDocument && $f(y.ownerDocument.documentElement, y)) {
            if (T !== null && Xu(y)) {
              var C = T.start,
                O = T.end;
              if ((O === void 0 && (O = C), "selectionStart" in y))
                ((y.selectionStart = C), (y.selectionEnd = Math.min(O, y.value.length)));
              else {
                var _ = y.ownerDocument || document,
                  z = (_ && _.defaultView) || window;
                if (z.getSelection) {
                  var j = z.getSelection(),
                    K = y.textContent.length,
                    nt = Math.min(T.start, K),
                    Ct = T.end === void 0 ? nt : Math.min(T.end, K);
                  !j.extend && nt > Ct && ((f = Ct), (Ct = nt), (nt = f));
                  var M = Wf(y, nt),
                    A = Wf(y, Ct);
                  if (
                    M &&
                    A &&
                    (j.rangeCount !== 1 ||
                      j.anchorNode !== M.node ||
                      j.anchorOffset !== M.offset ||
                      j.focusNode !== A.node ||
                      j.focusOffset !== A.offset)
                  ) {
                    var D = _.createRange();
                    (D.setStart(M.node, M.offset),
                      j.removeAllRanges(),
                      nt > Ct
                        ? (j.addRange(D), j.extend(A.node, A.offset))
                        : (D.setEnd(A.node, A.offset), j.addRange(D)));
                  }
                }
              }
            }
            for (_ = [], j = y; (j = j.parentNode); )
              j.nodeType === 1 && _.push({ element: j, left: j.scrollLeft, top: j.scrollTop });
            for (typeof y.focus == "function" && y.focus(), y = 0; y < _.length; y++) {
              var V = _[y];
              ((V.element.scrollLeft = V.left), (V.element.scrollTop = V.top));
            }
          }
          ((Ls = !!vr), (xr = vr = null));
        } finally {
          ((St = s), (X.p = a), (R.T = n));
        }
      }
      ((t.current = e), (Jt = 2));
    }
  }
  function mm() {
    if (Jt === 2) {
      Jt = 0;
      var t = Un,
        e = Ii,
        n = (e.flags & 8772) !== 0;
      if ((e.subtreeFlags & 8772) !== 0 || n) {
        ((n = R.T), (R.T = null));
        var a = X.p;
        X.p = 2;
        var s = St;
        St |= 4;
        try {
          Gh(t, e.alternate, e);
        } finally {
          ((St = s), (X.p = a), (R.T = n));
        }
      }
      Jt = 3;
    }
  }
  function pm() {
    if (Jt === 4 || Jt === 3) {
      ((Jt = 0), Vy());
      var t = Un,
        e = Ii,
        n = xn,
        a = em;
      (e.subtreeFlags & 10256) !== 0 || (e.flags & 10256) !== 0
        ? (Jt = 5)
        : ((Jt = 0), (Ii = Un = null), gm(t, t.pendingLanes));
      var s = t.pendingLanes;
      if (
        (s === 0 && (Bn = null),
        Eu(n),
        (e = e.stateNode),
        be && typeof be.onCommitFiberRoot == "function")
      )
        try {
          be.onCommitFiberRoot(xa, e, void 0, (e.current.flags & 128) === 128);
        } catch {}
      if (a !== null) {
        ((e = R.T), (s = X.p), (X.p = 2), (R.T = null));
        try {
          for (var r = t.onRecoverableError, f = 0; f < a.length; f++) {
            var y = a[f];
            r(y.value, { componentStack: y.stack });
          }
        } finally {
          ((R.T = e), (X.p = s));
        }
      }
      ((xn & 3) !== 0 && Ds(),
        $e(t),
        (s = t.pendingLanes),
        (n & 261930) !== 0 && (s & 42) !== 0 ? (t === lr ? tl++ : ((tl = 0), (lr = t))) : (tl = 0),
        el(0));
    }
  }
  function gm(t, e) {
    (t.pooledCacheLanes &= e) === 0 &&
      ((e = t.pooledCache), e != null && ((t.pooledCache = null), _a(e)));
  }
  function Ds() {
    return (hm(), mm(), pm(), ym());
  }
  function ym() {
    if (Jt !== 5) return !1;
    var t = Un,
      e = ir;
    ir = 0;
    var n = Eu(xn),
      a = R.T,
      s = X.p;
    try {
      ((X.p = 32 > n ? 32 : n), (R.T = null), (n = ar), (ar = null));
      var r = Un,
        f = xn;
      if (((Jt = 0), (Ii = Un = null), (xn = 0), (St & 6) !== 0)) throw Error(o(331));
      var y = St;
      if (
        ((St |= 4),
        $h(r.current),
        Fh(r, r.current, f, n),
        (St = y),
        el(0, !1),
        be && typeof be.onPostCommitFiberRoot == "function")
      )
        try {
          be.onPostCommitFiberRoot(xa, r);
        } catch {}
      return !0;
    } finally {
      ((X.p = s), (R.T = a), gm(t, e));
    }
  }
  function vm(t, e, n) {
    ((e = Ve(n, e)),
      (e = Uo(t.stateNode, e, 2)),
      (t = jn(t, e, 2)),
      t !== null && (Sa(t, 2), $e(t)));
  }
  function Et(t, e, n) {
    if (t.tag === 3) vm(t, t, n);
    else
      for (; e !== null; ) {
        if (e.tag === 3) {
          vm(e, t, n);
          break;
        } else if (e.tag === 1) {
          var a = e.stateNode;
          if (
            typeof e.type.getDerivedStateFromError == "function" ||
            (typeof a.componentDidCatch == "function" && (Bn === null || !Bn.has(a)))
          ) {
            ((t = Ve(n, t)),
              (n = xh(2)),
              (a = jn(e, n, 2)),
              a !== null && (bh(n, a, e, t), Sa(a, 2), $e(a)));
            break;
          }
        }
        e = e.return;
      }
  }
  function or(t, e, n) {
    var a = t.pingCache;
    if (a === null) {
      a = t.pingCache = new s1();
      var s = new Set();
      a.set(e, s);
    } else ((s = a.get(e)), s === void 0 && ((s = new Set()), a.set(e, s)));
    s.has(n) || ((tr = !0), s.add(n), (t = f1.bind(null, t, e, n)), e.then(t, t));
  }
  function f1(t, e, n) {
    var a = t.pingCache;
    (a !== null && a.delete(e),
      (t.pingedLanes |= t.suspendedLanes & n),
      (t.warmLanes &= ~n),
      Nt === t &&
        (ht & n) === n &&
        (Bt === 4 || (Bt === 3 && (ht & 62914560) === ht && 300 > xe() - bs)
          ? (St & 2) === 0 && ta(t, 0)
          : (er |= n),
        $i === ht && ($i = 0)),
      $e(t));
  }
  function xm(t, e) {
    (e === 0 && (e = hf()), (t = ai(t, e)), t !== null && (Sa(t, e), $e(t)));
  }
  function d1(t) {
    var e = t.memoizedState,
      n = 0;
    (e !== null && (n = e.retryLane), xm(t, n));
  }
  function h1(t, e) {
    var n = 0;
    switch (t.tag) {
      case 31:
      case 13:
        var a = t.stateNode,
          s = t.memoizedState;
        s !== null && (n = s.retryLane);
        break;
      case 19:
        a = t.stateNode;
        break;
      case 22:
        a = t.stateNode._retryCache;
        break;
      default:
        throw Error(o(314));
    }
    (a !== null && a.delete(e), xm(t, n));
  }
  function m1(t, e) {
    return bu(t, e);
  }
  var Cs = null,
    na = null,
    rr = !1,
    Ns = !1,
    cr = !1,
    Hn = 0;
  function $e(t) {
    (t !== na && t.next === null && (na === null ? (Cs = na = t) : (na = na.next = t)),
      (Ns = !0),
      rr || ((rr = !0), g1()));
  }
  function el(t, e) {
    if (!cr && Ns) {
      cr = !0;
      do
        for (var n = !1, a = Cs; a !== null; ) {
          if (t !== 0) {
            var s = a.pendingLanes;
            if (s === 0) var r = 0;
            else {
              var f = a.suspendedLanes,
                y = a.pingedLanes;
              ((r = (1 << (31 - Se(42 | t) + 1)) - 1),
                (r &= s & ~(f & ~y)),
                (r = r & 201326741 ? (r & 201326741) | 1 : r ? r | 2 : 0));
            }
            r !== 0 && ((n = !0), Am(a, r));
          } else
            ((r = ht),
              (r = Rl(
                a,
                a === Nt ? r : 0,
                a.cancelPendingCommit !== null || a.timeoutHandle !== -1,
              )),
              (r & 3) === 0 || ba(a, r) || ((n = !0), Am(a, r)));
          a = a.next;
        }
      while (n);
      cr = !1;
    }
  }
  function p1() {
    bm();
  }
  function bm() {
    Ns = rr = !1;
    var t = 0;
    Hn !== 0 && D1() && (t = Hn);
    for (var e = xe(), n = null, a = Cs; a !== null; ) {
      var s = a.next,
        r = Sm(a, e);
      (r === 0
        ? ((a.next = null), n === null ? (Cs = s) : (n.next = s), s === null && (na = n))
        : ((n = a), (t !== 0 || (r & 3) !== 0) && (Ns = !0)),
        (a = s));
    }
    ((Jt !== 0 && Jt !== 5) || el(t), Hn !== 0 && (Hn = 0));
  }
  function Sm(t, e) {
    for (
      var n = t.suspendedLanes,
        a = t.pingedLanes,
        s = t.expirationTimes,
        r = t.pendingLanes & -62914561;
      0 < r;
    ) {
      var f = 31 - Se(r),
        y = 1 << f,
        T = s[f];
      (T === -1
        ? ((y & n) === 0 || (y & a) !== 0) && (s[f] = Gy(y, e))
        : T <= e && (t.expiredLanes |= y),
        (r &= ~y));
    }
    if (
      ((e = Nt),
      (n = ht),
      (n = Rl(t, t === e ? n : 0, t.cancelPendingCommit !== null || t.timeoutHandle !== -1)),
      (a = t.callbackNode),
      n === 0 || (t === e && (At === 2 || At === 9)) || t.cancelPendingCommit !== null)
    )
      return (a !== null && a !== null && Su(a), (t.callbackNode = null), (t.callbackPriority = 0));
    if ((n & 3) === 0 || ba(t, n)) {
      if (((e = n & -n), e === t.callbackPriority)) return e;
      switch ((a !== null && Su(a), Eu(n))) {
        case 2:
        case 8:
          n = ff;
          break;
        case 32:
          n = Nl;
          break;
        case 268435456:
          n = df;
          break;
        default:
          n = Nl;
      }
      return (
        (a = Tm.bind(null, t)),
        (n = bu(n, a)),
        (t.callbackPriority = e),
        (t.callbackNode = n),
        e
      );
    }
    return (
      a !== null && a !== null && Su(a),
      (t.callbackPriority = 2),
      (t.callbackNode = null),
      2
    );
  }
  function Tm(t, e) {
    if (Jt !== 0 && Jt !== 5) return ((t.callbackNode = null), (t.callbackPriority = 0), null);
    var n = t.callbackNode;
    if (Ds() && t.callbackNode !== n) return null;
    var a = ht;
    return (
      (a = Rl(t, t === Nt ? a : 0, t.cancelPendingCommit !== null || t.timeoutHandle !== -1)),
      a === 0
        ? null
        : (im(t, a, e),
          Sm(t, xe()),
          t.callbackNode != null && t.callbackNode === n ? Tm.bind(null, t) : null)
    );
  }
  function Am(t, e) {
    if (Ds()) return null;
    im(t, e, !0);
  }
  function g1() {
    N1(function () {
      (St & 6) !== 0 ? bu(cf, p1) : bm();
    });
  }
  function fr() {
    if (Hn === 0) {
      var t = Yi;
      (t === 0 && ((t = zl), (zl <<= 1), (zl & 261888) === 0 && (zl = 256)), (Hn = t));
    }
    return Hn;
  }
  function Em(t) {
    return t == null || typeof t == "symbol" || typeof t == "boolean"
      ? null
      : typeof t == "function"
        ? t
        : Bl("" + t);
  }
  function Mm(t, e) {
    var n = e.ownerDocument.createElement("input");
    return (
      (n.name = e.name),
      (n.value = e.value),
      t.id && n.setAttribute("form", t.id),
      e.parentNode.insertBefore(n, e),
      (t = new FormData(t)),
      n.parentNode.removeChild(n),
      t
    );
  }
  function y1(t, e, n, a, s) {
    if (e === "submit" && n && n.stateNode === s) {
      var r = Em((s[de] || null).action),
        f = a.submitter;
      f &&
        ((e = (e = f[de] || null) ? Em(e.formAction) : f.getAttribute("formAction")),
        e !== null && ((r = e), (f = null)));
      var y = new ql("action", "action", null, a, s);
      t.push({
        event: y,
        listeners: [
          {
            instance: null,
            listener: function () {
              if (a.defaultPrevented) {
                if (Hn !== 0) {
                  var T = f ? Mm(s, f) : new FormData(s);
                  wo(n, { pending: !0, data: T, method: s.method, action: r }, null, T);
                }
              } else
                typeof r == "function" &&
                  (y.preventDefault(),
                  (T = f ? Mm(s, f) : new FormData(s)),
                  wo(n, { pending: !0, data: T, method: s.method, action: r }, r, T));
            },
            currentTarget: s,
          },
        ],
      });
    }
  }
  for (var dr = 0; dr < Ku.length; dr++) {
    var hr = Ku[dr],
      v1 = hr.toLowerCase(),
      x1 = hr[0].toUpperCase() + hr.slice(1);
    Xe(v1, "on" + x1);
  }
  (Xe(nd, "onAnimationEnd"),
    Xe(id, "onAnimationIteration"),
    Xe(ad, "onAnimationStart"),
    Xe("dblclick", "onDoubleClick"),
    Xe("focusin", "onFocus"),
    Xe("focusout", "onBlur"),
    Xe(_v, "onTransitionRun"),
    Xe(Bv, "onTransitionStart"),
    Xe(Uv, "onTransitionCancel"),
    Xe(ld, "onTransitionEnd"),
    Ci("onMouseEnter", ["mouseout", "mouseover"]),
    Ci("onMouseLeave", ["mouseout", "mouseover"]),
    Ci("onPointerEnter", ["pointerout", "pointerover"]),
    Ci("onPointerLeave", ["pointerout", "pointerover"]),
    ti("onChange", "change click focusin focusout input keydown keyup selectionchange".split(" ")),
    ti(
      "onSelect",
      "focusout contextmenu dragend focusin keydown keyup mousedown mouseup selectionchange".split(
        " ",
      ),
    ),
    ti("onBeforeInput", ["compositionend", "keypress", "textInput", "paste"]),
    ti("onCompositionEnd", "compositionend focusout keydown keypress keyup mousedown".split(" ")),
    ti(
      "onCompositionStart",
      "compositionstart focusout keydown keypress keyup mousedown".split(" "),
    ),
    ti(
      "onCompositionUpdate",
      "compositionupdate focusout keydown keypress keyup mousedown".split(" "),
    ));
  var nl =
      "abort canplay canplaythrough durationchange emptied encrypted ended error loadeddata loadedmetadata loadstart pause play playing progress ratechange resize seeked seeking stalled suspend timeupdate volumechange waiting".split(
        " ",
      ),
    b1 = new Set(
      "beforetoggle cancel close invalid load scroll scrollend toggle".split(" ").concat(nl),
    );
  function Dm(t, e) {
    e = (e & 4) !== 0;
    for (var n = 0; n < t.length; n++) {
      var a = t[n],
        s = a.event;
      a = a.listeners;
      t: {
        var r = void 0;
        if (e)
          for (var f = a.length - 1; 0 <= f; f--) {
            var y = a[f],
              T = y.instance,
              C = y.currentTarget;
            if (((y = y.listener), T !== r && s.isPropagationStopped())) break t;
            ((r = y), (s.currentTarget = C));
            try {
              r(s);
            } catch (O) {
              Xl(O);
            }
            ((s.currentTarget = null), (r = T));
          }
        else
          for (f = 0; f < a.length; f++) {
            if (
              ((y = a[f]),
              (T = y.instance),
              (C = y.currentTarget),
              (y = y.listener),
              T !== r && s.isPropagationStopped())
            )
              break t;
            ((r = y), (s.currentTarget = C));
            try {
              r(s);
            } catch (O) {
              Xl(O);
            }
            ((s.currentTarget = null), (r = T));
          }
      }
    }
  }
  function dt(t, e) {
    var n = e[Mu];
    n === void 0 && (n = e[Mu] = new Set());
    var a = t + "__bubble";
    n.has(a) || (Cm(e, t, 2, !1), n.add(a));
  }
  function mr(t, e, n) {
    var a = 0;
    (e && (a |= 4), Cm(n, t, a, e));
  }
  var zs = "_reactListening" + Math.random().toString(36).slice(2);
  function pr(t) {
    if (!t[zs]) {
      ((t[zs] = !0),
        bf.forEach(function (n) {
          n !== "selectionchange" && (b1.has(n) || mr(n, !1, t), mr(n, !0, t));
        }));
      var e = t.nodeType === 9 ? t : t.ownerDocument;
      e === null || e[zs] || ((e[zs] = !0), mr("selectionchange", !1, e));
    }
  }
  function Cm(t, e, n, a) {
    switch (np(e)) {
      case 2:
        var s = J1;
        break;
      case 8:
        s = F1;
        break;
      default:
        s = jr;
    }
    ((n = s.bind(null, e, n, t)),
      (s = void 0),
      !Vu || (e !== "touchstart" && e !== "touchmove" && e !== "wheel") || (s = !0),
      a
        ? s !== void 0
          ? t.addEventListener(e, n, { capture: !0, passive: s })
          : t.addEventListener(e, n, !0)
        : s !== void 0
          ? t.addEventListener(e, n, { passive: s })
          : t.addEventListener(e, n, !1));
  }
  function gr(t, e, n, a, s) {
    var r = a;
    if ((e & 1) === 0 && (e & 2) === 0 && a !== null)
      t: for (;;) {
        if (a === null) return;
        var f = a.tag;
        if (f === 3 || f === 4) {
          var y = a.stateNode.containerInfo;
          if (y === s) break;
          if (f === 4)
            for (f = a.return; f !== null; ) {
              var T = f.tag;
              if ((T === 3 || T === 4) && f.stateNode.containerInfo === s) return;
              f = f.return;
            }
          for (; y !== null; ) {
            if (((f = Ei(y)), f === null)) return;
            if (((T = f.tag), T === 5 || T === 6 || T === 26 || T === 27)) {
              a = r = f;
              continue t;
            }
            y = y.parentNode;
          }
        }
        a = a.return;
      }
    Rf(function () {
      var C = r,
        O = Ru(n),
        _ = [];
      t: {
        var z = sd.get(t);
        if (z !== void 0) {
          var j = ql,
            K = t;
          switch (t) {
            case "keypress":
              if (Ll(n) === 0) break t;
            case "keydown":
            case "keyup":
              j = hv;
              break;
            case "focusin":
              ((K = "focus"), (j = Lu));
              break;
            case "focusout":
              ((K = "blur"), (j = Lu));
              break;
            case "beforeblur":
            case "afterblur":
              j = Lu;
              break;
            case "click":
              if (n.button === 2) break t;
            case "auxclick":
            case "dblclick":
            case "mousedown":
            case "mousemove":
            case "mouseup":
            case "mouseout":
            case "mouseover":
            case "contextmenu":
              j = _f;
              break;
            case "drag":
            case "dragend":
            case "dragenter":
            case "dragexit":
            case "dragleave":
            case "dragover":
            case "dragstart":
            case "drop":
              j = ev;
              break;
            case "touchcancel":
            case "touchend":
            case "touchmove":
            case "touchstart":
              j = gv;
              break;
            case nd:
            case id:
            case ad:
              j = av;
              break;
            case ld:
              j = vv;
              break;
            case "scroll":
            case "scrollend":
              j = Iy;
              break;
            case "wheel":
              j = bv;
              break;
            case "copy":
            case "cut":
            case "paste":
              j = sv;
              break;
            case "gotpointercapture":
            case "lostpointercapture":
            case "pointercancel":
            case "pointerdown":
            case "pointermove":
            case "pointerout":
            case "pointerover":
            case "pointerup":
              j = Uf;
              break;
            case "toggle":
            case "beforetoggle":
              j = Tv;
          }
          var nt = (e & 4) !== 0,
            Ct = !nt && (t === "scroll" || t === "scrollend"),
            M = nt ? (z !== null ? z + "Capture" : null) : z;
          nt = [];
          for (var A = C, D; A !== null; ) {
            var V = A;
            if (
              ((D = V.stateNode),
              (V = V.tag),
              (V !== 5 && V !== 26 && V !== 27) ||
                D === null ||
                M === null ||
                ((V = Ea(A, M)), V != null && nt.push(il(A, V, D))),
              Ct)
            )
              break;
            A = A.return;
          }
          0 < nt.length && ((z = new j(z, K, null, n, O)), _.push({ event: z, listeners: nt }));
        }
      }
      if ((e & 7) === 0) {
        t: {
          if (
            ((z = t === "mouseover" || t === "pointerover"),
            (j = t === "mouseout" || t === "pointerout"),
            z && n !== wu && (K = n.relatedTarget || n.fromElement) && (Ei(K) || K[Ai]))
          )
            break t;
          if (
            (j || z) &&
            ((z =
              O.window === O
                ? O
                : (z = O.ownerDocument)
                  ? z.defaultView || z.parentWindow
                  : window),
            j
              ? ((K = n.relatedTarget || n.toElement),
                (j = C),
                (K = K ? Ei(K) : null),
                K !== null &&
                  ((Ct = h(K)), (nt = K.tag), K !== Ct || (nt !== 5 && nt !== 27 && nt !== 6)) &&
                  (K = null))
              : ((j = null), (K = C)),
            j !== K)
          ) {
            if (
              ((nt = _f),
              (V = "onMouseLeave"),
              (M = "onMouseEnter"),
              (A = "mouse"),
              (t === "pointerout" || t === "pointerover") &&
                ((nt = Uf), (V = "onPointerLeave"), (M = "onPointerEnter"), (A = "pointer")),
              (Ct = j == null ? z : Aa(j)),
              (D = K == null ? z : Aa(K)),
              (z = new nt(V, A + "leave", j, n, O)),
              (z.target = Ct),
              (z.relatedTarget = D),
              (V = null),
              Ei(O) === C &&
                ((nt = new nt(M, A + "enter", K, n, O)),
                (nt.target = D),
                (nt.relatedTarget = Ct),
                (V = nt)),
              (Ct = V),
              j && K)
            )
              e: {
                for (nt = S1, M = j, A = K, D = 0, V = M; V; V = nt(V)) D++;
                V = 0;
                for (var et = A; et; et = nt(et)) V++;
                for (; 0 < D - V; ) ((M = nt(M)), D--);
                for (; 0 < V - D; ) ((A = nt(A)), V--);
                for (; D--; ) {
                  if (M === A || (A !== null && M === A.alternate)) {
                    nt = M;
                    break e;
                  }
                  ((M = nt(M)), (A = nt(A)));
                }
                nt = null;
              }
            else nt = null;
            (j !== null && Nm(_, z, j, nt, !1), K !== null && Ct !== null && Nm(_, Ct, K, nt, !0));
          }
        }
        t: {
          if (
            ((z = C ? Aa(C) : window),
            (j = z.nodeName && z.nodeName.toLowerCase()),
            j === "select" || (j === "input" && z.type === "file"))
          )
            var xt = Qf;
          else if (Xf(z))
            if (Zf) xt = Rv;
            else {
              xt = jv;
              var W = zv;
            }
          else
            ((j = z.nodeName),
              !j || j.toLowerCase() !== "input" || (z.type !== "checkbox" && z.type !== "radio")
                ? C && ju(C.elementType) && (xt = Qf)
                : (xt = wv));
          if (xt && (xt = xt(t, C))) {
            kf(_, xt, n, O);
            break t;
          }
          (W && W(t, z, C),
            t === "focusout" &&
              C &&
              z.type === "number" &&
              C.memoizedProps.value != null &&
              zu(z, "number", z.value));
        }
        switch (((W = C ? Aa(C) : window), t)) {
          case "focusin":
            (Xf(W) || W.contentEditable === "true") && ((Oi = W), (ku = C), (Ra = null));
            break;
          case "focusout":
            Ra = ku = Oi = null;
            break;
          case "mousedown":
            Qu = !0;
            break;
          case "contextmenu":
          case "mouseup":
          case "dragend":
            ((Qu = !1), td(_, n, O));
            break;
          case "selectionchange":
            if (Vv) break;
          case "keydown":
          case "keyup":
            td(_, n, O);
        }
        var rt;
        if (qu)
          t: {
            switch (t) {
              case "compositionstart":
                var mt = "onCompositionStart";
                break t;
              case "compositionend":
                mt = "onCompositionEnd";
                break t;
              case "compositionupdate":
                mt = "onCompositionUpdate";
                break t;
            }
            mt = void 0;
          }
        else
          Ri
            ? Yf(t, n) && (mt = "onCompositionEnd")
            : t === "keydown" && n.keyCode === 229 && (mt = "onCompositionStart");
        (mt &&
          (Lf &&
            n.locale !== "ko" &&
            (Ri || mt !== "onCompositionStart"
              ? mt === "onCompositionEnd" && Ri && (rt = Of())
              : ((An = O), (_u = "value" in An ? An.value : An.textContent), (Ri = !0))),
          (W = js(C, mt)),
          0 < W.length &&
            ((mt = new Bf(mt, t, null, n, O)),
            _.push({ event: mt, listeners: W }),
            rt ? (mt.data = rt) : ((rt = Gf(n)), rt !== null && (mt.data = rt)))),
          (rt = Ev ? Mv(t, n) : Dv(t, n)) &&
            ((mt = js(C, "onBeforeInput")),
            0 < mt.length &&
              ((W = new Bf("onBeforeInput", "beforeinput", null, n, O)),
              _.push({ event: W, listeners: mt }),
              (W.data = rt))),
          y1(_, t, C, n, O));
      }
      Dm(_, e);
    });
  }
  function il(t, e, n) {
    return { instance: t, listener: e, currentTarget: n };
  }
  function js(t, e) {
    for (var n = e + "Capture", a = []; t !== null; ) {
      var s = t,
        r = s.stateNode;
      if (
        ((s = s.tag),
        (s !== 5 && s !== 26 && s !== 27) ||
          r === null ||
          ((s = Ea(t, n)),
          s != null && a.unshift(il(t, s, r)),
          (s = Ea(t, e)),
          s != null && a.push(il(t, s, r))),
        t.tag === 3)
      )
        return a;
      t = t.return;
    }
    return [];
  }
  function S1(t) {
    if (t === null) return null;
    do t = t.return;
    while (t && t.tag !== 5 && t.tag !== 27);
    return t || null;
  }
  function Nm(t, e, n, a, s) {
    for (var r = e._reactName, f = []; n !== null && n !== a; ) {
      var y = n,
        T = y.alternate,
        C = y.stateNode;
      if (((y = y.tag), T !== null && T === a)) break;
      ((y !== 5 && y !== 26 && y !== 27) ||
        C === null ||
        ((T = C),
        s
          ? ((C = Ea(n, r)), C != null && f.unshift(il(n, C, T)))
          : s || ((C = Ea(n, r)), C != null && f.push(il(n, C, T)))),
        (n = n.return));
    }
    f.length !== 0 && t.push({ event: e, listeners: f });
  }
  var T1 = /\r\n?/g,
    A1 = /\u0000|\uFFFD/g;
  function zm(t) {
    return (typeof t == "string" ? t : "" + t)
      .replace(
        T1,
        `
`,
      )
      .replace(A1, "");
  }
  function jm(t, e) {
    return ((e = zm(e)), zm(t) === e);
  }
  function Dt(t, e, n, a, s, r) {
    switch (n) {
      case "children":
        typeof a == "string"
          ? e === "body" || (e === "textarea" && a === "") || zi(t, a)
          : (typeof a == "number" || typeof a == "bigint") && e !== "body" && zi(t, "" + a);
        break;
      case "className":
        Vl(t, "class", a);
        break;
      case "tabIndex":
        Vl(t, "tabindex", a);
        break;
      case "dir":
      case "role":
      case "viewBox":
      case "width":
      case "height":
        Vl(t, n, a);
        break;
      case "style":
        jf(t, a, r);
        break;
      case "data":
        if (e !== "object") {
          Vl(t, "data", a);
          break;
        }
      case "src":
      case "href":
        if (a === "" && (e !== "a" || n !== "href")) {
          t.removeAttribute(n);
          break;
        }
        if (a == null || typeof a == "function" || typeof a == "symbol" || typeof a == "boolean") {
          t.removeAttribute(n);
          break;
        }
        ((a = Bl("" + a)), t.setAttribute(n, a));
        break;
      case "action":
      case "formAction":
        if (typeof a == "function") {
          t.setAttribute(
            n,
            "javascript:throw new Error('A React form was unexpectedly submitted. If you called form.submit() manually, consider using form.requestSubmit() instead. If you\\'re trying to use event.stopPropagation() in a submit event handler, consider also calling event.preventDefault().')",
          );
          break;
        } else
          typeof r == "function" &&
            (n === "formAction"
              ? (e !== "input" && Dt(t, e, "name", s.name, s, null),
                Dt(t, e, "formEncType", s.formEncType, s, null),
                Dt(t, e, "formMethod", s.formMethod, s, null),
                Dt(t, e, "formTarget", s.formTarget, s, null))
              : (Dt(t, e, "encType", s.encType, s, null),
                Dt(t, e, "method", s.method, s, null),
                Dt(t, e, "target", s.target, s, null)));
        if (a == null || typeof a == "symbol" || typeof a == "boolean") {
          t.removeAttribute(n);
          break;
        }
        ((a = Bl("" + a)), t.setAttribute(n, a));
        break;
      case "onClick":
        a != null && (t.onclick = an);
        break;
      case "onScroll":
        a != null && dt("scroll", t);
        break;
      case "onScrollEnd":
        a != null && dt("scrollend", t);
        break;
      case "dangerouslySetInnerHTML":
        if (a != null) {
          if (typeof a != "object" || !("__html" in a)) throw Error(o(61));
          if (((n = a.__html), n != null)) {
            if (s.children != null) throw Error(o(60));
            t.innerHTML = n;
          }
        }
        break;
      case "multiple":
        t.multiple = a && typeof a != "function" && typeof a != "symbol";
        break;
      case "muted":
        t.muted = a && typeof a != "function" && typeof a != "symbol";
        break;
      case "suppressContentEditableWarning":
      case "suppressHydrationWarning":
      case "defaultValue":
      case "defaultChecked":
      case "innerHTML":
      case "ref":
        break;
      case "autoFocus":
        break;
      case "xlinkHref":
        if (a == null || typeof a == "function" || typeof a == "boolean" || typeof a == "symbol") {
          t.removeAttribute("xlink:href");
          break;
        }
        ((n = Bl("" + a)), t.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", n));
        break;
      case "contentEditable":
      case "spellCheck":
      case "draggable":
      case "value":
      case "autoReverse":
      case "externalResourcesRequired":
      case "focusable":
      case "preserveAlpha":
        a != null && typeof a != "function" && typeof a != "symbol"
          ? t.setAttribute(n, "" + a)
          : t.removeAttribute(n);
        break;
      case "inert":
      case "allowFullScreen":
      case "async":
      case "autoPlay":
      case "controls":
      case "default":
      case "defer":
      case "disabled":
      case "disablePictureInPicture":
      case "disableRemotePlayback":
      case "formNoValidate":
      case "hidden":
      case "loop":
      case "noModule":
      case "noValidate":
      case "open":
      case "playsInline":
      case "readOnly":
      case "required":
      case "reversed":
      case "scoped":
      case "seamless":
      case "itemScope":
        a && typeof a != "function" && typeof a != "symbol"
          ? t.setAttribute(n, "")
          : t.removeAttribute(n);
        break;
      case "capture":
      case "download":
        a === !0
          ? t.setAttribute(n, "")
          : a !== !1 && a != null && typeof a != "function" && typeof a != "symbol"
            ? t.setAttribute(n, a)
            : t.removeAttribute(n);
        break;
      case "cols":
      case "rows":
      case "size":
      case "span":
        a != null && typeof a != "function" && typeof a != "symbol" && !isNaN(a) && 1 <= a
          ? t.setAttribute(n, a)
          : t.removeAttribute(n);
        break;
      case "rowSpan":
      case "start":
        a == null || typeof a == "function" || typeof a == "symbol" || isNaN(a)
          ? t.removeAttribute(n)
          : t.setAttribute(n, a);
        break;
      case "popover":
        (dt("beforetoggle", t), dt("toggle", t), Ol(t, "popover", a));
        break;
      case "xlinkActuate":
        nn(t, "http://www.w3.org/1999/xlink", "xlink:actuate", a);
        break;
      case "xlinkArcrole":
        nn(t, "http://www.w3.org/1999/xlink", "xlink:arcrole", a);
        break;
      case "xlinkRole":
        nn(t, "http://www.w3.org/1999/xlink", "xlink:role", a);
        break;
      case "xlinkShow":
        nn(t, "http://www.w3.org/1999/xlink", "xlink:show", a);
        break;
      case "xlinkTitle":
        nn(t, "http://www.w3.org/1999/xlink", "xlink:title", a);
        break;
      case "xlinkType":
        nn(t, "http://www.w3.org/1999/xlink", "xlink:type", a);
        break;
      case "xmlBase":
        nn(t, "http://www.w3.org/XML/1998/namespace", "xml:base", a);
        break;
      case "xmlLang":
        nn(t, "http://www.w3.org/XML/1998/namespace", "xml:lang", a);
        break;
      case "xmlSpace":
        nn(t, "http://www.w3.org/XML/1998/namespace", "xml:space", a);
        break;
      case "is":
        Ol(t, "is", a);
        break;
      case "innerText":
      case "textContent":
        break;
      default:
        (!(2 < n.length) || (n[0] !== "o" && n[0] !== "O") || (n[1] !== "n" && n[1] !== "N")) &&
          ((n = Wy.get(n) || n), Ol(t, n, a));
    }
  }
  function yr(t, e, n, a, s, r) {
    switch (n) {
      case "style":
        jf(t, a, r);
        break;
      case "dangerouslySetInnerHTML":
        if (a != null) {
          if (typeof a != "object" || !("__html" in a)) throw Error(o(61));
          if (((n = a.__html), n != null)) {
            if (s.children != null) throw Error(o(60));
            t.innerHTML = n;
          }
        }
        break;
      case "children":
        typeof a == "string"
          ? zi(t, a)
          : (typeof a == "number" || typeof a == "bigint") && zi(t, "" + a);
        break;
      case "onScroll":
        a != null && dt("scroll", t);
        break;
      case "onScrollEnd":
        a != null && dt("scrollend", t);
        break;
      case "onClick":
        a != null && (t.onclick = an);
        break;
      case "suppressContentEditableWarning":
      case "suppressHydrationWarning":
      case "innerHTML":
      case "ref":
        break;
      case "innerText":
      case "textContent":
        break;
      default:
        if (!Sf.hasOwnProperty(n))
          t: {
            if (
              n[0] === "o" &&
              n[1] === "n" &&
              ((s = n.endsWith("Capture")),
              (e = n.slice(2, s ? n.length - 7 : void 0)),
              (r = t[de] || null),
              (r = r != null ? r[n] : null),
              typeof r == "function" && t.removeEventListener(e, r, s),
              typeof a == "function")
            ) {
              (typeof r != "function" &&
                r !== null &&
                (n in t ? (t[n] = null) : t.hasAttribute(n) && t.removeAttribute(n)),
                t.addEventListener(e, a, s));
              break t;
            }
            n in t ? (t[n] = a) : a === !0 ? t.setAttribute(n, "") : Ol(t, n, a);
          }
    }
  }
  function ie(t, e, n) {
    switch (e) {
      case "div":
      case "span":
      case "svg":
      case "path":
      case "a":
      case "g":
      case "p":
      case "li":
        break;
      case "img":
        (dt("error", t), dt("load", t));
        var a = !1,
          s = !1,
          r;
        for (r in n)
          if (n.hasOwnProperty(r)) {
            var f = n[r];
            if (f != null)
              switch (r) {
                case "src":
                  a = !0;
                  break;
                case "srcSet":
                  s = !0;
                  break;
                case "children":
                case "dangerouslySetInnerHTML":
                  throw Error(o(137, e));
                default:
                  Dt(t, e, r, f, n, null);
              }
          }
        (s && Dt(t, e, "srcSet", n.srcSet, n, null), a && Dt(t, e, "src", n.src, n, null));
        return;
      case "input":
        dt("invalid", t);
        var y = (r = f = s = null),
          T = null,
          C = null;
        for (a in n)
          if (n.hasOwnProperty(a)) {
            var O = n[a];
            if (O != null)
              switch (a) {
                case "name":
                  s = O;
                  break;
                case "type":
                  f = O;
                  break;
                case "checked":
                  T = O;
                  break;
                case "defaultChecked":
                  C = O;
                  break;
                case "value":
                  r = O;
                  break;
                case "defaultValue":
                  y = O;
                  break;
                case "children":
                case "dangerouslySetInnerHTML":
                  if (O != null) throw Error(o(137, e));
                  break;
                default:
                  Dt(t, e, a, O, n, null);
              }
          }
        Df(t, r, y, T, C, f, s, !1);
        return;
      case "select":
        (dt("invalid", t), (a = f = r = null));
        for (s in n)
          if (n.hasOwnProperty(s) && ((y = n[s]), y != null))
            switch (s) {
              case "value":
                r = y;
                break;
              case "defaultValue":
                f = y;
                break;
              case "multiple":
                a = y;
              default:
                Dt(t, e, s, y, n, null);
            }
        ((e = r),
          (n = f),
          (t.multiple = !!a),
          e != null ? Ni(t, !!a, e, !1) : n != null && Ni(t, !!a, n, !0));
        return;
      case "textarea":
        (dt("invalid", t), (r = s = a = null));
        for (f in n)
          if (n.hasOwnProperty(f) && ((y = n[f]), y != null))
            switch (f) {
              case "value":
                a = y;
                break;
              case "defaultValue":
                s = y;
                break;
              case "children":
                r = y;
                break;
              case "dangerouslySetInnerHTML":
                if (y != null) throw Error(o(91));
                break;
              default:
                Dt(t, e, f, y, n, null);
            }
        Nf(t, a, s, r);
        return;
      case "option":
        for (T in n)
          if (n.hasOwnProperty(T) && ((a = n[T]), a != null))
            switch (T) {
              case "selected":
                t.selected = a && typeof a != "function" && typeof a != "symbol";
                break;
              default:
                Dt(t, e, T, a, n, null);
            }
        return;
      case "dialog":
        (dt("beforetoggle", t), dt("toggle", t), dt("cancel", t), dt("close", t));
        break;
      case "iframe":
      case "object":
        dt("load", t);
        break;
      case "video":
      case "audio":
        for (a = 0; a < nl.length; a++) dt(nl[a], t);
        break;
      case "image":
        (dt("error", t), dt("load", t));
        break;
      case "details":
        dt("toggle", t);
        break;
      case "embed":
      case "source":
      case "link":
        (dt("error", t), dt("load", t));
      case "area":
      case "base":
      case "br":
      case "col":
      case "hr":
      case "keygen":
      case "meta":
      case "param":
      case "track":
      case "wbr":
      case "menuitem":
        for (C in n)
          if (n.hasOwnProperty(C) && ((a = n[C]), a != null))
            switch (C) {
              case "children":
              case "dangerouslySetInnerHTML":
                throw Error(o(137, e));
              default:
                Dt(t, e, C, a, n, null);
            }
        return;
      default:
        if (ju(e)) {
          for (O in n)
            n.hasOwnProperty(O) && ((a = n[O]), a !== void 0 && yr(t, e, O, a, n, void 0));
          return;
        }
    }
    for (y in n) n.hasOwnProperty(y) && ((a = n[y]), a != null && Dt(t, e, y, a, n, null));
  }
  function E1(t, e, n, a) {
    switch (e) {
      case "div":
      case "span":
      case "svg":
      case "path":
      case "a":
      case "g":
      case "p":
      case "li":
        break;
      case "input":
        var s = null,
          r = null,
          f = null,
          y = null,
          T = null,
          C = null,
          O = null;
        for (j in n) {
          var _ = n[j];
          if (n.hasOwnProperty(j) && _ != null)
            switch (j) {
              case "checked":
                break;
              case "value":
                break;
              case "defaultValue":
                T = _;
              default:
                a.hasOwnProperty(j) || Dt(t, e, j, null, a, _);
            }
        }
        for (var z in a) {
          var j = a[z];
          if (((_ = n[z]), a.hasOwnProperty(z) && (j != null || _ != null)))
            switch (z) {
              case "type":
                r = j;
                break;
              case "name":
                s = j;
                break;
              case "checked":
                C = j;
                break;
              case "defaultChecked":
                O = j;
                break;
              case "value":
                f = j;
                break;
              case "defaultValue":
                y = j;
                break;
              case "children":
              case "dangerouslySetInnerHTML":
                if (j != null) throw Error(o(137, e));
                break;
              default:
                j !== _ && Dt(t, e, z, j, a, _);
            }
        }
        Nu(t, f, y, T, C, O, r, s);
        return;
      case "select":
        j = f = y = z = null;
        for (r in n)
          if (((T = n[r]), n.hasOwnProperty(r) && T != null))
            switch (r) {
              case "value":
                break;
              case "multiple":
                j = T;
              default:
                a.hasOwnProperty(r) || Dt(t, e, r, null, a, T);
            }
        for (s in a)
          if (((r = a[s]), (T = n[s]), a.hasOwnProperty(s) && (r != null || T != null)))
            switch (s) {
              case "value":
                z = r;
                break;
              case "defaultValue":
                y = r;
                break;
              case "multiple":
                f = r;
              default:
                r !== T && Dt(t, e, s, r, a, T);
            }
        ((e = y),
          (n = f),
          (a = j),
          z != null
            ? Ni(t, !!n, z, !1)
            : !!a != !!n && (e != null ? Ni(t, !!n, e, !0) : Ni(t, !!n, n ? [] : "", !1)));
        return;
      case "textarea":
        j = z = null;
        for (y in n)
          if (((s = n[y]), n.hasOwnProperty(y) && s != null && !a.hasOwnProperty(y)))
            switch (y) {
              case "value":
                break;
              case "children":
                break;
              default:
                Dt(t, e, y, null, a, s);
            }
        for (f in a)
          if (((s = a[f]), (r = n[f]), a.hasOwnProperty(f) && (s != null || r != null)))
            switch (f) {
              case "value":
                z = s;
                break;
              case "defaultValue":
                j = s;
                break;
              case "children":
                break;
              case "dangerouslySetInnerHTML":
                if (s != null) throw Error(o(91));
                break;
              default:
                s !== r && Dt(t, e, f, s, a, r);
            }
        Cf(t, z, j);
        return;
      case "option":
        for (var K in n)
          if (((z = n[K]), n.hasOwnProperty(K) && z != null && !a.hasOwnProperty(K)))
            switch (K) {
              case "selected":
                t.selected = !1;
                break;
              default:
                Dt(t, e, K, null, a, z);
            }
        for (T in a)
          if (((z = a[T]), (j = n[T]), a.hasOwnProperty(T) && z !== j && (z != null || j != null)))
            switch (T) {
              case "selected":
                t.selected = z && typeof z != "function" && typeof z != "symbol";
                break;
              default:
                Dt(t, e, T, z, a, j);
            }
        return;
      case "img":
      case "link":
      case "area":
      case "base":
      case "br":
      case "col":
      case "embed":
      case "hr":
      case "keygen":
      case "meta":
      case "param":
      case "source":
      case "track":
      case "wbr":
      case "menuitem":
        for (var nt in n)
          ((z = n[nt]),
            n.hasOwnProperty(nt) && z != null && !a.hasOwnProperty(nt) && Dt(t, e, nt, null, a, z));
        for (C in a)
          if (((z = a[C]), (j = n[C]), a.hasOwnProperty(C) && z !== j && (z != null || j != null)))
            switch (C) {
              case "children":
              case "dangerouslySetInnerHTML":
                if (z != null) throw Error(o(137, e));
                break;
              default:
                Dt(t, e, C, z, a, j);
            }
        return;
      default:
        if (ju(e)) {
          for (var Ct in n)
            ((z = n[Ct]),
              n.hasOwnProperty(Ct) &&
                z !== void 0 &&
                !a.hasOwnProperty(Ct) &&
                yr(t, e, Ct, void 0, a, z));
          for (O in a)
            ((z = a[O]),
              (j = n[O]),
              !a.hasOwnProperty(O) ||
                z === j ||
                (z === void 0 && j === void 0) ||
                yr(t, e, O, z, a, j));
          return;
        }
    }
    for (var M in n)
      ((z = n[M]),
        n.hasOwnProperty(M) && z != null && !a.hasOwnProperty(M) && Dt(t, e, M, null, a, z));
    for (_ in a)
      ((z = a[_]),
        (j = n[_]),
        !a.hasOwnProperty(_) || z === j || (z == null && j == null) || Dt(t, e, _, z, a, j));
  }
  function wm(t) {
    switch (t) {
      case "css":
      case "script":
      case "font":
      case "img":
      case "image":
      case "input":
      case "link":
        return !0;
      default:
        return !1;
    }
  }
  function M1() {
    if (typeof performance.getEntriesByType == "function") {
      for (
        var t = 0, e = 0, n = performance.getEntriesByType("resource"), a = 0;
        a < n.length;
        a++
      ) {
        var s = n[a],
          r = s.transferSize,
          f = s.initiatorType,
          y = s.duration;
        if (r && y && wm(f)) {
          for (f = 0, y = s.responseEnd, a += 1; a < n.length; a++) {
            var T = n[a],
              C = T.startTime;
            if (C > y) break;
            var O = T.transferSize,
              _ = T.initiatorType;
            O && wm(_) && ((T = T.responseEnd), (f += O * (T < y ? 1 : (y - C) / (T - C))));
          }
          if ((--a, (e += (8 * (r + f)) / (s.duration / 1e3)), t++, 10 < t)) break;
        }
      }
      if (0 < t) return e / t / 1e6;
    }
    return navigator.connection && ((t = navigator.connection.downlink), typeof t == "number")
      ? t
      : 5;
  }
  var vr = null,
    xr = null;
  function ws(t) {
    return t.nodeType === 9 ? t : t.ownerDocument;
  }
  function Rm(t) {
    switch (t) {
      case "http://www.w3.org/2000/svg":
        return 1;
      case "http://www.w3.org/1998/Math/MathML":
        return 2;
      default:
        return 0;
    }
  }
  function Om(t, e) {
    if (t === 0)
      switch (e) {
        case "svg":
          return 1;
        case "math":
          return 2;
        default:
          return 0;
      }
    return t === 1 && e === "foreignObject" ? 0 : t;
  }
  function br(t, e) {
    return (
      t === "textarea" ||
      t === "noscript" ||
      typeof e.children == "string" ||
      typeof e.children == "number" ||
      typeof e.children == "bigint" ||
      (typeof e.dangerouslySetInnerHTML == "object" &&
        e.dangerouslySetInnerHTML !== null &&
        e.dangerouslySetInnerHTML.__html != null)
    );
  }
  var Sr = null;
  function D1() {
    var t = window.event;
    return t && t.type === "popstate" ? (t === Sr ? !1 : ((Sr = t), !0)) : ((Sr = null), !1);
  }
  var Vm = typeof setTimeout == "function" ? setTimeout : void 0,
    C1 = typeof clearTimeout == "function" ? clearTimeout : void 0,
    _m = typeof Promise == "function" ? Promise : void 0,
    N1 =
      typeof queueMicrotask == "function"
        ? queueMicrotask
        : typeof _m < "u"
          ? function (t) {
              return _m.resolve(null).then(t).catch(z1);
            }
          : Vm;
  function z1(t) {
    setTimeout(function () {
      throw t;
    });
  }
  function qn(t) {
    return t === "head";
  }
  function Bm(t, e) {
    var n = e,
      a = 0;
    do {
      var s = n.nextSibling;
      if ((t.removeChild(n), s && s.nodeType === 8))
        if (((n = s.data), n === "/$" || n === "/&")) {
          if (a === 0) {
            (t.removeChild(s), sa(e));
            return;
          }
          a--;
        } else if (n === "$" || n === "$?" || n === "$~" || n === "$!" || n === "&") a++;
        else if (n === "html") al(t.ownerDocument.documentElement);
        else if (n === "head") {
          ((n = t.ownerDocument.head), al(n));
          for (var r = n.firstChild; r; ) {
            var f = r.nextSibling,
              y = r.nodeName;
            (r[Ta] ||
              y === "SCRIPT" ||
              y === "STYLE" ||
              (y === "LINK" && r.rel.toLowerCase() === "stylesheet") ||
              n.removeChild(r),
              (r = f));
          }
        } else n === "body" && al(t.ownerDocument.body);
      n = s;
    } while (n);
    sa(e);
  }
  function Um(t, e) {
    var n = t;
    t = 0;
    do {
      var a = n.nextSibling;
      if (
        (n.nodeType === 1
          ? e
            ? ((n._stashedDisplay = n.style.display), (n.style.display = "none"))
            : ((n.style.display = n._stashedDisplay || ""),
              n.getAttribute("style") === "" && n.removeAttribute("style"))
          : n.nodeType === 3 &&
            (e
              ? ((n._stashedText = n.nodeValue), (n.nodeValue = ""))
              : (n.nodeValue = n._stashedText || "")),
        a && a.nodeType === 8)
      )
        if (((n = a.data), n === "/$")) {
          if (t === 0) break;
          t--;
        } else (n !== "$" && n !== "$?" && n !== "$~" && n !== "$!") || t++;
      n = a;
    } while (n);
  }
  function Tr(t) {
    var e = t.firstChild;
    for (e && e.nodeType === 10 && (e = e.nextSibling); e; ) {
      var n = e;
      switch (((e = e.nextSibling), n.nodeName)) {
        case "HTML":
        case "HEAD":
        case "BODY":
          (Tr(n), Du(n));
          continue;
        case "SCRIPT":
        case "STYLE":
          continue;
        case "LINK":
          if (n.rel.toLowerCase() === "stylesheet") continue;
      }
      t.removeChild(n);
    }
  }
  function j1(t, e, n, a) {
    for (; t.nodeType === 1; ) {
      var s = n;
      if (t.nodeName.toLowerCase() !== e.toLowerCase()) {
        if (!a && (t.nodeName !== "INPUT" || t.type !== "hidden")) break;
      } else if (a) {
        if (!t[Ta])
          switch (e) {
            case "meta":
              if (!t.hasAttribute("itemprop")) break;
              return t;
            case "link":
              if (
                ((r = t.getAttribute("rel")),
                r === "stylesheet" && t.hasAttribute("data-precedence"))
              )
                break;
              if (
                r !== s.rel ||
                t.getAttribute("href") !== (s.href == null || s.href === "" ? null : s.href) ||
                t.getAttribute("crossorigin") !== (s.crossOrigin == null ? null : s.crossOrigin) ||
                t.getAttribute("title") !== (s.title == null ? null : s.title)
              )
                break;
              return t;
            case "style":
              if (t.hasAttribute("data-precedence")) break;
              return t;
            case "script":
              if (
                ((r = t.getAttribute("src")),
                (r !== (s.src == null ? null : s.src) ||
                  t.getAttribute("type") !== (s.type == null ? null : s.type) ||
                  t.getAttribute("crossorigin") !==
                    (s.crossOrigin == null ? null : s.crossOrigin)) &&
                  r &&
                  t.hasAttribute("async") &&
                  !t.hasAttribute("itemprop"))
              )
                break;
              return t;
            default:
              return t;
          }
      } else if (e === "input" && t.type === "hidden") {
        var r = s.name == null ? null : "" + s.name;
        if (s.type === "hidden" && t.getAttribute("name") === r) return t;
      } else return t;
      if (((t = He(t.nextSibling)), t === null)) break;
    }
    return null;
  }
  function w1(t, e, n) {
    if (e === "") return null;
    for (; t.nodeType !== 3; )
      if (
        ((t.nodeType !== 1 || t.nodeName !== "INPUT" || t.type !== "hidden") && !n) ||
        ((t = He(t.nextSibling)), t === null)
      )
        return null;
    return t;
  }
  function Lm(t, e) {
    for (; t.nodeType !== 8; )
      if (
        ((t.nodeType !== 1 || t.nodeName !== "INPUT" || t.type !== "hidden") && !e) ||
        ((t = He(t.nextSibling)), t === null)
      )
        return null;
    return t;
  }
  function Ar(t) {
    return t.data === "$?" || t.data === "$~";
  }
  function Er(t) {
    return t.data === "$!" || (t.data === "$?" && t.ownerDocument.readyState !== "loading");
  }
  function R1(t, e) {
    var n = t.ownerDocument;
    if (t.data === "$~") t._reactRetry = e;
    else if (t.data !== "$?" || n.readyState !== "loading") e();
    else {
      var a = function () {
        (e(), n.removeEventListener("DOMContentLoaded", a));
      };
      (n.addEventListener("DOMContentLoaded", a), (t._reactRetry = a));
    }
  }
  function He(t) {
    for (; t != null; t = t.nextSibling) {
      var e = t.nodeType;
      if (e === 1 || e === 3) break;
      if (e === 8) {
        if (
          ((e = t.data),
          e === "$" ||
            e === "$!" ||
            e === "$?" ||
            e === "$~" ||
            e === "&" ||
            e === "F!" ||
            e === "F")
        )
          break;
        if (e === "/$" || e === "/&") return null;
      }
    }
    return t;
  }
  var Mr = null;
  function Hm(t) {
    t = t.nextSibling;
    for (var e = 0; t; ) {
      if (t.nodeType === 8) {
        var n = t.data;
        if (n === "/$" || n === "/&") {
          if (e === 0) return He(t.nextSibling);
          e--;
        } else (n !== "$" && n !== "$!" && n !== "$?" && n !== "$~" && n !== "&") || e++;
      }
      t = t.nextSibling;
    }
    return null;
  }
  function qm(t) {
    t = t.previousSibling;
    for (var e = 0; t; ) {
      if (t.nodeType === 8) {
        var n = t.data;
        if (n === "$" || n === "$!" || n === "$?" || n === "$~" || n === "&") {
          if (e === 0) return t;
          e--;
        } else (n !== "/$" && n !== "/&") || e++;
      }
      t = t.previousSibling;
    }
    return null;
  }
  function Ym(t, e, n) {
    switch (((e = ws(n)), t)) {
      case "html":
        if (((t = e.documentElement), !t)) throw Error(o(452));
        return t;
      case "head":
        if (((t = e.head), !t)) throw Error(o(453));
        return t;
      case "body":
        if (((t = e.body), !t)) throw Error(o(454));
        return t;
      default:
        throw Error(o(451));
    }
  }
  function al(t) {
    for (var e = t.attributes; e.length; ) t.removeAttributeNode(e[0]);
    Du(t);
  }
  var qe = new Map(),
    Gm = new Set();
  function Rs(t) {
    return typeof t.getRootNode == "function"
      ? t.getRootNode()
      : t.nodeType === 9
        ? t
        : t.ownerDocument;
  }
  var bn = X.d;
  X.d = { f: O1, r: V1, D: _1, C: B1, L: U1, m: L1, X: q1, S: H1, M: Y1 };
  function O1() {
    var t = bn.f(),
      e = As();
    return t || e;
  }
  function V1(t) {
    var e = Mi(t);
    e !== null && e.tag === 5 && e.type === "form" ? lh(e) : bn.r(t);
  }
  var ia = typeof document > "u" ? null : document;
  function Xm(t, e, n) {
    var a = ia;
    if (a && typeof e == "string" && e) {
      var s = Re(e);
      ((s = 'link[rel="' + t + '"][href="' + s + '"]'),
        typeof n == "string" && (s += '[crossorigin="' + n + '"]'),
        Gm.has(s) ||
          (Gm.add(s),
          (t = { rel: t, crossOrigin: n, href: e }),
          a.querySelector(s) === null &&
            ((e = a.createElement("link")), ie(e, "link", t), Pt(e), a.head.appendChild(e))));
    }
  }
  function _1(t) {
    (bn.D(t), Xm("dns-prefetch", t, null));
  }
  function B1(t, e) {
    (bn.C(t, e), Xm("preconnect", t, e));
  }
  function U1(t, e, n) {
    bn.L(t, e, n);
    var a = ia;
    if (a && t && e) {
      var s = 'link[rel="preload"][as="' + Re(e) + '"]';
      e === "image" && n && n.imageSrcSet
        ? ((s += '[imagesrcset="' + Re(n.imageSrcSet) + '"]'),
          typeof n.imageSizes == "string" && (s += '[imagesizes="' + Re(n.imageSizes) + '"]'))
        : (s += '[href="' + Re(t) + '"]');
      var r = s;
      switch (e) {
        case "style":
          r = aa(t);
          break;
        case "script":
          r = la(t);
      }
      qe.has(r) ||
        ((t = x(
          { rel: "preload", href: e === "image" && n && n.imageSrcSet ? void 0 : t, as: e },
          n,
        )),
        qe.set(r, t),
        a.querySelector(s) !== null ||
          (e === "style" && a.querySelector(ll(r))) ||
          (e === "script" && a.querySelector(sl(r))) ||
          ((e = a.createElement("link")), ie(e, "link", t), Pt(e), a.head.appendChild(e)));
    }
  }
  function L1(t, e) {
    bn.m(t, e);
    var n = ia;
    if (n && t) {
      var a = e && typeof e.as == "string" ? e.as : "script",
        s = 'link[rel="modulepreload"][as="' + Re(a) + '"][href="' + Re(t) + '"]',
        r = s;
      switch (a) {
        case "audioworklet":
        case "paintworklet":
        case "serviceworker":
        case "sharedworker":
        case "worker":
        case "script":
          r = la(t);
      }
      if (
        !qe.has(r) &&
        ((t = x({ rel: "modulepreload", href: t }, e)), qe.set(r, t), n.querySelector(s) === null)
      ) {
        switch (a) {
          case "audioworklet":
          case "paintworklet":
          case "serviceworker":
          case "sharedworker":
          case "worker":
          case "script":
            if (n.querySelector(sl(r))) return;
        }
        ((a = n.createElement("link")), ie(a, "link", t), Pt(a), n.head.appendChild(a));
      }
    }
  }
  function H1(t, e, n) {
    bn.S(t, e, n);
    var a = ia;
    if (a && t) {
      var s = Di(a).hoistableStyles,
        r = aa(t);
      e = e || "default";
      var f = s.get(r);
      if (!f) {
        var y = { loading: 0, preload: null };
        if ((f = a.querySelector(ll(r)))) y.loading = 5;
        else {
          ((t = x({ rel: "stylesheet", href: t, "data-precedence": e }, n)),
            (n = qe.get(r)) && Dr(t, n));
          var T = (f = a.createElement("link"));
          (Pt(T),
            ie(T, "link", t),
            (T._p = new Promise(function (C, O) {
              ((T.onload = C), (T.onerror = O));
            })),
            T.addEventListener("load", function () {
              y.loading |= 1;
            }),
            T.addEventListener("error", function () {
              y.loading |= 2;
            }),
            (y.loading |= 4),
            Os(f, e, a));
        }
        ((f = { type: "stylesheet", instance: f, count: 1, state: y }), s.set(r, f));
      }
    }
  }
  function q1(t, e) {
    bn.X(t, e);
    var n = ia;
    if (n && t) {
      var a = Di(n).hoistableScripts,
        s = la(t),
        r = a.get(s);
      r ||
        ((r = n.querySelector(sl(s))),
        r ||
          ((t = x({ src: t, async: !0 }, e)),
          (e = qe.get(s)) && Cr(t, e),
          (r = n.createElement("script")),
          Pt(r),
          ie(r, "link", t),
          n.head.appendChild(r)),
        (r = { type: "script", instance: r, count: 1, state: null }),
        a.set(s, r));
    }
  }
  function Y1(t, e) {
    bn.M(t, e);
    var n = ia;
    if (n && t) {
      var a = Di(n).hoistableScripts,
        s = la(t),
        r = a.get(s);
      r ||
        ((r = n.querySelector(sl(s))),
        r ||
          ((t = x({ src: t, async: !0, type: "module" }, e)),
          (e = qe.get(s)) && Cr(t, e),
          (r = n.createElement("script")),
          Pt(r),
          ie(r, "link", t),
          n.head.appendChild(r)),
        (r = { type: "script", instance: r, count: 1, state: null }),
        a.set(s, r));
    }
  }
  function km(t, e, n, a) {
    var s = (s = ct.current) ? Rs(s) : null;
    if (!s) throw Error(o(446));
    switch (t) {
      case "meta":
      case "title":
        return null;
      case "style":
        return typeof n.precedence == "string" && typeof n.href == "string"
          ? ((e = aa(n.href)),
            (n = Di(s).hoistableStyles),
            (a = n.get(e)),
            a || ((a = { type: "style", instance: null, count: 0, state: null }), n.set(e, a)),
            a)
          : { type: "void", instance: null, count: 0, state: null };
      case "link":
        if (
          n.rel === "stylesheet" &&
          typeof n.href == "string" &&
          typeof n.precedence == "string"
        ) {
          t = aa(n.href);
          var r = Di(s).hoistableStyles,
            f = r.get(t);
          if (
            (f ||
              ((s = s.ownerDocument || s),
              (f = {
                type: "stylesheet",
                instance: null,
                count: 0,
                state: { loading: 0, preload: null },
              }),
              r.set(t, f),
              (r = s.querySelector(ll(t))) && !r._p && ((f.instance = r), (f.state.loading = 5)),
              qe.has(t) ||
                ((n = {
                  rel: "preload",
                  as: "style",
                  href: n.href,
                  crossOrigin: n.crossOrigin,
                  integrity: n.integrity,
                  media: n.media,
                  hrefLang: n.hrefLang,
                  referrerPolicy: n.referrerPolicy,
                }),
                qe.set(t, n),
                r || G1(s, t, n, f.state))),
            e && a === null)
          )
            throw Error(o(528, ""));
          return f;
        }
        if (e && a !== null) throw Error(o(529, ""));
        return null;
      case "script":
        return (
          (e = n.async),
          (n = n.src),
          typeof n == "string" && e && typeof e != "function" && typeof e != "symbol"
            ? ((e = la(n)),
              (n = Di(s).hoistableScripts),
              (a = n.get(e)),
              a || ((a = { type: "script", instance: null, count: 0, state: null }), n.set(e, a)),
              a)
            : { type: "void", instance: null, count: 0, state: null }
        );
      default:
        throw Error(o(444, t));
    }
  }
  function aa(t) {
    return 'href="' + Re(t) + '"';
  }
  function ll(t) {
    return 'link[rel="stylesheet"][' + t + "]";
  }
  function Qm(t) {
    return x({}, t, { "data-precedence": t.precedence, precedence: null });
  }
  function G1(t, e, n, a) {
    t.querySelector('link[rel="preload"][as="style"][' + e + "]")
      ? (a.loading = 1)
      : ((e = t.createElement("link")),
        (a.preload = e),
        e.addEventListener("load", function () {
          return (a.loading |= 1);
        }),
        e.addEventListener("error", function () {
          return (a.loading |= 2);
        }),
        ie(e, "link", n),
        Pt(e),
        t.head.appendChild(e));
  }
  function la(t) {
    return '[src="' + Re(t) + '"]';
  }
  function sl(t) {
    return "script[async]" + t;
  }
  function Zm(t, e, n) {
    if ((e.count++, e.instance === null))
      switch (e.type) {
        case "style":
          var a = t.querySelector('style[data-href~="' + Re(n.href) + '"]');
          if (a) return ((e.instance = a), Pt(a), a);
          var s = x({}, n, {
            "data-href": n.href,
            "data-precedence": n.precedence,
            href: null,
            precedence: null,
          });
          return (
            (a = (t.ownerDocument || t).createElement("style")),
            Pt(a),
            ie(a, "style", s),
            Os(a, n.precedence, t),
            (e.instance = a)
          );
        case "stylesheet":
          s = aa(n.href);
          var r = t.querySelector(ll(s));
          if (r) return ((e.state.loading |= 4), (e.instance = r), Pt(r), r);
          ((a = Qm(n)),
            (s = qe.get(s)) && Dr(a, s),
            (r = (t.ownerDocument || t).createElement("link")),
            Pt(r));
          var f = r;
          return (
            (f._p = new Promise(function (y, T) {
              ((f.onload = y), (f.onerror = T));
            })),
            ie(r, "link", a),
            (e.state.loading |= 4),
            Os(r, n.precedence, t),
            (e.instance = r)
          );
        case "script":
          return (
            (r = la(n.src)),
            (s = t.querySelector(sl(r)))
              ? ((e.instance = s), Pt(s), s)
              : ((a = n),
                (s = qe.get(r)) && ((a = x({}, n)), Cr(a, s)),
                (t = t.ownerDocument || t),
                (s = t.createElement("script")),
                Pt(s),
                ie(s, "link", a),
                t.head.appendChild(s),
                (e.instance = s))
          );
        case "void":
          return null;
        default:
          throw Error(o(443, e.type));
      }
    else
      e.type === "stylesheet" &&
        (e.state.loading & 4) === 0 &&
        ((a = e.instance), (e.state.loading |= 4), Os(a, n.precedence, t));
    return e.instance;
  }
  function Os(t, e, n) {
    for (
      var a = n.querySelectorAll('link[rel="stylesheet"][data-precedence],style[data-precedence]'),
        s = a.length ? a[a.length - 1] : null,
        r = s,
        f = 0;
      f < a.length;
      f++
    ) {
      var y = a[f];
      if (y.dataset.precedence === e) r = y;
      else if (r !== s) break;
    }
    r
      ? r.parentNode.insertBefore(t, r.nextSibling)
      : ((e = n.nodeType === 9 ? n.head : n), e.insertBefore(t, e.firstChild));
  }
  function Dr(t, e) {
    (t.crossOrigin == null && (t.crossOrigin = e.crossOrigin),
      t.referrerPolicy == null && (t.referrerPolicy = e.referrerPolicy),
      t.title == null && (t.title = e.title));
  }
  function Cr(t, e) {
    (t.crossOrigin == null && (t.crossOrigin = e.crossOrigin),
      t.referrerPolicy == null && (t.referrerPolicy = e.referrerPolicy),
      t.integrity == null && (t.integrity = e.integrity));
  }
  var Vs = null;
  function Km(t, e, n) {
    if (Vs === null) {
      var a = new Map(),
        s = (Vs = new Map());
      s.set(n, a);
    } else ((s = Vs), (a = s.get(n)), a || ((a = new Map()), s.set(n, a)));
    if (a.has(t)) return a;
    for (a.set(t, null), n = n.getElementsByTagName(t), s = 0; s < n.length; s++) {
      var r = n[s];
      if (
        !(r[Ta] || r[It] || (t === "link" && r.getAttribute("rel") === "stylesheet")) &&
        r.namespaceURI !== "http://www.w3.org/2000/svg"
      ) {
        var f = r.getAttribute(e) || "";
        f = t + f;
        var y = a.get(f);
        y ? y.push(r) : a.set(f, [r]);
      }
    }
    return a;
  }
  function Jm(t, e, n) {
    ((t = t.ownerDocument || t),
      t.head.insertBefore(n, e === "title" ? t.querySelector("head > title") : null));
  }
  function X1(t, e, n) {
    if (n === 1 || e.itemProp != null) return !1;
    switch (t) {
      case "meta":
      case "title":
        return !0;
      case "style":
        if (typeof e.precedence != "string" || typeof e.href != "string" || e.href === "") break;
        return !0;
      case "link":
        if (
          typeof e.rel != "string" ||
          typeof e.href != "string" ||
          e.href === "" ||
          e.onLoad ||
          e.onError
        )
          break;
        switch (e.rel) {
          case "stylesheet":
            return ((t = e.disabled), typeof e.precedence == "string" && t == null);
          default:
            return !0;
        }
      case "script":
        if (
          e.async &&
          typeof e.async != "function" &&
          typeof e.async != "symbol" &&
          !e.onLoad &&
          !e.onError &&
          e.src &&
          typeof e.src == "string"
        )
          return !0;
    }
    return !1;
  }
  function Fm(t) {
    return !(t.type === "stylesheet" && (t.state.loading & 3) === 0);
  }
  function k1(t, e, n, a) {
    if (
      n.type === "stylesheet" &&
      (typeof a.media != "string" || matchMedia(a.media).matches !== !1) &&
      (n.state.loading & 4) === 0
    ) {
      if (n.instance === null) {
        var s = aa(a.href),
          r = e.querySelector(ll(s));
        if (r) {
          ((e = r._p),
            e !== null &&
              typeof e == "object" &&
              typeof e.then == "function" &&
              (t.count++, (t = _s.bind(t)), e.then(t, t)),
            (n.state.loading |= 4),
            (n.instance = r),
            Pt(r));
          return;
        }
        ((r = e.ownerDocument || e),
          (a = Qm(a)),
          (s = qe.get(s)) && Dr(a, s),
          (r = r.createElement("link")),
          Pt(r));
        var f = r;
        ((f._p = new Promise(function (y, T) {
          ((f.onload = y), (f.onerror = T));
        })),
          ie(r, "link", a),
          (n.instance = r));
      }
      (t.stylesheets === null && (t.stylesheets = new Map()),
        t.stylesheets.set(n, e),
        (e = n.state.preload) &&
          (n.state.loading & 3) === 0 &&
          (t.count++,
          (n = _s.bind(t)),
          e.addEventListener("load", n),
          e.addEventListener("error", n)));
    }
  }
  var Nr = 0;
  function Q1(t, e) {
    return (
      t.stylesheets && t.count === 0 && Us(t, t.stylesheets),
      0 < t.count || 0 < t.imgCount
        ? function (n) {
            var a = setTimeout(function () {
              if ((t.stylesheets && Us(t, t.stylesheets), t.unsuspend)) {
                var r = t.unsuspend;
                ((t.unsuspend = null), r());
              }
            }, 6e4 + e);
            0 < t.imgBytes && Nr === 0 && (Nr = 62500 * M1());
            var s = setTimeout(
              function () {
                if (
                  ((t.waitingForImages = !1),
                  t.count === 0 && (t.stylesheets && Us(t, t.stylesheets), t.unsuspend))
                ) {
                  var r = t.unsuspend;
                  ((t.unsuspend = null), r());
                }
              },
              (t.imgBytes > Nr ? 50 : 800) + e,
            );
            return (
              (t.unsuspend = n),
              function () {
                ((t.unsuspend = null), clearTimeout(a), clearTimeout(s));
              }
            );
          }
        : null
    );
  }
  function _s() {
    if ((this.count--, this.count === 0 && (this.imgCount === 0 || !this.waitingForImages))) {
      if (this.stylesheets) Us(this, this.stylesheets);
      else if (this.unsuspend) {
        var t = this.unsuspend;
        ((this.unsuspend = null), t());
      }
    }
  }
  var Bs = null;
  function Us(t, e) {
    ((t.stylesheets = null),
      t.unsuspend !== null &&
        (t.count++, (Bs = new Map()), e.forEach(Z1, t), (Bs = null), _s.call(t)));
  }
  function Z1(t, e) {
    if (!(e.state.loading & 4)) {
      var n = Bs.get(t);
      if (n) var a = n.get(null);
      else {
        ((n = new Map()), Bs.set(t, n));
        for (
          var s = t.querySelectorAll("link[data-precedence],style[data-precedence]"), r = 0;
          r < s.length;
          r++
        ) {
          var f = s[r];
          (f.nodeName === "LINK" || f.getAttribute("media") !== "not all") &&
            (n.set(f.dataset.precedence, f), (a = f));
        }
        a && n.set(null, a);
      }
      ((s = e.instance),
        (f = s.getAttribute("data-precedence")),
        (r = n.get(f) || a),
        r === a && n.set(null, s),
        n.set(f, s),
        this.count++,
        (a = _s.bind(this)),
        s.addEventListener("load", a),
        s.addEventListener("error", a),
        r
          ? r.parentNode.insertBefore(s, r.nextSibling)
          : ((t = t.nodeType === 9 ? t.head : t), t.insertBefore(s, t.firstChild)),
        (e.state.loading |= 4));
    }
  }
  var ul = {
    $$typeof: q,
    Provider: null,
    Consumer: null,
    _currentValue: Z,
    _currentValue2: Z,
    _threadCount: 0,
  };
  function K1(t, e, n, a, s, r, f, y, T) {
    ((this.tag = 1),
      (this.containerInfo = t),
      (this.pingCache = this.current = this.pendingChildren = null),
      (this.timeoutHandle = -1),
      (this.callbackNode =
        this.next =
        this.pendingContext =
        this.context =
        this.cancelPendingCommit =
          null),
      (this.callbackPriority = 0),
      (this.expirationTimes = Tu(-1)),
      (this.entangledLanes =
        this.shellSuspendCounter =
        this.errorRecoveryDisabledLanes =
        this.expiredLanes =
        this.warmLanes =
        this.pingedLanes =
        this.suspendedLanes =
        this.pendingLanes =
          0),
      (this.entanglements = Tu(0)),
      (this.hiddenUpdates = Tu(null)),
      (this.identifierPrefix = a),
      (this.onUncaughtError = s),
      (this.onCaughtError = r),
      (this.onRecoverableError = f),
      (this.pooledCache = null),
      (this.pooledCacheLanes = 0),
      (this.formState = T),
      (this.incompleteTransitions = new Map()));
  }
  function Pm(t, e, n, a, s, r, f, y, T, C, O, _) {
    return (
      (t = new K1(t, e, n, f, T, C, O, _, y)),
      (e = 1),
      r === !0 && (e |= 24),
      (r = Ae(3, null, null, e)),
      (t.current = r),
      (r.stateNode = t),
      (e = so()),
      e.refCount++,
      (t.pooledCache = e),
      e.refCount++,
      (r.memoizedState = { element: a, isDehydrated: n, cache: e }),
      co(r),
      t
    );
  }
  function Wm(t) {
    return t ? ((t = Bi), t) : Bi;
  }
  function $m(t, e, n, a, s, r) {
    ((s = Wm(s)),
      a.context === null ? (a.context = s) : (a.pendingContext = s),
      (a = zn(e)),
      (a.payload = { element: n }),
      (r = r === void 0 ? null : r),
      r !== null && (a.callback = r),
      (n = jn(t, a, e)),
      n !== null && (ve(n, t, e), Ha(n, t, e)));
  }
  function Im(t, e) {
    if (((t = t.memoizedState), t !== null && t.dehydrated !== null)) {
      var n = t.retryLane;
      t.retryLane = n !== 0 && n < e ? n : e;
    }
  }
  function zr(t, e) {
    (Im(t, e), (t = t.alternate) && Im(t, e));
  }
  function tp(t) {
    if (t.tag === 13 || t.tag === 31) {
      var e = ai(t, 67108864);
      (e !== null && ve(e, t, 67108864), zr(t, 67108864));
    }
  }
  function ep(t) {
    if (t.tag === 13 || t.tag === 31) {
      var e = Ne();
      e = Au(e);
      var n = ai(t, e);
      (n !== null && ve(n, t, e), zr(t, e));
    }
  }
  var Ls = !0;
  function J1(t, e, n, a) {
    var s = R.T;
    R.T = null;
    var r = X.p;
    try {
      ((X.p = 2), jr(t, e, n, a));
    } finally {
      ((X.p = r), (R.T = s));
    }
  }
  function F1(t, e, n, a) {
    var s = R.T;
    R.T = null;
    var r = X.p;
    try {
      ((X.p = 8), jr(t, e, n, a));
    } finally {
      ((X.p = r), (R.T = s));
    }
  }
  function jr(t, e, n, a) {
    if (Ls) {
      var s = wr(a);
      if (s === null) (gr(t, e, a, Hs, n), ip(t, a));
      else if (W1(s, t, e, n, a)) a.stopPropagation();
      else if ((ip(t, a), e & 4 && -1 < P1.indexOf(t))) {
        for (; s !== null; ) {
          var r = Mi(s);
          if (r !== null)
            switch (r.tag) {
              case 3:
                if (((r = r.stateNode), r.current.memoizedState.isDehydrated)) {
                  var f = In(r.pendingLanes);
                  if (f !== 0) {
                    var y = r;
                    for (y.pendingLanes |= 2, y.entangledLanes |= 2; f; ) {
                      var T = 1 << (31 - Se(f));
                      ((y.entanglements[1] |= T), (f &= ~T));
                    }
                    ($e(r), (St & 6) === 0 && ((Ss = xe() + 500), el(0)));
                  }
                }
                break;
              case 31:
              case 13:
                ((y = ai(r, 2)), y !== null && ve(y, r, 2), As(), zr(r, 2));
            }
          if (((r = wr(a)), r === null && gr(t, e, a, Hs, n), r === s)) break;
          s = r;
        }
        s !== null && a.stopPropagation();
      } else gr(t, e, a, null, n);
    }
  }
  function wr(t) {
    return ((t = Ru(t)), Rr(t));
  }
  var Hs = null;
  function Rr(t) {
    if (((Hs = null), (t = Ei(t)), t !== null)) {
      var e = h(t);
      if (e === null) t = null;
      else {
        var n = e.tag;
        if (n === 13) {
          if (((t = d(e)), t !== null)) return t;
          t = null;
        } else if (n === 31) {
          if (((t = m(e)), t !== null)) return t;
          t = null;
        } else if (n === 3) {
          if (e.stateNode.current.memoizedState.isDehydrated)
            return e.tag === 3 ? e.stateNode.containerInfo : null;
          t = null;
        } else e !== t && (t = null);
      }
    }
    return ((Hs = t), null);
  }
  function np(t) {
    switch (t) {
      case "beforetoggle":
      case "cancel":
      case "click":
      case "close":
      case "contextmenu":
      case "copy":
      case "cut":
      case "auxclick":
      case "dblclick":
      case "dragend":
      case "dragstart":
      case "drop":
      case "focusin":
      case "focusout":
      case "input":
      case "invalid":
      case "keydown":
      case "keypress":
      case "keyup":
      case "mousedown":
      case "mouseup":
      case "paste":
      case "pause":
      case "play":
      case "pointercancel":
      case "pointerdown":
      case "pointerup":
      case "ratechange":
      case "reset":
      case "resize":
      case "seeked":
      case "submit":
      case "toggle":
      case "touchcancel":
      case "touchend":
      case "touchstart":
      case "volumechange":
      case "change":
      case "selectionchange":
      case "textInput":
      case "compositionstart":
      case "compositionend":
      case "compositionupdate":
      case "beforeblur":
      case "afterblur":
      case "beforeinput":
      case "blur":
      case "fullscreenchange":
      case "focus":
      case "hashchange":
      case "popstate":
      case "select":
      case "selectstart":
        return 2;
      case "drag":
      case "dragenter":
      case "dragexit":
      case "dragleave":
      case "dragover":
      case "mousemove":
      case "mouseout":
      case "mouseover":
      case "pointermove":
      case "pointerout":
      case "pointerover":
      case "scroll":
      case "touchmove":
      case "wheel":
      case "mouseenter":
      case "mouseleave":
      case "pointerenter":
      case "pointerleave":
        return 8;
      case "message":
        switch (_y()) {
          case cf:
            return 2;
          case ff:
            return 8;
          case Nl:
          case By:
            return 32;
          case df:
            return 268435456;
          default:
            return 32;
        }
      default:
        return 32;
    }
  }
  var Or = !1,
    Yn = null,
    Gn = null,
    Xn = null,
    ol = new Map(),
    rl = new Map(),
    kn = [],
    P1 =
      "mousedown mouseup touchcancel touchend touchstart auxclick dblclick pointercancel pointerdown pointerup dragend dragstart drop compositionend compositionstart keydown keypress keyup input textInput copy cut paste click change contextmenu reset".split(
        " ",
      );
  function ip(t, e) {
    switch (t) {
      case "focusin":
      case "focusout":
        Yn = null;
        break;
      case "dragenter":
      case "dragleave":
        Gn = null;
        break;
      case "mouseover":
      case "mouseout":
        Xn = null;
        break;
      case "pointerover":
      case "pointerout":
        ol.delete(e.pointerId);
        break;
      case "gotpointercapture":
      case "lostpointercapture":
        rl.delete(e.pointerId);
    }
  }
  function cl(t, e, n, a, s, r) {
    return t === null || t.nativeEvent !== r
      ? ((t = {
          blockedOn: e,
          domEventName: n,
          eventSystemFlags: a,
          nativeEvent: r,
          targetContainers: [s],
        }),
        e !== null && ((e = Mi(e)), e !== null && tp(e)),
        t)
      : ((t.eventSystemFlags |= a),
        (e = t.targetContainers),
        s !== null && e.indexOf(s) === -1 && e.push(s),
        t);
  }
  function W1(t, e, n, a, s) {
    switch (e) {
      case "focusin":
        return ((Yn = cl(Yn, t, e, n, a, s)), !0);
      case "dragenter":
        return ((Gn = cl(Gn, t, e, n, a, s)), !0);
      case "mouseover":
        return ((Xn = cl(Xn, t, e, n, a, s)), !0);
      case "pointerover":
        var r = s.pointerId;
        return (ol.set(r, cl(ol.get(r) || null, t, e, n, a, s)), !0);
      case "gotpointercapture":
        return ((r = s.pointerId), rl.set(r, cl(rl.get(r) || null, t, e, n, a, s)), !0);
    }
    return !1;
  }
  function ap(t) {
    var e = Ei(t.target);
    if (e !== null) {
      var n = h(e);
      if (n !== null) {
        if (((e = n.tag), e === 13)) {
          if (((e = d(n)), e !== null)) {
            ((t.blockedOn = e),
              vf(t.priority, function () {
                ep(n);
              }));
            return;
          }
        } else if (e === 31) {
          if (((e = m(n)), e !== null)) {
            ((t.blockedOn = e),
              vf(t.priority, function () {
                ep(n);
              }));
            return;
          }
        } else if (e === 3 && n.stateNode.current.memoizedState.isDehydrated) {
          t.blockedOn = n.tag === 3 ? n.stateNode.containerInfo : null;
          return;
        }
      }
    }
    t.blockedOn = null;
  }
  function qs(t) {
    if (t.blockedOn !== null) return !1;
    for (var e = t.targetContainers; 0 < e.length; ) {
      var n = wr(t.nativeEvent);
      if (n === null) {
        n = t.nativeEvent;
        var a = new n.constructor(n.type, n);
        ((wu = a), n.target.dispatchEvent(a), (wu = null));
      } else return ((e = Mi(n)), e !== null && tp(e), (t.blockedOn = n), !1);
      e.shift();
    }
    return !0;
  }
  function lp(t, e, n) {
    qs(t) && n.delete(e);
  }
  function $1() {
    ((Or = !1),
      Yn !== null && qs(Yn) && (Yn = null),
      Gn !== null && qs(Gn) && (Gn = null),
      Xn !== null && qs(Xn) && (Xn = null),
      ol.forEach(lp),
      rl.forEach(lp));
  }
  function Ys(t, e) {
    t.blockedOn === e &&
      ((t.blockedOn = null),
      Or || ((Or = !0), i.unstable_scheduleCallback(i.unstable_NormalPriority, $1)));
  }
  var Gs = null;
  function sp(t) {
    Gs !== t &&
      ((Gs = t),
      i.unstable_scheduleCallback(i.unstable_NormalPriority, function () {
        Gs === t && (Gs = null);
        for (var e = 0; e < t.length; e += 3) {
          var n = t[e],
            a = t[e + 1],
            s = t[e + 2];
          if (typeof a != "function") {
            if (Rr(a || n) === null) continue;
            break;
          }
          var r = Mi(n);
          r !== null &&
            (t.splice(e, 3),
            (e -= 3),
            wo(r, { pending: !0, data: s, method: n.method, action: a }, a, s));
        }
      }));
  }
  function sa(t) {
    function e(T) {
      return Ys(T, t);
    }
    (Yn !== null && Ys(Yn, t),
      Gn !== null && Ys(Gn, t),
      Xn !== null && Ys(Xn, t),
      ol.forEach(e),
      rl.forEach(e));
    for (var n = 0; n < kn.length; n++) {
      var a = kn[n];
      a.blockedOn === t && (a.blockedOn = null);
    }
    for (; 0 < kn.length && ((n = kn[0]), n.blockedOn === null); )
      (ap(n), n.blockedOn === null && kn.shift());
    if (((n = (t.ownerDocument || t).$$reactFormReplay), n != null))
      for (a = 0; a < n.length; a += 3) {
        var s = n[a],
          r = n[a + 1],
          f = s[de] || null;
        if (typeof r == "function") f || sp(n);
        else if (f) {
          var y = null;
          if (r && r.hasAttribute("formAction")) {
            if (((s = r), (f = r[de] || null))) y = f.formAction;
            else if (Rr(s) !== null) continue;
          } else y = f.action;
          (typeof y == "function" ? (n[a + 1] = y) : (n.splice(a, 3), (a -= 3)), sp(n));
        }
      }
  }
  function up() {
    function t(r) {
      r.canIntercept &&
        r.info === "react-transition" &&
        r.intercept({
          handler: function () {
            return new Promise(function (f) {
              return (s = f);
            });
          },
          focusReset: "manual",
          scroll: "manual",
        });
    }
    function e() {
      (s !== null && (s(), (s = null)), a || setTimeout(n, 20));
    }
    function n() {
      if (!a && !navigation.transition) {
        var r = navigation.currentEntry;
        r &&
          r.url != null &&
          navigation.navigate(r.url, {
            state: r.getState(),
            info: "react-transition",
            history: "replace",
          });
      }
    }
    if (typeof navigation == "object") {
      var a = !1,
        s = null;
      return (
        navigation.addEventListener("navigate", t),
        navigation.addEventListener("navigatesuccess", e),
        navigation.addEventListener("navigateerror", e),
        setTimeout(n, 100),
        function () {
          ((a = !0),
            navigation.removeEventListener("navigate", t),
            navigation.removeEventListener("navigatesuccess", e),
            navigation.removeEventListener("navigateerror", e),
            s !== null && (s(), (s = null)));
        }
      );
    }
  }
  function Vr(t) {
    this._internalRoot = t;
  }
  ((Xs.prototype.render = Vr.prototype.render =
    function (t) {
      var e = this._internalRoot;
      if (e === null) throw Error(o(409));
      var n = e.current,
        a = Ne();
      $m(n, a, t, e, null, null);
    }),
    (Xs.prototype.unmount = Vr.prototype.unmount =
      function () {
        var t = this._internalRoot;
        if (t !== null) {
          this._internalRoot = null;
          var e = t.containerInfo;
          ($m(t.current, 2, null, t, null, null), As(), (e[Ai] = null));
        }
      }));
  function Xs(t) {
    this._internalRoot = t;
  }
  Xs.prototype.unstable_scheduleHydration = function (t) {
    if (t) {
      var e = yf();
      t = { blockedOn: null, target: t, priority: e };
      for (var n = 0; n < kn.length && e !== 0 && e < kn[n].priority; n++);
      (kn.splice(n, 0, t), n === 0 && ap(t));
    }
  };
  var op = l.version;
  if (op !== "19.2.4") throw Error(o(527, op, "19.2.4"));
  X.findDOMNode = function (t) {
    var e = t._reactInternals;
    if (e === void 0)
      throw typeof t.render == "function"
        ? Error(o(188))
        : ((t = Object.keys(t).join(",")), Error(o(268, t)));
    return ((t = p(e)), (t = t !== null ? v(t) : null), (t = t === null ? null : t.stateNode), t);
  };
  var I1 = {
    bundleType: 0,
    version: "19.2.4",
    rendererPackageName: "react-dom",
    currentDispatcherRef: R,
    reconcilerVersion: "19.2.4",
  };
  if (typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ < "u") {
    var ks = __REACT_DEVTOOLS_GLOBAL_HOOK__;
    if (!ks.isDisabled && ks.supportsFiber)
      try {
        ((xa = ks.inject(I1)), (be = ks));
      } catch {}
  }
  return (
    (dl.createRoot = function (t, e) {
      if (!c(t)) throw Error(o(299));
      var n = !1,
        a = "",
        s = ph,
        r = gh,
        f = yh;
      return (
        e != null &&
          (e.unstable_strictMode === !0 && (n = !0),
          e.identifierPrefix !== void 0 && (a = e.identifierPrefix),
          e.onUncaughtError !== void 0 && (s = e.onUncaughtError),
          e.onCaughtError !== void 0 && (r = e.onCaughtError),
          e.onRecoverableError !== void 0 && (f = e.onRecoverableError)),
        (e = Pm(t, 1, !1, null, null, n, a, null, s, r, f, up)),
        (t[Ai] = e.current),
        pr(t),
        new Vr(e)
      );
    }),
    (dl.hydrateRoot = function (t, e, n) {
      if (!c(t)) throw Error(o(299));
      var a = !1,
        s = "",
        r = ph,
        f = gh,
        y = yh,
        T = null;
      return (
        n != null &&
          (n.unstable_strictMode === !0 && (a = !0),
          n.identifierPrefix !== void 0 && (s = n.identifierPrefix),
          n.onUncaughtError !== void 0 && (r = n.onUncaughtError),
          n.onCaughtError !== void 0 && (f = n.onCaughtError),
          n.onRecoverableError !== void 0 && (y = n.onRecoverableError),
          n.formState !== void 0 && (T = n.formState)),
        (e = Pm(t, 1, !0, e, n ?? null, a, s, T, r, f, y, up)),
        (e.context = Wm(null)),
        (n = e.current),
        (a = Ne()),
        (a = Au(a)),
        (s = zn(a)),
        (s.callback = null),
        jn(n, s, a),
        (n = a),
        (e.current.lanes = n),
        Sa(e, n),
        $e(e),
        (t[Ai] = e.current),
        pr(t),
        new Xs(e)
      );
    }),
    (dl.version = "19.2.4"),
    dl
  );
}
var vp;
function fx() {
  if (vp) return Ur.exports;
  vp = 1;
  function i() {
    if (
      !(
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
        typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
      )
    )
      try {
        __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(i);
      } catch (l) {
        console.error(l);
      }
  }
  return (i(), (Ur.exports = cx()), Ur.exports);
}
var dx = fx();
const jc = k.createContext({});
function wc(i) {
  const l = k.useRef(null);
  return (l.current === null && (l.current = i()), l.current);
}
const hx = typeof window < "u",
  X0 = hx ? k.useLayoutEffect : k.useEffect,
  du = k.createContext(null);
function Rc(i, l) {
  i.indexOf(l) === -1 && i.push(l);
}
function au(i, l) {
  const u = i.indexOf(l);
  u > -1 && i.splice(u, 1);
}
const en = (i, l, u) => (u > l ? l : u < i ? i : u);
let Oc = () => {};
const Fn = {},
  k0 = (i) => /^-?(?:\d+(?:\.\d+)?|\.\d+)$/u.test(i);
function Q0(i) {
  return typeof i == "object" && i !== null;
}
const Z0 = (i) => /^0[^.\s]+$/u.test(i);
function K0(i) {
  let l;
  return () => (l === void 0 && (l = i()), l);
}
const Ge = (i) => i,
  mx = (i, l) => (u) => l(i(u)),
  Al = (...i) => i.reduce(mx),
  xl = (i, l, u) => {
    const o = l - i;
    return o === 0 ? 1 : (u - i) / o;
  };
class Vc {
  constructor() {
    this.subscriptions = [];
  }
  add(l) {
    return (Rc(this.subscriptions, l), () => au(this.subscriptions, l));
  }
  notify(l, u, o) {
    const c = this.subscriptions.length;
    if (c)
      if (c === 1) this.subscriptions[0](l, u, o);
      else
        for (let h = 0; h < c; h++) {
          const d = this.subscriptions[h];
          d && d(l, u, o);
        }
  }
  getSize() {
    return this.subscriptions.length;
  }
  clear() {
    this.subscriptions.length = 0;
  }
}
const je = (i) => i * 1e3,
  Ye = (i) => i / 1e3;
function J0(i, l) {
  return l ? i * (1e3 / l) : 0;
}
const F0 = (i, l, u) => (((1 - 3 * u + 3 * l) * i + (3 * u - 6 * l)) * i + 3 * l) * i,
  px = 1e-7,
  gx = 12;
function yx(i, l, u, o, c) {
  let h,
    d,
    m = 0;
  do ((d = l + (u - l) / 2), (h = F0(d, o, c) - i), h > 0 ? (u = d) : (l = d));
  while (Math.abs(h) > px && ++m < gx);
  return d;
}
function El(i, l, u, o) {
  if (i === l && u === o) return Ge;
  const c = (h) => yx(h, 0, 1, i, u);
  return (h) => (h === 0 || h === 1 ? h : F0(c(h), l, o));
}
const P0 = (i) => (l) => (l <= 0.5 ? i(2 * l) / 2 : (2 - i(2 * (1 - l))) / 2),
  W0 = (i) => (l) => 1 - i(1 - l),
  $0 = El(0.33, 1.53, 0.69, 0.99),
  _c = W0($0),
  I0 = P0(_c),
  tg = (i) => (i >= 1 ? 1 : (i *= 2) < 1 ? 0.5 * _c(i) : 0.5 * (2 - Math.pow(2, -10 * (i - 1)))),
  Bc = (i) => 1 - Math.sin(Math.acos(i)),
  eg = W0(Bc),
  ng = P0(Bc),
  vx = El(0.42, 0, 1, 1),
  xx = El(0, 0, 0.58, 1),
  ig = El(0.42, 0, 0.58, 1),
  bx = (i) => Array.isArray(i) && typeof i[0] != "number",
  ag = (i) => Array.isArray(i) && typeof i[0] == "number",
  Sx = {
    linear: Ge,
    easeIn: vx,
    easeInOut: ig,
    easeOut: xx,
    circIn: Bc,
    circInOut: ng,
    circOut: eg,
    backIn: _c,
    backInOut: I0,
    backOut: $0,
    anticipate: tg,
  },
  Tx = (i) => typeof i == "string",
  xp = (i) => {
    if (ag(i)) {
      Oc(i.length === 4);
      const [l, u, o, c] = i;
      return El(l, u, o, c);
    } else if (Tx(i)) return Sx[i];
    return i;
  },
  Qs = [
    "setup",
    "read",
    "resolveKeyframes",
    "preUpdate",
    "update",
    "preRender",
    "render",
    "postRender",
  ];
function Ax(i, l) {
  let u = new Set(),
    o = new Set(),
    c = !1,
    h = !1;
  const d = new WeakSet();
  let m = { delta: 0, timestamp: 0, isProcessing: !1 };
  function g(v) {
    (d.has(v) && (p.schedule(v), i()), v(m));
  }
  const p = {
    schedule: (v, x = !1, S = !1) => {
      const w = S && c ? u : o;
      return (x && d.add(v), w.add(v), v);
    },
    cancel: (v) => {
      (o.delete(v), d.delete(v));
    },
    process: (v) => {
      if (((m = v), c)) {
        h = !0;
        return;
      }
      c = !0;
      const x = u;
      ((u = o), (o = x), u.forEach(g), u.clear(), (c = !1), h && ((h = !1), p.process(v)));
    },
  };
  return p;
}
const Ex = 40;
function lg(i, l) {
  let u = !1,
    o = !0;
  const c = { delta: 0, timestamp: 0, isProcessing: !1 },
    h = () => (u = !0),
    d = Qs.reduce((q, G) => ((q[G] = Ax(h)), q), {}),
    {
      setup: m,
      read: g,
      resolveKeyframes: p,
      preUpdate: v,
      update: x,
      preRender: S,
      render: N,
      postRender: w,
    } = d,
    U = () => {
      const q = Fn.useManualTiming,
        G = q ? c.timestamp : performance.now();
      ((u = !1),
        q || (c.delta = o ? 1e3 / 60 : Math.max(Math.min(G - c.timestamp, Ex), 1)),
        (c.timestamp = G),
        (c.isProcessing = !0),
        m.process(c),
        g.process(c),
        p.process(c),
        v.process(c),
        x.process(c),
        S.process(c),
        N.process(c),
        w.process(c),
        (c.isProcessing = !1),
        u && l && ((o = !1), i(U)));
    },
    H = () => {
      ((u = !0), (o = !0), c.isProcessing || i(U));
    };
  return {
    schedule: Qs.reduce((q, G) => {
      const F = d[G];
      return ((q[G] = (lt, I = !1, $ = !1) => (u || H(), F.schedule(lt, I, $))), q);
    }, {}),
    cancel: (q) => {
      for (let G = 0; G < Qs.length; G++) d[Qs[G]].cancel(q);
    },
    state: c,
    steps: d,
  };
}
const {
  schedule: zt,
  cancel: Pn,
  state: le,
  steps: Yr,
} = lg(typeof requestAnimationFrame < "u" ? requestAnimationFrame : Ge, !0);
let Ps;
function Mx() {
  Ps = void 0;
}
const ce = {
    now: () => (
      Ps === void 0 &&
        ce.set(le.isProcessing || Fn.useManualTiming ? le.timestamp : performance.now()),
      Ps
    ),
    set: (i) => {
      ((Ps = i), queueMicrotask(Mx));
    },
  },
  sg = (i) => (l) => typeof l == "string" && l.startsWith(i),
  ug = sg("--"),
  Dx = sg("var(--"),
  Uc = (i) => (Dx(i) ? Cx.test(i.split("/*")[0].trim()) : !1),
  Cx = /var\(--(?:[\w-]+\s*|[\w-]+\s*,(?:\s*[^)(\s]|\s*\((?:[^)(]|\([^)(]*\))*\))+\s*)\)$/iu;
function bp(i) {
  return typeof i != "string" ? !1 : i.split("/*")[0].includes("var(--");
}
const pa = { test: (i) => typeof i == "number", parse: parseFloat, transform: (i) => i },
  bl = { ...pa, transform: (i) => en(0, 1, i) },
  Zs = { ...pa, default: 1 },
  pl = (i) => Math.round(i * 1e5) / 1e5,
  Lc = /-?(?:\d+(?:\.\d+)?|\.\d+)/gu;
function Nx(i) {
  return i == null;
}
const zx =
    /^(?:#[\da-f]{3,8}|(?:rgb|hsl)a?\((?:-?[\d.]+%?[,\s]+){2}-?[\d.]+%?\s*(?:[,/]\s*)?(?:\b\d+(?:\.\d+)?|\.\d+)?%?\))$/iu,
  Hc = (i, l) => (u) =>
    !!(
      (typeof u == "string" && zx.test(u) && u.startsWith(i)) ||
      (l && !Nx(u) && Object.prototype.hasOwnProperty.call(u, l))
    ),
  og = (i, l, u) => (o) => {
    if (typeof o != "string") return o;
    const [c, h, d, m] = o.match(Lc);
    return {
      [i]: parseFloat(c),
      [l]: parseFloat(h),
      [u]: parseFloat(d),
      alpha: m !== void 0 ? parseFloat(m) : 1,
    };
  },
  jx = (i) => en(0, 255, i),
  Gr = { ...pa, transform: (i) => Math.round(jx(i)) },
  xi = {
    test: Hc("rgb", "red"),
    parse: og("red", "green", "blue"),
    transform: ({ red: i, green: l, blue: u, alpha: o = 1 }) =>
      "rgba(" +
      Gr.transform(i) +
      ", " +
      Gr.transform(l) +
      ", " +
      Gr.transform(u) +
      ", " +
      pl(bl.transform(o)) +
      ")",
  };
function wx(i) {
  let l = "",
    u = "",
    o = "",
    c = "";
  return (
    i.length > 5
      ? ((l = i.substring(1, 3)),
        (u = i.substring(3, 5)),
        (o = i.substring(5, 7)),
        (c = i.substring(7, 9)))
      : ((l = i.substring(1, 2)),
        (u = i.substring(2, 3)),
        (o = i.substring(3, 4)),
        (c = i.substring(4, 5)),
        (l += l),
        (u += u),
        (o += o),
        (c += c)),
    {
      red: parseInt(l, 16),
      green: parseInt(u, 16),
      blue: parseInt(o, 16),
      alpha: c ? parseInt(c, 16) / 255 : 1,
    }
  );
}
const lc = { test: Hc("#"), parse: wx, transform: xi.transform },
  Ml = (i) => ({
    test: (l) => typeof l == "string" && l.endsWith(i) && l.split(" ").length === 1,
    parse: parseFloat,
    transform: (l) => `${l}${i}`,
  }),
  Zn = Ml("deg"),
  tn = Ml("%"),
  J = Ml("px"),
  Rx = Ml("vh"),
  Ox = Ml("vw"),
  Sp = { ...tn, parse: (i) => tn.parse(i) / 100, transform: (i) => tn.transform(i * 100) },
  oa = {
    test: Hc("hsl", "hue"),
    parse: og("hue", "saturation", "lightness"),
    transform: ({ hue: i, saturation: l, lightness: u, alpha: o = 1 }) =>
      "hsla(" +
      Math.round(i) +
      ", " +
      tn.transform(pl(l)) +
      ", " +
      tn.transform(pl(u)) +
      ", " +
      pl(bl.transform(o)) +
      ")",
  },
  Kt = {
    test: (i) => xi.test(i) || lc.test(i) || oa.test(i),
    parse: (i) => (xi.test(i) ? xi.parse(i) : oa.test(i) ? oa.parse(i) : lc.parse(i)),
    transform: (i) =>
      typeof i == "string" ? i : i.hasOwnProperty("red") ? xi.transform(i) : oa.transform(i),
    getAnimatableNone: (i) => {
      const l = Kt.parse(i);
      return ((l.alpha = 0), Kt.transform(l));
    },
  },
  Vx =
    /(?:#[\da-f]{3,8}|(?:rgb|hsl)a?\((?:-?[\d.]+%?[,\s]+){2}-?[\d.]+%?\s*(?:[,/]\s*)?(?:\b\d+(?:\.\d+)?|\.\d+)?%?\))/giu;
function _x(i) {
  var l, u;
  return (
    isNaN(i) &&
    typeof i == "string" &&
    (((l = i.match(Lc)) == null ? void 0 : l.length) || 0) +
      (((u = i.match(Vx)) == null ? void 0 : u.length) || 0) >
      0
  );
}
const rg = "number",
  cg = "color",
  Bx = "var",
  Ux = "var(",
  Tp = "${}",
  Lx =
    /var\s*\(\s*--(?:[\w-]+\s*|[\w-]+\s*,(?:\s*[^)(\s]|\s*\((?:[^)(]|\([^)(]*\))*\))+\s*)\)|#[\da-f]{3,8}|(?:rgb|hsl)a?\((?:-?[\d.]+%?[,\s]+){2}-?[\d.]+%?\s*(?:[,/]\s*)?(?:\b\d+(?:\.\d+)?|\.\d+)?%?\)|-?(?:\d+(?:\.\d+)?|\.\d+)/giu;
function ha(i) {
  const l = i.toString(),
    u = [],
    o = { color: [], number: [], var: [] },
    c = [];
  let h = 0;
  const m = l
    .replace(
      Lx,
      (g) => (
        Kt.test(g)
          ? (o.color.push(h), c.push(cg), u.push(Kt.parse(g)))
          : g.startsWith(Ux)
            ? (o.var.push(h), c.push(Bx), u.push(g))
            : (o.number.push(h), c.push(rg), u.push(parseFloat(g))),
        ++h,
        Tp
      ),
    )
    .split(Tp);
  return { values: u, split: m, indexes: o, types: c };
}
function Hx(i) {
  return ha(i).values;
}
function fg({ split: i, types: l }) {
  const u = i.length;
  return (o) => {
    let c = "";
    for (let h = 0; h < u; h++)
      if (((c += i[h]), o[h] !== void 0)) {
        const d = l[h];
        d === rg ? (c += pl(o[h])) : d === cg ? (c += Kt.transform(o[h])) : (c += o[h]);
      }
    return c;
  };
}
function qx(i) {
  return fg(ha(i));
}
const Yx = (i) => (typeof i == "number" ? 0 : Kt.test(i) ? Kt.getAnimatableNone(i) : i),
  Gx = (i, l) => (typeof i == "number" ? (l != null && l.trim().endsWith("/") ? i : 0) : Yx(i));
function Xx(i) {
  const l = ha(i);
  return fg(l)(l.values.map((o, c) => Gx(o, l.split[c])));
}
const Je = { test: _x, parse: Hx, createTransformer: qx, getAnimatableNone: Xx };
function Xr(i, l, u) {
  return (
    u < 0 && (u += 1),
    u > 1 && (u -= 1),
    u < 1 / 6 ? i + (l - i) * 6 * u : u < 1 / 2 ? l : u < 2 / 3 ? i + (l - i) * (2 / 3 - u) * 6 : i
  );
}
function kx({ hue: i, saturation: l, lightness: u, alpha: o }) {
  ((i /= 360), (l /= 100), (u /= 100));
  let c = 0,
    h = 0,
    d = 0;
  if (!l) c = h = d = u;
  else {
    const m = u < 0.5 ? u * (1 + l) : u + l - u * l,
      g = 2 * u - m;
    ((c = Xr(g, m, i + 1 / 3)), (h = Xr(g, m, i)), (d = Xr(g, m, i - 1 / 3)));
  }
  return {
    red: Math.round(c * 255),
    green: Math.round(h * 255),
    blue: Math.round(d * 255),
    alpha: o,
  };
}
function lu(i, l) {
  return (u) => (u > 0 ? l : i);
}
const Ot = (i, l, u) => i + (l - i) * u,
  kr = (i, l, u) => {
    const o = i * i,
      c = u * (l * l - o) + o;
    return c < 0 ? 0 : Math.sqrt(c);
  },
  Qx = [lc, xi, oa],
  Zx = (i) => Qx.find((l) => l.test(i));
function Ap(i) {
  const l = Zx(i);
  if (!l) return !1;
  let u = l.parse(i);
  return (l === oa && (u = kx(u)), u);
}
const Ep = (i, l) => {
    const u = Ap(i),
      o = Ap(l);
    if (!u || !o) return lu(i, l);
    const c = { ...u };
    return (h) => (
      (c.red = kr(u.red, o.red, h)),
      (c.green = kr(u.green, o.green, h)),
      (c.blue = kr(u.blue, o.blue, h)),
      (c.alpha = Ot(u.alpha, o.alpha, h)),
      xi.transform(c)
    );
  },
  sc = new Set(["none", "hidden"]);
function Kx(i, l) {
  return sc.has(i) ? (u) => (u <= 0 ? i : l) : (u) => (u >= 1 ? l : i);
}
function Jx(i, l) {
  return (u) => Ot(i, l, u);
}
function qc(i) {
  return typeof i == "number"
    ? Jx
    : typeof i == "string"
      ? Uc(i)
        ? lu
        : Kt.test(i)
          ? Ep
          : Wx
      : Array.isArray(i)
        ? dg
        : typeof i == "object"
          ? Kt.test(i)
            ? Ep
            : Fx
          : lu;
}
function dg(i, l) {
  const u = [...i],
    o = u.length,
    c = i.map((h, d) => qc(h)(h, l[d]));
  return (h) => {
    for (let d = 0; d < o; d++) u[d] = c[d](h);
    return u;
  };
}
function Fx(i, l) {
  const u = { ...i, ...l },
    o = {};
  for (const c in u) i[c] !== void 0 && l[c] !== void 0 && (o[c] = qc(i[c])(i[c], l[c]));
  return (c) => {
    for (const h in o) u[h] = o[h](c);
    return u;
  };
}
function Px(i, l) {
  const u = [],
    o = { color: 0, var: 0, number: 0 };
  for (let c = 0; c < l.values.length; c++) {
    const h = l.types[c],
      d = i.indexes[h][o[h]],
      m = i.values[d] ?? 0;
    ((u[c] = m), o[h]++);
  }
  return u;
}
const Wx = (i, l) => {
  const u = Je.createTransformer(l),
    o = ha(i),
    c = ha(l);
  return o.indexes.var.length === c.indexes.var.length &&
    o.indexes.color.length === c.indexes.color.length &&
    o.indexes.number.length >= c.indexes.number.length
    ? (sc.has(i) && !c.values.length) || (sc.has(l) && !o.values.length)
      ? Kx(i, l)
      : Al(dg(Px(o, c), c.values), u)
    : lu(i, l);
};
function hg(i, l, u) {
  return typeof i == "number" && typeof l == "number" && typeof u == "number"
    ? Ot(i, l, u)
    : qc(i)(i, l);
}
const $x = (i) => {
    const l = ({ timestamp: u }) => i(u);
    return {
      start: (u = !0) => zt.update(l, u),
      stop: () => Pn(l),
      now: () => (le.isProcessing ? le.timestamp : ce.now()),
    };
  },
  mg = (i, l, u = 10) => {
    let o = "";
    const c = Math.max(Math.round(l / u), 2);
    for (let h = 0; h < c; h++) o += Math.round(i(h / (c - 1)) * 1e4) / 1e4 + ", ";
    return `linear(${o.substring(0, o.length - 2)})`;
  },
  su = 2e4;
function Yc(i) {
  let l = 0;
  const u = 50;
  let o = i.next(l);
  for (; !o.done && l < su; ) ((l += u), (o = i.next(l)));
  return l >= su ? 1 / 0 : l;
}
function Ix(i, l = 100, u) {
  const o = u({ ...i, keyframes: [0, l] }),
    c = Math.min(Yc(o), su);
  return { type: "keyframes", ease: (h) => o.next(c * h).value / l, duration: Ye(c) };
}
const Ut = {
  stiffness: 100,
  damping: 10,
  mass: 1,
  velocity: 0,
  duration: 800,
  bounce: 0.3,
  visualDuration: 0.3,
  restSpeed: { granular: 0.01, default: 2 },
  restDelta: { granular: 0.005, default: 0.5 },
  minDuration: 0.01,
  maxDuration: 10,
  minDamping: 0.05,
  maxDamping: 1,
};
function uc(i, l) {
  return i * Math.sqrt(1 - l * l);
}
const tb = 12;
function eb(i, l, u) {
  let o = u;
  for (let c = 1; c < tb; c++) o = o - i(o) / l(o);
  return o;
}
const Qr = 0.001;
function nb({
  duration: i = Ut.duration,
  bounce: l = Ut.bounce,
  velocity: u = Ut.velocity,
  mass: o = Ut.mass,
}) {
  let c,
    h,
    d = 1 - l;
  ((d = en(Ut.minDamping, Ut.maxDamping, d)),
    (i = en(Ut.minDuration, Ut.maxDuration, Ye(i))),
    d < 1
      ? ((c = (p) => {
          const v = p * d,
            x = v * i,
            S = v - u,
            N = uc(p, d),
            w = Math.exp(-x);
          return Qr - (S / N) * w;
        }),
        (h = (p) => {
          const x = p * d * i,
            S = x * u + u,
            N = Math.pow(d, 2) * Math.pow(p, 2) * i,
            w = Math.exp(-x),
            U = uc(Math.pow(p, 2), d);
          return ((-c(p) + Qr > 0 ? -1 : 1) * ((S - N) * w)) / U;
        }))
      : ((c = (p) => {
          const v = Math.exp(-p * i),
            x = (p - u) * i + 1;
          return -Qr + v * x;
        }),
        (h = (p) => {
          const v = Math.exp(-p * i),
            x = (u - p) * (i * i);
          return v * x;
        })));
  const m = 5 / i,
    g = eb(c, h, m);
  if (((i = je(i)), isNaN(g))) return { stiffness: Ut.stiffness, damping: Ut.damping, duration: i };
  {
    const p = Math.pow(g, 2) * o;
    return { stiffness: p, damping: d * 2 * Math.sqrt(o * p), duration: i };
  }
}
const ib = ["duration", "bounce"],
  ab = ["stiffness", "damping", "mass"];
function Mp(i, l) {
  return l.some((u) => i[u] !== void 0);
}
function lb(i) {
  let l = {
    velocity: Ut.velocity,
    stiffness: Ut.stiffness,
    damping: Ut.damping,
    mass: Ut.mass,
    isResolvedFromDuration: !1,
    ...i,
  };
  if (!Mp(i, ab) && Mp(i, ib))
    if (((l.velocity = 0), i.visualDuration)) {
      const u = i.visualDuration,
        o = (2 * Math.PI) / (u * 1.2),
        c = o * o,
        h = 2 * en(0.05, 1, 1 - (i.bounce || 0)) * Math.sqrt(c);
      l = { ...l, mass: Ut.mass, stiffness: c, damping: h };
    } else {
      const u = nb({ ...i, velocity: 0 });
      ((l = { ...l, ...u, mass: Ut.mass }), (l.isResolvedFromDuration = !0));
    }
  return l;
}
function uu(i = Ut.visualDuration, l = Ut.bounce) {
  const u = typeof i != "object" ? { visualDuration: i, keyframes: [0, 1], bounce: l } : i;
  let { restSpeed: o, restDelta: c } = u;
  const h = u.keyframes[0],
    d = u.keyframes[u.keyframes.length - 1],
    m = { done: !1, value: h },
    {
      stiffness: g,
      damping: p,
      mass: v,
      duration: x,
      velocity: S,
      isResolvedFromDuration: N,
    } = lb({ ...u, velocity: -Ye(u.velocity || 0) }),
    w = S || 0,
    U = p / (2 * Math.sqrt(g * v)),
    H = d - h,
    L = Ye(Math.sqrt(g / v)),
    Y = Math.abs(H) < 5;
  (o || (o = Y ? Ut.restSpeed.granular : Ut.restSpeed.default),
    c || (c = Y ? Ut.restDelta.granular : Ut.restDelta.default));
  let q, G, F, lt, I, $;
  if (U < 1)
    ((F = uc(L, U)),
      (lt = (w + U * L * H) / F),
      (q = (tt) => {
        const pt = Math.exp(-U * L * tt);
        return d - pt * (lt * Math.sin(F * tt) + H * Math.cos(F * tt));
      }),
      (I = U * L * lt + H * F),
      ($ = U * L * H - lt * F),
      (G = (tt) => Math.exp(-U * L * tt) * (I * Math.sin(F * tt) + $ * Math.cos(F * tt))));
  else if (U === 1) {
    q = (pt) => d - Math.exp(-L * pt) * (H + (w + L * H) * pt);
    const tt = w + L * H;
    G = (pt) => Math.exp(-L * pt) * (L * tt * pt - w);
  } else {
    const tt = L * Math.sqrt(U * U - 1);
    q = (Gt) => {
      const Lt = Math.exp(-U * L * Gt),
        R = Math.min(tt * Gt, 300);
      return d - (Lt * ((w + U * L * H) * Math.sinh(R) + tt * H * Math.cosh(R))) / tt;
    };
    const pt = (w + U * L * H) / tt,
      vt = U * L * pt - H * tt,
      $t = U * L * H - pt * tt;
    G = (Gt) => {
      const Lt = Math.exp(-U * L * Gt),
        R = Math.min(tt * Gt, 300);
      return Lt * (vt * Math.sinh(R) + $t * Math.cosh(R));
    };
  }
  const it = {
    calculatedDuration: (N && x) || null,
    velocity: (tt) => je(G(tt)),
    next: (tt) => {
      if (!N && U < 1) {
        const vt = Math.exp(-U * L * tt),
          $t = Math.sin(F * tt),
          Gt = Math.cos(F * tt),
          Lt = d - vt * (lt * $t + H * Gt),
          R = je(vt * (I * $t + $ * Gt));
        return (
          (m.done = Math.abs(R) <= o && Math.abs(d - Lt) <= c),
          (m.value = m.done ? d : Lt),
          m
        );
      }
      const pt = q(tt);
      if (N) m.done = tt >= x;
      else {
        const vt = je(G(tt));
        m.done = Math.abs(vt) <= o && Math.abs(d - pt) <= c;
      }
      return ((m.value = m.done ? d : pt), m);
    },
    toString: () => {
      const tt = Math.min(Yc(it), su),
        pt = mg((vt) => it.next(tt * vt).value, tt, 30);
      return tt + "ms " + pt;
    },
    toTransition: () => {},
  };
  return it;
}
uu.applyToOptions = (i) => {
  const l = Ix(i, 100, uu);
  return ((i.ease = l.ease), (i.duration = je(l.duration)), (i.type = "keyframes"), i);
};
const sb = 5;
function pg(i, l, u) {
  const o = Math.max(l - sb, 0);
  return J0(u - i(o), l - o);
}
function oc({
  keyframes: i,
  velocity: l = 0,
  power: u = 0.8,
  timeConstant: o = 325,
  bounceDamping: c = 10,
  bounceStiffness: h = 500,
  modifyTarget: d,
  min: m,
  max: g,
  restDelta: p = 0.5,
  restSpeed: v,
}) {
  const x = i[0],
    S = { done: !1, value: x },
    N = ($) => (m !== void 0 && $ < m) || (g !== void 0 && $ > g),
    w = ($) => (m === void 0 ? g : g === void 0 || Math.abs(m - $) < Math.abs(g - $) ? m : g);
  let U = u * l;
  const H = x + U,
    L = d === void 0 ? H : d(H);
  L !== H && (U = L - x);
  const Y = ($) => -U * Math.exp(-$ / o),
    q = ($) => L + Y($),
    G = ($) => {
      const it = Y($),
        tt = q($);
      ((S.done = Math.abs(it) <= p), (S.value = S.done ? L : tt));
    };
  let F, lt;
  const I = ($) => {
    N(S.value) &&
      ((F = $),
      (lt = uu({
        keyframes: [S.value, w(S.value)],
        velocity: pg(q, $, S.value),
        damping: c,
        stiffness: h,
        restDelta: p,
        restSpeed: v,
      })));
  };
  return (
    I(0),
    {
      calculatedDuration: null,
      next: ($) => {
        let it = !1;
        return (
          !lt && F === void 0 && ((it = !0), G($), I($)),
          F !== void 0 && $ >= F ? lt.next($ - F) : (!it && G($), S)
        );
      },
    }
  );
}
function ub(i, l, u) {
  const o = [],
    c = u || Fn.mix || hg,
    h = i.length - 1;
  for (let d = 0; d < h; d++) {
    let m = c(i[d], i[d + 1]);
    if (l) {
      const g = Array.isArray(l) ? l[d] || Ge : l;
      m = Al(g, m);
    }
    o.push(m);
  }
  return o;
}
function ob(i, l, { clamp: u = !0, ease: o, mixer: c } = {}) {
  const h = i.length;
  if ((Oc(h === l.length), h === 1)) return () => l[0];
  if (h === 2 && l[0] === l[1]) return () => l[1];
  const d = i[0] === i[1];
  i[0] > i[h - 1] && ((i = [...i].reverse()), (l = [...l].reverse()));
  const m = ub(l, o, c),
    g = m.length,
    p = (v) => {
      if (d && v < i[0]) return l[0];
      let x = 0;
      if (g > 1) for (; x < i.length - 2 && !(v < i[x + 1]); x++);
      const S = xl(i[x], i[x + 1], v);
      return m[x](S);
    };
  return u ? (v) => p(en(i[0], i[h - 1], v)) : p;
}
function rb(i, l) {
  const u = i[i.length - 1];
  for (let o = 1; o <= l; o++) {
    const c = xl(0, l, o);
    i.push(Ot(u, 1, c));
  }
}
function cb(i) {
  const l = [0];
  return (rb(l, i.length - 1), l);
}
function fb(i, l) {
  return i.map((u) => u * l);
}
function db(i, l) {
  return i.map(() => l || ig).splice(0, i.length - 1);
}
function gl({ duration: i = 300, keyframes: l, times: u, ease: o = "easeInOut" }) {
  const c = bx(o) ? o.map(xp) : xp(o),
    h = { done: !1, value: l[0] },
    d = fb(u && u.length === l.length ? u : cb(l), i),
    m = ob(d, l, { ease: Array.isArray(c) ? c : db(l, c) });
  return { calculatedDuration: i, next: (g) => ((h.value = m(g)), (h.done = g >= i), h) };
}
const hb = (i) => i !== null;
function Gc(i, { repeat: l, repeatType: u = "loop" }, o, c = 1) {
  const h = i.filter(hb),
    m = c < 0 || (l && u !== "loop" && l % 2 === 1) ? 0 : h.length - 1;
  return !m || o === void 0 ? h[m] : o;
}
const mb = { decay: oc, inertia: oc, tween: gl, keyframes: gl, spring: uu };
function gg(i) {
  typeof i.type == "string" && (i.type = mb[i.type]);
}
class Xc {
  constructor() {
    this.updateFinished();
  }
  get finished() {
    return this._finished;
  }
  updateFinished() {
    this._finished = new Promise((l) => {
      this.resolve = l;
    });
  }
  notifyFinished() {
    this.resolve();
  }
  then(l, u) {
    return this.finished.then(l, u);
  }
}
const pb = (i) => i / 100;
class kc extends Xc {
  constructor(l) {
    (super(),
      (this.state = "idle"),
      (this.startTime = null),
      (this.isStopped = !1),
      (this.currentTime = 0),
      (this.holdTime = null),
      (this.playbackSpeed = 1),
      (this.stop = () => {
        var o, c;
        const { motionValue: u } = this.options;
        (u && u.updatedAt !== ce.now() && this.tick(ce.now()),
          (this.isStopped = !0),
          this.state !== "idle" &&
            (this.teardown(), (c = (o = this.options).onStop) == null || c.call(o)));
      }),
      (this.options = l),
      this.initAnimation(),
      this.play(),
      l.autoplay === !1 && this.pause());
  }
  initAnimation() {
    const { options: l } = this;
    gg(l);
    const { type: u = gl, repeat: o = 0, repeatDelay: c = 0, repeatType: h, velocity: d = 0 } = l;
    let { keyframes: m } = l;
    const g = u || gl;
    g !== gl &&
      typeof m[0] != "number" &&
      ((this.mixKeyframes = Al(pb, hg(m[0], m[1]))), (m = [0, 100]));
    const p = g({ ...l, keyframes: m });
    (h === "mirror" &&
      (this.mirroredGenerator = g({ ...l, keyframes: [...m].reverse(), velocity: -d })),
      p.calculatedDuration === null && (p.calculatedDuration = Yc(p)));
    const { calculatedDuration: v } = p;
    ((this.calculatedDuration = v),
      (this.resolvedDuration = v + c),
      (this.totalDuration = this.resolvedDuration * (o + 1) - c),
      (this.generator = p));
  }
  updateTime(l) {
    const u = Math.round(l - this.startTime) * this.playbackSpeed;
    this.holdTime !== null ? (this.currentTime = this.holdTime) : (this.currentTime = u);
  }
  tick(l, u = !1) {
    const {
      generator: o,
      totalDuration: c,
      mixKeyframes: h,
      mirroredGenerator: d,
      resolvedDuration: m,
      calculatedDuration: g,
    } = this;
    if (this.startTime === null) return o.next(0);
    const {
      delay: p = 0,
      keyframes: v,
      repeat: x,
      repeatType: S,
      repeatDelay: N,
      type: w,
      onUpdate: U,
      finalKeyframe: H,
    } = this.options;
    (this.speed > 0
      ? (this.startTime = Math.min(this.startTime, l))
      : this.speed < 0 && (this.startTime = Math.min(l - c / this.speed, this.startTime)),
      u ? (this.currentTime = l) : this.updateTime(l));
    const L = this.currentTime - p * (this.playbackSpeed >= 0 ? 1 : -1),
      Y = this.playbackSpeed >= 0 ? L < 0 : L > c;
    ((this.currentTime = Math.max(L, 0)),
      this.state === "finished" && this.holdTime === null && (this.currentTime = c));
    let q = this.currentTime,
      G = o;
    if (x) {
      const $ = Math.min(this.currentTime, c) / m;
      let it = Math.floor($),
        tt = $ % 1;
      (!tt && $ >= 1 && (tt = 1),
        tt === 1 && it--,
        (it = Math.min(it, x + 1)),
        !!(it % 2) &&
          (S === "reverse" ? ((tt = 1 - tt), N && (tt -= N / m)) : S === "mirror" && (G = d)),
        (q = en(0, 1, tt) * m));
    }
    const F = Y ? { done: !1, value: v[0] } : G.next(q);
    h && !Y && (F.value = h(F.value));
    let { done: lt } = F;
    !Y &&
      g !== null &&
      (lt = this.playbackSpeed >= 0 ? this.currentTime >= c : this.currentTime <= 0);
    const I =
      this.holdTime === null && (this.state === "finished" || (this.state === "running" && lt));
    return (
      I && w !== oc && (F.value = Gc(v, this.options, H, this.speed)),
      U && U(F.value),
      I && this.finish(),
      F
    );
  }
  then(l, u) {
    return this.finished.then(l, u);
  }
  get duration() {
    return Ye(this.calculatedDuration);
  }
  get iterationDuration() {
    const { delay: l = 0 } = this.options || {};
    return this.duration + Ye(l);
  }
  get time() {
    return Ye(this.currentTime);
  }
  set time(l) {
    ((l = je(l)),
      (this.currentTime = l),
      this.startTime === null || this.holdTime !== null || this.playbackSpeed === 0
        ? (this.holdTime = l)
        : this.driver && (this.startTime = this.driver.now() - l / this.playbackSpeed),
      this.driver
        ? this.driver.start(!1)
        : ((this.startTime = 0), (this.state = "paused"), (this.holdTime = l), this.tick(l)));
  }
  getGeneratorVelocity() {
    const l = this.currentTime;
    if (l <= 0) return this.options.velocity || 0;
    if (this.generator.velocity) return this.generator.velocity(l);
    const u = this.generator.next(l).value;
    return pg((o) => this.generator.next(o).value, l, u);
  }
  get speed() {
    return this.playbackSpeed;
  }
  set speed(l) {
    const u = this.playbackSpeed !== l;
    (u && this.driver && this.updateTime(ce.now()),
      (this.playbackSpeed = l),
      u && this.driver && (this.time = Ye(this.currentTime)));
  }
  play() {
    var c, h;
    if (this.isStopped) return;
    const { driver: l = $x, startTime: u } = this.options;
    (this.driver || (this.driver = l((d) => this.tick(d))),
      (h = (c = this.options).onPlay) == null || h.call(c));
    const o = this.driver.now();
    (this.state === "finished"
      ? (this.updateFinished(), (this.startTime = o))
      : this.holdTime !== null
        ? (this.startTime = o - this.holdTime)
        : this.startTime || (this.startTime = u ?? o),
      this.state === "finished" && this.speed < 0 && (this.startTime += this.calculatedDuration),
      (this.holdTime = null),
      (this.state = "running"),
      this.driver.start());
  }
  pause() {
    ((this.state = "paused"), this.updateTime(ce.now()), (this.holdTime = this.currentTime));
  }
  complete() {
    (this.state !== "running" && this.play(), (this.state = "finished"), (this.holdTime = null));
  }
  finish() {
    var l, u;
    (this.notifyFinished(),
      this.teardown(),
      (this.state = "finished"),
      (u = (l = this.options).onComplete) == null || u.call(l));
  }
  cancel() {
    var l, u;
    ((this.holdTime = null),
      (this.startTime = 0),
      this.tick(0),
      this.teardown(),
      (u = (l = this.options).onCancel) == null || u.call(l));
  }
  teardown() {
    ((this.state = "idle"), this.stopDriver(), (this.startTime = this.holdTime = null));
  }
  stopDriver() {
    this.driver && (this.driver.stop(), (this.driver = void 0));
  }
  sample(l) {
    return ((this.startTime = 0), this.tick(l, !0));
  }
  attachTimeline(l) {
    var u;
    return (
      this.options.allowFlatten &&
        ((this.options.type = "keyframes"), (this.options.ease = "linear"), this.initAnimation()),
      (u = this.driver) == null || u.stop(),
      l.observe(this)
    );
  }
}
function gb(i) {
  for (let l = 1; l < i.length; l++) i[l] ?? (i[l] = i[l - 1]);
}
const bi = (i) => (i * 180) / Math.PI,
  rc = (i) => {
    const l = bi(Math.atan2(i[1], i[0]));
    return cc(l);
  },
  yb = {
    x: 4,
    y: 5,
    translateX: 4,
    translateY: 5,
    scaleX: 0,
    scaleY: 3,
    scale: (i) => (Math.abs(i[0]) + Math.abs(i[3])) / 2,
    rotate: rc,
    rotateZ: rc,
    skewX: (i) => bi(Math.atan(i[1])),
    skewY: (i) => bi(Math.atan(i[2])),
    skew: (i) => (Math.abs(i[1]) + Math.abs(i[2])) / 2,
  },
  cc = (i) => ((i = i % 360), i < 0 && (i += 360), i),
  Dp = rc,
  Cp = (i) => Math.sqrt(i[0] * i[0] + i[1] * i[1]),
  Np = (i) => Math.sqrt(i[4] * i[4] + i[5] * i[5]),
  vb = {
    x: 12,
    y: 13,
    z: 14,
    translateX: 12,
    translateY: 13,
    translateZ: 14,
    scaleX: Cp,
    scaleY: Np,
    scale: (i) => (Cp(i) + Np(i)) / 2,
    rotateX: (i) => cc(bi(Math.atan2(i[6], i[5]))),
    rotateY: (i) => cc(bi(Math.atan2(-i[2], i[0]))),
    rotateZ: Dp,
    rotate: Dp,
    skewX: (i) => bi(Math.atan(i[4])),
    skewY: (i) => bi(Math.atan(i[1])),
    skew: (i) => (Math.abs(i[1]) + Math.abs(i[4])) / 2,
  };
function fc(i) {
  return i.includes("scale") ? 1 : 0;
}
function dc(i, l) {
  if (!i || i === "none") return fc(l);
  const u = i.match(/^matrix3d\(([-\d.e\s,]+)\)$/u);
  let o, c;
  if (u) ((o = vb), (c = u));
  else {
    const m = i.match(/^matrix\(([-\d.e\s,]+)\)$/u);
    ((o = yb), (c = m));
  }
  if (!c) return fc(l);
  const h = o[l],
    d = c[1].split(",").map(bb);
  return typeof h == "function" ? h(d) : d[h];
}
const xb = (i, l) => {
  const { transform: u = "none" } = getComputedStyle(i);
  return dc(u, l);
};
function bb(i) {
  return parseFloat(i.trim());
}
const ga = [
    "transformPerspective",
    "x",
    "y",
    "z",
    "translateX",
    "translateY",
    "translateZ",
    "scale",
    "scaleX",
    "scaleY",
    "rotate",
    "rotateX",
    "rotateY",
    "rotateZ",
    "skew",
    "skewX",
    "skewY",
  ],
  ya = new Set(ga),
  zp = (i) => i === pa || i === J,
  Sb = new Set(["x", "y", "z"]),
  Tb = ga.filter((i) => !Sb.has(i));
function Ab(i) {
  const l = [];
  return (
    Tb.forEach((u) => {
      const o = i.getValue(u);
      o !== void 0 && (l.push([u, o.get()]), o.set(u.startsWith("scale") ? 1 : 0));
    }),
    l
  );
}
const Jn = {
  width: ({ x: i }, { paddingLeft: l = "0", paddingRight: u = "0", boxSizing: o }) => {
    const c = i.max - i.min;
    return o === "border-box" ? c : c - parseFloat(l) - parseFloat(u);
  },
  height: ({ y: i }, { paddingTop: l = "0", paddingBottom: u = "0", boxSizing: o }) => {
    const c = i.max - i.min;
    return o === "border-box" ? c : c - parseFloat(l) - parseFloat(u);
  },
  top: (i, { top: l }) => parseFloat(l),
  left: (i, { left: l }) => parseFloat(l),
  bottom: ({ y: i }, { top: l }) => parseFloat(l) + (i.max - i.min),
  right: ({ x: i }, { left: l }) => parseFloat(l) + (i.max - i.min),
  x: (i, { transform: l }) => dc(l, "x"),
  y: (i, { transform: l }) => dc(l, "y"),
};
Jn.translateX = Jn.x;
Jn.translateY = Jn.y;
const Si = new Set();
let hc = !1,
  mc = !1,
  pc = !1;
function yg() {
  if (mc) {
    const i = Array.from(Si).filter((o) => o.needsMeasurement),
      l = new Set(i.map((o) => o.element)),
      u = new Map();
    (l.forEach((o) => {
      const c = Ab(o);
      c.length && (u.set(o, c), o.render());
    }),
      i.forEach((o) => o.measureInitialState()),
      l.forEach((o) => {
        o.render();
        const c = u.get(o);
        c &&
          c.forEach(([h, d]) => {
            var m;
            (m = o.getValue(h)) == null || m.set(d);
          });
      }),
      i.forEach((o) => o.measureEndState()),
      i.forEach((o) => {
        o.suspendedScrollY !== void 0 && window.scrollTo(0, o.suspendedScrollY);
      }));
  }
  ((mc = !1), (hc = !1), Si.forEach((i) => i.complete(pc)), Si.clear());
}
function vg() {
  Si.forEach((i) => {
    (i.readKeyframes(), i.needsMeasurement && (mc = !0));
  });
}
function Eb() {
  ((pc = !0), vg(), yg(), (pc = !1));
}
class Qc {
  constructor(l, u, o, c, h, d = !1) {
    ((this.state = "pending"),
      (this.isAsync = !1),
      (this.needsMeasurement = !1),
      (this.unresolvedKeyframes = [...l]),
      (this.onComplete = u),
      (this.name = o),
      (this.motionValue = c),
      (this.element = h),
      (this.isAsync = d));
  }
  scheduleResolve() {
    ((this.state = "scheduled"),
      this.isAsync
        ? (Si.add(this), hc || ((hc = !0), zt.read(vg), zt.resolveKeyframes(yg)))
        : (this.readKeyframes(), this.complete()));
  }
  readKeyframes() {
    const { unresolvedKeyframes: l, name: u, element: o, motionValue: c } = this;
    if (l[0] === null) {
      const h = c == null ? void 0 : c.get(),
        d = l[l.length - 1];
      if (h !== void 0) l[0] = h;
      else if (o && u) {
        const m = o.readValue(u, d);
        m != null && (l[0] = m);
      }
      (l[0] === void 0 && (l[0] = d), c && h === void 0 && c.set(l[0]));
    }
    gb(l);
  }
  setFinalKeyframe() {}
  measureInitialState() {}
  renderEndStyles() {}
  measureEndState() {}
  complete(l = !1) {
    ((this.state = "complete"),
      this.onComplete(this.unresolvedKeyframes, this.finalKeyframe, l),
      Si.delete(this));
  }
  cancel() {
    this.state === "scheduled" && (Si.delete(this), (this.state = "pending"));
  }
  resume() {
    this.state === "pending" && this.scheduleResolve();
  }
}
const Mb = (i) => i.startsWith("--");
function xg(i, l, u) {
  Mb(l) ? i.style.setProperty(l, u) : (i.style[l] = u);
}
const Db = {};
function bg(i, l) {
  const u = K0(i);
  return () => Db[l] ?? u();
}
const Cb = bg(() => window.ScrollTimeline !== void 0, "scrollTimeline"),
  Sg = bg(() => {
    try {
      document.createElement("div").animate({ opacity: 0 }, { easing: "linear(0, 1)" });
    } catch {
      return !1;
    }
    return !0;
  }, "linearEasing"),
  ml = ([i, l, u, o]) => `cubic-bezier(${i}, ${l}, ${u}, ${o})`,
  jp = {
    linear: "linear",
    ease: "ease",
    easeIn: "ease-in",
    easeOut: "ease-out",
    easeInOut: "ease-in-out",
    circIn: ml([0, 0.65, 0.55, 1]),
    circOut: ml([0.55, 0, 1, 0.45]),
    backIn: ml([0.31, 0.01, 0.66, -0.59]),
    backOut: ml([0.33, 1.53, 0.69, 0.99]),
  };
function Tg(i, l) {
  if (i)
    return typeof i == "function"
      ? Sg()
        ? mg(i, l)
        : "ease-out"
      : ag(i)
        ? ml(i)
        : Array.isArray(i)
          ? i.map((u) => Tg(u, l) || jp.easeOut)
          : jp[i];
}
function Nb(
  i,
  l,
  u,
  {
    delay: o = 0,
    duration: c = 300,
    repeat: h = 0,
    repeatType: d = "loop",
    ease: m = "easeOut",
    times: g,
  } = {},
  p = void 0,
) {
  const v = { [l]: u };
  g && (v.offset = g);
  const x = Tg(m, c);
  Array.isArray(x) && (v.easing = x);
  const S = {
    delay: o,
    duration: c,
    easing: Array.isArray(x) ? "linear" : x,
    fill: "both",
    iterations: h + 1,
    direction: d === "reverse" ? "alternate" : "normal",
  };
  return (p && (S.pseudoElement = p), i.animate(v, S));
}
function Ag(i) {
  return typeof i == "function" && "applyToOptions" in i;
}
function zb({ type: i, ...l }) {
  return Ag(i) && Sg()
    ? i.applyToOptions(l)
    : (l.duration ?? (l.duration = 300), l.ease ?? (l.ease = "easeOut"), l);
}
class Eg extends Xc {
  constructor(l) {
    if (
      (super(),
      (this.finishedTime = null),
      (this.isStopped = !1),
      (this.manualStartTime = null),
      !l)
    )
      return;
    const {
      element: u,
      name: o,
      keyframes: c,
      pseudoElement: h,
      allowFlatten: d = !1,
      finalKeyframe: m,
      onComplete: g,
    } = l;
    ((this.isPseudoElement = !!h),
      (this.allowFlatten = d),
      (this.options = l),
      Oc(typeof l.type != "string"));
    const p = zb(l);
    ((this.animation = Nb(u, o, c, p, h)),
      p.autoplay === !1 && this.animation.pause(),
      (this.animation.onfinish = () => {
        if (((this.finishedTime = this.time), !h)) {
          const v = Gc(c, this.options, m, this.speed);
          (this.updateMotionValue && this.updateMotionValue(v),
            xg(u, o, v),
            this.animation.cancel());
        }
        (g == null || g(), this.notifyFinished());
      }));
  }
  play() {
    this.isStopped ||
      ((this.manualStartTime = null),
      this.animation.play(),
      this.state === "finished" && this.updateFinished());
  }
  pause() {
    this.animation.pause();
  }
  complete() {
    var l, u;
    (u = (l = this.animation).finish) == null || u.call(l);
  }
  cancel() {
    try {
      this.animation.cancel();
    } catch {}
  }
  stop() {
    if (this.isStopped) return;
    this.isStopped = !0;
    const { state: l } = this;
    l === "idle" ||
      l === "finished" ||
      (this.updateMotionValue ? this.updateMotionValue() : this.commitStyles(),
      this.isPseudoElement || this.cancel());
  }
  commitStyles() {
    var u, o, c;
    const l = (u = this.options) == null ? void 0 : u.element;
    !this.isPseudoElement &&
      l != null &&
      l.isConnected &&
      ((c = (o = this.animation).commitStyles) == null || c.call(o));
  }
  get duration() {
    var u, o;
    const l =
      ((o = (u = this.animation.effect) == null ? void 0 : u.getComputedTiming) == null
        ? void 0
        : o.call(u).duration) || 0;
    return Ye(Number(l));
  }
  get iterationDuration() {
    const { delay: l = 0 } = this.options || {};
    return this.duration + Ye(l);
  }
  get time() {
    return Ye(Number(this.animation.currentTime) || 0);
  }
  set time(l) {
    const u = this.finishedTime !== null;
    ((this.manualStartTime = null),
      (this.finishedTime = null),
      (this.animation.currentTime = je(l)),
      u && this.animation.pause());
  }
  get speed() {
    return this.animation.playbackRate;
  }
  set speed(l) {
    (l < 0 && (this.finishedTime = null), (this.animation.playbackRate = l));
  }
  get state() {
    return this.finishedTime !== null ? "finished" : this.animation.playState;
  }
  get startTime() {
    return this.manualStartTime ?? Number(this.animation.startTime);
  }
  set startTime(l) {
    this.manualStartTime = this.animation.startTime = l;
  }
  attachTimeline({ timeline: l, rangeStart: u, rangeEnd: o, observe: c }) {
    var h;
    return (
      this.allowFlatten &&
        ((h = this.animation.effect) == null || h.updateTiming({ easing: "linear" })),
      (this.animation.onfinish = null),
      l && Cb()
        ? ((this.animation.timeline = l),
          u && (this.animation.rangeStart = u),
          o && (this.animation.rangeEnd = o),
          Ge)
        : c(this)
    );
  }
}
const Mg = { anticipate: tg, backInOut: I0, circInOut: ng };
function jb(i) {
  return i in Mg;
}
function wb(i) {
  typeof i.ease == "string" && jb(i.ease) && (i.ease = Mg[i.ease]);
}
const Zr = 10;
class Rb extends Eg {
  constructor(l) {
    (wb(l),
      gg(l),
      super(l),
      l.startTime !== void 0 && l.autoplay !== !1 && (this.startTime = l.startTime),
      (this.options = l));
  }
  updateMotionValue(l) {
    const { motionValue: u, onUpdate: o, onComplete: c, element: h, ...d } = this.options;
    if (!u) return;
    if (l !== void 0) {
      u.set(l);
      return;
    }
    const m = new kc({ ...d, autoplay: !1 }),
      g = Math.max(Zr, ce.now() - this.startTime),
      p = en(0, Zr, g - Zr),
      v = m.sample(g).value,
      { name: x } = this.options;
    (h && x && xg(h, x, v), u.setWithVelocity(m.sample(Math.max(0, g - p)).value, v, p), m.stop());
  }
}
const wp = (i, l) =>
  l === "zIndex"
    ? !1
    : !!(
        typeof i == "number" ||
        Array.isArray(i) ||
        (typeof i == "string" && (Je.test(i) || i === "0") && !i.startsWith("url("))
      );
function Ob(i) {
  const l = i[0];
  if (i.length === 1) return !0;
  for (let u = 0; u < i.length; u++) if (i[u] !== l) return !0;
}
function Vb(i, l, u, o) {
  const c = i[0];
  if (c === null) return !1;
  if (l === "display" || l === "visibility") return !0;
  const h = i[i.length - 1],
    d = wp(c, l),
    m = wp(h, l);
  return !d || !m ? !1 : Ob(i) || ((u === "spring" || Ag(u)) && o);
}
function gc(i) {
  ((i.duration = 0), (i.type = "keyframes"));
}
const _b = new Set(["opacity", "clipPath", "filter", "transform"]),
  Bb = K0(() => Object.hasOwnProperty.call(Element.prototype, "animate"));
function Ub(i) {
  var v;
  const { motionValue: l, name: u, repeatDelay: o, repeatType: c, damping: h, type: d } = i;
  if (!(((v = l == null ? void 0 : l.owner) == null ? void 0 : v.current) instanceof HTMLElement))
    return !1;
  const { onUpdate: g, transformTemplate: p } = l.owner.getProps();
  return (
    Bb() &&
    u &&
    _b.has(u) &&
    (u !== "transform" || !p) &&
    !g &&
    !o &&
    c !== "mirror" &&
    h !== 0 &&
    d !== "inertia"
  );
}
const Lb = 40;
class Hb extends Xc {
  constructor({
    autoplay: l = !0,
    delay: u = 0,
    type: o = "keyframes",
    repeat: c = 0,
    repeatDelay: h = 0,
    repeatType: d = "loop",
    keyframes: m,
    name: g,
    motionValue: p,
    element: v,
    ...x
  }) {
    var w;
    (super(),
      (this.stop = () => {
        var U, H;
        (this._animation &&
          (this._animation.stop(), (U = this.stopTimeline) == null || U.call(this)),
          (H = this.keyframeResolver) == null || H.cancel());
      }),
      (this.createdAt = ce.now()));
    const S = {
        autoplay: l,
        delay: u,
        type: o,
        repeat: c,
        repeatDelay: h,
        repeatType: d,
        name: g,
        motionValue: p,
        element: v,
        ...x,
      },
      N = (v == null ? void 0 : v.KeyframeResolver) || Qc;
    ((this.keyframeResolver = new N(
      m,
      (U, H, L) => this.onKeyframesResolved(U, H, S, !L),
      g,
      p,
      v,
    )),
      (w = this.keyframeResolver) == null || w.scheduleResolve());
  }
  onKeyframesResolved(l, u, o, c) {
    var L, Y;
    this.keyframeResolver = void 0;
    const { name: h, type: d, velocity: m, delay: g, isHandoff: p, onUpdate: v } = o;
    this.resolvedAt = ce.now();
    let x = !0;
    Vb(l, h, d, m) ||
      ((x = !1),
      (Fn.instantAnimations || !g) && (v == null || v(Gc(l, o, u))),
      (l[0] = l[l.length - 1]),
      gc(o),
      (o.repeat = 0));
    const N = {
        startTime: c
          ? this.resolvedAt
            ? this.resolvedAt - this.createdAt > Lb
              ? this.resolvedAt
              : this.createdAt
            : this.createdAt
          : void 0,
        finalKeyframe: u,
        ...o,
        keyframes: l,
      },
      w = x && !p && Ub(N),
      U = (Y = (L = N.motionValue) == null ? void 0 : L.owner) == null ? void 0 : Y.current,
      H = w ? new Rb({ ...N, element: U }) : new kc(N);
    (H.finished
      .then(() => {
        this.notifyFinished();
      })
      .catch(Ge),
      this.pendingTimeline &&
        ((this.stopTimeline = H.attachTimeline(this.pendingTimeline)),
        (this.pendingTimeline = void 0)),
      (this._animation = H));
  }
  get finished() {
    return this._animation ? this.animation.finished : this._finished;
  }
  then(l, u) {
    return this.finished.finally(l).then(() => {});
  }
  get animation() {
    var l;
    return (
      this._animation || ((l = this.keyframeResolver) == null || l.resume(), Eb()),
      this._animation
    );
  }
  get duration() {
    return this.animation.duration;
  }
  get iterationDuration() {
    return this.animation.iterationDuration;
  }
  get time() {
    return this.animation.time;
  }
  set time(l) {
    this.animation.time = l;
  }
  get speed() {
    return this.animation.speed;
  }
  get state() {
    return this.animation.state;
  }
  set speed(l) {
    this.animation.speed = l;
  }
  get startTime() {
    return this.animation.startTime;
  }
  attachTimeline(l) {
    return (
      this._animation
        ? (this.stopTimeline = this.animation.attachTimeline(l))
        : (this.pendingTimeline = l),
      () => this.stop()
    );
  }
  play() {
    this.animation.play();
  }
  pause() {
    this.animation.pause();
  }
  complete() {
    this.animation.complete();
  }
  cancel() {
    var l;
    (this._animation && this.animation.cancel(), (l = this.keyframeResolver) == null || l.cancel());
  }
}
function Dg(i, l, u, o = 0, c = 1) {
  const h = Array.from(i)
      .sort((p, v) => p.sortNodePosition(v))
      .indexOf(l),
    d = i.size,
    m = (d - 1) * o;
  return typeof u == "function" ? u(h, d) : c === 1 ? h * o : m - h * o;
}
const qb = /^var\(--(?:([\w-]+)|([\w-]+), ?([a-zA-Z\d ()%#.,-]+))\)/u;
function Yb(i) {
  const l = qb.exec(i);
  if (!l) return [,];
  const [, u, o, c] = l;
  return [`--${u ?? o}`, c];
}
function Cg(i, l, u = 1) {
  const [o, c] = Yb(i);
  if (!o) return;
  const h = window.getComputedStyle(l).getPropertyValue(o);
  if (h) {
    const d = h.trim();
    return k0(d) ? parseFloat(d) : d;
  }
  return Uc(c) ? Cg(c, l, u + 1) : c;
}
const Gb = { type: "spring", stiffness: 500, damping: 25, restSpeed: 10 },
  Xb = (i) => ({
    type: "spring",
    stiffness: 550,
    damping: i === 0 ? 2 * Math.sqrt(550) : 30,
    restSpeed: 10,
  }),
  kb = { type: "keyframes", duration: 0.8 },
  Qb = { type: "keyframes", ease: [0.25, 0.1, 0.35, 1], duration: 0.3 },
  Zb = (i, { keyframes: l }) =>
    l.length > 2 ? kb : ya.has(i) ? (i.startsWith("scale") ? Xb(l[1]) : Gb) : Qb,
  Kb = (i) => i !== null;
function Jb(i, { repeat: l, repeatType: u = "loop" }, o) {
  const c = i.filter(Kb),
    h = l && u !== "loop" && l % 2 === 1 ? 0 : c.length - 1;
  return c[h];
}
function Ng(i, l) {
  if (i != null && i.inherit && l) {
    const { inherit: u, ...o } = i;
    return { ...l, ...o };
  }
  return i;
}
function Zc(i, l) {
  const u = (i == null ? void 0 : i[l]) ?? (i == null ? void 0 : i.default) ?? i;
  return u !== i ? Ng(u, i) : u;
}
function Fb({
  when: i,
  delay: l,
  delayChildren: u,
  staggerChildren: o,
  staggerDirection: c,
  repeat: h,
  repeatType: d,
  repeatDelay: m,
  from: g,
  elapsed: p,
  ...v
}) {
  return !!Object.keys(v).length;
}
const Kc =
  (i, l, u, o = {}, c, h) =>
  (d) => {
    const m = Zc(o, i) || {},
      g = m.delay || o.delay || 0;
    let { elapsed: p = 0 } = o;
    p = p - je(g);
    const v = {
      keyframes: Array.isArray(u) ? u : [null, u],
      ease: "easeOut",
      velocity: l.getVelocity(),
      ...m,
      delay: -p,
      onUpdate: (S) => {
        (l.set(S), m.onUpdate && m.onUpdate(S));
      },
      onComplete: () => {
        (d(), m.onComplete && m.onComplete());
      },
      name: i,
      motionValue: l,
      element: h ? void 0 : c,
    };
    (Fb(m) || Object.assign(v, Zb(i, v)),
      v.duration && (v.duration = je(v.duration)),
      v.repeatDelay && (v.repeatDelay = je(v.repeatDelay)),
      v.from !== void 0 && (v.keyframes[0] = v.from));
    let x = !1;
    if (
      ((v.type === !1 || (v.duration === 0 && !v.repeatDelay)) &&
        (gc(v), v.delay === 0 && (x = !0)),
      (Fn.instantAnimations || Fn.skipAnimations || (c != null && c.shouldSkipAnimations)) &&
        ((x = !0), gc(v), (v.delay = 0)),
      (v.allowFlatten = !m.type && !m.ease),
      x && !h && l.get() !== void 0)
    ) {
      const S = Jb(v.keyframes, m);
      if (S !== void 0) {
        zt.update(() => {
          (v.onUpdate(S), v.onComplete());
        });
        return;
      }
    }
    return m.isSync ? new kc(v) : new Hb(v);
  };
function Rp(i) {
  const l = [{}, {}];
  return (
    i == null ||
      i.values.forEach((u, o) => {
        ((l[0][o] = u.get()), (l[1][o] = u.getVelocity()));
      }),
    l
  );
}
function Jc(i, l, u, o) {
  if (typeof l == "function") {
    const [c, h] = Rp(o);
    l = l(u !== void 0 ? u : i.custom, c, h);
  }
  if ((typeof l == "string" && (l = i.variants && i.variants[l]), typeof l == "function")) {
    const [c, h] = Rp(o);
    l = l(u !== void 0 ? u : i.custom, c, h);
  }
  return l;
}
function Ti(i, l, u) {
  const o = i.getProps();
  return Jc(o, l, u !== void 0 ? u : o.custom, i);
}
const zg = new Set(["width", "height", "top", "left", "right", "bottom", ...ga]),
  Op = 30,
  Pb = (i) => !isNaN(parseFloat(i));
class Wb {
  constructor(l, u = {}) {
    ((this.canTrackVelocity = null),
      (this.events = {}),
      (this.updateAndNotify = (o) => {
        var h;
        const c = ce.now();
        if (
          (this.updatedAt !== c && this.setPrevFrameValue(),
          (this.prev = this.current),
          this.setCurrent(o),
          this.current !== this.prev &&
            ((h = this.events.change) == null || h.notify(this.current), this.dependents))
        )
          for (const d of this.dependents) d.dirty();
      }),
      (this.hasAnimated = !1),
      this.setCurrent(l),
      (this.owner = u.owner));
  }
  setCurrent(l) {
    ((this.current = l),
      (this.updatedAt = ce.now()),
      this.canTrackVelocity === null && l !== void 0 && (this.canTrackVelocity = Pb(this.current)));
  }
  setPrevFrameValue(l = this.current) {
    ((this.prevFrameValue = l), (this.prevUpdatedAt = this.updatedAt));
  }
  onChange(l) {
    return this.on("change", l);
  }
  on(l, u) {
    this.events[l] || (this.events[l] = new Vc());
    const o = this.events[l].add(u);
    return l === "change"
      ? () => {
          (o(),
            zt.read(() => {
              this.events.change.getSize() || this.stop();
            }));
        }
      : o;
  }
  clearListeners() {
    for (const l in this.events) this.events[l].clear();
  }
  attach(l, u) {
    ((this.passiveEffect = l), (this.stopPassiveEffect = u));
  }
  set(l) {
    this.passiveEffect ? this.passiveEffect(l, this.updateAndNotify) : this.updateAndNotify(l);
  }
  setWithVelocity(l, u, o) {
    (this.set(u),
      (this.prev = void 0),
      (this.prevFrameValue = l),
      (this.prevUpdatedAt = this.updatedAt - o));
  }
  jump(l, u = !0) {
    (this.updateAndNotify(l),
      (this.prev = l),
      (this.prevUpdatedAt = this.prevFrameValue = void 0),
      u && this.stop(),
      this.stopPassiveEffect && this.stopPassiveEffect());
  }
  dirty() {
    var l;
    (l = this.events.change) == null || l.notify(this.current);
  }
  addDependent(l) {
    (this.dependents || (this.dependents = new Set()), this.dependents.add(l));
  }
  removeDependent(l) {
    this.dependents && this.dependents.delete(l);
  }
  get() {
    return this.current;
  }
  getPrevious() {
    return this.prev;
  }
  getVelocity() {
    const l = ce.now();
    if (!this.canTrackVelocity || this.prevFrameValue === void 0 || l - this.updatedAt > Op)
      return 0;
    const u = Math.min(this.updatedAt - this.prevUpdatedAt, Op);
    return J0(parseFloat(this.current) - parseFloat(this.prevFrameValue), u);
  }
  start(l) {
    return (
      this.stop(),
      new Promise((u) => {
        ((this.hasAnimated = !0),
          (this.animation = l(u)),
          this.events.animationStart && this.events.animationStart.notify());
      }).then(() => {
        (this.events.animationComplete && this.events.animationComplete.notify(),
          this.clearAnimation());
      })
    );
  }
  stop() {
    (this.animation &&
      (this.animation.stop(), this.events.animationCancel && this.events.animationCancel.notify()),
      this.clearAnimation());
  }
  isAnimating() {
    return !!this.animation;
  }
  clearAnimation() {
    delete this.animation;
  }
  destroy() {
    var l, u;
    ((l = this.dependents) == null || l.clear(),
      (u = this.events.destroy) == null || u.notify(),
      this.clearListeners(),
      this.stop(),
      this.stopPassiveEffect && this.stopPassiveEffect());
  }
}
function ma(i, l) {
  return new Wb(i, l);
}
const yc = (i) => Array.isArray(i);
function $b(i, l, u) {
  i.hasValue(l) ? i.getValue(l).set(u) : i.addValue(l, ma(u));
}
function Ib(i) {
  return yc(i) ? i[i.length - 1] || 0 : i;
}
function t2(i, l) {
  const u = Ti(i, l);
  let { transitionEnd: o = {}, transition: c = {}, ...h } = u || {};
  h = { ...h, ...o };
  for (const d in h) {
    const m = Ib(h[d]);
    $b(i, d, m);
  }
}
const se = (i) => !!(i && i.getVelocity);
function e2(i) {
  return !!(se(i) && i.add);
}
function vc(i, l) {
  const u = i.getValue("willChange");
  if (e2(u)) return u.add(l);
  if (!u && Fn.WillChange) {
    const o = new Fn.WillChange("auto");
    (i.addValue("willChange", o), o.add(l));
  }
}
function Fc(i) {
  return i.replace(/([A-Z])/g, (l) => `-${l.toLowerCase()}`);
}
const n2 = "framerAppearId",
  jg = "data-" + Fc(n2);
function wg(i) {
  return i.props[jg];
}
function i2({ protectedKeys: i, needsAnimating: l }, u) {
  const o = i.hasOwnProperty(u) && l[u] !== !0;
  return ((l[u] = !1), o);
}
function Rg(i, l, { delay: u = 0, transitionOverride: o, type: c } = {}) {
  let { transition: h, transitionEnd: d, ...m } = l;
  const g = i.getDefaultTransition();
  h = h ? Ng(h, g) : g;
  const p = h == null ? void 0 : h.reduceMotion;
  o && (h = o);
  const v = [],
    x = c && i.animationState && i.animationState.getState()[c];
  for (const S in m) {
    const N = i.getValue(S, i.latestValues[S] ?? null),
      w = m[S];
    if (w === void 0 || (x && i2(x, S))) continue;
    const U = { delay: u, ...Zc(h || {}, S) },
      H = N.get();
    if (H !== void 0 && !N.isAnimating && !Array.isArray(w) && w === H && !U.velocity) continue;
    let L = !1;
    if (window.MotionHandoffAnimation) {
      const G = wg(i);
      if (G) {
        const F = window.MotionHandoffAnimation(G, S, zt);
        F !== null && ((U.startTime = F), (L = !0));
      }
    }
    vc(i, S);
    const Y = p ?? i.shouldReduceMotion;
    N.start(Kc(S, N, w, Y && zg.has(S) ? { type: !1 } : U, i, L));
    const q = N.animation;
    q && v.push(q);
  }
  if (d) {
    const S = () =>
      zt.update(() => {
        d && t2(i, d);
      });
    v.length ? Promise.all(v).then(S) : S();
  }
  return v;
}
function xc(i, l, u = {}) {
  var g;
  const o = Ti(
    i,
    l,
    u.type === "exit" ? ((g = i.presenceContext) == null ? void 0 : g.custom) : void 0,
  );
  let { transition: c = i.getDefaultTransition() || {} } = o || {};
  u.transitionOverride && (c = u.transitionOverride);
  const h = o ? () => Promise.all(Rg(i, o, u)) : () => Promise.resolve(),
    d =
      i.variantChildren && i.variantChildren.size
        ? (p = 0) => {
            const { delayChildren: v = 0, staggerChildren: x, staggerDirection: S } = c;
            return a2(i, l, p, v, x, S, u);
          }
        : () => Promise.resolve(),
    { when: m } = c;
  if (m) {
    const [p, v] = m === "beforeChildren" ? [h, d] : [d, h];
    return p().then(() => v());
  } else return Promise.all([h(), d(u.delay)]);
}
function a2(i, l, u = 0, o = 0, c = 0, h = 1, d) {
  const m = [];
  for (const g of i.variantChildren)
    (g.notify("AnimationStart", l),
      m.push(
        xc(g, l, {
          ...d,
          delay: u + (typeof o == "function" ? 0 : o) + Dg(i.variantChildren, g, o, c, h),
        }).then(() => g.notify("AnimationComplete", l)),
      ));
  return Promise.all(m);
}
function l2(i, l, u = {}) {
  i.notify("AnimationStart", l);
  let o;
  if (Array.isArray(l)) {
    const c = l.map((h) => xc(i, h, u));
    o = Promise.all(c);
  } else if (typeof l == "string") o = xc(i, l, u);
  else {
    const c = typeof l == "function" ? Ti(i, l, u.custom) : l;
    o = Promise.all(Rg(i, c, u));
  }
  return o.then(() => {
    i.notify("AnimationComplete", l);
  });
}
const s2 = { test: (i) => i === "auto", parse: (i) => i },
  Og = (i) => (l) => l.test(i),
  Vg = [pa, J, tn, Zn, Ox, Rx, s2],
  Vp = (i) => Vg.find(Og(i));
function u2(i) {
  return typeof i == "number" ? i === 0 : i !== null ? i === "none" || i === "0" || Z0(i) : !0;
}
const o2 = new Set(["brightness", "contrast", "saturate", "opacity"]);
function r2(i) {
  const [l, u] = i.slice(0, -1).split("(");
  if (l === "drop-shadow") return i;
  const [o] = u.match(Lc) || [];
  if (!o) return i;
  const c = u.replace(o, "");
  let h = o2.has(l) ? 1 : 0;
  return (o !== u && (h *= 100), l + "(" + h + c + ")");
}
const c2 = /\b([a-z-]*)\(.*?\)/gu,
  bc = {
    ...Je,
    getAnimatableNone: (i) => {
      const l = i.match(c2);
      return l ? l.map(r2).join(" ") : i;
    },
  },
  Sc = {
    ...Je,
    getAnimatableNone: (i) => {
      const l = Je.parse(i);
      return Je.createTransformer(i)(
        l.map((o) => (typeof o == "number" ? 0 : typeof o == "object" ? { ...o, alpha: 1 } : o)),
      );
    },
  },
  _p = { ...pa, transform: Math.round },
  f2 = {
    rotate: Zn,
    rotateX: Zn,
    rotateY: Zn,
    rotateZ: Zn,
    scale: Zs,
    scaleX: Zs,
    scaleY: Zs,
    scaleZ: Zs,
    skew: Zn,
    skewX: Zn,
    skewY: Zn,
    distance: J,
    translateX: J,
    translateY: J,
    translateZ: J,
    x: J,
    y: J,
    z: J,
    perspective: J,
    transformPerspective: J,
    opacity: bl,
    originX: Sp,
    originY: Sp,
    originZ: J,
  },
  Pc = {
    borderWidth: J,
    borderTopWidth: J,
    borderRightWidth: J,
    borderBottomWidth: J,
    borderLeftWidth: J,
    borderRadius: J,
    borderTopLeftRadius: J,
    borderTopRightRadius: J,
    borderBottomRightRadius: J,
    borderBottomLeftRadius: J,
    width: J,
    maxWidth: J,
    height: J,
    maxHeight: J,
    top: J,
    right: J,
    bottom: J,
    left: J,
    inset: J,
    insetBlock: J,
    insetBlockStart: J,
    insetBlockEnd: J,
    insetInline: J,
    insetInlineStart: J,
    insetInlineEnd: J,
    padding: J,
    paddingTop: J,
    paddingRight: J,
    paddingBottom: J,
    paddingLeft: J,
    paddingBlock: J,
    paddingBlockStart: J,
    paddingBlockEnd: J,
    paddingInline: J,
    paddingInlineStart: J,
    paddingInlineEnd: J,
    margin: J,
    marginTop: J,
    marginRight: J,
    marginBottom: J,
    marginLeft: J,
    marginBlock: J,
    marginBlockStart: J,
    marginBlockEnd: J,
    marginInline: J,
    marginInlineStart: J,
    marginInlineEnd: J,
    fontSize: J,
    backgroundPositionX: J,
    backgroundPositionY: J,
    ...f2,
    zIndex: _p,
    fillOpacity: bl,
    strokeOpacity: bl,
    numOctaves: _p,
  },
  d2 = {
    ...Pc,
    color: Kt,
    backgroundColor: Kt,
    outlineColor: Kt,
    fill: Kt,
    stroke: Kt,
    borderColor: Kt,
    borderTopColor: Kt,
    borderRightColor: Kt,
    borderBottomColor: Kt,
    borderLeftColor: Kt,
    filter: bc,
    WebkitFilter: bc,
    mask: Sc,
    WebkitMask: Sc,
  },
  _g = (i) => d2[i],
  h2 = new Set([bc, Sc]);
function Bg(i, l) {
  let u = _g(i);
  return (h2.has(u) || (u = Je), u.getAnimatableNone ? u.getAnimatableNone(l) : void 0);
}
const m2 = new Set(["auto", "none", "0"]);
function p2(i, l, u) {
  let o = 0,
    c;
  for (; o < i.length && !c; ) {
    const h = i[o];
    (typeof h == "string" && !m2.has(h) && ha(h).values.length && (c = i[o]), o++);
  }
  if (c && u) for (const h of l) i[h] = Bg(u, c);
}
class g2 extends Qc {
  constructor(l, u, o, c, h) {
    super(l, u, o, c, h, !0);
  }
  readKeyframes() {
    const { unresolvedKeyframes: l, element: u, name: o } = this;
    if (!u || !u.current) return;
    super.readKeyframes();
    for (let v = 0; v < l.length; v++) {
      let x = l[v];
      if (typeof x == "string" && ((x = x.trim()), Uc(x))) {
        const S = Cg(x, u.current);
        (S !== void 0 && (l[v] = S), v === l.length - 1 && (this.finalKeyframe = x));
      }
    }
    if ((this.resolveNoneKeyframes(), !zg.has(o) || l.length !== 2)) return;
    const [c, h] = l,
      d = Vp(c),
      m = Vp(h),
      g = bp(c),
      p = bp(h);
    if (g !== p && Jn[o]) {
      this.needsMeasurement = !0;
      return;
    }
    if (d !== m)
      if (zp(d) && zp(m))
        for (let v = 0; v < l.length; v++) {
          const x = l[v];
          typeof x == "string" && (l[v] = parseFloat(x));
        }
      else Jn[o] && (this.needsMeasurement = !0);
  }
  resolveNoneKeyframes() {
    const { unresolvedKeyframes: l, name: u } = this,
      o = [];
    for (let c = 0; c < l.length; c++) (l[c] === null || u2(l[c])) && o.push(c);
    o.length && p2(l, o, u);
  }
  measureInitialState() {
    const { element: l, unresolvedKeyframes: u, name: o } = this;
    if (!l || !l.current) return;
    (o === "height" && (this.suspendedScrollY = window.pageYOffset),
      (this.measuredOrigin = Jn[o](l.measureViewportBox(), window.getComputedStyle(l.current))),
      (u[0] = this.measuredOrigin));
    const c = u[u.length - 1];
    c !== void 0 && l.getValue(o, c).jump(c, !1);
  }
  measureEndState() {
    var m;
    const { element: l, name: u, unresolvedKeyframes: o } = this;
    if (!l || !l.current) return;
    const c = l.getValue(u);
    c && c.jump(this.measuredOrigin, !1);
    const h = o.length - 1,
      d = o[h];
    ((o[h] = Jn[u](l.measureViewportBox(), window.getComputedStyle(l.current))),
      d !== null && this.finalKeyframe === void 0 && (this.finalKeyframe = d),
      (m = this.removedTransforms) != null &&
        m.length &&
        this.removedTransforms.forEach(([g, p]) => {
          l.getValue(g).set(p);
        }),
      this.resolveNoneKeyframes());
  }
}
const y2 = new Set(["opacity", "clipPath", "filter", "transform"]);
function Ug(i, l, u) {
  if (i == null) return [];
  if (i instanceof EventTarget) return [i];
  if (typeof i == "string") {
    let o = document;
    const c = (u == null ? void 0 : u[i]) ?? o.querySelectorAll(i);
    return c ? Array.from(c) : [];
  }
  return Array.from(i).filter((o) => o != null);
}
const Lg = (i, l) => (l && typeof i == "number" ? l.transform(i) : i);
function Ws(i) {
  return Q0(i) && "offsetHeight" in i && !("ownerSVGElement" in i);
}
const { schedule: Wc } = lg(queueMicrotask, !1),
  Ke = { x: !1, y: !1 };
function Hg() {
  return Ke.x || Ke.y;
}
function v2(i) {
  return i === "x" || i === "y"
    ? Ke[i]
      ? null
      : ((Ke[i] = !0),
        () => {
          Ke[i] = !1;
        })
    : Ke.x || Ke.y
      ? null
      : ((Ke.x = Ke.y = !0),
        () => {
          Ke.x = Ke.y = !1;
        });
}
function qg(i, l) {
  const u = Ug(i),
    o = new AbortController(),
    c = { passive: !0, ...l, signal: o.signal };
  return [u, c, () => o.abort()];
}
function x2(i) {
  return !(i.pointerType === "touch" || Hg());
}
function b2(i, l, u = {}) {
  const [o, c, h] = qg(i, u);
  return (
    o.forEach((d) => {
      let m = !1,
        g = !1,
        p;
      const v = () => {
          d.removeEventListener("pointerleave", w);
        },
        x = (H) => {
          (p && (p(H), (p = void 0)), v());
        },
        S = (H) => {
          ((m = !1),
            window.removeEventListener("pointerup", S),
            window.removeEventListener("pointercancel", S),
            g && ((g = !1), x(H)));
        },
        N = () => {
          ((m = !0),
            window.addEventListener("pointerup", S, c),
            window.addEventListener("pointercancel", S, c));
        },
        w = (H) => {
          if (H.pointerType !== "touch") {
            if (m) {
              g = !0;
              return;
            }
            x(H);
          }
        },
        U = (H) => {
          if (!x2(H)) return;
          g = !1;
          const L = l(d, H);
          typeof L == "function" && ((p = L), d.addEventListener("pointerleave", w, c));
        };
      (d.addEventListener("pointerenter", U, c), d.addEventListener("pointerdown", N, c));
    }),
    h
  );
}
const Yg = (i, l) => (l ? (i === l ? !0 : Yg(i, l.parentElement)) : !1),
  $c = (i) =>
    i.pointerType === "mouse" ? typeof i.button != "number" || i.button <= 0 : i.isPrimary !== !1,
  S2 = new Set(["BUTTON", "INPUT", "SELECT", "TEXTAREA", "A"]);
function T2(i) {
  return S2.has(i.tagName) || i.isContentEditable === !0;
}
const A2 = new Set(["INPUT", "SELECT", "TEXTAREA"]);
function E2(i) {
  return A2.has(i.tagName) || i.isContentEditable === !0;
}
const $s = new WeakSet();
function Bp(i) {
  return (l) => {
    l.key === "Enter" && i(l);
  };
}
function Kr(i, l) {
  i.dispatchEvent(new PointerEvent("pointer" + l, { isPrimary: !0, bubbles: !0 }));
}
const M2 = (i, l) => {
  const u = i.currentTarget;
  if (!u) return;
  const o = Bp(() => {
    if ($s.has(u)) return;
    Kr(u, "down");
    const c = Bp(() => {
        Kr(u, "up");
      }),
      h = () => Kr(u, "cancel");
    (u.addEventListener("keyup", c, l), u.addEventListener("blur", h, l));
  });
  (u.addEventListener("keydown", o, l),
    u.addEventListener("blur", () => u.removeEventListener("keydown", o), l));
};
function Up(i) {
  return $c(i) && !Hg();
}
const Lp = new WeakSet();
function D2(i, l, u = {}) {
  const [o, c, h] = qg(i, u),
    d = (m) => {
      const g = m.currentTarget;
      if (!Up(m) || Lp.has(m)) return;
      ($s.add(g), u.stopPropagation && Lp.add(m));
      const p = l(g, m),
        v = (N, w) => {
          (window.removeEventListener("pointerup", x),
            window.removeEventListener("pointercancel", S),
            $s.has(g) && $s.delete(g),
            Up(N) && typeof p == "function" && p(N, { success: w }));
        },
        x = (N) => {
          v(N, g === window || g === document || u.useGlobalTarget || Yg(g, N.target));
        },
        S = (N) => {
          v(N, !1);
        };
      (window.addEventListener("pointerup", x, c), window.addEventListener("pointercancel", S, c));
    };
  return (
    o.forEach((m) => {
      ((u.useGlobalTarget ? window : m).addEventListener("pointerdown", d, c),
        Ws(m) &&
          (m.addEventListener("focus", (p) => M2(p, c)),
          !T2(m) && !m.hasAttribute("tabindex") && (m.tabIndex = 0)));
    }),
    h
  );
}
function Ic(i) {
  return Q0(i) && "ownerSVGElement" in i;
}
const Is = new WeakMap();
let Kn;
const Gg = (i, l, u) => (o, c) =>
    c && c[0] ? c[0][i + "Size"] : Ic(o) && "getBBox" in o ? o.getBBox()[l] : o[u],
  C2 = Gg("inline", "width", "offsetWidth"),
  N2 = Gg("block", "height", "offsetHeight");
function z2({ target: i, borderBoxSize: l }) {
  var u;
  (u = Is.get(i)) == null ||
    u.forEach((o) => {
      o(i, {
        get width() {
          return C2(i, l);
        },
        get height() {
          return N2(i, l);
        },
      });
    });
}
function j2(i) {
  i.forEach(z2);
}
function w2() {
  typeof ResizeObserver > "u" || (Kn = new ResizeObserver(j2));
}
function R2(i, l) {
  Kn || w2();
  const u = Ug(i);
  return (
    u.forEach((o) => {
      let c = Is.get(o);
      (c || ((c = new Set()), Is.set(o, c)), c.add(l), Kn == null || Kn.observe(o));
    }),
    () => {
      u.forEach((o) => {
        const c = Is.get(o);
        (c == null || c.delete(l), (c != null && c.size) || Kn == null || Kn.unobserve(o));
      });
    }
  );
}
const tu = new Set();
let ra;
function O2() {
  ((ra = () => {
    const i = {
      get width() {
        return window.innerWidth;
      },
      get height() {
        return window.innerHeight;
      },
    };
    tu.forEach((l) => l(i));
  }),
    window.addEventListener("resize", ra));
}
function V2(i) {
  return (
    tu.add(i),
    ra || O2(),
    () => {
      (tu.delete(i),
        !tu.size &&
          typeof ra == "function" &&
          (window.removeEventListener("resize", ra), (ra = void 0)));
    }
  );
}
function Hp(i, l) {
  return typeof i == "function" ? V2(i) : R2(i, l);
}
function _2(i) {
  return Ic(i) && i.tagName === "svg";
}
const B2 = [...Vg, Kt, Je],
  U2 = (i) => B2.find(Og(i)),
  qp = () => ({ translate: 0, scale: 1, origin: 0, originPoint: 0 }),
  ca = () => ({ x: qp(), y: qp() }),
  Yp = () => ({ min: 0, max: 0 }),
  Ft = () => ({ x: Yp(), y: Yp() }),
  L2 = new WeakMap();
function hu(i) {
  return i !== null && typeof i == "object" && typeof i.start == "function";
}
function Sl(i) {
  return typeof i == "string" || Array.isArray(i);
}
const tf = ["animate", "whileInView", "whileFocus", "whileHover", "whileTap", "whileDrag", "exit"],
  ef = ["initial", ...tf];
function mu(i) {
  return hu(i.animate) || ef.some((l) => Sl(i[l]));
}
function Xg(i) {
  return !!(mu(i) || i.variants);
}
function H2(i, l, u) {
  for (const o in l) {
    const c = l[o],
      h = u[o];
    if (se(c)) i.addValue(o, c);
    else if (se(h)) i.addValue(o, ma(c, { owner: i }));
    else if (h !== c)
      if (i.hasValue(o)) {
        const d = i.getValue(o);
        d.liveStyle === !0 ? d.jump(c) : d.hasAnimated || d.set(c);
      } else {
        const d = i.getStaticValue(o);
        i.addValue(o, ma(d !== void 0 ? d : c, { owner: i }));
      }
  }
  for (const o in u) l[o] === void 0 && i.removeValue(o);
  return l;
}
const Tc = { current: null },
  kg = { current: !1 },
  q2 = typeof window < "u";
function Y2() {
  if (((kg.current = !0), !!q2))
    if (window.matchMedia) {
      const i = window.matchMedia("(prefers-reduced-motion)"),
        l = () => (Tc.current = i.matches);
      (i.addEventListener("change", l), l());
    } else Tc.current = !1;
}
const Gp = [
  "AnimationStart",
  "AnimationComplete",
  "Update",
  "BeforeLayoutMeasure",
  "LayoutMeasure",
  "LayoutAnimationStart",
  "LayoutAnimationComplete",
];
let ou = {};
function Qg(i) {
  ou = i;
}
function G2() {
  return ou;
}
class X2 {
  scrapeMotionValuesFromProps(l, u, o) {
    return {};
  }
  constructor(
    {
      parent: l,
      props: u,
      presenceContext: o,
      reducedMotionConfig: c,
      skipAnimations: h,
      blockInitialAnimation: d,
      visualState: m,
    },
    g = {},
  ) {
    ((this.current = null),
      (this.children = new Set()),
      (this.isVariantNode = !1),
      (this.isControllingVariants = !1),
      (this.shouldReduceMotion = null),
      (this.shouldSkipAnimations = !1),
      (this.values = new Map()),
      (this.KeyframeResolver = Qc),
      (this.features = {}),
      (this.valueSubscriptions = new Map()),
      (this.prevMotionValues = {}),
      (this.hasBeenMounted = !1),
      (this.events = {}),
      (this.propEventSubscriptions = {}),
      (this.notifyUpdate = () => this.notify("Update", this.latestValues)),
      (this.render = () => {
        this.current &&
          (this.triggerBuild(),
          this.renderInstance(this.current, this.renderState, this.props.style, this.projection));
      }),
      (this.renderScheduledAt = 0),
      (this.scheduleRender = () => {
        const N = ce.now();
        this.renderScheduledAt < N &&
          ((this.renderScheduledAt = N), zt.render(this.render, !1, !0));
      }));
    const { latestValues: p, renderState: v } = m;
    ((this.latestValues = p),
      (this.baseTarget = { ...p }),
      (this.initialValues = u.initial ? { ...p } : {}),
      (this.renderState = v),
      (this.parent = l),
      (this.props = u),
      (this.presenceContext = o),
      (this.depth = l ? l.depth + 1 : 0),
      (this.reducedMotionConfig = c),
      (this.skipAnimationsConfig = h),
      (this.options = g),
      (this.blockInitialAnimation = !!d),
      (this.isControllingVariants = mu(u)),
      (this.isVariantNode = Xg(u)),
      this.isVariantNode && (this.variantChildren = new Set()),
      (this.manuallyAnimateOnMount = !!(l && l.current)));
    const { willChange: x, ...S } = this.scrapeMotionValuesFromProps(u, {}, this);
    for (const N in S) {
      const w = S[N];
      p[N] !== void 0 && se(w) && w.set(p[N]);
    }
  }
  mount(l) {
    var u, o;
    if (this.hasBeenMounted)
      for (const c in this.initialValues)
        ((u = this.values.get(c)) == null || u.jump(this.initialValues[c]),
          (this.latestValues[c] = this.initialValues[c]));
    ((this.current = l),
      L2.set(l, this),
      this.projection && !this.projection.instance && this.projection.mount(l),
      this.parent &&
        this.isVariantNode &&
        !this.isControllingVariants &&
        (this.removeFromVariantTree = this.parent.addVariantChild(this)),
      this.values.forEach((c, h) => this.bindToMotionValue(h, c)),
      this.reducedMotionConfig === "never"
        ? (this.shouldReduceMotion = !1)
        : this.reducedMotionConfig === "always"
          ? (this.shouldReduceMotion = !0)
          : (kg.current || Y2(), (this.shouldReduceMotion = Tc.current)),
      (this.shouldSkipAnimations = this.skipAnimationsConfig ?? !1),
      (o = this.parent) == null || o.addChild(this),
      this.update(this.props, this.presenceContext),
      (this.hasBeenMounted = !0));
  }
  unmount() {
    var l;
    (this.projection && this.projection.unmount(),
      Pn(this.notifyUpdate),
      Pn(this.render),
      this.valueSubscriptions.forEach((u) => u()),
      this.valueSubscriptions.clear(),
      this.removeFromVariantTree && this.removeFromVariantTree(),
      (l = this.parent) == null || l.removeChild(this));
    for (const u in this.events) this.events[u].clear();
    for (const u in this.features) {
      const o = this.features[u];
      o && (o.unmount(), (o.isMounted = !1));
    }
    this.current = null;
  }
  addChild(l) {
    (this.children.add(l),
      this.enteringChildren ?? (this.enteringChildren = new Set()),
      this.enteringChildren.add(l));
  }
  removeChild(l) {
    (this.children.delete(l), this.enteringChildren && this.enteringChildren.delete(l));
  }
  bindToMotionValue(l, u) {
    if (
      (this.valueSubscriptions.has(l) && this.valueSubscriptions.get(l)(),
      u.accelerate && y2.has(l) && this.current instanceof HTMLElement)
    ) {
      const { factory: d, keyframes: m, times: g, ease: p, duration: v } = u.accelerate,
        x = new Eg({
          element: this.current,
          name: l,
          keyframes: m,
          times: g,
          ease: p,
          duration: je(v),
        }),
        S = d(x);
      this.valueSubscriptions.set(l, () => {
        (S(), x.cancel());
      });
      return;
    }
    const o = ya.has(l);
    o && this.onBindTransform && this.onBindTransform();
    const c = u.on("change", (d) => {
      ((this.latestValues[l] = d),
        this.props.onUpdate && zt.preRender(this.notifyUpdate),
        o && this.projection && (this.projection.isTransformDirty = !0),
        this.scheduleRender());
    });
    let h;
    (typeof window < "u" &&
      window.MotionCheckAppearSync &&
      (h = window.MotionCheckAppearSync(this, l, u)),
      this.valueSubscriptions.set(l, () => {
        (c(), h && h(), u.owner && u.stop());
      }));
  }
  sortNodePosition(l) {
    return !this.current || !this.sortInstanceNodePosition || this.type !== l.type
      ? 0
      : this.sortInstanceNodePosition(this.current, l.current);
  }
  updateFeatures() {
    let l = "animation";
    for (l in ou) {
      const u = ou[l];
      if (!u) continue;
      const { isEnabled: o, Feature: c } = u;
      if (
        (!this.features[l] && c && o(this.props) && (this.features[l] = new c(this)),
        this.features[l])
      ) {
        const h = this.features[l];
        h.isMounted ? h.update() : (h.mount(), (h.isMounted = !0));
      }
    }
  }
  triggerBuild() {
    this.build(this.renderState, this.latestValues, this.props);
  }
  measureViewportBox() {
    return this.current ? this.measureInstanceViewportBox(this.current, this.props) : Ft();
  }
  getStaticValue(l) {
    return this.latestValues[l];
  }
  setStaticValue(l, u) {
    this.latestValues[l] = u;
  }
  update(l, u) {
    ((l.transformTemplate || this.props.transformTemplate) && this.scheduleRender(),
      (this.prevProps = this.props),
      (this.props = l),
      (this.prevPresenceContext = this.presenceContext),
      (this.presenceContext = u));
    for (let o = 0; o < Gp.length; o++) {
      const c = Gp[o];
      this.propEventSubscriptions[c] &&
        (this.propEventSubscriptions[c](), delete this.propEventSubscriptions[c]);
      const h = "on" + c,
        d = l[h];
      d && (this.propEventSubscriptions[c] = this.on(c, d));
    }
    ((this.prevMotionValues = H2(
      this,
      this.scrapeMotionValuesFromProps(l, this.prevProps || {}, this),
      this.prevMotionValues,
    )),
      this.handleChildMotionValue && this.handleChildMotionValue());
  }
  getProps() {
    return this.props;
  }
  getVariant(l) {
    return this.props.variants ? this.props.variants[l] : void 0;
  }
  getDefaultTransition() {
    return this.props.transition;
  }
  getTransformPagePoint() {
    return this.props.transformPagePoint;
  }
  getClosestVariantNode() {
    return this.isVariantNode ? this : this.parent ? this.parent.getClosestVariantNode() : void 0;
  }
  addVariantChild(l) {
    const u = this.getClosestVariantNode();
    if (u)
      return (u.variantChildren && u.variantChildren.add(l), () => u.variantChildren.delete(l));
  }
  addValue(l, u) {
    const o = this.values.get(l);
    u !== o &&
      (o && this.removeValue(l),
      this.bindToMotionValue(l, u),
      this.values.set(l, u),
      (this.latestValues[l] = u.get()));
  }
  removeValue(l) {
    this.values.delete(l);
    const u = this.valueSubscriptions.get(l);
    (u && (u(), this.valueSubscriptions.delete(l)),
      delete this.latestValues[l],
      this.removeValueFromRenderState(l, this.renderState));
  }
  hasValue(l) {
    return this.values.has(l);
  }
  getValue(l, u) {
    if (this.props.values && this.props.values[l]) return this.props.values[l];
    let o = this.values.get(l);
    return (
      o === void 0 &&
        u !== void 0 &&
        ((o = ma(u === null ? void 0 : u, { owner: this })), this.addValue(l, o)),
      o
    );
  }
  readValue(l, u) {
    let o =
      this.latestValues[l] !== void 0 || !this.current
        ? this.latestValues[l]
        : (this.getBaseTargetFromProps(this.props, l) ??
          this.readValueFromInstance(this.current, l, this.options));
    return (
      o != null &&
        (typeof o == "string" && (k0(o) || Z0(o))
          ? (o = parseFloat(o))
          : !U2(o) && Je.test(u) && (o = Bg(l, u)),
        this.setBaseTarget(l, se(o) ? o.get() : o)),
      se(o) ? o.get() : o
    );
  }
  setBaseTarget(l, u) {
    this.baseTarget[l] = u;
  }
  getBaseTarget(l) {
    var h;
    const { initial: u } = this.props;
    let o;
    if (typeof u == "string" || typeof u == "object") {
      const d = Jc(this.props, u, (h = this.presenceContext) == null ? void 0 : h.custom);
      d && (o = d[l]);
    }
    if (u && o !== void 0) return o;
    const c = this.getBaseTargetFromProps(this.props, l);
    return c !== void 0 && !se(c)
      ? c
      : this.initialValues[l] !== void 0 && o === void 0
        ? void 0
        : this.baseTarget[l];
  }
  on(l, u) {
    return (this.events[l] || (this.events[l] = new Vc()), this.events[l].add(u));
  }
  notify(l, ...u) {
    this.events[l] && this.events[l].notify(...u);
  }
  scheduleRenderMicrotask() {
    Wc.render(this.render);
  }
}
class Zg extends X2 {
  constructor() {
    (super(...arguments), (this.KeyframeResolver = g2));
  }
  sortInstanceNodePosition(l, u) {
    return l.compareDocumentPosition(u) & 2 ? 1 : -1;
  }
  getBaseTargetFromProps(l, u) {
    const o = l.style;
    return o ? o[u] : void 0;
  }
  removeValueFromRenderState(l, { vars: u, style: o }) {
    (delete u[l], delete o[l]);
  }
  handleChildMotionValue() {
    this.childSubscription && (this.childSubscription(), delete this.childSubscription);
    const { children: l } = this.props;
    se(l) &&
      (this.childSubscription = l.on("change", (u) => {
        this.current && (this.current.textContent = `${u}`);
      }));
  }
}
class Wn {
  constructor(l) {
    ((this.isMounted = !1), (this.node = l));
  }
  update() {}
}
function Kg({ top: i, left: l, right: u, bottom: o }) {
  return { x: { min: l, max: u }, y: { min: i, max: o } };
}
function k2({ x: i, y: l }) {
  return { top: l.min, right: i.max, bottom: l.max, left: i.min };
}
function Q2(i, l) {
  if (!l) return i;
  const u = l({ x: i.left, y: i.top }),
    o = l({ x: i.right, y: i.bottom });
  return { top: u.y, left: u.x, bottom: o.y, right: o.x };
}
function Jr(i) {
  return i === void 0 || i === 1;
}
function Ac({ scale: i, scaleX: l, scaleY: u }) {
  return !Jr(i) || !Jr(l) || !Jr(u);
}
function vi(i) {
  return Ac(i) || Jg(i) || i.z || i.rotate || i.rotateX || i.rotateY || i.skewX || i.skewY;
}
function Jg(i) {
  return Xp(i.x) || Xp(i.y);
}
function Xp(i) {
  return i && i !== "0%";
}
function ru(i, l, u) {
  const o = i - u,
    c = l * o;
  return u + c;
}
function kp(i, l, u, o, c) {
  return (c !== void 0 && (i = ru(i, c, o)), ru(i, u, o) + l);
}
function Ec(i, l = 0, u = 1, o, c) {
  ((i.min = kp(i.min, l, u, o, c)), (i.max = kp(i.max, l, u, o, c)));
}
function Fg(i, { x: l, y: u }) {
  (Ec(i.x, l.translate, l.scale, l.originPoint), Ec(i.y, u.translate, u.scale, u.originPoint));
}
const Qp = 0.999999999999,
  Zp = 1.0000000000001;
function Z2(i, l, u, o = !1) {
  var m;
  const c = u.length;
  if (!c) return;
  l.x = l.y = 1;
  let h, d;
  for (let g = 0; g < c; g++) {
    ((h = u[g]), (d = h.projectionDelta));
    const { visualElement: p } = h.options;
    (p && p.props.style && p.props.style.display === "contents") ||
      (o &&
        h.options.layoutScroll &&
        h.scroll &&
        h !== h.root &&
        da(i, { x: -h.scroll.offset.x, y: -h.scroll.offset.y }),
      d && ((l.x *= d.x.scale), (l.y *= d.y.scale), Fg(i, d)),
      o &&
        vi(h.latestValues) &&
        da(i, h.latestValues, (m = h.layout) == null ? void 0 : m.layoutBox));
  }
  (l.x < Zp && l.x > Qp && (l.x = 1), l.y < Zp && l.y > Qp && (l.y = 1));
}
function fa(i, l) {
  ((i.min = i.min + l), (i.max = i.max + l));
}
function Kp(i, l, u, o, c = 0.5) {
  const h = Ot(i.min, i.max, c);
  Ec(i, l, u, h, o);
}
function Jp(i, l) {
  return typeof i == "string" ? (parseFloat(i) / 100) * (l.max - l.min) : i;
}
function da(i, l, u) {
  const o = u ?? i;
  (Kp(i.x, Jp(l.x, o.x), l.scaleX, l.scale, l.originX),
    Kp(i.y, Jp(l.y, o.y), l.scaleY, l.scale, l.originY));
}
function Pg(i, l) {
  return Kg(Q2(i.getBoundingClientRect(), l));
}
function K2(i, l, u) {
  const o = Pg(i, u),
    { scroll: c } = l;
  return (c && (fa(o.x, c.offset.x), fa(o.y, c.offset.y)), o);
}
const J2 = {
    x: "translateX",
    y: "translateY",
    z: "translateZ",
    transformPerspective: "perspective",
  },
  F2 = ga.length;
function P2(i, l, u) {
  let o = "",
    c = !0;
  for (let h = 0; h < F2; h++) {
    const d = ga[h],
      m = i[d];
    if (m === void 0) continue;
    let g = !0;
    if (typeof m == "number") g = m === (d.startsWith("scale") ? 1 : 0);
    else {
      const p = parseFloat(m);
      g = d.startsWith("scale") ? p === 1 : p === 0;
    }
    if (!g || u) {
      const p = Lg(m, Pc[d]);
      if (!g) {
        c = !1;
        const v = J2[d] || d;
        o += `${v}(${p}) `;
      }
      u && (l[d] = p);
    }
  }
  return ((o = o.trim()), u ? (o = u(l, c ? "" : o)) : c && (o = "none"), o);
}
function nf(i, l, u) {
  const { style: o, vars: c, transformOrigin: h } = i;
  let d = !1,
    m = !1;
  for (const g in l) {
    const p = l[g];
    if (ya.has(g)) {
      d = !0;
      continue;
    } else if (ug(g)) {
      c[g] = p;
      continue;
    } else {
      const v = Lg(p, Pc[g]);
      g.startsWith("origin") ? ((m = !0), (h[g] = v)) : (o[g] = v);
    }
  }
  if (
    (l.transform ||
      (d || u ? (o.transform = P2(l, i.transform, u)) : o.transform && (o.transform = "none")),
    m)
  ) {
    const { originX: g = "50%", originY: p = "50%", originZ: v = 0 } = h;
    o.transformOrigin = `${g} ${p} ${v}`;
  }
}
function Wg(i, { style: l, vars: u }, o, c) {
  const h = i.style;
  let d;
  for (d in l) h[d] = l[d];
  c == null || c.applyProjectionStyles(h, o);
  for (d in u) h.setProperty(d, u[d]);
}
function Fp(i, l) {
  return l.max === l.min ? 0 : (i / (l.max - l.min)) * 100;
}
const hl = {
    correct: (i, l) => {
      if (!l.target) return i;
      if (typeof i == "string")
        if (J.test(i)) i = parseFloat(i);
        else return i;
      const u = Fp(i, l.target.x),
        o = Fp(i, l.target.y);
      return `${u}% ${o}%`;
    },
  },
  W2 = {
    correct: (i, { treeScale: l, projectionDelta: u }) => {
      const o = i,
        c = Je.parse(i);
      if (c.length > 5) return o;
      const h = Je.createTransformer(i),
        d = typeof c[0] != "number" ? 1 : 0,
        m = u.x.scale * l.x,
        g = u.y.scale * l.y;
      ((c[0 + d] /= m), (c[1 + d] /= g));
      const p = Ot(m, g, 0.5);
      return (
        typeof c[2 + d] == "number" && (c[2 + d] /= p),
        typeof c[3 + d] == "number" && (c[3 + d] /= p),
        h(c)
      );
    },
  },
  Mc = {
    borderRadius: {
      ...hl,
      applyTo: [
        "borderTopLeftRadius",
        "borderTopRightRadius",
        "borderBottomLeftRadius",
        "borderBottomRightRadius",
      ],
    },
    borderTopLeftRadius: hl,
    borderTopRightRadius: hl,
    borderBottomLeftRadius: hl,
    borderBottomRightRadius: hl,
    boxShadow: W2,
  };
function $g(i, { layout: l, layoutId: u }) {
  return (
    ya.has(i) || i.startsWith("origin") || ((l || u !== void 0) && (!!Mc[i] || i === "opacity"))
  );
}
function af(i, l, u) {
  var d;
  const o = i.style,
    c = l == null ? void 0 : l.style,
    h = {};
  if (!o) return h;
  for (const m in o)
    (se(o[m]) ||
      (c && se(c[m])) ||
      $g(m, i) ||
      ((d = u == null ? void 0 : u.getValue(m)) == null ? void 0 : d.liveStyle) !== void 0) &&
      (h[m] = o[m]);
  return h;
}
function $2(i) {
  return window.getComputedStyle(i);
}
class I2 extends Zg {
  constructor() {
    (super(...arguments), (this.type = "html"), (this.renderInstance = Wg));
  }
  readValueFromInstance(l, u) {
    var o;
    if (ya.has(u)) return (o = this.projection) != null && o.isProjecting ? fc(u) : xb(l, u);
    {
      const c = $2(l),
        h = (ug(u) ? c.getPropertyValue(u) : c[u]) || 0;
      return typeof h == "string" ? h.trim() : h;
    }
  }
  measureInstanceViewportBox(l, { transformPagePoint: u }) {
    return Pg(l, u);
  }
  build(l, u, o) {
    nf(l, u, o.transformTemplate);
  }
  scrapeMotionValuesFromProps(l, u, o) {
    return af(l, u, o);
  }
}
const tS = { offset: "stroke-dashoffset", array: "stroke-dasharray" },
  eS = { offset: "strokeDashoffset", array: "strokeDasharray" };
function nS(i, l, u = 1, o = 0, c = !0) {
  i.pathLength = 1;
  const h = c ? tS : eS;
  ((i[h.offset] = `${-o}`), (i[h.array] = `${l} ${u}`));
}
const iS = ["offsetDistance", "offsetPath", "offsetRotate", "offsetAnchor"];
function Ig(
  i,
  { attrX: l, attrY: u, attrScale: o, pathLength: c, pathSpacing: h = 1, pathOffset: d = 0, ...m },
  g,
  p,
  v,
) {
  if ((nf(i, m, p), g)) {
    i.style.viewBox && (i.attrs.viewBox = i.style.viewBox);
    return;
  }
  ((i.attrs = i.style), (i.style = {}));
  const { attrs: x, style: S } = i;
  (x.transform && ((S.transform = x.transform), delete x.transform),
    (S.transform || x.transformOrigin) &&
      ((S.transformOrigin = x.transformOrigin ?? "50% 50%"), delete x.transformOrigin),
    S.transform &&
      ((S.transformBox = (v == null ? void 0 : v.transformBox) ?? "fill-box"),
      delete x.transformBox));
  for (const N of iS) x[N] !== void 0 && ((S[N] = x[N]), delete x[N]);
  (l !== void 0 && (x.x = l),
    u !== void 0 && (x.y = u),
    o !== void 0 && (x.scale = o),
    c !== void 0 && nS(x, c, h, d, !1));
}
const ty = new Set([
    "baseFrequency",
    "diffuseConstant",
    "kernelMatrix",
    "kernelUnitLength",
    "keySplines",
    "keyTimes",
    "limitingConeAngle",
    "markerHeight",
    "markerWidth",
    "numOctaves",
    "targetX",
    "targetY",
    "surfaceScale",
    "specularConstant",
    "specularExponent",
    "stdDeviation",
    "tableValues",
    "viewBox",
    "gradientTransform",
    "pathLength",
    "startOffset",
    "textLength",
    "lengthAdjust",
  ]),
  ey = (i) => typeof i == "string" && i.toLowerCase() === "svg";
function aS(i, l, u, o) {
  Wg(i, l, void 0, o);
  for (const c in l.attrs) i.setAttribute(ty.has(c) ? c : Fc(c), l.attrs[c]);
}
function ny(i, l, u) {
  const o = af(i, l, u);
  for (const c in i)
    if (se(i[c]) || se(l[c])) {
      const h = ga.indexOf(c) !== -1 ? "attr" + c.charAt(0).toUpperCase() + c.substring(1) : c;
      o[h] = i[c];
    }
  return o;
}
class lS extends Zg {
  constructor() {
    (super(...arguments),
      (this.type = "svg"),
      (this.isSVGTag = !1),
      (this.measureInstanceViewportBox = Ft));
  }
  getBaseTargetFromProps(l, u) {
    return l[u];
  }
  readValueFromInstance(l, u) {
    if (ya.has(u)) {
      const o = _g(u);
      return (o && o.default) || 0;
    }
    return ((u = ty.has(u) ? u : Fc(u)), l.getAttribute(u));
  }
  scrapeMotionValuesFromProps(l, u, o) {
    return ny(l, u, o);
  }
  build(l, u, o) {
    Ig(l, u, this.isSVGTag, o.transformTemplate, o.style);
  }
  renderInstance(l, u, o, c) {
    aS(l, u, o, c);
  }
  mount(l) {
    ((this.isSVGTag = ey(l.tagName)), super.mount(l));
  }
}
const sS = ef.length;
function iy(i) {
  if (!i) return;
  if (!i.isControllingVariants) {
    const u = i.parent ? iy(i.parent) || {} : {};
    return (i.props.initial !== void 0 && (u.initial = i.props.initial), u);
  }
  const l = {};
  for (let u = 0; u < sS; u++) {
    const o = ef[u],
      c = i.props[o];
    (Sl(c) || c === !1) && (l[o] = c);
  }
  return l;
}
function ay(i, l) {
  if (!Array.isArray(l)) return !1;
  const u = l.length;
  if (u !== i.length) return !1;
  for (let o = 0; o < u; o++) if (l[o] !== i[o]) return !1;
  return !0;
}
const uS = [...tf].reverse(),
  oS = tf.length;
function rS(i) {
  return (l) => Promise.all(l.map(({ animation: u, options: o }) => l2(i, u, o)));
}
function cS(i) {
  let l = rS(i),
    u = Pp(),
    o = !0,
    c = !1;
  const h = (p) => (v, x) => {
    var N;
    const S = Ti(
      i,
      x,
      p === "exit" ? ((N = i.presenceContext) == null ? void 0 : N.custom) : void 0,
    );
    if (S) {
      const { transition: w, transitionEnd: U, ...H } = S;
      v = { ...v, ...H, ...U };
    }
    return v;
  };
  function d(p) {
    l = p(i);
  }
  function m(p) {
    const { props: v } = i,
      x = iy(i.parent) || {},
      S = [],
      N = new Set();
    let w = {},
      U = 1 / 0;
    for (let L = 0; L < oS; L++) {
      const Y = uS[L],
        q = u[Y],
        G = v[Y] !== void 0 ? v[Y] : x[Y],
        F = Sl(G),
        lt = Y === p ? q.isActive : null;
      lt === !1 && (U = L);
      let I = G === x[Y] && G !== v[Y] && F;
      if (
        (I && (o || c) && i.manuallyAnimateOnMount && (I = !1),
        (q.protectedKeys = { ...w }),
        (!q.isActive && lt === null) || (!G && !q.prevProp) || hu(G) || typeof G == "boolean")
      )
        continue;
      if (Y === "exit" && q.isActive && lt !== !0) {
        q.prevResolvedValues && (w = { ...w, ...q.prevResolvedValues });
        continue;
      }
      const $ = fS(q.prevProp, G);
      let it = $ || (Y === p && q.isActive && !I && F) || (L > U && F),
        tt = !1;
      const pt = Array.isArray(G) ? G : [G];
      let vt = pt.reduce(h(Y), {});
      lt === !1 && (vt = {});
      const { prevResolvedValues: $t = {} } = q,
        Gt = { ...$t, ...vt },
        Lt = (Z) => {
          ((it = !0), N.has(Z) && ((tt = !0), N.delete(Z)), (q.needsAnimating[Z] = !0));
          const ot = i.getValue(Z);
          ot && (ot.liveStyle = !1);
        };
      for (const Z in Gt) {
        const ot = vt[Z],
          gt = $t[Z];
        if (w.hasOwnProperty(Z)) continue;
        let E = !1;
        (yc(ot) && yc(gt) ? (E = !ay(ot, gt)) : (E = ot !== gt),
          E
            ? ot != null
              ? Lt(Z)
              : N.add(Z)
            : ot !== void 0 && N.has(Z)
              ? Lt(Z)
              : (q.protectedKeys[Z] = !0));
      }
      ((q.prevProp = G),
        (q.prevResolvedValues = vt),
        q.isActive && (w = { ...w, ...vt }),
        (o || c) && i.blockInitialAnimation && (it = !1));
      const R = I && $;
      it &&
        (!R || tt) &&
        S.push(
          ...pt.map((Z) => {
            const ot = { type: Y };
            if (typeof Z == "string" && (o || c) && !R && i.manuallyAnimateOnMount && i.parent) {
              const { parent: gt } = i,
                E = Ti(gt, Z);
              if (gt.enteringChildren && E) {
                const { delayChildren: B } = E.transition || {};
                ot.delay = Dg(gt.enteringChildren, i, B);
              }
            }
            return { animation: Z, options: ot };
          }),
        );
    }
    if (N.size) {
      const L = {};
      if (typeof v.initial != "boolean") {
        const Y = Ti(i, Array.isArray(v.initial) ? v.initial[0] : v.initial);
        Y && Y.transition && (L.transition = Y.transition);
      }
      (N.forEach((Y) => {
        const q = i.getBaseTarget(Y),
          G = i.getValue(Y);
        (G && (G.liveStyle = !0), (L[Y] = q ?? null));
      }),
        S.push({ animation: L }));
    }
    let H = !!S.length;
    return (
      o && (v.initial === !1 || v.initial === v.animate) && !i.manuallyAnimateOnMount && (H = !1),
      (o = !1),
      (c = !1),
      H ? l(S) : Promise.resolve()
    );
  }
  function g(p, v) {
    var S;
    if (u[p].isActive === v) return Promise.resolve();
    ((S = i.variantChildren) == null ||
      S.forEach((N) => {
        var w;
        return (w = N.animationState) == null ? void 0 : w.setActive(p, v);
      }),
      (u[p].isActive = v));
    const x = m(p);
    for (const N in u) u[N].protectedKeys = {};
    return x;
  }
  return {
    animateChanges: m,
    setActive: g,
    setAnimateFunction: d,
    getState: () => u,
    reset: () => {
      ((u = Pp()), (c = !0));
    },
  };
}
function fS(i, l) {
  return typeof l == "string" ? l !== i : Array.isArray(l) ? !ay(l, i) : !1;
}
function yi(i = !1) {
  return { isActive: i, protectedKeys: {}, needsAnimating: {}, prevResolvedValues: {} };
}
function Pp() {
  return {
    animate: yi(!0),
    whileInView: yi(),
    whileHover: yi(),
    whileTap: yi(),
    whileDrag: yi(),
    whileFocus: yi(),
    exit: yi(),
  };
}
function Dc(i, l) {
  ((i.min = l.min), (i.max = l.max));
}
function Ze(i, l) {
  (Dc(i.x, l.x), Dc(i.y, l.y));
}
function Wp(i, l) {
  ((i.translate = l.translate),
    (i.scale = l.scale),
    (i.originPoint = l.originPoint),
    (i.origin = l.origin));
}
const ly = 1e-4,
  dS = 1 - ly,
  hS = 1 + ly,
  sy = 0.01,
  mS = 0 - sy,
  pS = 0 + sy;
function fe(i) {
  return i.max - i.min;
}
function gS(i, l, u) {
  return Math.abs(i - l) <= u;
}
function $p(i, l, u, o = 0.5) {
  ((i.origin = o),
    (i.originPoint = Ot(l.min, l.max, i.origin)),
    (i.scale = fe(u) / fe(l)),
    (i.translate = Ot(u.min, u.max, i.origin) - i.originPoint),
    ((i.scale >= dS && i.scale <= hS) || isNaN(i.scale)) && (i.scale = 1),
    ((i.translate >= mS && i.translate <= pS) || isNaN(i.translate)) && (i.translate = 0));
}
function yl(i, l, u, o) {
  ($p(i.x, l.x, u.x, o ? o.originX : void 0), $p(i.y, l.y, u.y, o ? o.originY : void 0));
}
function Ip(i, l, u) {
  ((i.min = u.min + l.min), (i.max = i.min + fe(l)));
}
function yS(i, l, u) {
  (Ip(i.x, l.x, u.x), Ip(i.y, l.y, u.y));
}
function t0(i, l, u) {
  ((i.min = l.min - u.min), (i.max = i.min + fe(l)));
}
function cu(i, l, u) {
  (t0(i.x, l.x, u.x), t0(i.y, l.y, u.y));
}
function e0(i, l, u, o, c) {
  return ((i -= l), (i = ru(i, 1 / u, o)), c !== void 0 && (i = ru(i, 1 / c, o)), i);
}
function vS(i, l = 0, u = 1, o = 0.5, c, h = i, d = i) {
  if (
    (tn.test(l) && ((l = parseFloat(l)), (l = Ot(d.min, d.max, l / 100) - d.min)),
    typeof l != "number")
  )
    return;
  let m = Ot(h.min, h.max, o);
  (i === h && (m -= l), (i.min = e0(i.min, l, u, m, c)), (i.max = e0(i.max, l, u, m, c)));
}
function n0(i, l, [u, o, c], h, d) {
  vS(i, l[u], l[o], l[c], l.scale, h, d);
}
const xS = ["x", "scaleX", "originX"],
  bS = ["y", "scaleY", "originY"];
function i0(i, l, u, o) {
  (n0(i.x, l, xS, u ? u.x : void 0, o ? o.x : void 0),
    n0(i.y, l, bS, u ? u.y : void 0, o ? o.y : void 0));
}
function a0(i) {
  return i.translate === 0 && i.scale === 1;
}
function uy(i) {
  return a0(i.x) && a0(i.y);
}
function l0(i, l) {
  return i.min === l.min && i.max === l.max;
}
function SS(i, l) {
  return l0(i.x, l.x) && l0(i.y, l.y);
}
function s0(i, l) {
  return Math.round(i.min) === Math.round(l.min) && Math.round(i.max) === Math.round(l.max);
}
function oy(i, l) {
  return s0(i.x, l.x) && s0(i.y, l.y);
}
function u0(i) {
  return fe(i.x) / fe(i.y);
}
function o0(i, l) {
  return i.translate === l.translate && i.scale === l.scale && i.originPoint === l.originPoint;
}
function Ie(i) {
  return [i("x"), i("y")];
}
function TS(i, l, u) {
  let o = "";
  const c = i.x.translate / l.x,
    h = i.y.translate / l.y,
    d = (u == null ? void 0 : u.z) || 0;
  if (
    ((c || h || d) && (o = `translate3d(${c}px, ${h}px, ${d}px) `),
    (l.x !== 1 || l.y !== 1) && (o += `scale(${1 / l.x}, ${1 / l.y}) `),
    u)
  ) {
    const { transformPerspective: p, rotate: v, rotateX: x, rotateY: S, skewX: N, skewY: w } = u;
    (p && (o = `perspective(${p}px) ${o}`),
      v && (o += `rotate(${v}deg) `),
      x && (o += `rotateX(${x}deg) `),
      S && (o += `rotateY(${S}deg) `),
      N && (o += `skewX(${N}deg) `),
      w && (o += `skewY(${w}deg) `));
  }
  const m = i.x.scale * l.x,
    g = i.y.scale * l.y;
  return ((m !== 1 || g !== 1) && (o += `scale(${m}, ${g})`), o || "none");
}
const ry = ["TopLeft", "TopRight", "BottomLeft", "BottomRight"],
  AS = ry.length,
  r0 = (i) => (typeof i == "string" ? parseFloat(i) : i),
  c0 = (i) => typeof i == "number" || J.test(i);
function ES(i, l, u, o, c, h) {
  c
    ? ((i.opacity = Ot(0, u.opacity ?? 1, MS(o))), (i.opacityExit = Ot(l.opacity ?? 1, 0, DS(o))))
    : h && (i.opacity = Ot(l.opacity ?? 1, u.opacity ?? 1, o));
  for (let d = 0; d < AS; d++) {
    const m = `border${ry[d]}Radius`;
    let g = f0(l, m),
      p = f0(u, m);
    if (g === void 0 && p === void 0) continue;
    (g || (g = 0),
      p || (p = 0),
      g === 0 || p === 0 || c0(g) === c0(p)
        ? ((i[m] = Math.max(Ot(r0(g), r0(p), o), 0)), (tn.test(p) || tn.test(g)) && (i[m] += "%"))
        : (i[m] = p));
  }
  (l.rotate || u.rotate) && (i.rotate = Ot(l.rotate || 0, u.rotate || 0, o));
}
function f0(i, l) {
  return i[l] !== void 0 ? i[l] : i.borderRadius;
}
const MS = cy(0, 0.5, eg),
  DS = cy(0.5, 0.95, Ge);
function cy(i, l, u) {
  return (o) => (o < i ? 0 : o > l ? 1 : u(xl(i, l, o)));
}
function CS(i, l, u) {
  const o = se(i) ? i : ma(i);
  return (o.start(Kc("", o, l, u)), o.animation);
}
function Tl(i, l, u, o = { passive: !0 }) {
  return (i.addEventListener(l, u, o), () => i.removeEventListener(l, u));
}
const NS = (i, l) => i.depth - l.depth;
class zS {
  constructor() {
    ((this.children = []), (this.isDirty = !1));
  }
  add(l) {
    (Rc(this.children, l), (this.isDirty = !0));
  }
  remove(l) {
    (au(this.children, l), (this.isDirty = !0));
  }
  forEach(l) {
    (this.isDirty && this.children.sort(NS), (this.isDirty = !1), this.children.forEach(l));
  }
}
function jS(i, l) {
  const u = ce.now(),
    o = ({ timestamp: c }) => {
      const h = c - u;
      h >= l && (Pn(o), i(h - l));
    };
  return (zt.setup(o, !0), () => Pn(o));
}
function eu(i) {
  return se(i) ? i.get() : i;
}
class wS {
  constructor() {
    this.members = [];
  }
  add(l) {
    Rc(this.members, l);
    for (let u = this.members.length - 1; u >= 0; u--) {
      const o = this.members[u];
      if (o === l || o === this.lead || o === this.prevLead) continue;
      const c = o.instance;
      (!c || c.isConnected === !1) && !o.snapshot && (au(this.members, o), o.unmount());
    }
    l.scheduleRender();
  }
  remove(l) {
    if ((au(this.members, l), l === this.prevLead && (this.prevLead = void 0), l === this.lead)) {
      const u = this.members[this.members.length - 1];
      u && this.promote(u);
    }
  }
  relegate(l) {
    var u;
    for (let o = this.members.indexOf(l) - 1; o >= 0; o--) {
      const c = this.members[o];
      if (c.isPresent !== !1 && ((u = c.instance) == null ? void 0 : u.isConnected) !== !1)
        return (this.promote(c), !0);
    }
    return !1;
  }
  promote(l, u) {
    var c;
    const o = this.lead;
    if (l !== o && ((this.prevLead = o), (this.lead = l), l.show(), o)) {
      (o.updateSnapshot(), l.scheduleRender());
      const { layoutDependency: h } = o.options,
        { layoutDependency: d } = l.options;
      ((h === void 0 || h !== d) &&
        ((l.resumeFrom = o),
        u && (o.preserveOpacity = !0),
        o.snapshot &&
          ((l.snapshot = o.snapshot),
          (l.snapshot.latestValues = o.animationValues || o.latestValues)),
        (c = l.root) != null && c.isUpdating && (l.isLayoutDirty = !0)),
        l.options.crossfade === !1 && o.hide());
    }
  }
  exitAnimationComplete() {
    this.members.forEach((l) => {
      var u, o, c, h, d;
      ((o = (u = l.options).onExitComplete) == null || o.call(u),
        (d = (c = l.resumingFrom) == null ? void 0 : (h = c.options).onExitComplete) == null ||
          d.call(h));
    });
  }
  scheduleRender() {
    this.members.forEach((l) => l.instance && l.scheduleRender(!1));
  }
  removeLeadSnapshot() {
    var l;
    (l = this.lead) != null && l.snapshot && (this.lead.snapshot = void 0);
  }
}
const nu = { hasAnimatedSinceResize: !0, hasEverUpdated: !1 },
  Fr = ["", "X", "Y", "Z"],
  RS = 1e3;
let OS = 0;
function Pr(i, l, u, o) {
  const { latestValues: c } = l;
  c[i] && ((u[i] = c[i]), l.setStaticValue(i, 0), o && (o[i] = 0));
}
function fy(i) {
  if (((i.hasCheckedOptimisedAppear = !0), i.root === i)) return;
  const { visualElement: l } = i.options;
  if (!l) return;
  const u = wg(l);
  if (window.MotionHasOptimisedAnimation(u, "transform")) {
    const { layout: c, layoutId: h } = i.options;
    window.MotionCancelOptimisedAnimation(u, "transform", zt, !(c || h));
  }
  const { parent: o } = i;
  o && !o.hasCheckedOptimisedAppear && fy(o);
}
function dy({
  attachResizeListener: i,
  defaultParent: l,
  measureScroll: u,
  checkIsScrollRoot: o,
  resetTransform: c,
}) {
  return class {
    constructor(d = {}, m = l == null ? void 0 : l()) {
      ((this.id = OS++),
        (this.animationId = 0),
        (this.animationCommitId = 0),
        (this.children = new Set()),
        (this.options = {}),
        (this.isTreeAnimating = !1),
        (this.isAnimationBlocked = !1),
        (this.isLayoutDirty = !1),
        (this.isProjectionDirty = !1),
        (this.isSharedProjectionDirty = !1),
        (this.isTransformDirty = !1),
        (this.updateManuallyBlocked = !1),
        (this.updateBlockedByResize = !1),
        (this.isUpdating = !1),
        (this.isSVG = !1),
        (this.needsReset = !1),
        (this.shouldResetTransform = !1),
        (this.hasCheckedOptimisedAppear = !1),
        (this.treeScale = { x: 1, y: 1 }),
        (this.eventHandlers = new Map()),
        (this.hasTreeAnimated = !1),
        (this.layoutVersion = 0),
        (this.updateScheduled = !1),
        (this.scheduleUpdate = () => this.update()),
        (this.projectionUpdateScheduled = !1),
        (this.checkUpdateFailed = () => {
          this.isUpdating && ((this.isUpdating = !1), this.clearAllSnapshots());
        }),
        (this.updateProjection = () => {
          ((this.projectionUpdateScheduled = !1),
            this.nodes.forEach(BS),
            this.nodes.forEach(qS),
            this.nodes.forEach(YS),
            this.nodes.forEach(US));
        }),
        (this.resolvedRelativeTargetAt = 0),
        (this.linkedParentVersion = 0),
        (this.hasProjected = !1),
        (this.isVisible = !0),
        (this.animationProgress = 0),
        (this.sharedNodes = new Map()),
        (this.latestValues = d),
        (this.root = m ? m.root || m : this),
        (this.path = m ? [...m.path, m] : []),
        (this.parent = m),
        (this.depth = m ? m.depth + 1 : 0));
      for (let g = 0; g < this.path.length; g++) this.path[g].shouldResetTransform = !0;
      this.root === this && (this.nodes = new zS());
    }
    addEventListener(d, m) {
      return (
        this.eventHandlers.has(d) || this.eventHandlers.set(d, new Vc()),
        this.eventHandlers.get(d).add(m)
      );
    }
    notifyListeners(d, ...m) {
      const g = this.eventHandlers.get(d);
      g && g.notify(...m);
    }
    hasListeners(d) {
      return this.eventHandlers.has(d);
    }
    mount(d) {
      if (this.instance) return;
      ((this.isSVG = Ic(d) && !_2(d)), (this.instance = d));
      const { layoutId: m, layout: g, visualElement: p } = this.options;
      if (
        (p && !p.current && p.mount(d),
        this.root.nodes.add(this),
        this.parent && this.parent.children.add(this),
        this.root.hasTreeAnimated && (g || m) && (this.isLayoutDirty = !0),
        i)
      ) {
        let v,
          x = 0;
        const S = () => (this.root.updateBlockedByResize = !1);
        (zt.read(() => {
          x = window.innerWidth;
        }),
          i(d, () => {
            const N = window.innerWidth;
            N !== x &&
              ((x = N),
              (this.root.updateBlockedByResize = !0),
              v && v(),
              (v = jS(S, 250)),
              nu.hasAnimatedSinceResize &&
                ((nu.hasAnimatedSinceResize = !1), this.nodes.forEach(m0)));
          }));
      }
      (m && this.root.registerSharedNode(m, this),
        this.options.animate !== !1 &&
          p &&
          (m || g) &&
          this.addEventListener(
            "didUpdate",
            ({ delta: v, hasLayoutChanged: x, hasRelativeLayoutChanged: S, layout: N }) => {
              if (this.isTreeAnimationBlocked()) {
                ((this.target = void 0), (this.relativeTarget = void 0));
                return;
              }
              const w = this.options.transition || p.getDefaultTransition() || ZS,
                { onLayoutAnimationStart: U, onLayoutAnimationComplete: H } = p.getProps(),
                L = !this.targetLayout || !oy(this.targetLayout, N),
                Y = !x && S;
              if (
                this.options.layoutRoot ||
                this.resumeFrom ||
                Y ||
                (x && (L || !this.currentAnimation))
              ) {
                this.resumeFrom &&
                  ((this.resumingFrom = this.resumeFrom),
                  (this.resumingFrom.resumingFrom = void 0));
                const q = { ...Zc(w, "layout"), onPlay: U, onComplete: H };
                ((p.shouldReduceMotion || this.options.layoutRoot) &&
                  ((q.delay = 0), (q.type = !1)),
                  this.startAnimation(q),
                  this.setAnimationOrigin(v, Y));
              } else
                (x || m0(this),
                  this.isLead() && this.options.onExitComplete && this.options.onExitComplete());
              this.targetLayout = N;
            },
          ));
    }
    unmount() {
      (this.options.layoutId && this.willUpdate(), this.root.nodes.remove(this));
      const d = this.getStack();
      (d && d.remove(this),
        this.parent && this.parent.children.delete(this),
        (this.instance = void 0),
        this.eventHandlers.clear(),
        Pn(this.updateProjection));
    }
    blockUpdate() {
      this.updateManuallyBlocked = !0;
    }
    unblockUpdate() {
      this.updateManuallyBlocked = !1;
    }
    isUpdateBlocked() {
      return this.updateManuallyBlocked || this.updateBlockedByResize;
    }
    isTreeAnimationBlocked() {
      return this.isAnimationBlocked || (this.parent && this.parent.isTreeAnimationBlocked()) || !1;
    }
    startUpdate() {
      this.isUpdateBlocked() ||
        ((this.isUpdating = !0), this.nodes && this.nodes.forEach(GS), this.animationId++);
    }
    getTransformTemplate() {
      const { visualElement: d } = this.options;
      return d && d.getProps().transformTemplate;
    }
    willUpdate(d = !0) {
      if (((this.root.hasTreeAnimated = !0), this.root.isUpdateBlocked())) {
        this.options.onExitComplete && this.options.onExitComplete();
        return;
      }
      if (
        (window.MotionCancelOptimisedAnimation && !this.hasCheckedOptimisedAppear && fy(this),
        !this.root.isUpdating && this.root.startUpdate(),
        this.isLayoutDirty)
      )
        return;
      this.isLayoutDirty = !0;
      for (let v = 0; v < this.path.length; v++) {
        const x = this.path[v];
        ((x.shouldResetTransform = !0),
          (typeof x.latestValues.x == "string" || typeof x.latestValues.y == "string") &&
            (x.isLayoutDirty = !0),
          x.updateScroll("snapshot"),
          x.options.layoutRoot && x.willUpdate(!1));
      }
      const { layoutId: m, layout: g } = this.options;
      if (m === void 0 && !g) return;
      const p = this.getTransformTemplate();
      ((this.prevTransformTemplateValue = p ? p(this.latestValues, "") : void 0),
        this.updateSnapshot(),
        d && this.notifyListeners("willUpdate"));
    }
    update() {
      if (((this.updateScheduled = !1), this.isUpdateBlocked())) {
        (this.unblockUpdate(), this.clearAllSnapshots(), this.nodes.forEach(d0));
        return;
      }
      if (this.animationId <= this.animationCommitId) {
        this.nodes.forEach(h0);
        return;
      }
      ((this.animationCommitId = this.animationId),
        this.isUpdating
          ? ((this.isUpdating = !1),
            this.nodes.forEach(HS),
            this.nodes.forEach(VS),
            this.nodes.forEach(_S))
          : this.nodes.forEach(h0),
        this.clearAllSnapshots());
      const m = ce.now();
      ((le.delta = en(0, 1e3 / 60, m - le.timestamp)),
        (le.timestamp = m),
        (le.isProcessing = !0),
        Yr.update.process(le),
        Yr.preRender.process(le),
        Yr.render.process(le),
        (le.isProcessing = !1));
    }
    didUpdate() {
      this.updateScheduled || ((this.updateScheduled = !0), Wc.read(this.scheduleUpdate));
    }
    clearAllSnapshots() {
      (this.nodes.forEach(LS), this.sharedNodes.forEach(XS));
    }
    scheduleUpdateProjection() {
      this.projectionUpdateScheduled ||
        ((this.projectionUpdateScheduled = !0), zt.preRender(this.updateProjection, !1, !0));
    }
    scheduleCheckAfterUnmount() {
      zt.postRender(() => {
        this.isLayoutDirty ? this.root.didUpdate() : this.root.checkUpdateFailed();
      });
    }
    updateSnapshot() {
      this.snapshot ||
        !this.instance ||
        ((this.snapshot = this.measure()),
        this.snapshot &&
          !fe(this.snapshot.measuredBox.x) &&
          !fe(this.snapshot.measuredBox.y) &&
          (this.snapshot = void 0));
    }
    updateLayout() {
      if (
        !this.instance ||
        (this.updateScroll(),
        !(this.options.alwaysMeasureLayout && this.isLead()) && !this.isLayoutDirty)
      )
        return;
      if (this.resumeFrom && !this.resumeFrom.instance)
        for (let g = 0; g < this.path.length; g++) this.path[g].updateScroll();
      const d = this.layout;
      ((this.layout = this.measure(!1)),
        this.layoutVersion++,
        (this.layoutCorrected = Ft()),
        (this.isLayoutDirty = !1),
        (this.projectionDelta = void 0),
        this.notifyListeners("measure", this.layout.layoutBox));
      const { visualElement: m } = this.options;
      m && m.notify("LayoutMeasure", this.layout.layoutBox, d ? d.layoutBox : void 0);
    }
    updateScroll(d = "measure") {
      let m = !!(this.options.layoutScroll && this.instance);
      if (
        (this.scroll &&
          this.scroll.animationId === this.root.animationId &&
          this.scroll.phase === d &&
          (m = !1),
        m && this.instance)
      ) {
        const g = o(this.instance);
        this.scroll = {
          animationId: this.root.animationId,
          phase: d,
          isRoot: g,
          offset: u(this.instance),
          wasRoot: this.scroll ? this.scroll.isRoot : g,
        };
      }
    }
    resetTransform() {
      if (!c) return;
      const d = this.isLayoutDirty || this.shouldResetTransform || this.options.alwaysMeasureLayout,
        m = this.projectionDelta && !uy(this.projectionDelta),
        g = this.getTransformTemplate(),
        p = g ? g(this.latestValues, "") : void 0,
        v = p !== this.prevTransformTemplateValue;
      d &&
        this.instance &&
        (m || vi(this.latestValues) || v) &&
        (c(this.instance, p), (this.shouldResetTransform = !1), this.scheduleRender());
    }
    measure(d = !0) {
      const m = this.measurePageBox();
      let g = this.removeElementScroll(m);
      return (
        d && (g = this.removeTransform(g)),
        KS(g),
        {
          animationId: this.root.animationId,
          measuredBox: m,
          layoutBox: g,
          latestValues: {},
          source: this.id,
        }
      );
    }
    measurePageBox() {
      var p;
      const { visualElement: d } = this.options;
      if (!d) return Ft();
      const m = d.measureViewportBox();
      if (!(((p = this.scroll) == null ? void 0 : p.wasRoot) || this.path.some(JS))) {
        const { scroll: v } = this.root;
        v && (fa(m.x, v.offset.x), fa(m.y, v.offset.y));
      }
      return m;
    }
    removeElementScroll(d) {
      var g;
      const m = Ft();
      if ((Ze(m, d), (g = this.scroll) != null && g.wasRoot)) return m;
      for (let p = 0; p < this.path.length; p++) {
        const v = this.path[p],
          { scroll: x, options: S } = v;
        v !== this.root &&
          x &&
          S.layoutScroll &&
          (x.wasRoot && Ze(m, d), fa(m.x, x.offset.x), fa(m.y, x.offset.y));
      }
      return m;
    }
    applyTransform(d, m = !1) {
      var p, v;
      const g = Ft();
      Ze(g, d);
      for (let x = 0; x < this.path.length; x++) {
        const S = this.path[x];
        (!m &&
          S.options.layoutScroll &&
          S.scroll &&
          S !== S.root &&
          da(g, { x: -S.scroll.offset.x, y: -S.scroll.offset.y }),
          vi(S.latestValues) &&
            da(g, S.latestValues, (p = S.layout) == null ? void 0 : p.layoutBox));
      }
      return (
        vi(this.latestValues) &&
          da(g, this.latestValues, (v = this.layout) == null ? void 0 : v.layoutBox),
        g
      );
    }
    removeTransform(d) {
      var g;
      const m = Ft();
      Ze(m, d);
      for (let p = 0; p < this.path.length; p++) {
        const v = this.path[p];
        if (!vi(v.latestValues)) continue;
        let x;
        (v.instance &&
          (Ac(v.latestValues) && v.updateSnapshot(), (x = Ft()), Ze(x, v.measurePageBox())),
          i0(m, v.latestValues, (g = v.snapshot) == null ? void 0 : g.layoutBox, x));
      }
      return (vi(this.latestValues) && i0(m, this.latestValues), m);
    }
    setTargetDelta(d) {
      ((this.targetDelta = d), this.root.scheduleUpdateProjection(), (this.isProjectionDirty = !0));
    }
    setOptions(d) {
      this.options = {
        ...this.options,
        ...d,
        crossfade: d.crossfade !== void 0 ? d.crossfade : !0,
      };
    }
    clearMeasurements() {
      ((this.scroll = void 0),
        (this.layout = void 0),
        (this.snapshot = void 0),
        (this.prevTransformTemplateValue = void 0),
        (this.targetDelta = void 0),
        (this.target = void 0),
        (this.isLayoutDirty = !1));
    }
    forceRelativeParentToResolveTarget() {
      this.relativeParent &&
        this.relativeParent.resolvedRelativeTargetAt !== le.timestamp &&
        this.relativeParent.resolveTargetDelta(!0);
    }
    resolveTargetDelta(d = !1) {
      var N;
      const m = this.getLead();
      (this.isProjectionDirty || (this.isProjectionDirty = m.isProjectionDirty),
        this.isTransformDirty || (this.isTransformDirty = m.isTransformDirty),
        this.isSharedProjectionDirty || (this.isSharedProjectionDirty = m.isSharedProjectionDirty));
      const g = !!this.resumingFrom || this !== m;
      if (
        !(
          d ||
          (g && this.isSharedProjectionDirty) ||
          this.isProjectionDirty ||
          ((N = this.parent) != null && N.isProjectionDirty) ||
          this.attemptToResolveRelativeTarget ||
          this.root.updateBlockedByResize
        )
      )
        return;
      const { layout: v, layoutId: x } = this.options;
      if (!this.layout || !(v || x)) return;
      this.resolvedRelativeTargetAt = le.timestamp;
      const S = this.getClosestProjectingParent();
      (S &&
        this.linkedParentVersion !== S.layoutVersion &&
        !S.options.layoutRoot &&
        this.removeRelativeTarget(),
        !this.targetDelta &&
          !this.relativeTarget &&
          (S && S.layout
            ? this.createRelativeTarget(S, this.layout.layoutBox, S.layout.layoutBox)
            : this.removeRelativeTarget()),
        !(!this.relativeTarget && !this.targetDelta) &&
          (this.target || ((this.target = Ft()), (this.targetWithTransforms = Ft())),
          this.relativeTarget &&
          this.relativeTargetOrigin &&
          this.relativeParent &&
          this.relativeParent.target
            ? (this.forceRelativeParentToResolveTarget(),
              yS(this.target, this.relativeTarget, this.relativeParent.target))
            : this.targetDelta
              ? (this.resumingFrom
                  ? (this.target = this.applyTransform(this.layout.layoutBox))
                  : Ze(this.target, this.layout.layoutBox),
                Fg(this.target, this.targetDelta))
              : Ze(this.target, this.layout.layoutBox),
          this.attemptToResolveRelativeTarget &&
            ((this.attemptToResolveRelativeTarget = !1),
            S &&
            !!S.resumingFrom == !!this.resumingFrom &&
            !S.options.layoutScroll &&
            S.target &&
            this.animationProgress !== 1
              ? this.createRelativeTarget(S, this.target, S.target)
              : (this.relativeParent = this.relativeTarget = void 0))));
    }
    getClosestProjectingParent() {
      if (!(!this.parent || Ac(this.parent.latestValues) || Jg(this.parent.latestValues)))
        return this.parent.isProjecting() ? this.parent : this.parent.getClosestProjectingParent();
    }
    isProjecting() {
      return !!(
        (this.relativeTarget || this.targetDelta || this.options.layoutRoot) &&
        this.layout
      );
    }
    createRelativeTarget(d, m, g) {
      ((this.relativeParent = d),
        (this.linkedParentVersion = d.layoutVersion),
        this.forceRelativeParentToResolveTarget(),
        (this.relativeTarget = Ft()),
        (this.relativeTargetOrigin = Ft()),
        cu(this.relativeTargetOrigin, m, g),
        Ze(this.relativeTarget, this.relativeTargetOrigin));
    }
    removeRelativeTarget() {
      this.relativeParent = this.relativeTarget = void 0;
    }
    calcProjection() {
      var w;
      const d = this.getLead(),
        m = !!this.resumingFrom || this !== d;
      let g = !0;
      if (
        ((this.isProjectionDirty || ((w = this.parent) != null && w.isProjectionDirty)) && (g = !1),
        m && (this.isSharedProjectionDirty || this.isTransformDirty) && (g = !1),
        this.resolvedRelativeTargetAt === le.timestamp && (g = !1),
        g)
      )
        return;
      const { layout: p, layoutId: v } = this.options;
      if (
        ((this.isTreeAnimating = !!(
          (this.parent && this.parent.isTreeAnimating) ||
          this.currentAnimation ||
          this.pendingAnimation
        )),
        this.isTreeAnimating || (this.targetDelta = this.relativeTarget = void 0),
        !this.layout || !(p || v))
      )
        return;
      Ze(this.layoutCorrected, this.layout.layoutBox);
      const x = this.treeScale.x,
        S = this.treeScale.y;
      (Z2(this.layoutCorrected, this.treeScale, this.path, m),
        d.layout &&
          !d.target &&
          (this.treeScale.x !== 1 || this.treeScale.y !== 1) &&
          ((d.target = d.layout.layoutBox), (d.targetWithTransforms = Ft())));
      const { target: N } = d;
      if (!N) {
        this.prevProjectionDelta && (this.createProjectionDeltas(), this.scheduleRender());
        return;
      }
      (!this.projectionDelta || !this.prevProjectionDelta
        ? this.createProjectionDeltas()
        : (Wp(this.prevProjectionDelta.x, this.projectionDelta.x),
          Wp(this.prevProjectionDelta.y, this.projectionDelta.y)),
        yl(this.projectionDelta, this.layoutCorrected, N, this.latestValues),
        (this.treeScale.x !== x ||
          this.treeScale.y !== S ||
          !o0(this.projectionDelta.x, this.prevProjectionDelta.x) ||
          !o0(this.projectionDelta.y, this.prevProjectionDelta.y)) &&
          ((this.hasProjected = !0),
          this.scheduleRender(),
          this.notifyListeners("projectionUpdate", N)));
    }
    hide() {
      this.isVisible = !1;
    }
    show() {
      this.isVisible = !0;
    }
    scheduleRender(d = !0) {
      var m;
      if (((m = this.options.visualElement) == null || m.scheduleRender(), d)) {
        const g = this.getStack();
        g && g.scheduleRender();
      }
      this.resumingFrom && !this.resumingFrom.instance && (this.resumingFrom = void 0);
    }
    createProjectionDeltas() {
      ((this.prevProjectionDelta = ca()),
        (this.projectionDelta = ca()),
        (this.projectionDeltaWithTransform = ca()));
    }
    setAnimationOrigin(d, m = !1) {
      const g = this.snapshot,
        p = g ? g.latestValues : {},
        v = { ...this.latestValues },
        x = ca();
      ((!this.relativeParent || !this.relativeParent.options.layoutRoot) &&
        (this.relativeTarget = this.relativeTargetOrigin = void 0),
        (this.attemptToResolveRelativeTarget = !m));
      const S = Ft(),
        N = g ? g.source : void 0,
        w = this.layout ? this.layout.source : void 0,
        U = N !== w,
        H = this.getStack(),
        L = !H || H.members.length <= 1,
        Y = !!(U && !L && this.options.crossfade === !0 && !this.path.some(QS));
      this.animationProgress = 0;
      let q;
      ((this.mixTargetDelta = (G) => {
        const F = G / 1e3;
        (p0(x.x, d.x, F),
          p0(x.y, d.y, F),
          this.setTargetDelta(x),
          this.relativeTarget &&
            this.relativeTargetOrigin &&
            this.layout &&
            this.relativeParent &&
            this.relativeParent.layout &&
            (cu(S, this.layout.layoutBox, this.relativeParent.layout.layoutBox),
            kS(this.relativeTarget, this.relativeTargetOrigin, S, F),
            q && SS(this.relativeTarget, q) && (this.isProjectionDirty = !1),
            q || (q = Ft()),
            Ze(q, this.relativeTarget)),
          U && ((this.animationValues = v), ES(v, p, this.latestValues, F, Y, L)),
          this.root.scheduleUpdateProjection(),
          this.scheduleRender(),
          (this.animationProgress = F));
      }),
        this.mixTargetDelta(this.options.layoutRoot ? 1e3 : 0));
    }
    startAnimation(d) {
      var m, g, p;
      (this.notifyListeners("animationStart"),
        (m = this.currentAnimation) == null || m.stop(),
        (p = (g = this.resumingFrom) == null ? void 0 : g.currentAnimation) == null || p.stop(),
        this.pendingAnimation && (Pn(this.pendingAnimation), (this.pendingAnimation = void 0)),
        (this.pendingAnimation = zt.update(() => {
          ((nu.hasAnimatedSinceResize = !0),
            this.motionValue || (this.motionValue = ma(0)),
            this.motionValue.jump(0, !1),
            (this.currentAnimation = CS(this.motionValue, [0, 1e3], {
              ...d,
              velocity: 0,
              isSync: !0,
              onUpdate: (v) => {
                (this.mixTargetDelta(v), d.onUpdate && d.onUpdate(v));
              },
              onStop: () => {},
              onComplete: () => {
                (d.onComplete && d.onComplete(), this.completeAnimation());
              },
            })),
            this.resumingFrom && (this.resumingFrom.currentAnimation = this.currentAnimation),
            (this.pendingAnimation = void 0));
        })));
    }
    completeAnimation() {
      this.resumingFrom &&
        ((this.resumingFrom.currentAnimation = void 0),
        (this.resumingFrom.preserveOpacity = void 0));
      const d = this.getStack();
      (d && d.exitAnimationComplete(),
        (this.resumingFrom = this.currentAnimation = this.animationValues = void 0),
        this.notifyListeners("animationComplete"));
    }
    finishAnimation() {
      (this.currentAnimation &&
        (this.mixTargetDelta && this.mixTargetDelta(RS), this.currentAnimation.stop()),
        this.completeAnimation());
    }
    applyTransformsToTarget() {
      const d = this.getLead();
      let { targetWithTransforms: m, target: g, layout: p, latestValues: v } = d;
      if (!(!m || !g || !p)) {
        if (
          this !== d &&
          this.layout &&
          p &&
          hy(this.options.animationType, this.layout.layoutBox, p.layoutBox)
        ) {
          g = this.target || Ft();
          const x = fe(this.layout.layoutBox.x);
          ((g.x.min = d.target.x.min), (g.x.max = g.x.min + x));
          const S = fe(this.layout.layoutBox.y);
          ((g.y.min = d.target.y.min), (g.y.max = g.y.min + S));
        }
        (Ze(m, g), da(m, v), yl(this.projectionDeltaWithTransform, this.layoutCorrected, m, v));
      }
    }
    registerSharedNode(d, m) {
      (this.sharedNodes.has(d) || this.sharedNodes.set(d, new wS()),
        this.sharedNodes.get(d).add(m));
      const p = m.options.initialPromotionConfig;
      m.promote({
        transition: p ? p.transition : void 0,
        preserveFollowOpacity:
          p && p.shouldPreserveFollowOpacity ? p.shouldPreserveFollowOpacity(m) : void 0,
      });
    }
    isLead() {
      const d = this.getStack();
      return d ? d.lead === this : !0;
    }
    getLead() {
      var m;
      const { layoutId: d } = this.options;
      return d ? ((m = this.getStack()) == null ? void 0 : m.lead) || this : this;
    }
    getPrevLead() {
      var m;
      const { layoutId: d } = this.options;
      return d ? ((m = this.getStack()) == null ? void 0 : m.prevLead) : void 0;
    }
    getStack() {
      const { layoutId: d } = this.options;
      if (d) return this.root.sharedNodes.get(d);
    }
    promote({ needsReset: d, transition: m, preserveFollowOpacity: g } = {}) {
      const p = this.getStack();
      (p && p.promote(this, g),
        d && ((this.projectionDelta = void 0), (this.needsReset = !0)),
        m && this.setOptions({ transition: m }));
    }
    relegate() {
      const d = this.getStack();
      return d ? d.relegate(this) : !1;
    }
    resetSkewAndRotation() {
      const { visualElement: d } = this.options;
      if (!d) return;
      let m = !1;
      const { latestValues: g } = d;
      if (
        ((g.z || g.rotate || g.rotateX || g.rotateY || g.rotateZ || g.skewX || g.skewY) && (m = !0),
        !m)
      )
        return;
      const p = {};
      g.z && Pr("z", d, p, this.animationValues);
      for (let v = 0; v < Fr.length; v++)
        (Pr(`rotate${Fr[v]}`, d, p, this.animationValues),
          Pr(`skew${Fr[v]}`, d, p, this.animationValues));
      d.render();
      for (const v in p)
        (d.setStaticValue(v, p[v]), this.animationValues && (this.animationValues[v] = p[v]));
      d.scheduleRender();
    }
    applyProjectionStyles(d, m) {
      if (!this.instance || this.isSVG) return;
      if (!this.isVisible) {
        d.visibility = "hidden";
        return;
      }
      const g = this.getTransformTemplate();
      if (this.needsReset) {
        ((this.needsReset = !1),
          (d.visibility = ""),
          (d.opacity = ""),
          (d.pointerEvents = eu(m == null ? void 0 : m.pointerEvents) || ""),
          (d.transform = g ? g(this.latestValues, "") : "none"));
        return;
      }
      const p = this.getLead();
      if (!this.projectionDelta || !this.layout || !p.target) {
        (this.options.layoutId &&
          ((d.opacity = this.latestValues.opacity !== void 0 ? this.latestValues.opacity : 1),
          (d.pointerEvents = eu(m == null ? void 0 : m.pointerEvents) || "")),
          this.hasProjected &&
            !vi(this.latestValues) &&
            ((d.transform = g ? g({}, "") : "none"), (this.hasProjected = !1)));
        return;
      }
      d.visibility = "";
      const v = p.animationValues || p.latestValues;
      this.applyTransformsToTarget();
      let x = TS(this.projectionDeltaWithTransform, this.treeScale, v);
      (g && (x = g(v, x)), (d.transform = x));
      const { x: S, y: N } = this.projectionDelta;
      ((d.transformOrigin = `${S.origin * 100}% ${N.origin * 100}% 0`),
        p.animationValues
          ? (d.opacity =
              p === this
                ? (v.opacity ?? this.latestValues.opacity ?? 1)
                : this.preserveOpacity
                  ? this.latestValues.opacity
                  : v.opacityExit)
          : (d.opacity =
              p === this
                ? v.opacity !== void 0
                  ? v.opacity
                  : ""
                : v.opacityExit !== void 0
                  ? v.opacityExit
                  : 0));
      for (const w in Mc) {
        if (v[w] === void 0) continue;
        const { correct: U, applyTo: H, isCSSVariable: L } = Mc[w],
          Y = x === "none" ? v[w] : U(v[w], p);
        if (H) {
          const q = H.length;
          for (let G = 0; G < q; G++) d[H[G]] = Y;
        } else L ? (this.options.visualElement.renderState.vars[w] = Y) : (d[w] = Y);
      }
      this.options.layoutId &&
        (d.pointerEvents = p === this ? eu(m == null ? void 0 : m.pointerEvents) || "" : "none");
    }
    clearSnapshot() {
      this.resumeFrom = this.snapshot = void 0;
    }
    resetTree() {
      (this.root.nodes.forEach((d) => {
        var m;
        return (m = d.currentAnimation) == null ? void 0 : m.stop();
      }),
        this.root.nodes.forEach(d0),
        this.root.sharedNodes.clear());
    }
  };
}
function VS(i) {
  i.updateLayout();
}
function _S(i) {
  var u;
  const l = ((u = i.resumeFrom) == null ? void 0 : u.snapshot) || i.snapshot;
  if (i.isLead() && i.layout && l && i.hasListeners("didUpdate")) {
    const { layoutBox: o, measuredBox: c } = i.layout,
      { animationType: h } = i.options,
      d = l.source !== i.layout.source;
    if (h === "size")
      Ie((x) => {
        const S = d ? l.measuredBox[x] : l.layoutBox[x],
          N = fe(S);
        ((S.min = o[x].min), (S.max = S.min + N));
      });
    else if (h === "x" || h === "y") {
      const x = h === "x" ? "y" : "x";
      Dc(d ? l.measuredBox[x] : l.layoutBox[x], o[x]);
    } else
      hy(h, l.layoutBox, o) &&
        Ie((x) => {
          const S = d ? l.measuredBox[x] : l.layoutBox[x],
            N = fe(o[x]);
          ((S.max = S.min + N),
            i.relativeTarget &&
              !i.currentAnimation &&
              ((i.isProjectionDirty = !0),
              (i.relativeTarget[x].max = i.relativeTarget[x].min + N)));
        });
    const m = ca();
    yl(m, o, l.layoutBox);
    const g = ca();
    d ? yl(g, i.applyTransform(c, !0), l.measuredBox) : yl(g, o, l.layoutBox);
    const p = !uy(m);
    let v = !1;
    if (!i.resumeFrom) {
      const x = i.getClosestProjectingParent();
      if (x && !x.resumeFrom) {
        const { snapshot: S, layout: N } = x;
        if (S && N) {
          const w = Ft();
          cu(w, l.layoutBox, S.layoutBox);
          const U = Ft();
          (cu(U, o, N.layoutBox),
            oy(w, U) || (v = !0),
            x.options.layoutRoot &&
              ((i.relativeTarget = U), (i.relativeTargetOrigin = w), (i.relativeParent = x)));
        }
      }
    }
    i.notifyListeners("didUpdate", {
      layout: o,
      snapshot: l,
      delta: g,
      layoutDelta: m,
      hasLayoutChanged: p,
      hasRelativeLayoutChanged: v,
    });
  } else if (i.isLead()) {
    const { onExitComplete: o } = i.options;
    o && o();
  }
  i.options.transition = void 0;
}
function BS(i) {
  i.parent &&
    (i.isProjecting() || (i.isProjectionDirty = i.parent.isProjectionDirty),
    i.isSharedProjectionDirty ||
      (i.isSharedProjectionDirty = !!(
        i.isProjectionDirty ||
        i.parent.isProjectionDirty ||
        i.parent.isSharedProjectionDirty
      )),
    i.isTransformDirty || (i.isTransformDirty = i.parent.isTransformDirty));
}
function US(i) {
  i.isProjectionDirty = i.isSharedProjectionDirty = i.isTransformDirty = !1;
}
function LS(i) {
  i.clearSnapshot();
}
function d0(i) {
  i.clearMeasurements();
}
function h0(i) {
  i.isLayoutDirty = !1;
}
function HS(i) {
  const { visualElement: l } = i.options;
  (l && l.getProps().onBeforeLayoutMeasure && l.notify("BeforeLayoutMeasure"), i.resetTransform());
}
function m0(i) {
  (i.finishAnimation(),
    (i.targetDelta = i.relativeTarget = i.target = void 0),
    (i.isProjectionDirty = !0));
}
function qS(i) {
  i.resolveTargetDelta();
}
function YS(i) {
  i.calcProjection();
}
function GS(i) {
  i.resetSkewAndRotation();
}
function XS(i) {
  i.removeLeadSnapshot();
}
function p0(i, l, u) {
  ((i.translate = Ot(l.translate, 0, u)),
    (i.scale = Ot(l.scale, 1, u)),
    (i.origin = l.origin),
    (i.originPoint = l.originPoint));
}
function g0(i, l, u, o) {
  ((i.min = Ot(l.min, u.min, o)), (i.max = Ot(l.max, u.max, o)));
}
function kS(i, l, u, o) {
  (g0(i.x, l.x, u.x, o), g0(i.y, l.y, u.y, o));
}
function QS(i) {
  return i.animationValues && i.animationValues.opacityExit !== void 0;
}
const ZS = { duration: 0.45, ease: [0.4, 0, 0.1, 1] },
  y0 = (i) =>
    typeof navigator < "u" && navigator.userAgent && navigator.userAgent.toLowerCase().includes(i),
  v0 = y0("applewebkit/") && !y0("chrome/") ? Math.round : Ge;
function x0(i) {
  ((i.min = v0(i.min)), (i.max = v0(i.max)));
}
function KS(i) {
  (x0(i.x), x0(i.y));
}
function hy(i, l, u) {
  return i === "position" || (i === "preserve-aspect" && !gS(u0(l), u0(u), 0.2));
}
function JS(i) {
  var l;
  return i !== i.root && ((l = i.scroll) == null ? void 0 : l.wasRoot);
}
const FS = dy({
    attachResizeListener: (i, l) => Tl(i, "resize", l),
    measureScroll: () => {
      var i, l;
      return {
        x:
          document.documentElement.scrollLeft ||
          ((i = document.body) == null ? void 0 : i.scrollLeft) ||
          0,
        y:
          document.documentElement.scrollTop ||
          ((l = document.body) == null ? void 0 : l.scrollTop) ||
          0,
      };
    },
    checkIsScrollRoot: () => !0,
  }),
  Wr = { current: void 0 },
  my = dy({
    measureScroll: (i) => ({ x: i.scrollLeft, y: i.scrollTop }),
    defaultParent: () => {
      if (!Wr.current) {
        const i = new FS({});
        (i.mount(window), i.setOptions({ layoutScroll: !0 }), (Wr.current = i));
      }
      return Wr.current;
    },
    resetTransform: (i, l) => {
      i.style.transform = l !== void 0 ? l : "none";
    },
    checkIsScrollRoot: (i) => window.getComputedStyle(i).position === "fixed",
  }),
  lf = k.createContext({ transformPagePoint: (i) => i, isStatic: !1, reducedMotion: "never" });
function b0(i, l) {
  if (typeof i == "function") return i(l);
  i != null && (i.current = l);
}
function PS(...i) {
  return (l) => {
    let u = !1;
    const o = i.map((c) => {
      const h = b0(c, l);
      return (!u && typeof h == "function" && (u = !0), h);
    });
    if (u)
      return () => {
        for (let c = 0; c < o.length; c++) {
          const h = o[c];
          typeof h == "function" ? h() : b0(i[c], null);
        }
      };
  };
}
function WS(...i) {
  return k.useCallback(PS(...i), i);
}
class $S extends k.Component {
  getSnapshotBeforeUpdate(l) {
    const u = this.props.childRef.current;
    if (Ws(u) && l.isPresent && !this.props.isPresent && this.props.pop !== !1) {
      const o = u.offsetParent,
        c = (Ws(o) && o.offsetWidth) || 0,
        h = (Ws(o) && o.offsetHeight) || 0,
        d = getComputedStyle(u),
        m = this.props.sizeRef.current;
      ((m.height = parseFloat(d.height)),
        (m.width = parseFloat(d.width)),
        (m.top = u.offsetTop),
        (m.left = u.offsetLeft),
        (m.right = c - m.width - m.left),
        (m.bottom = h - m.height - m.top));
    }
    return null;
  }
  componentDidUpdate() {}
  render() {
    return this.props.children;
  }
}
function IS({ children: i, isPresent: l, anchorX: u, anchorY: o, root: c, pop: h }) {
  var S;
  const d = k.useId(),
    m = k.useRef(null),
    g = k.useRef({ width: 0, height: 0, top: 0, left: 0, right: 0, bottom: 0 }),
    { nonce: p } = k.useContext(lf),
    v = ((S = i.props) == null ? void 0 : S.ref) ?? (i == null ? void 0 : i.ref),
    x = WS(m, v);
  return (
    k.useInsertionEffect(() => {
      const { width: N, height: w, top: U, left: H, right: L, bottom: Y } = g.current;
      if (l || h === !1 || !m.current || !N || !w) return;
      const q = u === "left" ? `left: ${H}` : `right: ${L}`,
        G = o === "bottom" ? `bottom: ${Y}` : `top: ${U}`;
      m.current.dataset.motionPopId = d;
      const F = document.createElement("style");
      p && (F.nonce = p);
      const lt = c ?? document.head;
      return (
        lt.appendChild(F),
        F.sheet &&
          F.sheet.insertRule(`
          [data-motion-pop-id="${d}"] {
            position: absolute !important;
            width: ${N}px !important;
            height: ${w}px !important;
            ${q}px !important;
            ${G}px !important;
          }
        `),
        () => {
          var I;
          ((I = m.current) == null || I.removeAttribute("data-motion-pop-id"),
            lt.contains(F) && lt.removeChild(F));
        }
      );
    }, [l]),
    b.jsx($S, {
      isPresent: l,
      childRef: m,
      sizeRef: g,
      pop: h,
      children: h === !1 ? i : k.cloneElement(i, { ref: x }),
    })
  );
}
const tT = ({
  children: i,
  initial: l,
  isPresent: u,
  onExitComplete: o,
  custom: c,
  presenceAffectsLayout: h,
  mode: d,
  anchorX: m,
  anchorY: g,
  root: p,
}) => {
  const v = wc(eT),
    x = k.useId();
  let S = !0,
    N = k.useMemo(
      () => (
        (S = !1),
        {
          id: x,
          initial: l,
          isPresent: u,
          custom: c,
          onExitComplete: (w) => {
            v.set(w, !0);
            for (const U of v.values()) if (!U) return;
            o && o();
          },
          register: (w) => (v.set(w, !1), () => v.delete(w)),
        }
      ),
      [u, v, o],
    );
  return (
    h && S && (N = { ...N }),
    k.useMemo(() => {
      v.forEach((w, U) => v.set(U, !1));
    }, [u]),
    k.useEffect(() => {
      !u && !v.size && o && o();
    }, [u]),
    (i = b.jsx(IS, {
      pop: d === "popLayout",
      isPresent: u,
      anchorX: m,
      anchorY: g,
      root: p,
      children: i,
    })),
    b.jsx(du.Provider, { value: N, children: i })
  );
};
function eT() {
  return new Map();
}
function py(i = !0) {
  const l = k.useContext(du);
  if (l === null) return [!0, null];
  const { isPresent: u, onExitComplete: o, register: c } = l,
    h = k.useId();
  k.useEffect(() => {
    if (i) return c(h);
  }, [i]);
  const d = k.useCallback(() => i && o && o(h), [h, o, i]);
  return !u && o ? [!1, d] : [!0];
}
const Ks = (i) => i.key || "";
function S0(i) {
  const l = [];
  return (
    k.Children.forEach(i, (u) => {
      k.isValidElement(u) && l.push(u);
    }),
    l
  );
}
const T0 = ({
    children: i,
    custom: l,
    initial: u = !0,
    onExitComplete: o,
    presenceAffectsLayout: c = !0,
    mode: h = "sync",
    propagate: d = !1,
    anchorX: m = "left",
    anchorY: g = "top",
    root: p,
  }) => {
    const [v, x] = py(d),
      S = k.useMemo(() => S0(i), [i]),
      N = d && !v ? [] : S.map(Ks),
      w = k.useRef(!0),
      U = k.useRef(S),
      H = wc(() => new Map()),
      L = k.useRef(new Set()),
      [Y, q] = k.useState(S),
      [G, F] = k.useState(S);
    X0(() => {
      ((w.current = !1), (U.current = S));
      for (let $ = 0; $ < G.length; $++) {
        const it = Ks(G[$]);
        N.includes(it) ? (H.delete(it), L.current.delete(it)) : H.get(it) !== !0 && H.set(it, !1);
      }
    }, [G, N.length, N.join("-")]);
    const lt = [];
    if (S !== Y) {
      let $ = [...S];
      for (let it = 0; it < G.length; it++) {
        const tt = G[it],
          pt = Ks(tt);
        N.includes(pt) || ($.splice(it, 0, tt), lt.push(tt));
      }
      return (h === "wait" && lt.length && ($ = lt), F(S0($)), q(S), null);
    }
    const { forceRender: I } = k.useContext(jc);
    return b.jsx(b.Fragment, {
      children: G.map(($) => {
        const it = Ks($),
          tt = d && !v ? !1 : S === G || N.includes(it),
          pt = () => {
            if (L.current.has(it)) return;
            if (H.has(it)) (L.current.add(it), H.set(it, !0));
            else return;
            let vt = !0;
            (H.forEach(($t) => {
              $t || (vt = !1);
            }),
              vt && (I == null || I(), F(U.current), d && (x == null || x()), o && o()));
          };
        return b.jsx(
          tT,
          {
            isPresent: tt,
            initial: !w.current || u ? void 0 : !1,
            custom: l,
            presenceAffectsLayout: c,
            mode: h,
            root: p,
            onExitComplete: tt ? void 0 : pt,
            anchorX: m,
            anchorY: g,
            children: $,
          },
          it,
        );
      }),
    });
  },
  gy = k.createContext({ strict: !1 }),
  A0 = {
    animation: [
      "animate",
      "variants",
      "whileHover",
      "whileTap",
      "exit",
      "whileInView",
      "whileFocus",
      "whileDrag",
    ],
    exit: ["exit"],
    drag: ["drag", "dragControls"],
    focus: ["whileFocus"],
    hover: ["whileHover", "onHoverStart", "onHoverEnd"],
    tap: ["whileTap", "onTap", "onTapStart", "onTapCancel"],
    pan: ["onPan", "onPanStart", "onPanSessionStart", "onPanEnd"],
    inView: ["whileInView", "onViewportEnter", "onViewportLeave"],
    layout: ["layout", "layoutId"],
  };
let E0 = !1;
function nT() {
  if (E0) return;
  const i = {};
  for (const l in A0) i[l] = { isEnabled: (u) => A0[l].some((o) => !!u[o]) };
  (Qg(i), (E0 = !0));
}
function yy() {
  return (nT(), G2());
}
function iT(i) {
  const l = yy();
  for (const u in i) l[u] = { ...l[u], ...i[u] };
  Qg(l);
}
const aT = new Set([
  "animate",
  "exit",
  "variants",
  "initial",
  "style",
  "values",
  "variants",
  "transition",
  "transformTemplate",
  "custom",
  "inherit",
  "onBeforeLayoutMeasure",
  "onAnimationStart",
  "onAnimationComplete",
  "onUpdate",
  "onDragStart",
  "onDrag",
  "onDragEnd",
  "onMeasureDragConstraints",
  "onDirectionLock",
  "onDragTransitionEnd",
  "_dragX",
  "_dragY",
  "onHoverStart",
  "onHoverEnd",
  "onViewportEnter",
  "onViewportLeave",
  "globalTapTarget",
  "propagate",
  "ignoreStrict",
  "viewport",
]);
function fu(i) {
  return (
    i.startsWith("while") ||
    (i.startsWith("drag") && i !== "draggable") ||
    i.startsWith("layout") ||
    i.startsWith("onTap") ||
    i.startsWith("onPan") ||
    i.startsWith("onLayout") ||
    aT.has(i)
  );
}
let vy = (i) => !fu(i);
function lT(i) {
  typeof i == "function" && (vy = (l) => (l.startsWith("on") ? !fu(l) : i(l)));
}
try {
  lT(require("@emotion/is-prop-valid").default);
} catch {}
function sT(i, l, u) {
  const o = {};
  for (const c in i)
    (c === "values" && typeof i.values == "object") ||
      se(i[c]) ||
      ((vy(c) ||
        (u === !0 && fu(c)) ||
        (!l && !fu(c)) ||
        (i.draggable && c.startsWith("onDrag"))) &&
        (o[c] = i[c]));
  return o;
}
const pu = k.createContext({});
function uT(i, l) {
  if (mu(i)) {
    const { initial: u, animate: o } = i;
    return { initial: u === !1 || Sl(u) ? u : void 0, animate: Sl(o) ? o : void 0 };
  }
  return i.inherit !== !1 ? l : {};
}
function oT(i) {
  const { initial: l, animate: u } = uT(i, k.useContext(pu));
  return k.useMemo(() => ({ initial: l, animate: u }), [M0(l), M0(u)]);
}
function M0(i) {
  return Array.isArray(i) ? i.join(" ") : i;
}
const sf = () => ({ style: {}, transform: {}, transformOrigin: {}, vars: {} });
function xy(i, l, u) {
  for (const o in l) !se(l[o]) && !$g(o, u) && (i[o] = l[o]);
}
function rT({ transformTemplate: i }, l) {
  return k.useMemo(() => {
    const u = sf();
    return (nf(u, l, i), Object.assign({}, u.vars, u.style));
  }, [l]);
}
function cT(i, l) {
  const u = i.style || {},
    o = {};
  return (xy(o, u, i), Object.assign(o, rT(i, l)), o);
}
function fT(i, l) {
  const u = {},
    o = cT(i, l);
  return (
    i.drag &&
      i.dragListener !== !1 &&
      ((u.draggable = !1),
      (o.userSelect = o.WebkitUserSelect = o.WebkitTouchCallout = "none"),
      (o.touchAction = i.drag === !0 ? "none" : `pan-${i.drag === "x" ? "y" : "x"}`)),
    i.tabIndex === void 0 && (i.onTap || i.onTapStart || i.whileTap) && (u.tabIndex = 0),
    (u.style = o),
    u
  );
}
const by = () => ({ ...sf(), attrs: {} });
function dT(i, l, u, o) {
  const c = k.useMemo(() => {
    const h = by();
    return (Ig(h, l, ey(o), i.transformTemplate, i.style), { ...h.attrs, style: { ...h.style } });
  }, [l]);
  if (i.style) {
    const h = {};
    (xy(h, i.style, i), (c.style = { ...h, ...c.style }));
  }
  return c;
}
const hT = [
  "animate",
  "circle",
  "defs",
  "desc",
  "ellipse",
  "g",
  "image",
  "line",
  "filter",
  "marker",
  "mask",
  "metadata",
  "path",
  "pattern",
  "polygon",
  "polyline",
  "rect",
  "stop",
  "switch",
  "symbol",
  "svg",
  "text",
  "tspan",
  "use",
  "view",
];
function uf(i) {
  return typeof i != "string" || i.includes("-") ? !1 : !!(hT.indexOf(i) > -1 || /[A-Z]/u.test(i));
}
function mT(i, l, u, { latestValues: o }, c, h = !1, d) {
  const g = ((d ?? uf(i)) ? dT : fT)(l, o, c, i),
    p = sT(l, typeof i == "string", h),
    v = i !== k.Fragment ? { ...p, ...g, ref: u } : {},
    { children: x } = l,
    S = k.useMemo(() => (se(x) ? x.get() : x), [x]);
  return k.createElement(i, { ...v, children: S });
}
function pT({ scrapeMotionValuesFromProps: i, createRenderState: l }, u, o, c) {
  return { latestValues: gT(u, o, c, i), renderState: l() };
}
function gT(i, l, u, o) {
  const c = {},
    h = o(i, {});
  for (const S in h) c[S] = eu(h[S]);
  let { initial: d, animate: m } = i;
  const g = mu(i),
    p = Xg(i);
  l &&
    p &&
    !g &&
    i.inherit !== !1 &&
    (d === void 0 && (d = l.initial), m === void 0 && (m = l.animate));
  let v = u ? u.initial === !1 : !1;
  v = v || d === !1;
  const x = v ? m : d;
  if (x && typeof x != "boolean" && !hu(x)) {
    const S = Array.isArray(x) ? x : [x];
    for (let N = 0; N < S.length; N++) {
      const w = Jc(i, S[N]);
      if (w) {
        const { transitionEnd: U, transition: H, ...L } = w;
        for (const Y in L) {
          let q = L[Y];
          if (Array.isArray(q)) {
            const G = v ? q.length - 1 : 0;
            q = q[G];
          }
          q !== null && (c[Y] = q);
        }
        for (const Y in U) c[Y] = U[Y];
      }
    }
  }
  return c;
}
const Sy = (i) => (l, u) => {
    const o = k.useContext(pu),
      c = k.useContext(du),
      h = () => pT(i, l, o, c);
    return u ? h() : wc(h);
  },
  yT = Sy({ scrapeMotionValuesFromProps: af, createRenderState: sf }),
  vT = Sy({ scrapeMotionValuesFromProps: ny, createRenderState: by }),
  xT = Symbol.for("motionComponentSymbol");
function bT(i, l, u) {
  const o = k.useRef(u);
  k.useInsertionEffect(() => {
    o.current = u;
  });
  const c = k.useRef(null);
  return k.useCallback(
    (h) => {
      var m;
      h && ((m = i.onMount) == null || m.call(i, h));
      const d = o.current;
      if (typeof d == "function")
        if (h) {
          const g = d(h);
          typeof g == "function" && (c.current = g);
        } else c.current ? (c.current(), (c.current = null)) : d(h);
      else d && (d.current = h);
      l && (h ? l.mount(h) : l.unmount());
    },
    [l],
  );
}
const Ty = k.createContext({});
function ua(i) {
  return i && typeof i == "object" && Object.prototype.hasOwnProperty.call(i, "current");
}
function ST(i, l, u, o, c, h) {
  var q, G;
  const { visualElement: d } = k.useContext(pu),
    m = k.useContext(gy),
    g = k.useContext(du),
    p = k.useContext(lf),
    v = p.reducedMotion,
    x = p.skipAnimations,
    S = k.useRef(null),
    N = k.useRef(!1);
  ((o = o || m.renderer),
    !S.current &&
      o &&
      ((S.current = o(i, {
        visualState: l,
        parent: d,
        props: u,
        presenceContext: g,
        blockInitialAnimation: g ? g.initial === !1 : !1,
        reducedMotionConfig: v,
        skipAnimations: x,
        isSVG: h,
      })),
      N.current && S.current && (S.current.manuallyAnimateOnMount = !0)));
  const w = S.current,
    U = k.useContext(Ty);
  w && !w.projection && c && (w.type === "html" || w.type === "svg") && TT(S.current, u, c, U);
  const H = k.useRef(!1);
  k.useInsertionEffect(() => {
    w && H.current && w.update(u, g);
  });
  const L = u[jg],
    Y = k.useRef(
      !!L &&
        typeof window < "u" &&
        !((q = window.MotionHandoffIsComplete) != null && q.call(window, L)) &&
        ((G = window.MotionHasOptimisedAnimation) == null ? void 0 : G.call(window, L)),
    );
  return (
    X0(() => {
      ((N.current = !0),
        w &&
          ((H.current = !0),
          (window.MotionIsMounted = !0),
          w.updateFeatures(),
          w.scheduleRenderMicrotask(),
          Y.current && w.animationState && w.animationState.animateChanges()));
    }),
    k.useEffect(() => {
      w &&
        (!Y.current && w.animationState && w.animationState.animateChanges(),
        Y.current &&
          (queueMicrotask(() => {
            var F;
            (F = window.MotionHandoffMarkAsComplete) == null || F.call(window, L);
          }),
          (Y.current = !1)),
        (w.enteringChildren = void 0));
    }),
    w
  );
}
function TT(i, l, u, o) {
  const {
    layoutId: c,
    layout: h,
    drag: d,
    dragConstraints: m,
    layoutScroll: g,
    layoutRoot: p,
    layoutCrossfade: v,
  } = l;
  ((i.projection = new u(i.latestValues, l["data-framer-portal-id"] ? void 0 : Ay(i.parent))),
    i.projection.setOptions({
      layoutId: c,
      layout: h,
      alwaysMeasureLayout: !!d || (m && ua(m)),
      visualElement: i,
      animationType: typeof h == "string" ? h : "both",
      initialPromotionConfig: o,
      crossfade: v,
      layoutScroll: g,
      layoutRoot: p,
    }));
}
function Ay(i) {
  if (i) return i.options.allowProjection !== !1 ? i.projection : Ay(i.parent);
}
function $r(i, { forwardMotionProps: l = !1, type: u } = {}, o, c) {
  o && iT(o);
  const h = u ? u === "svg" : uf(i),
    d = h ? vT : yT;
  function m(p, v) {
    let x;
    const S = { ...k.useContext(lf), ...p, layoutId: AT(p) },
      { isStatic: N } = S,
      w = oT(p),
      U = d(p, N);
    if (!N && typeof window < "u") {
      ET();
      const H = MT(S);
      ((x = H.MeasureLayout), (w.visualElement = ST(i, U, S, c, H.ProjectionNode, h)));
    }
    return b.jsxs(pu.Provider, {
      value: w,
      children: [
        x && w.visualElement ? b.jsx(x, { visualElement: w.visualElement, ...S }) : null,
        mT(i, p, bT(U, w.visualElement, v), U, N, l, h),
      ],
    });
  }
  m.displayName = `motion.${typeof i == "string" ? i : `create(${i.displayName ?? i.name ?? ""})`}`;
  const g = k.forwardRef(m);
  return ((g[xT] = i), g);
}
function AT({ layoutId: i }) {
  const l = k.useContext(jc).id;
  return l && i !== void 0 ? l + "-" + i : i;
}
function ET(i, l) {
  k.useContext(gy).strict;
}
function MT(i) {
  const l = yy(),
    { drag: u, layout: o } = l;
  if (!u && !o) return {};
  const c = { ...u, ...o };
  return {
    MeasureLayout:
      (u != null && u.isEnabled(i)) || (o != null && o.isEnabled(i)) ? c.MeasureLayout : void 0,
    ProjectionNode: c.ProjectionNode,
  };
}
function DT(i, l) {
  if (typeof Proxy > "u") return $r;
  const u = new Map(),
    o = (h, d) => $r(h, d, i, l),
    c = (h, d) => o(h, d);
  return new Proxy(c, {
    get: (h, d) => (d === "create" ? o : (u.has(d) || u.set(d, $r(d, void 0, i, l)), u.get(d))),
  });
}
const CT = (i, l) =>
  (l.isSVG ?? uf(i)) ? new lS(l) : new I2(l, { allowProjection: i !== k.Fragment });
class NT extends Wn {
  constructor(l) {
    (super(l), l.animationState || (l.animationState = cS(l)));
  }
  updateAnimationControlsSubscription() {
    const { animate: l } = this.node.getProps();
    hu(l) && (this.unmountControls = l.subscribe(this.node));
  }
  mount() {
    this.updateAnimationControlsSubscription();
  }
  update() {
    const { animate: l } = this.node.getProps(),
      { animate: u } = this.node.prevProps || {};
    l !== u && this.updateAnimationControlsSubscription();
  }
  unmount() {
    var l;
    (this.node.animationState.reset(), (l = this.unmountControls) == null || l.call(this));
  }
}
let zT = 0;
class jT extends Wn {
  constructor() {
    (super(...arguments), (this.id = zT++), (this.isExitComplete = !1));
  }
  update() {
    var h;
    if (!this.node.presenceContext) return;
    const { isPresent: l, onExitComplete: u } = this.node.presenceContext,
      { isPresent: o } = this.node.prevPresenceContext || {};
    if (!this.node.animationState || l === o) return;
    if (l && o === !1) {
      if (this.isExitComplete) {
        const { initial: d, custom: m } = this.node.getProps();
        if (typeof d == "string") {
          const g = Ti(this.node, d, m);
          if (g) {
            const { transition: p, transitionEnd: v, ...x } = g;
            for (const S in x) (h = this.node.getValue(S)) == null || h.jump(x[S]);
          }
        }
        (this.node.animationState.reset(), this.node.animationState.animateChanges());
      } else this.node.animationState.setActive("exit", !1);
      this.isExitComplete = !1;
      return;
    }
    const c = this.node.animationState.setActive("exit", !l);
    u &&
      !l &&
      c.then(() => {
        ((this.isExitComplete = !0), u(this.id));
      });
  }
  mount() {
    const { register: l, onExitComplete: u } = this.node.presenceContext || {};
    (u && u(this.id), l && (this.unmount = l(this.id)));
  }
  unmount() {}
}
const wT = { animation: { Feature: NT }, exit: { Feature: jT } };
function Dl(i) {
  return { point: { x: i.pageX, y: i.pageY } };
}
const RT = (i) => (l) => $c(l) && i(l, Dl(l));
function vl(i, l, u, o) {
  return Tl(i, l, RT(u), o);
}
const Ey = ({ current: i }) => (i ? i.ownerDocument.defaultView : null),
  D0 = (i, l) => Math.abs(i - l);
function OT(i, l) {
  const u = D0(i.x, l.x),
    o = D0(i.y, l.y);
  return Math.sqrt(u ** 2 + o ** 2);
}
const C0 = new Set(["auto", "scroll"]);
class My {
  constructor(
    l,
    u,
    {
      transformPagePoint: o,
      contextWindow: c = window,
      dragSnapToOrigin: h = !1,
      distanceThreshold: d = 3,
      element: m,
    } = {},
  ) {
    if (
      ((this.startEvent = null),
      (this.lastMoveEvent = null),
      (this.lastMoveEventInfo = null),
      (this.lastRawMoveEventInfo = null),
      (this.handlers = {}),
      (this.contextWindow = window),
      (this.scrollPositions = new Map()),
      (this.removeScrollListeners = null),
      (this.onElementScroll = (N) => {
        this.handleScroll(N.target);
      }),
      (this.onWindowScroll = () => {
        this.handleScroll(window);
      }),
      (this.updatePoint = () => {
        if (!(this.lastMoveEvent && this.lastMoveEventInfo)) return;
        this.lastRawMoveEventInfo &&
          (this.lastMoveEventInfo = Js(this.lastRawMoveEventInfo, this.transformPagePoint));
        const N = Ir(this.lastMoveEventInfo, this.history),
          w = this.startEvent !== null,
          U = OT(N.offset, { x: 0, y: 0 }) >= this.distanceThreshold;
        if (!w && !U) return;
        const { point: H } = N,
          { timestamp: L } = le;
        this.history.push({ ...H, timestamp: L });
        const { onStart: Y, onMove: q } = this.handlers;
        (w || (Y && Y(this.lastMoveEvent, N), (this.startEvent = this.lastMoveEvent)),
          q && q(this.lastMoveEvent, N));
      }),
      (this.handlePointerMove = (N, w) => {
        ((this.lastMoveEvent = N),
          (this.lastRawMoveEventInfo = w),
          (this.lastMoveEventInfo = Js(w, this.transformPagePoint)),
          zt.update(this.updatePoint, !0));
      }),
      (this.handlePointerUp = (N, w) => {
        this.end();
        const { onEnd: U, onSessionEnd: H, resumeAnimation: L } = this.handlers;
        if (
          ((this.dragSnapToOrigin || !this.startEvent) && L && L(),
          !(this.lastMoveEvent && this.lastMoveEventInfo))
        )
          return;
        const Y = Ir(
          N.type === "pointercancel" ? this.lastMoveEventInfo : Js(w, this.transformPagePoint),
          this.history,
        );
        (this.startEvent && U && U(N, Y), H && H(N, Y));
      }),
      !$c(l))
    )
      return;
    ((this.dragSnapToOrigin = h),
      (this.handlers = u),
      (this.transformPagePoint = o),
      (this.distanceThreshold = d),
      (this.contextWindow = c || window));
    const g = Dl(l),
      p = Js(g, this.transformPagePoint),
      { point: v } = p,
      { timestamp: x } = le;
    this.history = [{ ...v, timestamp: x }];
    const { onSessionStart: S } = u;
    (S && S(l, Ir(p, this.history)),
      (this.removeListeners = Al(
        vl(this.contextWindow, "pointermove", this.handlePointerMove),
        vl(this.contextWindow, "pointerup", this.handlePointerUp),
        vl(this.contextWindow, "pointercancel", this.handlePointerUp),
      )),
      m && this.startScrollTracking(m));
  }
  startScrollTracking(l) {
    let u = l.parentElement;
    for (; u; ) {
      const o = getComputedStyle(u);
      ((C0.has(o.overflowX) || C0.has(o.overflowY)) &&
        this.scrollPositions.set(u, { x: u.scrollLeft, y: u.scrollTop }),
        (u = u.parentElement));
    }
    (this.scrollPositions.set(window, { x: window.scrollX, y: window.scrollY }),
      window.addEventListener("scroll", this.onElementScroll, { capture: !0 }),
      window.addEventListener("scroll", this.onWindowScroll),
      (this.removeScrollListeners = () => {
        (window.removeEventListener("scroll", this.onElementScroll, { capture: !0 }),
          window.removeEventListener("scroll", this.onWindowScroll));
      }));
  }
  handleScroll(l) {
    const u = this.scrollPositions.get(l);
    if (!u) return;
    const o = l === window,
      c = o ? { x: window.scrollX, y: window.scrollY } : { x: l.scrollLeft, y: l.scrollTop },
      h = { x: c.x - u.x, y: c.y - u.y };
    (h.x === 0 && h.y === 0) ||
      (o
        ? this.lastMoveEventInfo &&
          ((this.lastMoveEventInfo.point.x += h.x), (this.lastMoveEventInfo.point.y += h.y))
        : this.history.length > 0 && ((this.history[0].x -= h.x), (this.history[0].y -= h.y)),
      this.scrollPositions.set(l, c),
      zt.update(this.updatePoint, !0));
  }
  updateHandlers(l) {
    this.handlers = l;
  }
  end() {
    (this.removeListeners && this.removeListeners(),
      this.removeScrollListeners && this.removeScrollListeners(),
      this.scrollPositions.clear(),
      Pn(this.updatePoint));
  }
}
function Js(i, l) {
  return l ? { point: l(i.point) } : i;
}
function N0(i, l) {
  return { x: i.x - l.x, y: i.y - l.y };
}
function Ir({ point: i }, l) {
  return { point: i, delta: N0(i, Dy(l)), offset: N0(i, VT(l)), velocity: _T(l, 0.1) };
}
function VT(i) {
  return i[0];
}
function Dy(i) {
  return i[i.length - 1];
}
function _T(i, l) {
  if (i.length < 2) return { x: 0, y: 0 };
  let u = i.length - 1,
    o = null;
  const c = Dy(i);
  for (; u >= 0 && ((o = i[u]), !(c.timestamp - o.timestamp > je(l))); ) u--;
  if (!o) return { x: 0, y: 0 };
  o === i[0] && i.length > 2 && c.timestamp - o.timestamp > je(l) * 2 && (o = i[1]);
  const h = Ye(c.timestamp - o.timestamp);
  if (h === 0) return { x: 0, y: 0 };
  const d = { x: (c.x - o.x) / h, y: (c.y - o.y) / h };
  return (d.x === 1 / 0 && (d.x = 0), d.y === 1 / 0 && (d.y = 0), d);
}
function BT(i, { min: l, max: u }, o) {
  return (
    l !== void 0 && i < l
      ? (i = o ? Ot(l, i, o.min) : Math.max(i, l))
      : u !== void 0 && i > u && (i = o ? Ot(u, i, o.max) : Math.min(i, u)),
    i
  );
}
function z0(i, l, u) {
  return {
    min: l !== void 0 ? i.min + l : void 0,
    max: u !== void 0 ? i.max + u - (i.max - i.min) : void 0,
  };
}
function UT(i, { top: l, left: u, bottom: o, right: c }) {
  return { x: z0(i.x, u, c), y: z0(i.y, l, o) };
}
function j0(i, l) {
  let u = l.min - i.min,
    o = l.max - i.max;
  return (l.max - l.min < i.max - i.min && ([u, o] = [o, u]), { min: u, max: o });
}
function LT(i, l) {
  return { x: j0(i.x, l.x), y: j0(i.y, l.y) };
}
function HT(i, l) {
  let u = 0.5;
  const o = fe(i),
    c = fe(l);
  return (
    c > o ? (u = xl(l.min, l.max - o, i.min)) : o > c && (u = xl(i.min, i.max - c, l.min)),
    en(0, 1, u)
  );
}
function qT(i, l) {
  const u = {};
  return (
    l.min !== void 0 && (u.min = l.min - i.min),
    l.max !== void 0 && (u.max = l.max - i.min),
    u
  );
}
const Cc = 0.35;
function YT(i = Cc) {
  return (
    i === !1 ? (i = 0) : i === !0 && (i = Cc),
    { x: w0(i, "left", "right"), y: w0(i, "top", "bottom") }
  );
}
function w0(i, l, u) {
  return { min: R0(i, l), max: R0(i, u) };
}
function R0(i, l) {
  return typeof i == "number" ? i : i[l] || 0;
}
const GT = new WeakMap();
class XT {
  constructor(l) {
    ((this.openDragLock = null),
      (this.isDragging = !1),
      (this.currentDirection = null),
      (this.originPoint = { x: 0, y: 0 }),
      (this.constraints = !1),
      (this.hasMutatedConstraints = !1),
      (this.elastic = Ft()),
      (this.latestPointerEvent = null),
      (this.latestPanInfo = null),
      (this.visualElement = l));
  }
  start(l, { snapToCursor: u = !1, distanceThreshold: o } = {}) {
    const { presenceContext: c } = this.visualElement;
    if (c && c.isPresent === !1) return;
    const h = (x) => {
        (u && this.snapToCursor(Dl(x).point), this.stopAnimation());
      },
      d = (x, S) => {
        const { drag: N, dragPropagation: w, onDragStart: U } = this.getProps();
        if (
          N &&
          !w &&
          (this.openDragLock && this.openDragLock(),
          (this.openDragLock = v2(N)),
          !this.openDragLock)
        )
          return;
        ((this.latestPointerEvent = x),
          (this.latestPanInfo = S),
          (this.isDragging = !0),
          (this.currentDirection = null),
          this.resolveConstraints(),
          this.visualElement.projection &&
            ((this.visualElement.projection.isAnimationBlocked = !0),
            (this.visualElement.projection.target = void 0)),
          Ie((L) => {
            let Y = this.getAxisMotionValue(L).get() || 0;
            if (tn.test(Y)) {
              const { projection: q } = this.visualElement;
              if (q && q.layout) {
                const G = q.layout.layoutBox[L];
                G && (Y = fe(G) * (parseFloat(Y) / 100));
              }
            }
            this.originPoint[L] = Y;
          }),
          U && zt.update(() => U(x, S), !1, !0),
          vc(this.visualElement, "transform"));
        const { animationState: H } = this.visualElement;
        H && H.setActive("whileDrag", !0);
      },
      m = (x, S) => {
        ((this.latestPointerEvent = x), (this.latestPanInfo = S));
        const {
          dragPropagation: N,
          dragDirectionLock: w,
          onDirectionLock: U,
          onDrag: H,
        } = this.getProps();
        if (!N && !this.openDragLock) return;
        const { offset: L } = S;
        if (w && this.currentDirection === null) {
          ((this.currentDirection = QT(L)),
            this.currentDirection !== null && U && U(this.currentDirection));
          return;
        }
        (this.updateAxis("x", S.point, L),
          this.updateAxis("y", S.point, L),
          this.visualElement.render(),
          H && zt.update(() => H(x, S), !1, !0));
      },
      g = (x, S) => {
        ((this.latestPointerEvent = x),
          (this.latestPanInfo = S),
          this.stop(x, S),
          (this.latestPointerEvent = null),
          (this.latestPanInfo = null));
      },
      p = () => {
        const { dragSnapToOrigin: x } = this.getProps();
        (x || this.constraints) && this.startAnimation({ x: 0, y: 0 });
      },
      { dragSnapToOrigin: v } = this.getProps();
    this.panSession = new My(
      l,
      { onSessionStart: h, onStart: d, onMove: m, onSessionEnd: g, resumeAnimation: p },
      {
        transformPagePoint: this.visualElement.getTransformPagePoint(),
        dragSnapToOrigin: v,
        distanceThreshold: o,
        contextWindow: Ey(this.visualElement),
        element: this.visualElement.current,
      },
    );
  }
  stop(l, u) {
    const o = l || this.latestPointerEvent,
      c = u || this.latestPanInfo,
      h = this.isDragging;
    if ((this.cancel(), !h || !c || !o)) return;
    const { velocity: d } = c;
    this.startAnimation(d);
    const { onDragEnd: m } = this.getProps();
    m && zt.postRender(() => m(o, c));
  }
  cancel() {
    this.isDragging = !1;
    const { projection: l, animationState: u } = this.visualElement;
    (l && (l.isAnimationBlocked = !1), this.endPanSession());
    const { dragPropagation: o } = this.getProps();
    (!o && this.openDragLock && (this.openDragLock(), (this.openDragLock = null)),
      u && u.setActive("whileDrag", !1));
  }
  endPanSession() {
    (this.panSession && this.panSession.end(), (this.panSession = void 0));
  }
  updateAxis(l, u, o) {
    const { drag: c } = this.getProps();
    if (!o || !Fs(l, c, this.currentDirection)) return;
    const h = this.getAxisMotionValue(l);
    let d = this.originPoint[l] + o[l];
    (this.constraints && this.constraints[l] && (d = BT(d, this.constraints[l], this.elastic[l])),
      h.set(d));
  }
  resolveConstraints() {
    var h;
    const { dragConstraints: l, dragElastic: u } = this.getProps(),
      o =
        this.visualElement.projection && !this.visualElement.projection.layout
          ? this.visualElement.projection.measure(!1)
          : (h = this.visualElement.projection) == null
            ? void 0
            : h.layout,
      c = this.constraints;
    (l && ua(l)
      ? this.constraints || (this.constraints = this.resolveRefConstraints())
      : l && o
        ? (this.constraints = UT(o.layoutBox, l))
        : (this.constraints = !1),
      (this.elastic = YT(u)),
      c !== this.constraints &&
        !ua(l) &&
        o &&
        this.constraints &&
        !this.hasMutatedConstraints &&
        Ie((d) => {
          this.constraints !== !1 &&
            this.getAxisMotionValue(d) &&
            (this.constraints[d] = qT(o.layoutBox[d], this.constraints[d]));
        }));
  }
  resolveRefConstraints() {
    const { dragConstraints: l, onMeasureDragConstraints: u } = this.getProps();
    if (!l || !ua(l)) return !1;
    const o = l.current,
      { projection: c } = this.visualElement;
    if (!c || !c.layout) return !1;
    const h = K2(o, c.root, this.visualElement.getTransformPagePoint());
    let d = LT(c.layout.layoutBox, h);
    if (u) {
      const m = u(k2(d));
      ((this.hasMutatedConstraints = !!m), m && (d = Kg(m)));
    }
    return d;
  }
  startAnimation(l) {
    const {
        drag: u,
        dragMomentum: o,
        dragElastic: c,
        dragTransition: h,
        dragSnapToOrigin: d,
        onDragTransitionEnd: m,
      } = this.getProps(),
      g = this.constraints || {},
      p = Ie((v) => {
        if (!Fs(v, u, this.currentDirection)) return;
        let x = (g && g[v]) || {};
        (d === !0 || d === v) && (x = { min: 0, max: 0 });
        const S = c ? 200 : 1e6,
          N = c ? 40 : 1e7,
          w = {
            type: "inertia",
            velocity: o ? l[v] : 0,
            bounceStiffness: S,
            bounceDamping: N,
            timeConstant: 750,
            restDelta: 1,
            restSpeed: 10,
            ...h,
            ...x,
          };
        return this.startAxisValueAnimation(v, w);
      });
    return Promise.all(p).then(m);
  }
  startAxisValueAnimation(l, u) {
    const o = this.getAxisMotionValue(l);
    return (vc(this.visualElement, l), o.start(Kc(l, o, 0, u, this.visualElement, !1)));
  }
  stopAnimation() {
    Ie((l) => this.getAxisMotionValue(l).stop());
  }
  getAxisMotionValue(l) {
    const u = `_drag${l.toUpperCase()}`,
      o = this.visualElement.getProps(),
      c = o[u];
    return c || this.visualElement.getValue(l, (o.initial ? o.initial[l] : void 0) || 0);
  }
  snapToCursor(l) {
    Ie((u) => {
      const { drag: o } = this.getProps();
      if (!Fs(u, o, this.currentDirection)) return;
      const { projection: c } = this.visualElement,
        h = this.getAxisMotionValue(u);
      if (c && c.layout) {
        const { min: d, max: m } = c.layout.layoutBox[u],
          g = h.get() || 0;
        h.set(l[u] - Ot(d, m, 0.5) + g);
      }
    });
  }
  scalePositionWithinConstraints() {
    if (!this.visualElement.current) return;
    const { drag: l, dragConstraints: u } = this.getProps(),
      { projection: o } = this.visualElement;
    if (!ua(u) || !o || !this.constraints) return;
    this.stopAnimation();
    const c = { x: 0, y: 0 };
    Ie((d) => {
      const m = this.getAxisMotionValue(d);
      if (m && this.constraints !== !1) {
        const g = m.get();
        c[d] = HT({ min: g, max: g }, this.constraints[d]);
      }
    });
    const { transformTemplate: h } = this.visualElement.getProps();
    ((this.visualElement.current.style.transform = h ? h({}, "") : "none"),
      o.root && o.root.updateScroll(),
      o.updateLayout(),
      (this.constraints = !1),
      this.resolveConstraints(),
      Ie((d) => {
        if (!Fs(d, l, null)) return;
        const m = this.getAxisMotionValue(d),
          { min: g, max: p } = this.constraints[d];
        m.set(Ot(g, p, c[d]));
      }),
      this.visualElement.render());
  }
  addListeners() {
    if (!this.visualElement.current) return;
    GT.set(this.visualElement, this);
    const l = this.visualElement.current,
      u = vl(l, "pointerdown", (p) => {
        const { drag: v, dragListener: x = !0 } = this.getProps(),
          S = p.target,
          N = S !== l && E2(S);
        v && x && !N && this.start(p);
      });
    let o;
    const c = () => {
        const { dragConstraints: p } = this.getProps();
        ua(p) &&
          p.current &&
          ((this.constraints = this.resolveRefConstraints()),
          o || (o = kT(l, p.current, () => this.scalePositionWithinConstraints())));
      },
      { projection: h } = this.visualElement,
      d = h.addEventListener("measure", c);
    (h && !h.layout && (h.root && h.root.updateScroll(), h.updateLayout()), zt.read(c));
    const m = Tl(window, "resize", () => this.scalePositionWithinConstraints()),
      g = h.addEventListener("didUpdate", ({ delta: p, hasLayoutChanged: v }) => {
        this.isDragging &&
          v &&
          (Ie((x) => {
            const S = this.getAxisMotionValue(x);
            S && ((this.originPoint[x] += p[x].translate), S.set(S.get() + p[x].translate));
          }),
          this.visualElement.render());
      });
    return () => {
      (m(), u(), d(), g && g(), o && o());
    };
  }
  getProps() {
    const l = this.visualElement.getProps(),
      {
        drag: u = !1,
        dragDirectionLock: o = !1,
        dragPropagation: c = !1,
        dragConstraints: h = !1,
        dragElastic: d = Cc,
        dragMomentum: m = !0,
      } = l;
    return {
      ...l,
      drag: u,
      dragDirectionLock: o,
      dragPropagation: c,
      dragConstraints: h,
      dragElastic: d,
      dragMomentum: m,
    };
  }
}
function O0(i) {
  let l = !0;
  return () => {
    if (l) {
      l = !1;
      return;
    }
    i();
  };
}
function kT(i, l, u) {
  const o = Hp(i, O0(u)),
    c = Hp(l, O0(u));
  return () => {
    (o(), c());
  };
}
function Fs(i, l, u) {
  return (l === !0 || l === i) && (u === null || u === i);
}
function QT(i, l = 10) {
  let u = null;
  return (Math.abs(i.y) > l ? (u = "y") : Math.abs(i.x) > l && (u = "x"), u);
}
class ZT extends Wn {
  constructor(l) {
    (super(l),
      (this.removeGroupControls = Ge),
      (this.removeListeners = Ge),
      (this.controls = new XT(l)));
  }
  mount() {
    const { dragControls: l } = this.node.getProps();
    (l && (this.removeGroupControls = l.subscribe(this.controls)),
      (this.removeListeners = this.controls.addListeners() || Ge));
  }
  update() {
    const { dragControls: l } = this.node.getProps(),
      { dragControls: u } = this.node.prevProps || {};
    l !== u &&
      (this.removeGroupControls(), l && (this.removeGroupControls = l.subscribe(this.controls)));
  }
  unmount() {
    (this.removeGroupControls(),
      this.removeListeners(),
      this.controls.isDragging || this.controls.endPanSession());
  }
}
const tc = (i) => (l, u) => {
  i && zt.update(() => i(l, u), !1, !0);
};
class KT extends Wn {
  constructor() {
    (super(...arguments), (this.removePointerDownListener = Ge));
  }
  onPointerDown(l) {
    this.session = new My(l, this.createPanHandlers(), {
      transformPagePoint: this.node.getTransformPagePoint(),
      contextWindow: Ey(this.node),
    });
  }
  createPanHandlers() {
    const { onPanSessionStart: l, onPanStart: u, onPan: o, onPanEnd: c } = this.node.getProps();
    return {
      onSessionStart: tc(l),
      onStart: tc(u),
      onMove: tc(o),
      onEnd: (h, d) => {
        (delete this.session, c && zt.postRender(() => c(h, d)));
      },
    };
  }
  mount() {
    this.removePointerDownListener = vl(this.node.current, "pointerdown", (l) =>
      this.onPointerDown(l),
    );
  }
  update() {
    this.session && this.session.updateHandlers(this.createPanHandlers());
  }
  unmount() {
    (this.removePointerDownListener(), this.session && this.session.end());
  }
}
let ec = !1;
class JT extends k.Component {
  componentDidMount() {
    const { visualElement: l, layoutGroup: u, switchLayoutGroup: o, layoutId: c } = this.props,
      { projection: h } = l;
    (h &&
      (u.group && u.group.add(h),
      o && o.register && c && o.register(h),
      ec && h.root.didUpdate(),
      h.addEventListener("animationComplete", () => {
        this.safeToRemove();
      }),
      h.setOptions({
        ...h.options,
        layoutDependency: this.props.layoutDependency,
        onExitComplete: () => this.safeToRemove(),
      })),
      (nu.hasEverUpdated = !0));
  }
  getSnapshotBeforeUpdate(l) {
    const { layoutDependency: u, visualElement: o, drag: c, isPresent: h } = this.props,
      { projection: d } = o;
    return (
      d &&
        ((d.isPresent = h),
        l.layoutDependency !== u && d.setOptions({ ...d.options, layoutDependency: u }),
        (ec = !0),
        c || l.layoutDependency !== u || u === void 0 || l.isPresent !== h
          ? d.willUpdate()
          : this.safeToRemove(),
        l.isPresent !== h &&
          (h
            ? d.promote()
            : d.relegate() ||
              zt.postRender(() => {
                const m = d.getStack();
                (!m || !m.members.length) && this.safeToRemove();
              }))),
      null
    );
  }
  componentDidUpdate() {
    const { projection: l } = this.props.visualElement;
    l &&
      (l.root.didUpdate(),
      Wc.postRender(() => {
        !l.currentAnimation && l.isLead() && this.safeToRemove();
      }));
  }
  componentWillUnmount() {
    const { visualElement: l, layoutGroup: u, switchLayoutGroup: o } = this.props,
      { projection: c } = l;
    ((ec = !0),
      c &&
        (c.scheduleCheckAfterUnmount(),
        u && u.group && u.group.remove(c),
        o && o.deregister && o.deregister(c)));
  }
  safeToRemove() {
    const { safeToRemove: l } = this.props;
    l && l();
  }
  render() {
    return null;
  }
}
function Cy(i) {
  const [l, u] = py(),
    o = k.useContext(jc);
  return b.jsx(JT, {
    ...i,
    layoutGroup: o,
    switchLayoutGroup: k.useContext(Ty),
    isPresent: l,
    safeToRemove: u,
  });
}
const FT = { pan: { Feature: KT }, drag: { Feature: ZT, ProjectionNode: my, MeasureLayout: Cy } };
function V0(i, l, u) {
  const { props: o } = i;
  i.animationState && o.whileHover && i.animationState.setActive("whileHover", u === "Start");
  const c = "onHover" + u,
    h = o[c];
  h && zt.postRender(() => h(l, Dl(l)));
}
class PT extends Wn {
  mount() {
    const { current: l } = this.node;
    l &&
      (this.unmount = b2(l, (u, o) => (V0(this.node, o, "Start"), (c) => V0(this.node, c, "End"))));
  }
  unmount() {}
}
class WT extends Wn {
  constructor() {
    (super(...arguments), (this.isActive = !1));
  }
  onFocus() {
    let l = !1;
    try {
      l = this.node.current.matches(":focus-visible");
    } catch {
      l = !0;
    }
    !l ||
      !this.node.animationState ||
      (this.node.animationState.setActive("whileFocus", !0), (this.isActive = !0));
  }
  onBlur() {
    !this.isActive ||
      !this.node.animationState ||
      (this.node.animationState.setActive("whileFocus", !1), (this.isActive = !1));
  }
  mount() {
    this.unmount = Al(
      Tl(this.node.current, "focus", () => this.onFocus()),
      Tl(this.node.current, "blur", () => this.onBlur()),
    );
  }
  unmount() {}
}
function _0(i, l, u) {
  const { props: o } = i;
  if (i.current instanceof HTMLButtonElement && i.current.disabled) return;
  i.animationState && o.whileTap && i.animationState.setActive("whileTap", u === "Start");
  const c = "onTap" + (u === "End" ? "" : u),
    h = o[c];
  h && zt.postRender(() => h(l, Dl(l)));
}
class $T extends Wn {
  mount() {
    const { current: l } = this.node;
    if (!l) return;
    const { globalTapTarget: u, propagate: o } = this.node.props;
    this.unmount = D2(
      l,
      (c, h) => (
        _0(this.node, h, "Start"),
        (d, { success: m }) => _0(this.node, d, m ? "End" : "Cancel")
      ),
      { useGlobalTarget: u, stopPropagation: (o == null ? void 0 : o.tap) === !1 },
    );
  }
  unmount() {}
}
const Nc = new WeakMap(),
  nc = new WeakMap(),
  IT = (i) => {
    const l = Nc.get(i.target);
    l && l(i);
  },
  tA = (i) => {
    i.forEach(IT);
  };
function eA({ root: i, ...l }) {
  const u = i || document;
  nc.has(u) || nc.set(u, {});
  const o = nc.get(u),
    c = JSON.stringify(l);
  return (o[c] || (o[c] = new IntersectionObserver(tA, { root: i, ...l })), o[c]);
}
function nA(i, l, u) {
  const o = eA(l);
  return (
    Nc.set(i, u),
    o.observe(i),
    () => {
      (Nc.delete(i), o.unobserve(i));
    }
  );
}
const iA = { some: 0, all: 1 };
class aA extends Wn {
  constructor() {
    (super(...arguments), (this.hasEnteredView = !1), (this.isInView = !1));
  }
  startObserver() {
    this.unmount();
    const { viewport: l = {} } = this.node.getProps(),
      { root: u, margin: o, amount: c = "some", once: h } = l,
      d = {
        root: u ? u.current : void 0,
        rootMargin: o,
        threshold: typeof c == "number" ? c : iA[c],
      },
      m = (g) => {
        const { isIntersecting: p } = g;
        if (this.isInView === p || ((this.isInView = p), h && !p && this.hasEnteredView)) return;
        (p && (this.hasEnteredView = !0),
          this.node.animationState && this.node.animationState.setActive("whileInView", p));
        const { onViewportEnter: v, onViewportLeave: x } = this.node.getProps(),
          S = p ? v : x;
        S && S(g);
      };
    return nA(this.node.current, d, m);
  }
  mount() {
    this.startObserver();
  }
  update() {
    if (typeof IntersectionObserver > "u") return;
    const { props: l, prevProps: u } = this.node;
    ["amount", "margin", "root"].some(lA(l, u)) && this.startObserver();
  }
  unmount() {}
}
function lA({ viewport: i = {} }, { viewport: l = {} } = {}) {
  return (u) => i[u] !== l[u];
}
const sA = {
    inView: { Feature: aA },
    tap: { Feature: $T },
    focus: { Feature: WT },
    hover: { Feature: PT },
  },
  uA = { layout: { ProjectionNode: my, MeasureLayout: Cy } },
  oA = { ...wT, ...sA, ...FT, ...uA },
  ze = DT(oA, CT);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const rA = (i) => i.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(),
  cA = (i) =>
    i.replace(/^([A-Z])|[\s-_]+(\w)/g, (l, u, o) => (o ? o.toUpperCase() : u.toLowerCase())),
  B0 = (i) => {
    const l = cA(i);
    return l.charAt(0).toUpperCase() + l.slice(1);
  },
  Ny = (...i) =>
    i
      .filter((l, u, o) => !!l && l.trim() !== "" && o.indexOf(l) === u)
      .join(" ")
      .trim(),
  fA = (i) => {
    for (const l in i) if (l.startsWith("aria-") || l === "role" || l === "title") return !0;
  };
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ var dA = {
  xmlns: "http://www.w3.org/2000/svg",
  width: 24,
  height: 24,
  viewBox: "0 0 24 24",
  fill: "none",
  stroke: "currentColor",
  strokeWidth: 2,
  strokeLinecap: "round",
  strokeLinejoin: "round",
};
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const hA = k.forwardRef(
  (
    {
      color: i = "currentColor",
      size: l = 24,
      strokeWidth: u = 2,
      absoluteStrokeWidth: o,
      className: c = "",
      children: h,
      iconNode: d,
      ...m
    },
    g,
  ) =>
    k.createElement(
      "svg",
      {
        ref: g,
        ...dA,
        width: l,
        height: l,
        stroke: i,
        strokeWidth: o ? (Number(u) * 24) / Number(l) : u,
        className: Ny("lucide", c),
        ...(!h && !fA(m) && { "aria-hidden": "true" }),
        ...m,
      },
      [...d.map(([p, v]) => k.createElement(p, v)), ...(Array.isArray(h) ? h : [h])],
    ),
);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const Vt = (i, l) => {
  const u = k.forwardRef(({ className: o, ...c }, h) =>
    k.createElement(hA, {
      ref: h,
      iconNode: l,
      className: Ny(`lucide-${rA(B0(i))}`, `lucide-${i}`, o),
      ...c,
    }),
  );
  return ((u.displayName = B0(i)), u);
};
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const mA = [
    [
      "path",
      {
        d: "M22 12h-2.48a2 2 0 0 0-1.93 1.46l-2.35 8.36a.25.25 0 0 1-.48 0L9.24 2.18a.25.25 0 0 0-.48 0l-2.35 8.36A2 2 0 0 1 4.49 12H2",
        key: "169zse",
      },
    ],
  ],
  zy = Vt("activity", mA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const pA = [
    ["path", { d: "M7 7h10v10", key: "1tivn9" }],
    ["path", { d: "M7 17 17 7", key: "1vkiza" }],
  ],
  U0 = Vt("arrow-up-right", pA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const gA = [
    ["path", { d: "M16 20V4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16", key: "jecpp" }],
    ["rect", { width: "20", height: "14", x: "2", y: "6", rx: "2", key: "i6l2r4" }],
  ],
  jy = Vt("briefcase", gA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const yA = [["path", { d: "m9 18 6-6-6-6", key: "mthhwq" }]],
  vA = Vt("chevron-right", yA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const xA = [
    ["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
    ["path", { d: "m9 12 2 2 4-4", key: "dzmm74" }],
  ],
  L0 = Vt("circle-check", xA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const bA = [
    ["path", { d: "M17.5 19H9a7 7 0 1 1 6.71-9h1.79a4.5 4.5 0 1 1 0 9Z", key: "p7xjir" }],
  ],
  SA = Vt("cloud", bA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const TA = [
    ["path", { d: "m18 16 4-4-4-4", key: "1inbqp" }],
    ["path", { d: "m6 8-4 4 4 4", key: "15zrgr" }],
    ["path", { d: "m14.5 4-5 16", key: "e7oirm" }],
  ],
  AA = Vt("code-xml", TA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const EA = [
    ["ellipse", { cx: "12", cy: "5", rx: "9", ry: "3", key: "msslwz" }],
    ["path", { d: "M3 5V19A9 3 0 0 0 21 19V5", key: "1wlel7" }],
    ["path", { d: "M3 12A9 3 0 0 0 21 12", key: "mv7ke4" }],
  ],
  MA = Vt("database", EA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const DA = [
    ["path", { d: "M12 15V3", key: "m9g1x1" }],
    ["path", { d: "M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4", key: "ih7n3h" }],
    ["path", { d: "m7 10 5 5 5-5", key: "brsn70" }],
  ],
  CA = Vt("download", DA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const NA = [
    ["path", { d: "M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z", key: "1rqfz7" }],
    ["path", { d: "M14 2v4a2 2 0 0 0 2 2h4", key: "tnqrlb" }],
    ["path", { d: "M10 9H8", key: "b1mrlr" }],
    ["path", { d: "M16 13H8", key: "t4e002" }],
    ["path", { d: "M16 17H8", key: "z1uh3a" }],
  ],
  zA = Vt("file-text", NA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const jA = [
    [
      "path",
      {
        d: "M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4",
        key: "tonef",
      },
    ],
    ["path", { d: "M9 18c-4.51 2-5-2-7-2", key: "9comsn" }],
  ],
  H0 = Vt("github", jA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const wA = [
    [
      "path",
      {
        d: "M12.83 2.18a2 2 0 0 0-1.66 0L2.6 6.08a1 1 0 0 0 0 1.83l8.58 3.91a2 2 0 0 0 1.66 0l8.58-3.9a1 1 0 0 0 0-1.83z",
        key: "zw3jo",
      },
    ],
    [
      "path",
      {
        d: "M2 12a1 1 0 0 0 .58.91l8.6 3.91a2 2 0 0 0 1.65 0l8.58-3.9A1 1 0 0 0 22 12",
        key: "1wduqc",
      },
    ],
    [
      "path",
      {
        d: "M2 17a1 1 0 0 0 .58.91l8.6 3.91a2 2 0 0 0 1.65 0l8.58-3.9A1 1 0 0 0 22 17",
        key: "kqbvx6",
      },
    ],
  ],
  RA = Vt("layers", wA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const OA = [
    [
      "path",
      {
        d: "M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z",
        key: "c2jq9f",
      },
    ],
    ["rect", { width: "4", height: "12", x: "2", y: "9", key: "mk3on5" }],
    ["circle", { cx: "4", cy: "4", r: "2", key: "bt5ra8" }],
  ],
  q0 = Vt("linkedin", OA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const VA = [
    ["path", { d: "m22 7-8.991 5.727a2 2 0 0 1-2.009 0L2 7", key: "132q7q" }],
    ["rect", { x: "2", y: "4", width: "20", height: "16", rx: "2", key: "izxlao" }],
  ],
  ic = Vt("mail", VA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const _A = [
    [
      "path",
      {
        d: "M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0",
        key: "1r0f0z",
      },
    ],
    ["circle", { cx: "12", cy: "10", r: "3", key: "ilqhr7" }],
  ],
  ac = Vt("map-pin", _A);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const BA = [
    ["path", { d: "M4 5h16", key: "1tepv9" }],
    ["path", { d: "M4 12h16", key: "1lakjw" }],
    ["path", { d: "M4 19h16", key: "1djgab" }],
  ],
  UA = Vt("menu", BA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const LA = [
    ["rect", { width: "18", height: "18", x: "3", y: "3", rx: "2", key: "afitv7" }],
    ["path", { d: "M3 9h18", key: "1pudct" }],
    ["path", { d: "M9 21V9", key: "1oto5p" }],
  ],
  HA = Vt("panels-top-left", LA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const qA = [
    [
      "path",
      {
        d: "M13.832 16.568a1 1 0 0 0 1.213-.303l.355-.465A2 2 0 0 1 17 15h3a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2A18 18 0 0 1 2 4a2 2 0 0 1 2-2h3a2 2 0 0 1 2 2v3a2 2 0 0 1-.8 1.6l-.468.351a1 1 0 0 0-.292 1.233 14 14 0 0 0 6.392 6.384",
        key: "9njp5v",
      },
    ],
  ],
  YA = Vt("phone", qA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const GA = [
    ["rect", { width: "20", height: "8", x: "2", y: "2", rx: "2", ry: "2", key: "ngkwjq" }],
    ["rect", { width: "20", height: "8", x: "2", y: "14", rx: "2", ry: "2", key: "iecqi9" }],
    ["line", { x1: "6", x2: "6.01", y1: "6", y2: "6", key: "16zg32" }],
    ["line", { x1: "6", x2: "6.01", y1: "18", y2: "18", key: "nzw8ys" }],
  ],
  XA = Vt("server", GA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const kA = [
    [
      "path",
      {
        d: "M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z",
        key: "oel41y",
      },
    ],
    ["path", { d: "m9 12 2 2 4-4", key: "dzmm74" }],
  ],
  QA = Vt("shield-check", kA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const ZA = [
    ["path", { d: "M12 19h8", key: "baeox8" }],
    ["path", { d: "m4 17 6-6-6-6", key: "1yngyt" }],
  ],
  wy = Vt("terminal", ZA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const KA = [
    ["path", { d: "M18 6 6 18", key: "1bl5f8" }],
    ["path", { d: "m6 6 12 12", key: "d8bk6v" }],
  ],
  Y0 = Vt("x", KA);
/**
 * @license lucide-react v0.546.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */ const JA = [
    [
      "path",
      {
        d: "M4 14a1 1 0 0 1-.78-1.63l9.9-10.2a.5.5 0 0 1 .86.46l-1.92 6.02A1 1 0 0 0 13 10h7a1 1 0 0 1 .78 1.63l-9.9 10.2a.5.5 0 0 1-.86-.46l1.92-6.02A1 1 0 0 0 11 14z",
        key: "1xq2db",
      },
    ],
  ],
  iu = Vt("zap", JA),
  ae = {
    name: "Nasir Shahzad",
    email: "nasirshahzad.dev@gmail.com",
    phone: "+92 333 81 52 157",
    location: "Lahore, Pakistan",
    linkedin: "https://www.linkedin.com/in/nasir-shahzad-525110210/",
    github: "https://github.com/nasirshahzaddev",
    resumeUrl: "/Nasir-CV-M.pdf",
    whatsapp: "https://wa.me/923338152157",
    profileImage: "/Nasir.jpg",
    summary:
      "Full-Stack Software Engineer (5 years) with a strong backend focus on Node.js and NestJS. Skilled in building scalable, high-performance systems, designing robust architectures, optimizing databases, implementing event-driven workflows, and delivering production-ready full-stack applications across diverse domains.",
  },
  FA = [
    { title: "Languages", skills: ["JavaScript", "TypeScript"], icon: AA },
    { title: "Backend", skills: ["NodeJS", "NestJS", "ExpressJS"], icon: XA },
    { title: "Frontend", skills: ["React", "HTML", "CSS", "Tailwind"], icon: HA },
    { title: "Databases", skills: ["MongoDB", "PostgreSQL"], icon: MA },
    {
      title: "Architecture",
      skills: ["REST APIs", "Microservices", "Monolith", "Monorepo", "gRPC", "TCP"],
      icon: RA,
    },
    {
      title: "Performance",
      skills: ["Caching", "Job Queues", "Query Optimization", "Indexing"],
      icon: iu,
    },
    { title: "Messaging", skills: ["RabbitMQ", "WebSockets", "Socket.io"], icon: zy },
    { title: "Cloud & DevOps", skills: ["AWS", "Lambda", "Webhooks", "Cron Jobs"], icon: SA },
  ],
  PA = [
    {
      title: "Multi-Tenant Architecture",
      description:
        "Designed and implemented a multi-tenant backend architecture with tenant isolation, domain-based resolution, and dynamic RBAC supporting multiple organizations.",
    },
    {
      title: "Latency Optimization",
      description:
        "Optimized tenant resolution logic using in-memory lookup structures, reducing identification latency from 800ms to 20ms.",
    },
    {
      title: "Performance Tuning",
      description:
        "Improved backend performance through Redis caching and query optimizations, reducing average API response times by 30–40%.",
    },
    {
      title: "Healthcare Security",
      description:
        "Designed secure authentication and authorization systems using RBAC for healthcare platforms handling sensitive patient data.",
    },
    {
      title: "Flexible RBAC",
      description:
        "Built a flexible RBAC system with dynamic roles and predefined permissions, ensuring granular access control for all endpoints.",
    },
    {
      title: "Serverless Pipeline",
      description:
        "Designed a serverless transcription pipeline using AWS Transcribe and Lambda to process medical audio and generate clinical documentation.",
    },
    {
      title: "Audit Logging",
      description:
        "Implemented a centralized audit logging system resolving MongoDB ObjectId references across 15+ entity types, eliminating N+1 queries.",
    },
  ],
  WA = [
    {
      company: "Malind Tech",
      period: "April 2026 - Present",
      role: "Senior Full Software Engineer",
      location: "Lahore, Pakistan",
      bullets: [
        "Lead design and development of scalable full-stack systems using Node.js, NestJS, and modern frontend stacks.",
        "Own architecture decisions for multi-tenant SaaS platforms focused on scalability, security, and maintainability.",
        "Design and optimize RESTful and event-driven APIs using PostgreSQL and MongoDB.",
        "Improve performance through query optimization, indexing strategies, caching, and Redis tuning.",
        "Build cloud-native, event-driven workflows using AWS Lambda and distributed system patterns.",
        "Define and enforce RBAC-based security across services and APIs.",
        "Mentor developers and lead code reviews to maintain engineering standards.",
      ],
    },
    {
      company: "Brackets Private Limited",
      period: "October 2022 - April 2026",
      role: "Software Engineer",
      location: "Lahore, Pakistan",
      bullets: [
        "Developed and maintained scalable backend services using NodeJS and NestJS for healthcare and SaaS platforms.",
        "Designed and implemented RESTful APIs using MongoDB and PostgreSQL.",
        "Developed a reusable audit logging system to capture entity changes across platform modules.",
        "Optimized database queries, indexing strategies, and Redis caching mechanisms.",
        "Contributed to the design of multi-tenant backend architectures.",
        "Built event-driven and serverless workflows using AWS Lambda.",
        "Implemented secure authentication and authorization systems using RBAC.",
        "Mentored junior developers and participated in code reviews.",
      ],
    },
  ],
  $A = [
    {
      title: "Cairasu (Home Care)",
      subtitle: "Multi-Tenant Caregiver Marketplace",
      description:
        "Designed backend architecture for a multi-tenant platform connecting individuals with caregivers. Implemented tenant isolation, domain-based resolution, and dynamic RBAC.",
      tech: ["NestJS", "NodeJS", "MongoDB", "Redis", "REST APIs", "AWS"],
      image:
        "https://images.unsplash.com/photo-1576091160550-2173dba999ef?q=80&w=2070&auto=format&fit=crop",
      details: [
        "Architected multi-tenant database structure with strict data isolation.",
        "Implemented domain-based tenant resolution reducing latency by 90%.",
        "Developed dynamic RBAC system for granular access control.",
        "Integrated secure payment gateways and real-time notification systems.",
      ],
    },
    {
      title: "SOAP Notes Generation",
      subtitle: "Clinical Documentation Automation",
      description:
        "Designed and implemented backend services for a healthcare platform that automatically generates SOAP clinical notes from audio recordings using AWS Transcribe and Lambda.",
      tech: ["NodeJS", "NestJS", "AWS Transcribe", "AWS Lambda", "PostgreSQL"],
      image:
        "https://images.unsplash.com/photo-1505751172876-fa1923c5c528?q=80&w=2070&auto=format&fit=crop",
      details: [
        "Built serverless audio processing pipeline using AWS Lambda.",
        "Integrated AWS Transcribe for high-accuracy medical speech-to-text.",
        "Developed NLP-based parsing to structure transcriptions into SOAP format.",
        "Ensured HIPAA compliance for all data processing and storage.",
      ],
    },
    {
      title: "Business Process Automation",
      subtitle: "Enterprise Workflow System",
      description:
        "Comprehensive enterprise platform to automate core business workflows including expenses, payroll, and audits. Built modular backend services and scalable APIs.",
      tech: ["NodeJS", "ExpressJS", "MongoDB", "REST APIs"],
      image:
        "https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=2015&auto=format&fit=crop",
      details: [
        "Automated complex payroll calculations and expense approval workflows.",
        "Built a modular audit system tracking changes across 15+ entities.",
        "Designed high-performance reporting engine for business analytics.",
        "Implemented secure document storage and versioning.",
      ],
    },
    {
      title: "Healthcare Compliance",
      subtitle: "HIPAA Monitoring Platform",
      description:
        "Built backend services for a healthcare compliance platform that helps hospitals and clinics monitor HIPAA adherence. Implemented secure APIs and tracking workflows.",
      tech: ["NestJS", "NodeJS", "MongoDB", "PostgreSQL", "Microservices"],
      image:
        "https://images.unsplash.com/photo-1584036561566-baf8f5f1b144?q=80&w=2064&auto=format&fit=crop",
      details: [
        "Developed real-time monitoring dashboard for compliance metrics.",
        "Implemented automated risk assessment and reporting tools.",
        "Built secure communication channels for compliance officers.",
        "Designed scalable microservices architecture for high availability.",
      ],
    },
    {
      title: "HealthQ (Dental Match)",
      subtitle: "Clinic Collaboration Platform",
      description:
        "Platform connecting dental clinics with dentists for job posting, applications, and communication. Built scalable APIs and core modules for job management.",
      tech: ["NestJS", "NodeJS", "PostgreSQL", "Redis", "AWS"],
      image:
        "https://images.unsplash.com/photo-1588776814546-1ffcf47267a5?q=80&w=2070&auto=format&fit=crop",
      details: [
        "Built job matching algorithm based on location and specialization.",
        "Implemented real-time chat system for clinic-dentist communication.",
        "Developed secure application tracking and interview scheduling.",
        "Optimized search performance using Redis caching.",
      ],
    },
  ],
  IA = [
    { degree: "M.Phil (Software Engineering)", school: "University of Lahore" },
    { degree: "M.Sc (Information Technology)", school: "University of Sargodha" },
  ],
  tE = [
    { name: "Jira", category: "Project Management", icon: jy },
    { name: "ClickUp", category: "Workflow Automation", icon: zy },
    { name: "Discord", category: "Communication", icon: iu },
    { name: "SonarQube", category: "Code Quality & Fixes", icon: QA },
    { name: "Postman", category: "API Testing", icon: wy },
    { name: "Slack", category: "Communication", icon: iu },
    { name: "Swagger", category: "API Documentation", icon: zA },
    { name: "Redis", category: "Caching", icon: iu },
  ],
  eE = [
    { title: "Scrum Fundamentals Certified (SFC™)", issuer: "Scrum Study" },
    { title: "Developing Back-End Apps with NodeJS and Express", issuer: "IBM / Coursera" },
    { title: "Managing Project Risks and Changes", issuer: "UC Irvine / Coursera" },
    { title: "AWS Cloud Practitioner Essentials", issuer: "AWS / Coursera" },
    { title: "Fundamentals of NestJS", issuer: "Board Infinity / Coursera" },
  ],
  G0 = ({ className: i = "", onClick: l = () => {} }) =>
    b.jsxs("div", {
      className: `flex items-center gap-8 ${i}`,
      children: [
        b.jsx("a", {
          href: "#about",
          onClick: l,
          className: "hover:text-indigo-400 transition-colors",
          children: "About",
        }),
        b.jsx("a", {
          href: "#highlights",
          onClick: l,
          className: "hover:text-indigo-400 transition-colors",
          children: "Architecture",
        }),
        b.jsx("a", {
          href: "#experience",
          onClick: l,
          className: "hover:text-indigo-400 transition-colors",
          children: "Experience",
        }),
        b.jsx("a", {
          href: "#projects",
          onClick: l,
          className: "hover:text-indigo-400 transition-colors",
          children: "Projects",
        }),
        b.jsxs("a", {
          href: `/my_portfolio${ae.resumeUrl}`,
          download: !0,
          className:
            "px-5 py-2 rounded-full border border-indigo-500/30 text-indigo-600 hover:bg-indigo-500 hover:text-white transition-all flex items-center gap-2",
          children: ["Resume ", b.jsx(CA, { size: 14 })],
        }),
      ],
    });
function nE() {
  var d;
  const [i, l] = k.useState(!1),
    [u, o] = k.useState(null),
    c = lx.useRef(null),
    h = () => {
      c.current && c.current.scrollTo({ top: c.current.scrollTop + 300, behavior: "smooth" });
    };
  return b.jsxs("div", {
    className: "min-h-screen font-sans",
    children: [
      b.jsx(T0, {
        children:
          u &&
          b.jsxs("div", {
            className: "fixed inset-0 z-[100] flex items-center justify-center p-6",
            children: [
              b.jsx(ze.div, {
                initial: { opacity: 0 },
                animate: { opacity: 1 },
                exit: { opacity: 0 },
                onClick: () => o(null),
                className: "absolute inset-0 bg-slate-950/90 backdrop-blur-xl",
              }),
              b.jsxs(ze.div, {
                initial: { opacity: 0, scale: 0.9, y: 20 },
                animate: { opacity: 1, scale: 1, y: 0 },
                exit: { opacity: 0, scale: 0.9, y: 20 },
                className:
                  "relative w-full max-w-4xl max-h-[90vh] flex flex-col rounded-[2rem] lg:rounded-[3rem] glass border border-white/10 shadow-2xl overflow-hidden",
                children: [
                  b.jsxs("div", {
                    ref: c,
                    className: "flex-1 overflow-y-auto p-6 lg:p-12 scrollbar-hide",
                    children: [
                      b.jsx("button", {
                        onClick: () => o(null),
                        className:
                          "absolute top-4 right-4 lg:top-8 lg:right-8 p-2 lg:p-3 rounded-xl lg:rounded-2xl bg-white/5 hover:bg-white/10 transition-colors z-10",
                        children: b.jsx(Y0, { size: 20, className: "lg:w-6 lg:h-6" }),
                      }),
                      b.jsxs("div", {
                        className: "grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12",
                        children: [
                          b.jsxs("div", {
                            className: "space-y-8",
                            children: [
                              b.jsx("div", {
                                className:
                                  "aspect-video rounded-[2rem] overflow-hidden border border-white/5",
                                children: b.jsx("img", {
                                  src: u.image,
                                  alt: u.title,
                                  className: "w-full h-full object-cover",
                                }),
                              }),
                              b.jsx("div", {
                                className: "flex flex-wrap gap-3",
                                children: u.tech.map((m) =>
                                  b.jsx(
                                    "span",
                                    {
                                      className:
                                        "px-4 py-2 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-[12px] font-bold text-indigo-400 uppercase tracking-widest",
                                      children: m,
                                    },
                                    m,
                                  ),
                                ),
                              }),
                            ],
                          }),
                          b.jsxs("div", {
                            className: "space-y-8",
                            children: [
                              b.jsxs("div", {
                                children: [
                                  b.jsx("div", {
                                    className:
                                      "text-indigo-500 font-black uppercase tracking-[0.3em] text-[12px] mb-4",
                                    children: u.subtitle,
                                  }),
                                  b.jsx("h2", {
                                    className: "text-4xl font-black tracking-tighter mb-6",
                                    children: u.title,
                                  }),
                                  b.jsx("div", {
                                    className: "h-1.5 w-20 bg-indigo-600 rounded-full mb-8",
                                  }),
                                  b.jsx("p", {
                                    className:
                                      "text-slate-400 text-lg leading-relaxed text-justify",
                                    children: u.description,
                                  }),
                                ],
                              }),
                              b.jsxs("div", {
                                className: "space-y-6",
                                children: [
                                  b.jsx("h4", {
                                    className:
                                      "text-xs font-black uppercase tracking-widest text-slate-200",
                                    children: "Key Contributions",
                                  }),
                                  b.jsx("ul", {
                                    className: "space-y-4",
                                    children:
                                      ((d = u.details) == null
                                        ? void 0
                                        : d.map((m, g) =>
                                            b.jsxs(
                                              "li",
                                              {
                                                className:
                                                  "flex items-start gap-4 text-slate-400 text-sm leading-relaxed",
                                                children: [
                                                  b.jsx(L0, {
                                                    size: 18,
                                                    className:
                                                      "text-indigo-500 flex-shrink-0 mt-0.5",
                                                  }),
                                                  m,
                                                ],
                                              },
                                              g,
                                            ),
                                          )) ||
                                      b.jsx("li", {
                                        className: "text-slate-500 italic text-sm",
                                        children: "Detailed breakdown coming soon...",
                                      }),
                                  }),
                                ],
                              }),
                            ],
                          }),
                        ],
                      }),
                    ],
                  }),
                  b.jsx("div", {
                    className:
                      "p-4 border-t border-white/5 flex justify-center bg-slate-900/80 backdrop-blur-md",
                    children: b.jsxs("button", {
                      onClick: h,
                      className:
                        "flex items-center gap-2 px-6 py-2 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-[10px] font-bold uppercase tracking-widest text-indigo-400 hover:bg-indigo-500 hover:text-white transition-all group",
                      children: [
                        "Explore Details",
                        " ",
                        b.jsx(vA, {
                          size: 14,
                          className: "rotate-90 group-hover:translate-y-0.5 transition-transform",
                        }),
                      ],
                    }),
                  }),
                ],
              }),
            ],
          }),
      }),
      b.jsx("header", {
        className: "fixed top-0 left-0 w-full z-50 glass py-4 lg:py-5",
        children: b.jsxs("div", {
          className: "container mx-auto px-6 flex justify-between items-center",
          children: [
            b.jsxs("div", {
              className: "flex items-center gap-10",
              children: [
                b.jsxs("div", {
                  className: "text-xl lg:text-2xl font-black tracking-tighter",
                  children: ["NASIR SHAHZAD", b.jsx("span", { className: "text-indigo-500" })],
                }),
                b.jsxs("div", {
                  className:
                    "hidden lg:flex items-center gap-3 text-[12px] font-bold uppercase tracking-widest text-slate-500 bg-white/5 px-4 py-2 rounded-full border border-white/5",
                  children: [
                    b.jsx(ac, { size: 12, className: "text-indigo-500" }),
                    " ",
                    ae.location,
                  ],
                }),
              ],
            }),
            b.jsx("nav", {
              className:
                "hidden lg:block text-[11px] font-bold uppercase tracking-widest text-slate-400",
              children: b.jsx(G0, {}),
            }),
            b.jsx("button", {
              className: "lg:hidden p-2 text-slate-400 hover:text-white",
              onClick: () => l(!i),
              children: i ? b.jsx(Y0, { size: 24 }) : b.jsx(UA, { size: 24 }),
            }),
          ],
        }),
      }),
      b.jsx(T0, {
        children:
          i &&
          b.jsx(ze.div, {
            initial: { opacity: 0, y: -20 },
            animate: { opacity: 1, y: 0 },
            exit: { opacity: 0, y: -20 },
            className: "lg:hidden fixed inset-0 z-40 bg-[#0a0f1e] pt-24 px-6",
            children: b.jsxs("div", {
              className:
                "flex flex-col gap-8 text-lg font-bold uppercase tracking-widest text-slate-400",
              children: [
                b.jsx(G0, { className: "flex-col items-start", onClick: () => l(!1) }),
                b.jsxs("div", {
                  className: "pt-8 border-t border-white/5 flex gap-6",
                  children: [
                    b.jsx("a", {
                      href: ae.github,
                      target: "_blank",
                      className: "text-slate-500 hover:text-cyan-400 transition-colors",
                      children: b.jsx(H0, { size: 24 }),
                    }),
                    b.jsx("a", {
                      href: ae.linkedin,
                      target: "_blank",
                      className: "text-slate-500 hover:text-cyan-400 transition-colors",
                      children: b.jsx(q0, { size: 24 }),
                    }),
                    b.jsx("a", {
                      href: `mailto:${ae.email}`,
                      className: "text-slate-500 hover:text-cyan-400 transition-colors",
                      children: b.jsx(ic, { size: 24 }),
                    }),
                  ],
                }),
              ],
            }),
          }),
      }),
      b.jsxs("section", {
        id: "about",
        className: "pt-32 lg:pt-48 pb-16 lg:pb-24 relative overflow-hidden bg-[#0a0f1e]",
        children: [
          b.jsx("div", {
            className:
              "absolute top-[-10%] left-[-10%] w-[60%] h-[60%] bg-indigo-600/15 blur-[140px] rounded-full -z-10",
          }),
          b.jsx("div", {
            className:
              "absolute bottom-[10%] right-[-5%] w-[40%] h-[40%] bg-cyan-600/15 blur-[120px] rounded-full -z-10",
          }),
          b.jsx("div", {
            className: "container mx-auto px-6",
            children: b.jsx("div", {
              className: "max-w-5xl",
              children: b.jsxs(ze.div, {
                initial: { opacity: 0, y: 30 },
                animate: { opacity: 1, y: 0 },
                transition: { duration: 0.8 },
                children: [
                  b.jsxs("div", {
                    className: "flex items-center gap-3 mb-6",
                    children: [
                      b.jsx("div", { className: "h-px w-8 bg-cyan-500" }),
                      b.jsx("div", {
                        className:
                          "text-cyan-400 font-black uppercase tracking-[0.3em] text-[12px]",
                        children: "Software Engineer",
                      }),
                    ],
                  }),
                  b.jsxs("h1", {
                    className:
                      "text-3xl sm:text-5xl lg:text-7xl font-black tracking-tighter leading-[1.1] lg:leading-[0.95] mb-8 lg:mb-12 text-gradient",
                    children: [
                      "ARCHITECTING HIGH-PERFORMANCE",
                      b.jsx("br", { className: "hidden sm:block" }),
                      " BACKEND ECOSYSTEMS",
                    ],
                  }),
                  b.jsx("p", {
                    className:
                      "text-slate-300 text-base lg:text-xl max-w-3xl mb-12 lg:mb-16 leading-relaxed text-justify border-l-2 border-cyan-500/20 pl-6 lg:pl-8",
                    children: ae.summary,
                  }),
                  b.jsxs("div", {
                    className: "flex flex-wrap gap-4 lg:gap-6 mb-16 lg:mb-24",
                    children: [
                      b.jsx("a", {
                        href: "#projects",
                        className:
                          "w-full sm:w-auto text-center px-8 lg:px-10 py-4 lg:py-5 rounded-2xl bg-indigo-600 text-white font-black uppercase tracking-widest hover:bg-indigo-500 transition-all duration-300 shadow-lg shadow-indigo-600/30",
                        children: "Explore Projects",
                      }),
                      b.jsx("a", {
                        href: ae.whatsapp,
                        target: "_blank",
                        className:
                          "w-full sm:w-auto text-center px-8 lg:px-10 py-4 lg:py-5 rounded-2xl border border-slate-700 text-slate-200 font-black uppercase tracking-widest hover:bg-slate-800 transition-all duration-300 backdrop-blur-sm",
                        children: "Get in Touch",
                      }),
                    ],
                  }),
                  b.jsx("div", {
                    className: "grid grid-cols-2 md:grid-cols-4 gap-6 lg:gap-12",
                    children: [
                      { label: "Years Industry Exp", value: "5" },
                      { label: "Key Enterprise Projects", value: "8+" },
                      { label: "Cloud Practitioner", value: "AWS" },
                    ].map((m, g) =>
                      b.jsxs(
                        "div",
                        {
                          className: "relative group",
                          children: [
                            b.jsx("div", {
                              className:
                                "text-3xl lg:text-5xl font-black mb-2 text-white group-hover:text-cyan-400 transition-colors",
                              children: m.value,
                            }),
                            b.jsx("div", {
                              className:
                                "text-[10px] lg:text-[12px] font-bold uppercase tracking-widest text-slate-400 group-hover:text-slate-300 transition-colors",
                              children: m.label,
                            }),
                          ],
                        },
                        g,
                      ),
                    ),
                  }),
                ],
              }),
            }),
          }),
        ],
      }),
      b.jsxs("section", {
        className: "py-20 lg:py-24 relative overflow-hidden bg-[#111827]",
        children: [
          b.jsx("div", {
            className:
              "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] h-[80%] bg-cyan-500/10 blur-[180px] rounded-full -z-10",
          }),
          b.jsx("div", {
            className: "container mx-auto px-6",
            children: b.jsxs("div", {
              className: "grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center",
              children: [
                b.jsx(ze.div, {
                  initial: { opacity: 0, x: -50 },
                  whileInView: { opacity: 1, x: 0 },
                  viewport: { once: !0 },
                  className: "lg:col-span-5 flex justify-center",
                  children: b.jsxs("div", {
                    className:
                      "relative group w-full max-w-[280px] sm:max-w-[320px] md:max-w-md lg:max-w-none",
                    children: [
                      b.jsx("div", {
                        className:
                          "absolute -inset-4 bg-cyan-500/20 rounded-[2rem] lg:rounded-[4rem] blur-2xl group-hover:bg-cyan-500/30 transition-all duration-700",
                      }),
                      b.jsxs("div", {
                        className:
                          "relative aspect-square lg:aspect-[4/5] rounded-[2rem] lg:rounded-[3.5rem] overflow-hidden border border-slate-700 shadow-2xl bg-[#0a0f1e]",
                        children: [
                          b.jsx("img", {
                            src: `/my_portfolio${ae.profileImage}`,
                            alt: ae.name,
                            className:
                              "w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover: transition-all duration-1000 scale-105 group-hover:scale-100 ",
                            referrerPolicy: "no-referrer",
                          }),
                          b.jsx("div", {
                            className:
                              "absolute inset-0 bg-gradient-to-t from-[#0a0f1e]/90 via-transparent to-transparent opacity-80",
                          }),
                          b.jsxs("div", {
                            className: "absolute bottom-6 left-6 lg:bottom-10 lg:left-10",
                            children: [
                              b.jsx("div", {
                                className:
                                  "text-lg lg:text-2xl font-black tracking-tight text-white",
                                children: ae.name,
                              }),
                              b.jsx("div", {
                                className:
                                  "text-cyan-400 font-bold uppercase tracking-widest text-[8px] lg:text-[10px]",
                                children: "Architecting Excellence",
                              }),
                            ],
                          }),
                        ],
                      }),
                    ],
                  }),
                }),
                b.jsxs(ze.div, {
                  initial: { opacity: 0, x: 50 },
                  whileInView: { opacity: 1, x: 0 },
                  viewport: { once: !0 },
                  className: "lg:col-span-7 space-y-8 lg:space-y-10",
                  children: [
                    b.jsxs("div", {
                      className: "space-y-4",
                      children: [
                        b.jsx("div", {
                          className:
                            "text-cyan-400 font-black uppercase tracking-[0.3em] text-[18px]",
                          children: "The Architect",
                        }),
                        b.jsxs("h2", {
                          className: "text-3xl lg:text-6xl font-black tracking-tighter text-white",
                          children: [
                            "Professional Profile",
                            b.jsx("div", {
                              className:
                                "h-1 w-20  bg-gradient-to-r from-indigo-600 to-cyan-500  mt-4  rounded-full",
                            }),
                          ],
                        }),
                      ],
                    }),
                    b.jsx("p", {
                      className: "text-slate-300 text-base lg:text-xl leading-relaxed",
                      children:
                        "I am a dedicated Software Engineer with a passion for architecting robust backend solutions. My approach combines technical precision with a deep understanding of business requirements, ensuring that every system I build is not only functional but also scalable and future-proof.",
                    }),
                    b.jsx("div", {
                      className: "grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6",
                      children: [
                        { icon: ic, label: "Email", value: ae.email, color: "text-indigo-400" },
                        { icon: YA, label: "Phone", value: ae.phone, color: "text-cyan-400" },
                        {
                          icon: ac,
                          label: "Location",
                          value: ae.location,
                          color: "text-indigo-400",
                        },
                      ].map((m, g) =>
                        b.jsxs(
                          "div",
                          {
                            className:
                              "flex items-center gap-4 p-4 lg:p-6 rounded-2xl lg:rounded-3xl bg-[#0a0f1e]/50 border border-slate-700 hover:border-cyan-500/30 transition-all group",
                            children: [
                              b.jsx("div", {
                                className: `w-10 h-10 lg:w-12 lg:h-12 rounded-xl lg:rounded-2xl bg-[#111827] flex items-center justify-center ${m.color} group-hover:scale-110 transition-transform`,
                                children: b.jsx(m.icon, { size: 18 }),
                              }),
                              b.jsxs("div", {
                                className: "min-w-0",
                                children: [
                                  b.jsx("div", {
                                    className:
                                      "text-[10px] lg:text-[12px] font-bold uppercase tracking-widest text-slate-500",
                                    children: m.label,
                                  }),
                                  b.jsx("div", {
                                    className:
                                      "text-[10px] lg:text-sm font-bold text-slate-200 truncate",
                                    children: m.value,
                                  }),
                                ],
                              }),
                            ],
                          },
                          g,
                        ),
                      ),
                    }),
                  ],
                }),
              ],
            }),
          }),
        ],
      }),
      b.jsxs("section", {
        id: "highlights",
        className: "py-32 relative bg-[#0a0f1e] border-y border-slate-900",
        children: [
          b.jsx("div", {
            className:
              "absolute top-0 right-0 w-[50%] h-[50%] bg-cyan-500/10 blur-[140px] rounded-full -z-10",
          }),
          b.jsxs("div", {
            className: "container mx-auto px-6",
            children: [
              b.jsxs("div", {
                className: "max-w-2xl mb-20",
                children: [
                  b.jsx("div", {
                    className:
                      "text-cyan-400 font-black uppercase tracking-[0.3em] text-[12px] mb-4",
                    children: "Precision Engineering",
                  }),
                  b.jsxs("h2", {
                    className: "text-4xl lg:text-6xl font-black tracking-tighter mb-6 text-white",
                    children: [
                      "Engineering Highlights",
                      b.jsx("div", {
                        className:
                          "h-1 w-20  bg-gradient-to-r from-indigo-600 to-cyan-500  mt-4  rounded-full",
                      }),
                      " ",
                    ],
                  }),
                ],
              }),
              b.jsx("div", {
                className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6",
                children: PA.map((m, g) =>
                  b.jsxs(
                    ze.div,
                    {
                      whileHover: { y: -5 },
                      className:
                        "p-6 lg:p-8 rounded-3xl bg-[#111827]/60 backdrop-blur-xl border border-slate-800/50 hover:border-cyan-400/30 transition-all duration-500 group",
                      children: [
                        b.jsx("div", {
                          className:
                            "w-12 h-12 rounded-xl bg-[#0a0f1e] border border-slate-800 flex items-center justify-center mb-6 group-hover:bg-cyan-500 group-hover:text-white transition-all duration-500",
                          children: b.jsx(wy, {
                            size: 24,
                            className: "text-cyan-400 group-hover:text-inherit",
                          }),
                        }),
                        b.jsx("h3", {
                          className:
                            "text-lg font-bold mb-3 text-slate-100 group-hover:text-cyan-400 transition-colors",
                          children: m.title,
                        }),
                        b.jsx("p", {
                          className: "text-slate-400 text-xs leading-relaxed",
                          children: m.description,
                        }),
                      ],
                    },
                    g,
                  ),
                ),
              }),
            ],
          }),
        ],
      }),
      b.jsx("section", {
        className: "py-20 lg:py-32 bg-[#111827]",
        children: b.jsxs("div", {
          className: "container mx-auto px-6",
          children: [
            b.jsxs("div", {
              className: "text-center max-w-3xl mx-auto mb-16 lg:mb-24",
              children: [
                b.jsx("h2", {
                  className: "text-3xl lg:text-6xl font-black tracking-tighter mb-6 text-white",
                  children: "Technical Arsenal",
                }),
                b.jsx("p", {
                  className: "text-slate-400 text-base lg:text-lg",
                  children:
                    "A comprehensive overview of the technologies and methodologies I employ to build high-performance systems.",
                }),
              ],
            }),
            b.jsx("div", {
              className:
                "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6",
              children: FA.map((m, g) =>
                b.jsxs(
                  ze.div,
                  {
                    initial: { opacity: 0, y: 20 },
                    whileInView: { opacity: 1, y: 0 },
                    viewport: { once: !0 },
                    transition: { delay: g * 0.05 },
                    className:
                      "p-6 rounded-3xl bg-[#0a0f1e]/60 backdrop-blur-xl border border-slate-800/50 flex flex-col gap-6 group hover:bg-[#0a0f1e] hover:shadow-2xl hover:border-cyan-500/30 transition-all duration-500",
                    children: [
                      b.jsxs("div", {
                        className: "flex items-center gap-4",
                        children: [
                          b.jsx("div", {
                            className:
                              "w-12 h-12 rounded-2xl bg-[#111827] border border-slate-800 flex items-center justify-center text-cyan-400 group-hover:bg-cyan-500 group-hover:text-white transition-all duration-500 shadow-lg shadow-cyan-500/5",
                            children: b.jsx(m.icon, { size: 24 }),
                          }),
                          b.jsx("h3", {
                            className: "text-lg font-black tracking-tight text-slate-100",
                            children: m.title,
                          }),
                        ],
                      }),
                      b.jsx("div", {
                        className: "flex flex-wrap gap-2",
                        children: m.skills.map((p) =>
                          b.jsx(
                            "span",
                            {
                              className:
                                "px-3 py-1.5 rounded-lg bg-[#111827] border border-slate-800 text-[12px] font-bold text-slate-400 group-hover:text-cyan-400 transition-colors",
                              children: p,
                            },
                            p,
                          ),
                        ),
                      }),
                    ],
                  },
                  g,
                ),
              ),
            }),
          ],
        }),
      }),
      b.jsxs("section", {
        id: "tools",
        className: "py-20 lg:py-32 relative overflow-hidden bg-[#0a0f1e]",
        children: [
          b.jsx("div", {
            className:
              "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60%] h-[60%] bg-indigo-500/5 blur-[120px] rounded-full -z-10",
          }),
          b.jsxs("div", {
            className: "container mx-auto px-6",
            children: [
              b.jsxs("div", {
                className:
                  "flex flex-col lg:flex-row justify-between items-start lg:items-end mb-16 lg:mb-24 gap-8",
                children: [
                  b.jsxs("div", {
                    className: "max-w-2xl",
                    children: [
                      b.jsx("div", {
                        className:
                          "text-cyan-400 font-black uppercase tracking-[0.3em] text-[12px] mb-4",
                        children: "Productivity Arsenal",
                      }),
                      b.jsxs("h2", {
                        className:
                          "text-3xl lg:text-6xl font-black tracking-tighter mb-6 text-white",
                        children: [
                          "Tools & Ecosystem",
                          b.jsx("div", {
                            className:
                              "h-1 w-20  bg-gradient-to-r from-indigo-600 to-cyan-500  mt-4  rounded-full",
                          }),
                          " ",
                        ],
                      }),
                    ],
                  }),
                  b.jsx("p", {
                    className: "text-slate-400 text-sm lg:text-base max-w-md lg:text-right",
                    children:
                      "Proficient in industry-standard tools for project management, code quality, and collaborative engineering workflows.",
                  }),
                ],
              }),
              b.jsx("div", {
                className: "grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6",
                children: tE.map((m, g) =>
                  b.jsx(
                    ze.div,
                    {
                      initial: { opacity: 0, scale: 0.95 },
                      whileInView: { opacity: 1, scale: 1 },
                      viewport: { once: !0 },
                      transition: { delay: g * 0.05 },
                      className:
                        "p-4 lg:p-6 rounded-2xl lg:rounded-3xl bg-[#111827]/40 backdrop-blur-md border border-slate-800/50 hover:border-indigo-500/30 transition-all group",
                      children: b.jsxs("div", {
                        className: "flex items-center gap-4",
                        children: [
                          b.jsx("div", {
                            className:
                              "w-10 h-10 lg:w-12 lg:h-12 rounded-xl lg:rounded-2xl bg-[#0a0f1e] flex items-center justify-center text-indigo-400 group-hover:bg-indigo-600 group-hover:text-white transition-all",
                            children: b.jsx(m.icon, { size: 20 }),
                          }),
                          b.jsxs("div", {
                            children: [
                              b.jsx("div", {
                                className: "text-sm lg:text-base font-bold text-slate-100",
                                children: m.name,
                              }),
                              b.jsx("div", {
                                className:
                                  "text-[8px] lg:text-[10px] font-bold uppercase tracking-widest text-slate-500 group-hover:text-indigo-400 transition-colors",
                                children: m.category,
                              }),
                            ],
                          }),
                        ],
                      }),
                    },
                    g,
                  ),
                ),
              }),
            ],
          }),
        ],
      }),
      b.jsxs("section", {
        id: "experience",
        className: "py-20 lg:py-32 relative overflow-hidden bg-[#111827] border-y border-slate-900",
        children: [
          b.jsx("div", {
            className:
              "absolute top-1/2 left-0 w-[40%] h-[40%] bg-cyan-600/10 blur-[150px] rounded-full -z-10",
          }),
          b.jsxs("div", {
            className: "container mx-auto px-6",
            children: [
              b.jsxs("h2", {
                className:
                  "text-3xl lg:text-6xl font-black tracking-tighter mb-6 lg:mb-6 text-white",
                children: [
                  "Professional Experience",
                  b.jsx("div", {
                    className:
                      "h-1 w-20  bg-gradient-to-r from-indigo-600 to-cyan-500  mt-4  rounded-full",
                  }),
                  " ",
                ],
              }),
              b.jsx("div", {
                className: "space-y-8 lg:space-y-12",
                children: WA.map((m, g) =>
                  b.jsx(
                    ze.div,
                    {
                      initial: { opacity: 0, x: -30 },
                      whileInView: { opacity: 1, x: 0 },
                      viewport: { once: !0 },
                      className:
                        "p-6 lg:p-12 rounded-[2rem] lg:rounded-[3.5rem] bg-[#111827]/60 backdrop-blur-xl border border-slate-800/50 shadow-2xl",
                      children: b.jsxs("div", {
                        className:
                          "flex flex-col lg:flex-row justify-between items-start gap-8 lg:gap-10",
                        children: [
                          b.jsxs("div", {
                            className: "lg:w-1/3",
                            children: [
                              b.jsxs("div", {
                                className: "flex items-center gap-4 mb-6",
                                children: [
                                  b.jsx("div", {
                                    className:
                                      "w-12 h-12 lg:w-14 lg:h-14 rounded-2xl bg-cyan-500/10 flex items-center justify-center text-cyan-400",
                                    children: b.jsx(jy, { size: 24 }),
                                  }),
                                  b.jsxs("div", {
                                    children: [
                                      b.jsx("h3", {
                                        className: "text-xl lg:text-2xl font-black text-slate-100",
                                        children: m.company,
                                      }),
                                      b.jsx("p", {
                                        className:
                                          "text-cyan-400 font-bold uppercase tracking-widest text-[12px]",
                                        children: m.period,
                                      }),
                                    ],
                                  }),
                                ],
                              }),
                              b.jsx("div", {
                                className: "text-lg lg:text-xl font-bold text-slate-300 mb-2",
                                children: m.role,
                              }),
                              b.jsxs("div", {
                                className:
                                  "flex items-center gap-2 text-slate-500 text-[10px] font-bold uppercase tracking-widest",
                                children: [b.jsx(ac, { size: 12 }), " ", m.location],
                              }),
                            ],
                          }),
                          b.jsx("div", {
                            className: "lg:w-2/3",
                            children: b.jsx("ul", {
                              className: "space-y-3 lg:space-y-4",
                              children: m.bullets.map((p, v) =>
                                b.jsxs(
                                  "li",
                                  {
                                    className:
                                      "flex items-start gap-3 lg:gap-4 text-slate-400 text-sm lg:text-base leading-relaxed",
                                    children: [
                                      b.jsx("div", {
                                        className:
                                          "mt-2 w-1.5 h-1.5 rounded-full bg-cyan-500 flex-shrink-0",
                                      }),
                                      p,
                                    ],
                                  },
                                  v,
                                ),
                              ),
                            }),
                          }),
                        ],
                      }),
                    },
                    g,
                  ),
                ),
              }),
            ],
          }),
        ],
      }),
      b.jsxs("section", {
        id: "projects",
        className: "py-20 lg:py-32 relative overflow-hidden bg-[#0a0f1e]",
        children: [
          b.jsx("div", {
            className:
              "absolute bottom-0 right-0 w-[50%] h-[50%] bg-cyan-600/10 blur-[160px] rounded-full -z-10",
          }),
          b.jsxs("div", {
            className: "container mx-auto px-6",
            children: [
              b.jsxs("div", {
                className:
                  "flex flex-col md:flex-row justify-between items-start md:items-end mb-16 lg:mb-24 gap-8",
                children: [
                  b.jsxs("h2", {
                    className:
                      "text-3xl lg:text-6xl font-black tracking-tighter max-w-2xl text-white",
                    children: [
                      "Key Projects",
                      b.jsx("div", {
                        className:
                          "h-1 w-20  bg-gradient-to-r from-indigo-600 to-cyan-500  mt-4  rounded-full",
                      }),
                      " ",
                    ],
                  }),
                  b.jsxs("a", {
                    href: ae.github,
                    target: "_blank",
                    className:
                      "flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-cyan-400 hover:text-white transition-colors group",
                    children: [
                      "View GitHub",
                      " ",
                      b.jsx(U0, {
                        size: 18,
                        className:
                          "group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform",
                      }),
                    ],
                  }),
                ],
              }),
              b.jsx("div", {
                className: "grid grid-cols-1 gap-16 lg:gap-24",
                children: $A.map((m, g) =>
                  b.jsxs(
                    ze.div,
                    {
                      initial: { opacity: 0, y: 50 },
                      whileInView: { opacity: 1, y: 0 },
                      viewport: { once: !0 },
                      className:
                        "group grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center",
                      children: [
                        b.jsx("div", {
                          className: `lg:col-span-7 ${g % 2 !== 0 ? "lg:order-2" : ""}`,
                          children: b.jsxs("div", {
                            className:
                              "aspect-[16/9] rounded-[2rem] lg:rounded-[4rem] overflow-hidden border border-slate-800 relative shadow-2xl bg-[#0a0f1e]",
                            children: [
                              b.jsx("img", {
                                src: m.image,
                                alt: m.title,
                                className:
                                  "w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-1000 scale-105 group-hover:scale-100",
                                referrerPolicy: "no-referrer",
                              }),
                              b.jsx("div", {
                                className:
                                  "absolute inset-0 bg-cyan-600/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500",
                              }),
                            ],
                          }),
                        }),
                        b.jsxs("div", {
                          className: "lg:col-span-5",
                          children: [
                            b.jsx("div", {
                              className:
                                "text-cyan-400 font-black uppercase tracking-[0.3em] text-[12px] mb-3 lg:mb-4",
                              children: m.subtitle,
                            }),
                            b.jsx("h3", {
                              className:
                                "text-2xl lg:text-4xl font-black mb-4 lg:mb-6 text-slate-100 group-hover:text-cyan-400 transition-colors",
                              children: m.title,
                            }),
                            b.jsx("p", {
                              className:
                                "text-slate-400 text-sm lg:text-lg mb-6 lg:mb-10 leading-relaxed",
                              children: m.description,
                            }),
                            b.jsx("div", {
                              className: "flex flex-wrap gap-2 lg:gap-3 mb-8 lg:mb-12",
                              children: m.tech.map((p) =>
                                b.jsx(
                                  "span",
                                  {
                                    className:
                                      "px-3 lg:px-4 py-1.5 lg:py-2 rounded-xl bg-[#0a0f1e] border border-slate-800 text-[10px] lg:text-[12px] font-bold text-slate-500 uppercase tracking-widest",
                                    children: p,
                                  },
                                  p,
                                ),
                              ),
                            }),
                            b.jsxs("button", {
                              onClick: () => o(m),
                              className:
                                "inline-flex items-center gap-3 text-[12px] font-black uppercase tracking-widest text-cyan-400 hover:text-white transition-colors",
                              children: ["Project Details ", b.jsx(U0, { size: 16 })],
                            }),
                          ],
                        }),
                      ],
                    },
                    g,
                  ),
                ),
              }),
            ],
          }),
        ],
      }),
      b.jsx("section", {
        className: "py-20 lg:py-32 bg-[#111827] border-y border-slate-800",
        children: b.jsx("div", {
          className: "container mx-auto px-6",
          children: b.jsxs("div", {
            className: "grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24",
            children: [
              b.jsxs("div", {
                children: [
                  b.jsxs("h2", {
                    className:
                      "text-3xl lg:text-6xl font-black tracking-tighter mb-12 lg:mb-20 text-white",
                    children: [
                      "Education",
                      b.jsx("div", {
                        className:
                          "h-1 w-20  bg-gradient-to-r from-indigo-600 to-cyan-500  mt-4  rounded-full",
                      }),
                      " ",
                    ],
                  }),
                  b.jsx("div", {
                    className: "space-y-8 lg:space-y-12",
                    children: IA.map((m, g) =>
                      b.jsxs(
                        ze.div,
                        {
                          initial: { opacity: 0, y: 20 },
                          whileInView: { opacity: 1, y: 0 },
                          viewport: { once: !0 },
                          className: "relative pl-10 lg:pl-12 border-l-2 border-cyan-500/20",
                          children: [
                            b.jsx("div", {
                              className:
                                "absolute top-0 left-[-9px] w-4 h-4 rounded-full bg-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.5)]",
                            }),
                            b.jsx("h3", {
                              className: "text-xl lg:text-2xl font-black mb-2 text-slate-100",
                              children: m.degree,
                            }),
                            b.jsx("p", {
                              className:
                                "text-slate-400 font-bold uppercase tracking-widest text-[10px] lg:text-[12px]",
                              children: m.school,
                            }),
                          ],
                        },
                        g,
                      ),
                    ),
                  }),
                ],
              }),
              b.jsxs("div", {
                children: [
                  b.jsxs("h2", {
                    className:
                      "text-3xl lg:text-6xl font-black tracking-tighter mb-12 lg:mb-20 text-white",
                    children: [
                      "Certifications",
                      b.jsx("div", {
                        className:
                          "h-1 w-20  bg-gradient-to-r from-indigo-600 to-cyan-500  mt-4  rounded-full",
                      }),
                      " ",
                    ],
                  }),
                  b.jsx("div", {
                    className: "grid grid-cols-1 gap-4",
                    children: eE.map((m, g) =>
                      b.jsxs(
                        ze.div,
                        {
                          initial: { opacity: 0, x: 20 },
                          whileInView: { opacity: 1, x: 0 },
                          viewport: { once: !0 },
                          transition: { delay: g * 0.1 },
                          className:
                            "p-4 lg:p-6 rounded-2xl bg-[#0a0f1e]/60 backdrop-blur-xl border border-slate-800 flex items-center gap-4 lg:gap-6 group hover:border-cyan-500/30 transition-all shadow-2xl",
                          children: [
                            b.jsx("div", {
                              className:
                                "w-10 h-10 lg:w-12 lg:h-12 rounded-xl bg-[#111827] flex items-center justify-center text-cyan-400 group-hover:bg-cyan-500 group-hover:text-white transition-all",
                              children: b.jsx(L0, { size: 20 }),
                            }),
                            b.jsxs("div", {
                              children: [
                                b.jsx("div", {
                                  className: "text-xs lg:text-sm font-bold text-slate-100 mb-1",
                                  children: m.title,
                                }),
                                b.jsx("div", {
                                  className:
                                    "text-[10px] lg:text-[12px] font-bold uppercase tracking-widest text-slate-500",
                                  children: m.issuer,
                                }),
                              ],
                            }),
                          ],
                        },
                        g,
                      ),
                    ),
                  }),
                ],
              }),
            ],
          }),
        }),
      }),
      b.jsx("footer", {
        className: "py-24 bg-slate-950 border-t border-slate-900",
        children: b.jsx("div", {
          className: "container mx-auto px-6",
          children: b.jsxs("div", {
            className: "flex flex-col lg:flex-row justify-between items-center gap-12",
            children: [
              b.jsx("div", {
                className: "text-center lg:text-left",
                children: b.jsx("p", {
                  className: "text-slate-500 text-sm font-bold uppercase tracking-widest",
                  children: "Architecting Excellence",
                }),
              }),
              b.jsx("div", {
                className: "flex gap-4",
                children: [
                  { icon: H0, href: ae.github },
                  { icon: q0, href: ae.linkedin },
                  { icon: ic, href: `mailto:${ae.email}` },
                ].map((m, g) =>
                  b.jsx(
                    "a",
                    {
                      href: m.href,
                      target: "_blank",
                      className:
                        "w-14 h-14 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-400 hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-all duration-300 group shadow-2xl",
                      children: b.jsx(m.icon, {
                        size: 20,
                        className: "group-hover:scale-110 transition-transform",
                      }),
                    },
                    g,
                  ),
                ),
              }),
            ],
          }),
        }),
      }),
    ],
  });
}
dx.createRoot(document.getElementById("root")).render(
  b.jsx(k.StrictMode, { children: b.jsx(nE, {}) }),
);
